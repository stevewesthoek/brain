...
Handoff and parallel-briefs policy canonical source: `brain/ai/policy/handoff-and-parallel-briefs.md`.
...
- Treat `/code` as the conceptual entry point for coding work even when the literal Claude skill runtime is not active: classify intent, preserve scope, use dormant skill source docs/registries, review before shipping, and separate deterministic checks into hooks/CI/manual gates.
...
- For tasks requiring persistent memory, cross-repo context, architecture across multiple systems, or long iterative editing: prepare a handoff and tell the user to switch to Claude Code or the appropriate orchestrating surface.
- For isolated reviews, focused patches, validation, and well-scoped implementation: handle it directly, escalating tiers as needed.
...
For Stripe CLI auth and profile handling, use the standard Stripe runbook.
...
For large local JSON, logs, or text, use `brain-compress` automatically when exact retrieval may be needed. Store/retrieve originals by hash under `~/.brain/cache/compression/`; do not route model calls through it. For repeated failed paths, command forms, or local runtime gotchas, run `brain-learn-failures --repo . --write-report` before deciding whether `/learner` should promote a pattern. Runbook: `brain/operations/runbooks/context-compression.md`.
For ALL design work (new projects, reference mimics, existing site upgrades), use the shared `/design` orchestrator at `brain/ai/skills/custom/design/SKILL.md`. It sequences design subskills by function. `/web-design` produces implementation-ready web/SaaS specs. `/awesome-design-md` at `brain/ai/skills/custom/awesome-design-md/SKILL.md` provides public-brand DESIGN.md references for style direction and token cues when no approved project `DESIGN.md` exists. For motion audits specifically, use `/design-motion-principles` at `brain/ai/skills/vendors/kylezantos/design-motion-principles/SKILL.md`.
...
At the start of every session, read `~/.brain/memory-context.md` when present. It is generated by the explicit memory-context refresh procedure, not guaranteed by the production scheduler. If it is absent or stale, call `mem-search` instead.
...
Codex has no hook mechanism — use `~/.brain/memory-context.md` at session start instead.
