...
Default active skills: `/code`, `/research`, `/memory`, `/review`, `/qa`, `/handoff`, `/careful`, `/capability-discovery`. For every natural-language request that may need a skill, CLI, MCP server, or runbook, use the shared read-only query `node /Users/Office/Repos/stevewesthoek/brain/tools/discover-capabilities.mjs --query "<request>" --format compact`, then read the selected source documentation. The user must not need to know internal names or profiles. Use `brain/ai/policy/capability-discovery.md`, `brain/docs/skills/skill-index.md`, `brain/operations/CLI-MANIFEST.md`, and `brain/operations/AI-CONFIG-INDEX.md` as the canonical sources.
