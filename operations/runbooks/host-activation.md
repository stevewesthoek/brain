...
- a repeat `execute` against an already-active canonical packet fails before
  mutation; explicit rollback is idempotent after its receipt reaches phase 0;
- a completed run retains all backups and the old canonical Brain archive.

The runner never calls `launchctl`, starts/stops a service, weakens host-key
...
0. **Preflight:** verify users, exact clean packet commits, old canonical/Mind
   ...
   quiescence, both fixed SSH routes, authentication, and free space.
1. **Backup/receipts:** copy and digest Office `.claude`, `.cursor`, `.gemini`,
   ...
   standalone canonical candidate. Backup failure stops before mutation.
2. **Stage:** copy and digest physical Office runtime stages for the four
   legacy whole-root symlinks.
...
9. **Application acceptance:** reopen one item at a time and type `PASS` only
...
backup, stage, socket, config, approval, or Git change.
...
mutation and prints its socket path. **Do not close the Terminal, kill the
...
Type exactly `PASS` for each verified item. Any other response stops acceptance.
