...
2. Creates individual task `.md` files in `notes/04-tasks/{list-slug}/{number}-{task-slug}.md`
...
2. Creates `.md` files in `notes/04-tasks/`
...
Task file generation (YAML + markdown)
        ↓
Write to notes/04-tasks/{list}/{number}-{slug}.md
        ↓
Git: stage → commit → pull → push
        ↓
Kanban Syncer (runs every 10 min)
        ↓
notes/kanban.md updated with all tasks
        ↓
...
1. Check git history: `git log --oneline notes/04-tasks/`
2. Identify duplicate commits
3. Reset to before the first import: `git reset --soft <commit-hash>`
4. Delete task folders: `rm -rf notes/04-tasks/{list-name}`
