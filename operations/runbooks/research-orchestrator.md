# Research Orchestrator — Implementation Runbook

**Status:** Proposed implementation package  
**Intended source skill:** `brain/ai/skills/custom/research/SKILL.md`  
**Active profile policy:** Keep `/research` small enough for the default profile; keep `/bible-research` in the research profile unless it becomes a daily always-on need.

---

## Purpose

`/research` is the master natural-language entry point for research work. It decides the evidence standard, source strategy, domain routing, and output shape. It is not a replacement for `/web`; `/web` remains the browser, scraping, and web-automation orchestrator.

Use `/research` when the user asks to understand, compare, verify, synthesize, brief, or document a topic from sources.
...
User asks a research question
  ↓
/research classifies the task and evidence standard
  ↓
Routes to one or more capabilities:
  - /web for online acquisition, scraping, browser interaction, or automation
  - /bible-research for Scripture and theological research
  - /gemini for large-context preprocessing
  - /memory for durable findings
  - /review for critique and adversarial source checks
  ↓
Produces sourced synthesis, report, brief, table, or research note
```
...
The current repo architecture intentionally keeps `ai/skills/active/` small for Claude Code, Codex, Gemini, Cursor, Kiro, and Antigravity.

Recommended policy:

- Add `research` to `docs/skills/profiles/default.txt` only if broad research is a daily top-level task.
- Add `bible-research` to `docs/skills/profiles/research.txt`, not default.
- Do not add all research subskills to default.
- Keep `/web` independently callable for browser/scraping tasks.
- Keep `/bible-research` independently callable for power users and activated by the research profile.

---

## Intended `/research` Skill Source
...
1. Create `ai/skills/custom/research/SKILL.md` from the source block above.
2. Create `ai/skills/custom/bible-research/SKILL.md` from `operations/runbooks/bible-research.md`.
3. Add `research` to `docs/skills/profiles/default.txt` only if it should be always-on.
4. Add `research` and `bible-research` to `docs/skills/profiles/research.txt`.
5. Update `docs/skills/skill-index.md` to list both skills.
6. Run:
...
7. Apply a profile only after dry-run succeeds.
