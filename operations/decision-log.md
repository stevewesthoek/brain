...
- Date: 2026-09-07 (Stitch OAuth transport correction)
- Decision: Treat the installed Stitch SDK's authentication contract as authoritative. Do not use `STITCH_API_KEY = "gcloud-adc"`; it is sent literally as an API key and is rejected by the Stitch MCP endpoint. Standard stdio clients launch the centralized `stitch-oauth-proxy.sh`, which obtains a short-lived access token from gcloud Application Default Credentials at process start and passes it as `STITCH_ACCESS_TOKEN`.
- Context: A fresh official Google OAuth flow produced ADC successfully, but the previous Codex MCP configuration still returned HTTP 401 stating that API keys are unsupported. A direct OAuth wrapper probe reached the provider and returned the distinct IAM 403 boundary, proving the transport correction.
- Impact: All tracked stdio templates and setup guidance use the wrapper; tokens remain application/process-owned and are not persisted in Brain or MCP config. The setup script no longer escalates a failed read-only doctor into an automatic IAM/API mutation.
- Verification: `bash -n operations/system-configs/mcp/stitch/stitch-oauth-proxy.sh`; wrapper MCP probe reaches the provider with OAuth and returns only the expected project-permission boundary; targeted Codex/runtime/identity/Keychain tests pass.
- Rollback: Restore the prior tracked templates/config only if a compatible future SDK explicitly restores ADC-sentinel support. Never restore the sentinel blindly; revalidate the installed SDK first.

- Date: 2026-09-07 (Stitch provider IAM recovery)
- Decision: Keep Stitch provider authorization narrowly scoped to the existing project and required Service Usage permission. Grant the active Stitch ADC user `roles/serviceusage.serviceUsageConsumer` on `project-d63f458f-8fba-450e-acf`, and enable only `stitch.googleapis.com` for that project.
- Context: After the OAuth transport correction, Stitch reached the provider but returned HTTP 403 for the missing `serviceusage.services.use` permission. The installed Stitch setup documentation also requires the Stitch API to be enabled. The operator explicitly confirmed both exact external changes.
- Impact: The corrected wrapper probe now completes MCP initialization and a provider tool call. No broad IAM role, alternate API, token inspection, OAuth copy, WebGPT change, or shared Codex-root mutation was performed. The already-running desktop Codex process still needs a controlled reload to consume the corrected configuration.
- Verification: Persisted project policy contains the required role binding; `stitch.googleapis.com` is enabled; the wrapper probe reports provider success with no raw token or policy output. The live loaded desktop process is intentionally not restarted during this active session.
- Rollback: Remove only the exact project-level Service Usage Consumer binding if the operator later authorizes rollback, and disable only the Stitch API if no in-scope consumer remains. Re-run the provider probe after any rollback.

- Date: 2026-09-07 (Stitch managed projection refresh)
- Decision: Refresh only the machine-local Stitch MCP projection through the supported `codex mcp remove/add stitch` path, using the canonical OAuth wrapper and preserving all non-Stitch configuration semantics. Treat any still-loaded task-level MCP binding as a separate re-handshake boundary.
- Context: The live generated Codex config had drifted to the old direct `npx` registration with a Stitch API-key reference, while the canonical main configuration used the OAuth wrapper. A disposable Codex-home test showed the CLI reserialization changes formatting but preserves non-Stitch semantics. The corrected projection and a normal Desktop quit/reopen were completed, but the already-established task binding continued to return the historical API-key-mode 401.
- Impact: The local projection now points to the validated wrapper with no Stitch API-key reference; WebGPT remained untouched; no auth file, token, OAuth callback, global shutdown, or unrelated configuration was changed. Independent wrapper and provider checks remain successful; a fresh task-level MCP handshake is still required for this task's live Stitch tool binding.
- Verification: Projection invariant inventory, TOML parse, workstation/identity/credential-health validators, dedicated profile doctors, `codex_apps`, WebGPT owner doctor, Keychain metadata inventory, Codex status, and Tailscale path checks pass or remain explicitly classified. No secret-sensitive output was persisted.
- Rollback: Restore only the prior Stitch projection if a validated compatibility reason requires it; do not restore the rejected API-key registration or alter unrelated MCP/application state.

- Date: 2026-09-07 (CLR5 conversation evidence closeout)
- Decision: Treat selected conversation/session records as bounded evidence only. Preserve source ownership, provenance, actor/claim distinction, freshness, and review boundaries; route infrastructure-classified evidence to IKHP candidates without mutating IKHP canonical state.
- Context: CLR5 was explicitly authorized after Identity & Access closeout. The existing explicit Claude/Codex/Workbench session-reference foundation was extended with watermarks, deterministic event hashes, privacy redaction, duplicate collapse, contradiction reporting, and a report-only CLI/schema path.
...
- Date: 2026-09-07 (Brain Credentials Vault production closeout)
- Decision: Declare the Brain Credentials Vault `PRODUCTION_READY_EMPTY`. The macOS login Keychain under `tools.prochat.brain` is the authoritative host-local Brain secret store, while application/provider/host/unknown credentials remain with their owners.
- Context: The adapter and synthetic lifecycle were already accepted; the remaining work was the safe operator command surface, single opaque-reference resolver, inventory/doctor reconciliation, hidden native add/update/rotate flow, explicit deletion/revocation semantics, recovery documentation, and final audit.
- Impact: Added the production `vault` CLI, catalog reconciliation and bounded consumer resolution, native hidden-input modes, operator runbook, and production closeout report. The canonical catalog still has zero real Brain-managed credentials; no OAuth state or production secret was migrated.
- Verification: Live status/list/doctor report an available login Keychain with zero items. Native adapter 14/14, enrollment 2/2, bounded verification 6/6, provider verifier 13/13, vault UX 9/9, credential health 5/5, governance 7/7, observation admission 10/10, schema validators, and secret-output review pass.
- Rollback: Remove the scoped CLI/resolver/docs and restore the prior adapter-only operator surface; no production secret or canonical credential record requires rollback.
