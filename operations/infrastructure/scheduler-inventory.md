...
| `8` | `model-router-dry-run` | `com.office.nightly-scheduler` | Under 5 min | `Low` | Never stops chain. Runs after auth refresh as report-only validation. | **REPORT-only mode:** Runs `projects/model-router` CI and writes runtime status to `runtime/local/model-router/latest.md` and `.json`. It does not write, move, delete, archive, compact, split, or rewrite Mind files. Log: `~/Library/Logs/office-scheduler/model-router-dry-run.log`. Source: `tools/scripts/model-router-dry-run-report.sh`. |
