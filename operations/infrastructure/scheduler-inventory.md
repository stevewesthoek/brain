...
| `11` | `graphify-nightly` | `com.office.nightly-scheduler` | Up to 6 hours | `High` local LLM and disk during first-time semantic extraction; `Low` to `Medium` for existing graph updates | Never stops chain. Selector burst guard may skip heavy builds. | Maintains per-repo `graphify-out/` graphs for Git repositories under `/Users/Office/Repos`. First-time builds call AI Model Selector task `codebase_semantic_graph` with `local_only=true`; the selector admits `qwen2.5:32b` only when local load, macOS memory pressure, and Ollama model occupancy are safe, otherwise falls back to 14B or skips. The runner unloads the selected Ollama model after the extraction attempt. Defaults: one first-time build and up to 12 updates per night. Log: `~/Library/Logs/office-scheduler/graphify-nightly.log`. Source: `tools/scripts/graphify-nightly.sh`. |
