...
| `3` | `com.office.supabase-recovery-copy-backup` | macOS `launchd` LaunchAgent (prepared, disabled) | `05:30` daily after the Azure VM Backup window | Up to 4 hours; dynamic latest recovery point selection | `High` Azure restore, isolated PostgreSQL logical dump, and Blob upload | Must remain disabled until the Phase 3X manual acceptance, idempotency, telemetry, cleanup, and post-health gates are all proven. The job has its own lock and never uses the production logical-dump path. | Repo artifact: `operations/system-configs/launchagents/com.office.supabase-recovery-copy-backup.plist`; runner: `tools/scripts/supabase-recovery-copy-backup.sh`. |
...
| `12` | `supabase-recovery-copy-backup` | Prepared independent LaunchAgent; not enabled | `05:30` local time, after the expected Azure Daily recovery point window | Up to 4 hours | `High` temporary Azure VM restore plus sequential PG15 dumps and off-host Blob upload | Dynamic latest recovery point; clean no-op when the selected point was already processed; fail closed on Azure delay, active prior run, restore failure, Blob collision/unavailability, or any production preflight failure. The repository artifact is explicitly disabled and is not installed or started in Phase 3X. | Source: `tools/scripts/supabase-recovery-copy-backup.sh`; receipt: `runtime/local/infrastructure/backup-runtime-state.json`; legacy `pgdump-upload.timer` remains disabled/inactive. |
