...
| Token Name | Storage | Purpose | Scopes | Expiry | Rotation | Regenerate |
|------------|---------|---------|--------|--------|----------|-----------|
| `n8n-github-mind` | `~/.config/github/.env` | Fine-grained PAT — mind repo only (Contents: read + write) · Used by n8n Mind Inbox workflow | Contents: read + write | No expiration | Manually rotate if compromised | [GitHub → Settings → Developer settings → Personal access tokens](https://github.com/settings/tokens?type=beta) |
| `Dokploy` (GHCR pull) | `~/.config/dokploy/.env` as `GHCR_DOKPLOY_PULL_PAT` | Classic PAT — `read:packages` scope only. Used by Dokploy (vm-dokploy Azure) to pull private Docker images from GHCR (`ghcr.io`) across all orgs (prochattools, yeshuaacademy) and personal account. Registered once in Dokploy Settings → Registry as `dokploy-ghcr-pull`. Never needs to be re-entered per app. | `read:packages`, `repo`, `workflow`, `gist`, `read:org` | No expiration set | Rotate if compromised: update in Dokploy Settings → Registry → edit `dokploy-ghcr-pull` | [GitHub → Settings → Developer settings → Personal access tokens → Classic](https://github.com/settings/tokens) |
| `GHCR Write` (Docker push) | `~/.config/github/.ghcr-write-token` | Classic PAT — `write:packages`, `repo`, `workflow`, `gist`, `read:org` scopes. Used to **push** Docker images to GHCR (`ghcr.io`) for prochattools org. Used by local development workflow: `echo <token> \| docker login ghcr.io -u prochattools --password-stdin` then `docker push`. Separate from `dokploy-ghcr-pull` (read-only pull token). | `write:packages`, `repo`, `workflow`, `gist`, `read:org` | No expiration set | Rotate if compromised or after 90 days | [GitHub → Settings → Developer settings → Personal access tokens → Classic](https://github.com/settings/tokens) → Generate new token with `write:packages` scope |
...
Current CLI evidence (2026-08-26): the active selectors `prochat portugal`,
`jpv-bootcamp`, `prochat studio`, `says the bible`, and `yeshua academy`—
displayed respectively as ProChat Portugal, JPV Bootcamp, ProChat Nederland,
Says the Bible, and Yeshua Academy—all pass account and subscription reads.
The Dashboard account switcher may show multiple accounts for one user, but
Stripe CLI access remains profile/account scoped; `stripe accounts list`
returning an empty list means no Connect subaccounts were returned for that
account.

| Account | Live Account ID | Profile | Test Access | Notes |
|---------|-----------------|---------|-------------|-------|
| ProChat Studio | `acct_1SxE7KDmzmvnZunZ` | `prochat-main` | Same profile has test mode | Legacy profile probe failed on 2026-08-26; separate from the active `prochat studio` / ProChat Nederland profile |
...
Re-authenticate: `stripe login --project-name "ProChat Studio"` (opens browser; requires owner approval). The resulting CLI selector is `prochat-main`. Switch profiles with the profile-specific login flow only after confirming the intended account.

### Stripe inventory notes

- Current Dashboard-to-CLI mappings are maintained in the central Stripe health registry; do not infer selectors from display labels. The separate `prochat-main` / ProChat Studio profile is legacy and currently rejected.
...
- Profile configuration and key presence are not proof of current API access; use the central read-only health check and account probes.
- The 2026-08-26 full active-set probe passed for all five Dashboard accounts. Older duplicate/out-of-set selectors remain disabled in the health registry.
...
- Current live status is maintained by `node tools/check-cli-access-health.mjs --write --notify`; do not rely on the historical 2026-04-11 authentication claims above.
...
| `NEW_RELIC_USER_API_KEY` | `~/.config/probot/.env` | 2026-06-20 |
| `NEW_RELIC_ACCOUNT_ID` | `~/.config/probot/.env` | 2026-06-20 |
| `CLOUDFLARE_API_TOKEN` | `~/.config/probot/.env` | 2026-06-20 |
| `UMAMI_URL` | `~/.config/probot/.env` | 2026-06-20 |
| `UMAMI_USERNAME` | `~/.config/probot/.env` | 2026-06-20 |
| `UMAMI_PASSWORD` | `~/.config/probot/.env` | 2026-06-20 |
| `XGROW_URL` | `~/.config/probot/.env` | 2026-06-20 |
| `XGROW_API_KEY` | `~/.config/probot/.env` | 2026-06-20 |
| `XGROW_CRON_SECRET` | `~/.config/probot/.env` | 2026-06-20 |
| `YOUTUBE_ACCOUNT_LABEL` | `~/.config/youtube/prochat.env` | 2026-06-20 |
| `YOUTUBE_CHANNEL_TITLE` | `~/.config/youtube/prochat.env` | 2026-06-20 |
| `YOUTUBE_CLIENT_SECRET_JSON` | `~/.config/youtube/prochat.env` | 2026-06-20 |
| `YOUTUBE_TOKEN_FILE` | `~/.config/youtube/prochat.env` | 2026-06-20 |
| `YOUTUBE_ACCOUNT_LABEL` | `~/.config/youtube/says-the-bible.env` | 2026-06-20 |
| `YOUTUBE_CHANNEL_TITLE` | `~/.config/youtube/says-the-bible.env` | 2026-06-20 |
