...
- **Status:** complete; canonical Contract V2.1 accepted on 2026-08-10. Run `b8-1-v2-1-validation-fixed-20260810` completed 5/5 repetitions with every acceptance gate passing: 99.2063% aggregate structural coverage with zero unknown files, 90% file accuracy, 87.5% applicable-line accuracy, MRR 0.4944, set outcome accuracy 1.0, caller/callee F1 1.0, exact-source and fallback accuracy 1.0, lifecycle/resource headroom, isolation, restoration, and cleanup all passed. The accepted structural candidate is unchanged `codebase-memory-mcp 0.9.0` full mode with deterministic exact-source fallback; Graphify remained excluded. A post-acceptance cleanup-metadata defect was recovered without rerunning benchmark work, and final canonical evidence validates. P8 subsequently completed 6/6 on 2026-08-11. Evidence: [canonical acceptance](../reports/b8-1-v2-1-canonical-accepted-2026-08-10.md), [canonical evidence](../reports/b8-1-v2-evidence/b8-1-v2-canonical-evidence.json), [canonical disposition](../reports/b8-1-v2-evidence/disposition.json).
- **Scope:** Codebase Memory MCP and deterministic exact-source exploration on pinned Brain, Workbench, and ProChat repositories. Graphify is separate and excluded from B8.1 Contract V2; Mind is untouched and out of scope.
- **Change:** after explicit benchmark authorization and approval of the exact emitted Contract V2 plan digest, record full-mode coverage and fallback, indexing time, incremental refresh latency, CPU, peak memory, disk use, retrieval quality, caller/callee F1, isolation, source restoration, and cleanup across five independent runs; keep all tests local and read-only. Partial evidence is not accepted.
...
- **Status:** complete (2026-08-11). Canonically accepted `codebase-memory-mcp 0.9.0` is now `active-local` for Brain only, bound to the isolated Brain cache with `auto_index=false` and `auto_watch=false`; exact 14-tool inventory, credential-free stdio transport, pinned executable provenance, exact-source fallback, and rollback/uninstall dry-run all pass. Runtime-truth reports B8.2 accepted with Workbench/Mind registrations intact and no bare credentials. Wider repository rollout subsequently remained per-repository admission-bound through B8.6; P8 completed 6/6 on 2026-08-11. Evidence: [B8.2 Codebase Memory admission](../reports/b8-2-codebase-memory-admission-2026-08-11.md).
...
- **Status:** complete (2026-08-11). The read-only Brain+ProChat pilot passed 2/2 with 100% structural probe hits and exact-source fallback, inherited cold/refresh/index limits, change→refresh→query→restore, fresh-state rebuild, graceful degradation, Graphify semantic disablement, uninstall dry-run, disposable rollback, and unchanged source availability. An initial full-source retrieval pilot exposed excessive navigation output; B8.6 optimized CBM navigation to `files` mode with 5 candidates, reducing estimated CBM navigation output from 64,343 to 397 tokens (~99.4%) while preserving 100% probe hits. Rollout decision: approve the architecture but reject blanket global rollout; additional repositories require explicit admission. Evidence: [B8.6 pilot acceptance](../reports/b8-6-context-memory-pilot-acceptance-2026-08-11.md), [optimized evidence](../reports/b8-6-context-memory-pilot-evidence-optimized.json). B8.1–B8.6 are 6/6 accepted; final P8 verification passed on 2026-08-11 and remaining required work under this plan is 0.
