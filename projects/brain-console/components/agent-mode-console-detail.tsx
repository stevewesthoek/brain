...
type AgentModeRunDetail = Extract<AgentModeConsoleDetailResponse, { status: 'available' }>['detail'] & { kind: 'run' };
...
function ControlAuditList({ audits }: { audits: AgentModeRunDetail['controlAudits'] }) {
  return audits.length === 0 ? <p className="meta">No durable controls are recorded for this run.</p> : <div className="stack compact-detail-stack">{audits.map((audit) => <div className="agent-console-detail-list-row" key={audit.receiptRef}><div className="min-w-0"><div><StatusBadge status={audit.status} /> <span>{audit.decision ?? audit.action} · {audit.operatorId ?? audit.serviceActor ?? audit.actor}</span></div><div className="meta">{audit.reasonCode} · {timeAgo(audit.occurredAt)} · {audit.processIdentityVerified === true ? 'process identity verified' : audit.signalSent === true ? 'signal sent' : 'no signal metadata'}</div></div><Value value={audit.receiptRef} /></div>)}</div>;
}
