...
export function OperationsOverview() {
  ...
  const lastRunDuration = typeof schedulerData?.lastRun?.durationSeconds === 'number' ? `${schedulerData.lastRun.durationSeconds}s` : '—';
  ...
  return (
    <div className="workspace-screen operations-workspace">
      ...
      <div className="workspace-grid operations-primary-grid">
        ...
        <section className="workspace-panel" aria-labelledby="operations-scheduler"><div className="workspace-panel-heading"><div><div className="eyebrow">Scheduled work</div><h2 id="operations-scheduler">Scheduler posture</h2></div><Link href="/scheduler" className="workspace-link">History <ArrowUpRight size={13} /></Link></div><div className="workspace-kpi-row"><div><strong>{schedulerData?.successfulJobs ?? '—'}</strong><span className="meta">successful</span></div><div><strong>{failedJobs}</strong><span className="meta">failures</span></div><div><strong>{schedulerData?.runningJobs ?? '—'}</strong><span className="meta">running</span></div></div><div className="workspace-list"><div className="workspace-status-row"><div className="workspace-status-name"><StatusBadge status={lastRunStatus} /><strong>Latest run</strong></div><span className="meta">{dateLabel(schedulerData?.lastRun?.endedAt)} · {lastRunDuration}</span></div><div className="workspace-status-row"><div className="workspace-status-name"><OperationalStateBadge state="CURRENT" /><strong>Next run</strong></div><span className="meta">{dateLabel(schedulerData?.nextRunAt)}</span></div></div><div className="workspace-panel-footer"><ShieldCheck size={14} /> <span className="meta">{schedulerData?.launchMechanism ?? 'Scheduler source unavailable'} · read-only</span></div></section>
