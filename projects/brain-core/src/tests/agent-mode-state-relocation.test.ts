...
test('relocation readiness is evidence-backed and legacy caller booleans cannot bypass blockers', () => {
  ...
  try {
    mkdirSync(path.join(sourceRoot, 'projects', 'brain-core', 'dist'), { recursive: true }); mkdirSync(path.join(sourceRoot, 'projects', 'brain-console', '.next', 'standalone'), { recursive: true });
