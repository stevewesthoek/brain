...
test('lists only persisted decisions unmatched by visible or suppressed latest-report findings', async () => {
  ...
  assert.equal(
    (output as { matchedDecisionCount: number }).matchedDecisionCount,
    2,
  );
...
test('reports partial, full, and zero-decision coverage consistently', async () => {
  const createDecision = (findingId: string, deduplicationKey: string, index: number) => ({
    findingId,
    deduplicationKey,
    sourceReportId: 'report-coverage',
    sourceCommit: 'abc1234',
    reviewedBy: 'Steve Westhoek',
    reviewedAt: `2026-06-15T10:${String(index).padStart(2, '0')}:00.000Z`,
    decision: 'accepted' as const,
  ...
  const scenarios = [
    {
      name: 'partial',
      decisions: [
        createDecision('partial-a', 'partial:a', 0),
        createDecision('partial-b', 'partial:b', 1),
        createDecision('partial-c', 'partial:c', 2),
      ],
      visible: ['partial:a'],
      suppressed: ['partial:b'],
      expected: { total: 3, matched: 2, unmatched: 1, matchedPercent: 66.67 },
    },
    {
      name: 'full',
      decisions: [
        createDecision('full-a', 'full:a', 3),
        createDecision('full-b', 'full:b', 4),
      ],
      visible: ['full:a'],
      suppressed: ['full:b'],
      expected: { total: 2, matched: 2, unmatched: 0, matchedPercent: 100 },
    },
    {
      name: 'zero',
      decisions: [],
      visible: [],
      suppressed: [],
      expected: { total: 0, matched: 0, unmatched: 0, matchedPercent: 100 },
    },
  ] as const;
  ...
  for (const scenario of scenarios) {
    ...
    const result = await runMindMaintenancePilotCli(
      ...
      createDependencies({
        ...
        loadLatestReport: async () => ({
          reportId: `mind-maintenance-${scenario.name}`,
          findings: scenario.visible.map((deduplicationKey) => ({ deduplicationKey })),
          suppressedFindings: scenario.suppressed.map((deduplicationKey) => ({ deduplicationKey })),
        } as never),
      }),
    );
    ...
    assert.deepEqual(output.decisionCoverage, scenario.expected, scenario.name);
    assert.equal(output.matchedDecisionCount, scenario.expected.matched, scenario.name);
    assert.equal(output.unmatchedDecisionCount, scenario.expected.unmatched, scenario.name);
  }
});

test('compiled CLI reports decision coverage without changing either input file', async (context) => {
...
`,
  ...
  const decisionBefore = `${JSON.stringify({
    ...
    decisions: [
      ...
      {
        findingId: 'coverage-unmatched',
        deduplicationKey: 'historical:coverage',
        sourceReportId: 'older-report',
        sourceCommit: 'def5678',
        reviewedBy: 'Steve Westhoek',
        reviewedAt: '2026-06-15T10:20:00.000Z',
        decision: 'accepted',
        reason: 'Historical finding.',
  ...
  assert.deepEqual(output.decisionCoverage, {
    total: 2,
    matched: 1,
    unmatched: 1,
    matchedPercent: 50,
...
test('renders exact empty, complete, and partial decision coverage actions', async () => {
  const decision = (findingId: string, deduplicationKey: string, minute: string) => ({
    findingId,
    deduplicationKey,
    sourceReportId: 'action-report',
    sourceCommit: 'abc1234',
    reviewedBy: 'Steve Westhoek',
    reviewedAt: `2026-06-15T13:${minute}:00.000Z`,
    decision: 'accepted' as const,
    reason: 'Coverage action fixture.',
    nextAction: 'Review the finding.',
    resolutionRef: null,
    suppressionUntil: null,
  });
  const scenarios = [
    {
      ...
      findings: [],
      suppressedFindings: [],
      expected: 'No coverage action required; there are no persisted decisions.',
    },
    {
      name: 'complete',
      decisions: [decision('complete-action', 'action:complete', '01')],
      findings: [{ deduplicationKey: 'action:complete' }],
      suppressedFindings: [],
      expected: 'No coverage action required; all persisted decisions match the latest report.',
    },
    {
      name: 'partial',
      decisions: [
        decision('partial-action-a', 'action:partial:a', '02'),
        decision('partial-action-b', 'action:partial:b', '03'),
      ],
      findings: [{ deduplicationKey: 'action:partial:a' }],
      suppressedFindings: [],
      expected: 'Review unmatched persisted decisions against the latest report.',
    },
  ] as const;
  ...
  for (const scenario of scenarios) {
    ...
    const result = await runMindMaintenancePilotCli(
      ...
      createDependencies({
        ...
        loadLatestReport: async () => ({
          reportId: `mind-maintenance-action-${scenario.name}`,
          findings: [...scenario.findings],
          suppressedFindings: [...scenario.suppressedFindings],
        } as never),
      }),
    ...
    assert.equal(captured.stderr.length, 0, scenario.name);
    const output = JSON.parse(captured.stdout.join('')) as {
      decisionCoverageAction: string;
    };
    assert.equal(output.decisionCoverageAction, scenario.expected, scenario.name);
  }
});

test('compiled CLI renders the exact partial decision coverage action without modifying inputs', async (context) => {
...
`,
  ...
  const decisionBefore = `${JSON.stringify({
    ...
    decisions: [
      {
        findingId: 'action-matched',
        deduplicationKey: matchedKey,
        sourceReportId: report.reportId,
        sourceCommit: 'abc1234',
        reviewedBy: 'Steve Westhoek',
        reviewedAt: '2026-06-15T13:10:00.000Z',
        decision: 'accepted',
        reason: 'Current finding.',
        nextAction: 'Review it.',
        resolutionRef: null,
      ...
      {
        findingId: 'action-unmatched',
        deduplicationKey: 'historical:action',
        sourceReportId: 'older-report',
        sourceCommit: 'def5678',
        reviewedBy: 'Steve Westhoek',
        reviewedAt: '2026-06-15T13:20:00.000Z',
        decision: 'accepted',
        reason: 'Historical finding.',
        nextAction: 'Review whether it remains relevant.',
        resolutionRef: null,
