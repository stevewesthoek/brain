...
import { assertPackagedConsoleBoot, buildRuntimePackage, verifyRuntimePackage } from '../agent-mode/runtime-package.js';
...
import { createReleaseManifest, createReleaseManifestWithSigner, readReleaseManifest, retainVerifiedRelease, verifyReleaseManifest, writeReleaseManifest } from '../agent-mode/release-maintenance.js';
...
import { resolveBrainServiceIdentity } from '../agent-mode/service-identity.js';
import { runInteractiveTerminalSession } from '../agent-mode/interactive-terminal-session.js';
import { startTerminalSubmissionFeedback, type TerminalSubmissionFeedback } from '../agent-mode/terminal-console.js';
import type { TerminalExecutionStatus } from '../agent-mode/terminal-intake.js';
import { JARVIS_CONTEXT_INTAKE_SCHEMA_VERSION } from '../agent-mode/jarvis-context-intake.js';
import type { JarvisContextRequestV1 } from '../agent-mode/jarvis-local-context.js';
...
async function main(): Promise<void> {
  ...
  if (command === 'video') {
    ...
    for (let index = 5; index < process.argv.length; index += 1) {
      const argument = process.argv[index];
      if (argument === '--save-to-mind') {
        persistToMind = true;
      } else if (argument === '--focus') {
        focus = process.argv[index + 1];
        index += 1;
    ...
    try {
      const result = await analyzeVideo({ source, caller, allow_local_file: true, persist_to_mind: persistToMind, ...(focus ? { focus } : {}) });
