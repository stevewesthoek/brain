...
export type BrainRuntimeConfig = {
  ...
  eligibleLocalRoots: string[];
  writableLocalRoots: string[];
};
...
function repositoryRoots(value: unknown, label: string, home: string): string[] {
  ...
  return value.map((entry, index) => {
    const resolved = portablePath(entry, `${label}[${index}]`, home);
    if (resolved === home || resolved === path.parse(resolved).root) throw new Error(`${label}[${index}] is too broad`);
    return resolved;
  });
}

function localRoots(value: unknown, label: string, home: string, allowHome: boolean): string[] {
  if (!Array.isArray(value) || value.length > 32) throw new Error(`${label} must be an array of at most 32 paths`);
  return value.map((entry, index) => {
    const resolved = portablePath(entry, `${label}[${index}]`, home);
    if ((!allowHome && resolved === home) || resolved === path.parse(resolved).root) throw new Error(`${label}[${index}] is too broad`);
    return resolved;
  });
}
...
function parseLayer(value: unknown, label: string, home: string): BrainRuntimeConfigLayer {
  ...
  rejectUnknownKeys(input, ['schemaVersion', 'profile', 'deploymentMode', 'stateRoot', 'stateStore', 'core', 'console', 'node', 'execution', 'optionalCapabilities', 'providerRefs', 'resourceRefs', 'repositoryRoots', 'eligibleLocalRoots', 'writableLocalRoots'], label);
  ...
  return {
    ...
    ...(input.eligibleLocalRoots === undefined ? {} : { eligibleLocalRoots: localRoots(input.eligibleLocalRoots, `${label}.eligibleLocalRoots`, home, true) }),
    ...(input.writableLocalRoots === undefined ? {} : { writableLocalRoots: localRoots(input.writableLocalRoots, `${label}.writableLocalRoots`, home, false) }),
  };
}
...
function defaults(home: string): BrainRuntimeConfig {
  ...
  return {
    ...
    providerRefs: [],
    resourceRefs: [],
    repositoryRoots: [path.join(home, 'Repos')],
    eligibleLocalRoots: [home],
    writableLocalRoots: [],
  };
}
...
export function loadBrainRuntimeConfig(input: BrainRuntimeConfigLoadInput = {}): BrainRuntimeConfig {
  ...
  const merged = {
    ...
    eligibleLocalRoots: effective.eligibleLocalRoots ?? derivedBase.eligibleLocalRoots,
    writableLocalRoots: effective.writableLocalRoots ?? derivedBase.writableLocalRoots,
