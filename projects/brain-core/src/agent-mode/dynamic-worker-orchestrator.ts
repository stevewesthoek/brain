...
import {
  ...
  SPAWN_POLICY_CONTEXT_READ_ONLY,
  SPAWN_ROLE_READ_ONLY,
  SPAWN_ROLE_CONTEXT_READ_ONLY,
...
import {
  ...
  MODEL_GATEWAY_RUNTIME_PROFILE_REF,
  MODEL_GATEWAY_RUNTIME_REF,
  CLAUDE_CODE_RUNTIME_PROFILE_REF,
  CLAUDE_CODE_RUNTIME_REF,
  assignmentIntentKey,
...
import { AGENT_MODE_MODEL_ROUTES } from './model-gateway.js';
import type {
...
export const TERMINAL_INTAKE_AUTO_TASK_SPEC_REF = 'task-spec:agent-mode-terminal-auto-read-only-v1' as const;
export const TERMINAL_INTAKE_CONTROLLER_REF = 'controller:agent-mode-terminal-intake-v1' as const;
export const JARVIS_CONTEXT_ACTION_RULE_ID = 'agent-mode.action.jarvis-context-codex-read-only.v1' as const;
export const JARVIS_CONTEXT_TASK_SPEC_REF = 'task-spec:agent-mode-jarvis-context-read-only-v2' as const;
export const JARVIS_CONTEXT_AUTO_TASK_SPEC_REF = 'task-spec:agent-mode-jarvis-context-auto-read-only-v2' as const;
export const JARVIS_CONTEXT_CONTROLLER_REF = 'controller:agent-mode-jarvis-context-v2' as const;
...
export type SchedulerEventActionRule = {
  ...
  scopeMode: 'event.repository' | 'event.context' | 'none';
};
...
/** Core-owned v2 context execution rule; injected by the typed context intake service. */
export const JARVIS_CONTEXT_ACTION_RULE: SchedulerEventActionRule = Object.freeze({
  ruleId: JARVIS_CONTEXT_ACTION_RULE_ID,
  version: 1,
  enabled: true,
  sourceType: BRAIN_JARVIS_CONTEXT_SOURCE,
  eventType: JARVIS_CONTEXT_READY_EVENT,
  spawnPolicyId: SPAWN_POLICY_CONTEXT_READ_ONLY,
  spawnPolicyVersion: 1,
  roleTemplateId: SPAWN_ROLE_CONTEXT_READ_ONLY,
  roleTemplateVersion: 1,
  runtimeRef: CODEX_CLI_RUNTIME_REF,
  runtimeProfileRef: CODEX_CLI_RUNTIME_PROFILE_REF,
  taskSpecRef: JARVIS_CONTEXT_TASK_SPEC_REF,
  requestedTtl: 10 * 60 * 1000,
  requestedSteps: 1,
  requestedCost: 0,
  requestedTokens: 4_000,
  requestedCapabilities: ['repo.read'],
  scopeMode: 'event.context',
});
...
function validRule(rule: SchedulerEventActionRule, policies: readonly AgentSpawnPolicy[], templates: readonly AgentRoleTemplate[]): string | undefined {
  ...
  const isModelGateway = rule.runtimeRef === MODEL_GATEWAY_RUNTIME_REF && rule.runtimeProfileRef === MODEL_GATEWAY_RUNTIME_PROFILE_REF;
  const isClaudeCode = rule.runtimeRef === CLAUDE_CODE_RUNTIME_REF && rule.runtimeProfileRef === CLAUDE_CODE_RUNTIME_PROFILE_REF;
  if (!isMock && !isHarness && !isCodex && !isModelGateway && !isClaudeCode) return 'RULE_RUNTIME_NOT_ALLOWED';
  if (rule.modelRef !== undefined && (!(rule.modelRef in AGENT_MODE_MODEL_ROUTES) || !SAFE_REF.test(rule.modelRef))) return 'RULE_MODEL_NOT_ALLOWED';
  if ((isHarness && rule.requestedCapabilities.length !== 0) || (isMock && ![E1_FIXTURE_TASK_SPEC_REF, TERMINAL_INTAKE_AUTO_TASK_SPEC_REF, JARVIS_CONTEXT_AUTO_TASK_SPEC_REF].includes(rule.taskSpecRef as typeof E1_FIXTURE_TASK_SPEC_REF | typeof TERMINAL_INTAKE_AUTO_TASK_SPEC_REF | typeof JARVIS_CONTEXT_AUTO_TASK_SPEC_REF)) || (isHarness && ![E2_FIXTURE_TASK_SPEC_REF, K43A_LIVE_MINIMAX_TASK_SPEC_REF].includes(rule.taskSpecRef as typeof E2_FIXTURE_TASK_SPEC_REF | typeof K43A_LIVE_MINIMAX_TASK_SPEC_REF)) || ((isCodex || isModelGateway || isClaudeCode) && ![TERMINAL_INTAKE_TASK_SPEC_REF, TERMINAL_INTAKE_AUTO_TASK_SPEC_REF, JARVIS_CONTEXT_TASK_SPEC_REF, JARVIS_CONTEXT_AUTO_TASK_SPEC_REF].includes(rule.taskSpecRef as typeof TERMINAL_INTAKE_TASK_SPEC_REF | typeof TERMINAL_INTAKE_AUTO_TASK_SPEC_REF | typeof JARVIS_CONTEXT_TASK_SPEC_REF | typeof JARVIS_CONTEXT_AUTO_TASK_SPEC_REF)) || !SAFE_REF.test(rule.taskSpecRef) || rule.taskSpecRef.length > MAX_TASK_SPEC_LENGTH) return 'RULE_TASK_SPEC_INVALID';
  ...
  if (rule.scopeMode !== 'event.repository' && rule.scopeMode !== 'event.context' && rule.scopeMode !== 'none') return 'RULE_SCOPE_MODE_INVALID';
  ...
  if (template.allowsChildSpawning || ![SPAWN_ROLE_READ_ONLY, SPAWN_ROLE_CONTEXT_READ_ONLY].includes(rule.roleTemplateId as typeof SPAWN_ROLE_READ_ONLY | typeof SPAWN_ROLE_CONTEXT_READ_ONLY)) return 'RULE_ROLE_INCOMPATIBLE';
  if (rule.scopeMode === 'event.repository' && template.repositoryScope !== 'requested') return 'RULE_SCOPE_INCOMPATIBLE';
  if (rule.scopeMode === 'event.context' && (template.repositoryScope !== 'none' || template.resourceScope !== 'requested')) return 'RULE_SCOPE_INCOMPATIBLE';
...
function deriveLifecycle(store: AgentModeSqliteStateStore, event: AgentModeSchedulerEvent, rule: SchedulerEventActionRule, facts: SpawnAuthorityFacts, now: string): DerivedLifecycle | undefined {
  ...
  // Jarvis context events share one stable source identity. Their admitted
  // context set remains event-scoped and is carried in the structured payload,
  // so source registration no longer has to encode per-root resource scope.
  const eventContextRef = typeof event.payload.contextSetId === 'string' ? event.payload.contextSetId : source.repositoryRef;
  const resourceRef = rule.scopeMode === 'event.context' ? eventContextRef : null;
  ...
  // A context expansion can arrive after the root event was accepted. Bound
  // the child TTL to the root's remaining deadline so a valid expansion is not
  // rejected merely because the original ten-minute root window has started.
  // K4 still enforces the resulting deadline and all other mutable authority.
  const remainingRootTtl = Math.max(1, Date.parse(facts.root.deadline) - Date.parse(requestedAt));
  const requestedTtl = Math.min(rule.requestedTtl, remainingRootTtl);
  ...
  const request: SpawnRequest = {
    ...
    eventScope: { repositoryRef, resourceRef },
    parentAgentId: null,
    parentTaskId: null,
    parentRunId: null,
    rootGoalId,
    requestedScope: { repositoryRef, resourceRef },
    ...
    requestedAt,
    deadline: toDeadline(requestedAt, requestedTtl, event.deadline),
  ...
  const assignmentRequest: AgentModeChildAssignmentRequest = {
    ...
    ...(rule.modelRef === undefined ? {} : { modelRef: rule.modelRef }),
