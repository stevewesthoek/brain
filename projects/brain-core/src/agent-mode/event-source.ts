...
/** Internal Brain-owned Jarvis context causation; it is not an external adapter. */
export const BRAIN_JARVIS_CONTEXT_SOURCE = 'brain.jarvis.context' as const;
/** One stable internal source owns all Jarvis context causation events. */
export const JARVIS_CONTEXT_SOURCE_ID = 'source:jarvis-context:canonical:v1' as const;
export const JARVIS_CONTEXT_READY_EVENT = 'jarvis.context.ready' as const;
...
export async function pollEventSourcesOnce(options: { store: AgentModeSqliteStateStore; adapters: readonly EventSourceAdapter<GitRepositoryCommitEvent | BrainTaskLifecycleEvent | HostHealthTransitionEvent | CiWorkflowSemanticEvent>[]; now?: string; clock?: () => string; maxSources?: number; observationTimeoutMs?: number }): Promise<EventSourcePollResult> {
  ...
  // Jarvis context events are internal causation records created by the
  // authenticated intake service; they are not an external source to poll.
  const pollable = options.store.listEventSources().filter((source) => source.enabled && source.sourceType !== BRAIN_JARVIS_CONTEXT_SOURCE);
  const configured = pollable.slice(0, maxSources);
  const sources: AgentModeEventSourceObservationResult[] = [];
  let deferred = Math.max(0, pollable.length - configured.length);
