...
export const BRAIN_SERVICE_STATE_STORE_SCHEMA_VERSION = 11 as const;
...
function terminalSecretPath(descriptor: LaunchdDescriptor | undefined): string | undefined {
  const environmentPath = descriptor?.environmentVariables.BRAIN_SECRETS_FILE;
  if (environmentPath) return environmentPath;
  const envFileIndex = descriptor?.programArguments.indexOf('--env-file') ?? -1;
  return envFileIndex >= 0 ? descriptor?.programArguments[envFileIndex + 1] : undefined;
}

function checkTerminalReadiness(checks: ServiceDoctorCheck[], expected: ServiceDoctorExpected, observed: ServiceDoctorObserved): void {
  if (!expected.terminalReadiness) return;

  const secretPath = terminalSecretPath(observed.coreDescriptor);
  let secretPrivate = false;
  if (secretPath) {
    try {
      const mode = statSync(secretPath).mode & 0o777;
      secretPrivate = (mode & 0o077) === 0;
    } catch {
      secretPrivate = false;
    }
  }
  checks.push({ id: 'terminal.secret-file', status: secretPrivate ? 'pass' : 'fail', detail: secretPrivate ? 'external service secret file exists with private permissions' : 'external service secret file is missing or permissions are too broad' });

  let repositoryPolicyLoaded = false;
  try {
    const config = JSON.parse(readFileSync(expected.configPath ?? '', 'utf8')) as { repositoryRoots?: unknown };
    repositoryPolicyLoaded = Array.isArray(config.repositoryRoots) && config.repositoryRoots.length > 0 && config.repositoryRoots.length <= 16 && config.repositoryRoots.every((root) => typeof root === 'string' && root.length > 0);
  } catch {
    repositoryPolicyLoaded = false;
  }
  checks.push({ id: 'terminal.repository-policy', status: repositoryPolicyLoaded ? 'pass' : 'fail', detail: repositoryPolicyLoaded ? 'bounded repository admission roots are configured' : 'repository admission roots are missing or invalid' });

  const codexRuntimePath = path.join(expected.runtimeRoot, 'core', 'dist', 'agent-mode', 'codex-cli-agent-runtime.js');
  checks.push({ id: 'terminal.codex-runtime', status: existsSync(codexRuntimePath) ? 'pass' : 'fail', detail: existsSync(codexRuntimePath) ? 'packaged Codex read-only runtime is present' : 'packaged Codex read-only runtime is missing' });
}
...
export function evaluateServiceDoctor(expected: ServiceDoctorExpected, observed: ServiceDoctorObserved): ServiceDoctorResult {
  ...
  checks.push({ id: 'state-store', status: observed.store.exists && observed.store.schemaVersion === BRAIN_SERVICE_STATE_STORE_SCHEMA_VERSION && observed.store.integrity === 'ok' && observed.store.foreignKeyErrors === 0 ? 'pass' : 'fail', detail: observed.store.exists && observed.store.schemaVersion === BRAIN_SERVICE_STATE_STORE_SCHEMA_VERSION && observed.store.integrity === 'ok' && observed.store.foreignKeyErrors === 0 ? 'schema 11, integrity clean, foreign keys clean' : 'StateStore is missing or failed schema/integrity/foreign-key checks' });
  checkTerminalReadiness(checks, expected, observed);
