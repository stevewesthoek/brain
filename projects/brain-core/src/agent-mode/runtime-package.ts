import { chmodSync, cpSync, existsSync, lstatSync, mkdirSync, mkdtempSync, readdirSync, readFileSync, rmSync, statSync, writeFileSync } from 'node:fs';
...
const SECRET_PATH_SUFFIX = ['Library', 'Application Support', 'Brain', 'agent-mode', 'config', 'secrets.env'].join('/');
const FORBIDDEN_SECRET_CONTENT_PATTERN = new RegExp([`${['brain', 'package', 'root'].join('-')}/${SECRET_PATH_SUFFIX}`, `${['', 'Users', 'Office'].join('/')}/${SECRET_PATH_SUFFIX}`].join('|'), 'iu');
...
function requiredArtifacts(sourceRoot: string): { core: string; console: string; staticAssets: string | null; publicAssets: string | null } {
  ...
  if (!existsSync(path.join(console, 'node_modules', 'next', 'package.json'))) throw new Error('Console standalone dependency closure is incomplete: next/package.json is missing');
...
export function buildRuntimePackage(input: RuntimePackageInput): BrainRuntimePackage {
  ...
  try {
    ...
    if (artifacts.staticAssets) copyTree(artifacts.staticAssets, output.root, 'brain-console', 'static-asset', files, 'console/standalone/.next/static', () => true, sourceRoot);
    if (artifacts.publicAssets) copyTree(artifacts.publicAssets, output.root, 'brain-console', 'static-asset', files, 'console/standalone/public', () => true, sourceRoot);
    const configPath = path.join(output.root, 'config', 'brain-runtime-config.example.json');
    mkdirSync(path.dirname(configPath), { recursive: true });
    writeFileSync(configPath, `${JSON.stringify({ schemaVersion: 'brain-runtime-config-v1', profile: 'core', deploymentMode: 'server', stateRoot: '~/state', stateStore: { kind: 'sqlite', path: '~/state/agent-mode/agent-mode.db' }, core: { bindHost: '127.0.0.1', port: 4877 }, console: { coreUrl: 'http://127.0.0.1:4877', port: 4881 }, execution: { codexCliPath: null }, optionalCapabilities: { voiceStt: 'unavailable', voiceTts: 'client-capability', modelBedrock: 'unavailable', nodeLocal: 'unavailable', workcells: 'unavailable' }, providerRefs: [], resourceRefs: [], repositoryRoots: ['~/Repos'] }, null, 2)}\n`);
...
export function verifyRuntimePackage(packageRoot: string): RuntimePackageVerification {
  ...
  try {
    ...
    for (const file of manifest.files) {
      ...
      if (PERSONAL_PATH_PATTERN.test(content)) return { ok: false, reason: 'forbidden-content', detail: `personal path content: ${relative}` };
      if (FORBIDDEN_SECRET_CONTENT_PATTERN.test(content)) return { ok: false, reason: 'forbidden-content', detail: `secret path or fixture secret content: ${relative}` };
...
/**
 * Boots the packaged Console in an isolated process before release signing.
 *
 * This deliberately does not use the source checkout, NODE_PATH, or the
 * caller's HOME. The package must prove that its standalone dependency
 * closure serves the operator route and at least one generated static asset.
 */
export function assertPackagedConsoleBoot(packageRoot: string, timeoutMs = 12_000): void {
  const root = absolutePath(packageRoot, 'packageRoot');
  const verification = verifyRuntimePackage(root);
  if (!verification.ok) throw new Error('package is not verified for Console boot: ' + verification.detail);
  if (!Number.isInteger(timeoutMs) || timeoutMs < 1_000 || timeoutMs > 30_000) throw new Error('Console boot timeout is outside the bounded range');
  const isolatedHome = mkdtempSync(path.join(tmpdir(), 'brain-console-package-smoke-'));
  const script = [
    "import { createServer } from 'node:net';",
    "import { spawn } from 'node:child_process';",
    'const packageRoot = process.env.BRAIN_PACKAGE_ROOT;',
    'const timeoutMs = Number(process.env.BRAIN_PACKAGE_SMOKE_TIMEOUT_MS);',
    'const probe = createServer();',
    "await new Promise((resolve, reject) => { probe.once('error', reject); probe.listen(0, '127.0.0.1', resolve); });",
    'const address = probe.address();',
    "if (!address || typeof address === 'string') throw new Error('probe did not receive a TCP port');",
    'const port = address.port;',
    'await new Promise((resolve) => probe.close(resolve));',
    "const child = spawn(process.execPath, ['console/standalone/server.js'], {",
    '  cwd: packageRoot,',
    "  env: { NODE_ENV: 'production', HOSTNAME: '127.0.0.1', PORT: String(port), HOME: process.env.BRAIN_PACKAGE_SMOKE_HOME ?? '', NODE_PATH: '' },",
    "  stdio: 'ignore',",
    '});',
    ...
    "        const asset = body.match(/\\/_next\\/static\\/[^\"'\\s<]+/u)?.[0];",
    '        if (asset) {',
    "          const assetResponse = await fetch('http://127.0.0.1:' + port + asset);",
    "          if (!assetResponse.ok) throw new Error('generated static asset was not served');",
    '        }',
    '        success = true;',
    '        break;',
    '      }',
  ...
  try {
    execFileSync(process.execPath, ['--input-type=module', '--eval', script], {
      cwd: root,
      env: { BRAIN_PACKAGE_ROOT: root, BRAIN_PACKAGE_SMOKE_HOME: isolatedHome, BRAIN_PACKAGE_SMOKE_TIMEOUT_MS: String(timeoutMs) },
      stdio: ['ignore', 'ignore', 'ignore'],
      timeout: timeoutMs + 5_000,
    });
  } catch {
    throw new Error('packaged Console standalone boot gate failed');
  } finally {
