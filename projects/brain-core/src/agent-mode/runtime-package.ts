...
export function buildRuntimePackage(input: RuntimePackageInput): BrainRuntimePackage {
  ...
  try {
    ...
    writeFileSync(configPath, `${JSON.stringify({ schemaVersion: 'brain-runtime-config-v1', profile: 'core', deploymentMode: 'server', stateRoot: '~/state', stateStore: { kind: 'sqlite', path: '~/state/agent-mode/agent-mode.db' }, core: { bindHost: '127.0.0.1', port: 4877 }, console: { coreUrl: 'http://127.0.0.1:4877', port: 4881 }, optionalCapabilities: { voiceStt: 'unavailable', voiceTts: 'client-capability', modelBedrock: 'unavailable', nodeLocal: 'unavailable', workcells: 'unavailable' }, providerRefs: [], resourceRefs: [], repositoryRoots: [] }, null, 2)}\n`);
