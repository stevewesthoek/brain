...
export type AgentModeObserverProjection = {
  ...
  contextExpansions: Array<Record<string, unknown>>;
  attemptContextScopes: Array<Record<string, unknown>>;
...
export function readAgentModeObserver(now = new Date().toISOString(), databasePath = defaultAgentModeDatabasePath(), operatorId?: string): AgentModeObserverProjection {
  ...
  try {
    ...
    const contextExpansions: Array<Record<string, unknown>> = [];
    const attemptContextScopes: Array<Record<string, unknown>> = [];
    for (const intake of store.listJarvisIntakes(50)) {
      for (const context of store.listJarvisContexts(intake.rootGoalId).slice(0, 32)) contexts.push({ rootGoalId: intake.rootGoalId, contextId: context.contextId, kind: context.kind, requestedAccess: context.requestedAccess, admittedAccess: context.admittedAccess, recursive: context.recursive, origin: context.origin, admissionStatus: context.admissionStatus, admissionRef: context.admissionRef });
      for (const expansion of store.listJarvisContextExpansions(intake.rootGoalId).slice(0, 32)) contextExpansions.push({ rootGoalId: expansion.rootGoalId, expansionId: expansion.expansionId, admittedContextIds: expansion.admittedContextIds, reasonCode: expansion.reasonCode, causationRef: expansion.causationRef, createdAt: expansion.createdAt });
    }
    for (const attempt of store.listAttempts().slice(0, 100)) {
      const scope = store.getJarvisAttemptExecutionScope(attempt.attemptId);
      if (scope) attemptContextScopes.push({ scopeId: scope.scopeId, rootGoalId: scope.rootGoalId, attemptId: scope.attemptId, scopeDigest: scope.scopeDigest, contexts: scope.contexts.map((context) => ({ contextId: context.contextId, kind: context.kind, admittedAccess: context.admittedAccess, recursive: context.recursive })) });
    }
    for (const plan of store.listOrganizationPlans(16)) {
      ...
      for (const item of store.listOrganizationWorkItems(plan.organizationPlanId).slice(0, 16)) {
        const itemExecution = executionById.get(item.workItemId);
        organizationWorkItems.push({
          ...
          boundChildAgentId: item.boundChildAgentId, boundTaskId: item.boundTaskId, runId: itemExecution?.runId ?? null, attemptId: itemExecution?.attemptId ?? null,
          terminalStatus: itemExecution?.terminalStatus ?? null, resultRef: itemExecution?.resultRef ?? null, evidenceRefCount: itemExecution?.evidenceRefCount ?? 0,
          settledCost: itemExecution?.settledCost ?? 0,
        });
      }
    }
    ...
    return {
      ...
      agents, tasks, runs, attempts, events, recovery, operations, runtimeDispatches, modelOperations, workcells, workcellWrites, workcellValidations, workcellLeases, workcellDiffs, results, reviewRequests, reviewDecisions, targetRefLeases, commitOperations, mergeApprovals, mergeOperations, mergeReceipts, schedulerEvents, schedulerSchedules, sourceWatermarks, latestSchedulerTick, eventSources, hostHealthStates, ciWorkflowStates, spawnAdmissionControls, spawnRootStates, childAssignments, dynamicWorkerOrchestrations, organizationPlans, organizationWorkItems, organizationExecution, organizationFinalResults, contexts: contexts.slice(0, 100), contextExpansions: contextExpansions.slice(0, 100), attemptContextScopes: attemptContextScopes.slice(0, 100), controlAudits, escalations, notifications, attentionSummary,
    };
  } finally {
    store.close();
  }
