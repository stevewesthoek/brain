...
export function getVideoOrchestratorStatus(): BrainCoreVideoOrchestratorStatus {
  ...
  return {
    ...
    summary: `Video orchestrator ${percent}% complete. ${partial} modules designed; 0 modules live. Migration from STB planned for Stage 3.`,
    limitations: [
      'No live queue (no execution yet)',
      'No current task tracking (not running)',
      'No runtime metrics (design-phase only)',
      'Cannot compare STB vs video output (no test runs)',
      'Cannot publish to platforms (read-only view)',
      'Thumbnail design module blocked (design orchestrator not built)',
    ],
    actions: {
      canPreview: false,
      canRequestRun: false,
