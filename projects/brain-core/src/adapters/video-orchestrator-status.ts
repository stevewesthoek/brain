export interface BrainCoreVideoOrchestratorStatus {
  ...
  generationModeRuntime: 'fixture' | 'hybrid' | 'hybrid_tts' | 'hybrid_storyboard' | 'hybrid_slideshow' | 'hybrid_image_slideshow' | 'ai';
...
function getGenerationMode(): 'fixture' | 'hybrid' | 'hybrid_tts' | 'hybrid_storyboard' | 'hybrid_slideshow' | 'hybrid_image_slideshow' | 'ai' {
  const mode = process.env.AWS_VIDEO_GENERATION_MODE;
  if (mode === 'ai') return 'ai';
  if (mode === 'hybrid') return 'hybrid';
  if (mode === 'hybrid_tts' || mode === 'hybrid_storyboard' || mode === 'hybrid_slideshow' || mode === 'hybrid_image_slideshow') return mode;
  return 'fixture';
}
