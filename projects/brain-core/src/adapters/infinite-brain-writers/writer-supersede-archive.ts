/**
 * Infinite Brain Supersede/Archive Writer
 * Moves one explicitly approved existing Markdown file to one exact archive path.
 * No deletion, overwrite, folder move, glob, or autonomous destination selection.
 */
...
import type {
  InfiniteBrainSupersedeArchiveMoveInput,
  InfiniteBrainSupersedeArchiveMoveReport,
} from './types.js';

const MODULE_DIR = path.dirname(fileURLToPath(import.meta.url));
const BRAIN_ROOT = path.resolve(MODULE_DIR, '..', '..', '..', '..');
const DEFAULT_ROLLBACK_DIR = 'runtime/local/infinite-brain/supersede-archive-rollbacks';
const DEFAULT_REPORT_PATH = 'runtime/local/infinite-brain/supersede-archive-latest.json';
...
function makeId(prefix: string, input: InfiniteBrainSupersedeArchiveMoveInput, hash: string): string {
...
function persist(report: InfiniteBrainSupersedeArchiveMoveReport): InfiniteBrainSupersedeArchiveMoveReport {
  ...
  const reportPath = configuredPath('IBR_SUPERSEDE_ARCHIVE_REPORT_PATH', DEFAULT_REPORT_PATH);
...
function createReport(
  ...
  return persist({
    moveId: makeId('supersede-archive-move', input, sourceHash ?? status),
...
export function runSupersedeArchiveMove(
  input: InfiniteBrainSupersedeArchiveMoveInput,
): InfiniteBrainSupersedeArchiveMoveReport {
  const blockers: string[] = [];
  const sourcePath = normalizeExactMarkdownPath(input.sourcePath);
  const destinationPath = normalizeExactMarkdownPath(input.destinationPath, 'archive/');
  ...
  if (!input.contradictionSummary?.trim()) blockers.push('contradictionSummaryRequired');
  if (!input.supersessionReason?.trim()) blockers.push('supersessionReasonRequired');
  ...
  if (!sourcePath || sourcePath.startsWith('archive/')) blockers.push('invalidExactSourcePath');
  ...
  if (!destination) blockers.push('availableArchiveDestinationRequired');
  ...
  const rollbackId = makeId('supersede-archive-rollback', input, sourceHash);
  const rollbackPath = path.join(
    configuredPath('IBR_SUPERSEDE_ARCHIVE_ROLLBACK_DIR', DEFAULT_ROLLBACK_DIR),
    `${rollbackId}.json`,
  );
  ...
  try {
    ...
    return createReport(input, 'failed', ['archiveMoveFailed'], sourceHash, null, rollbackId, rollbackPath, []);
  }
  ...
  try {
    ...
    return createReport(input, 'failed', ['archiveVerificationReadFailed'], sourceHash, null, rollbackId, rollbackPath, [sourcePath, destinationPath]);
  }
  if (destinationHash !== sourceHash || fs.existsSync(source)) {
    return createReport(input, 'failed', ['archiveVerificationFailed'], sourceHash, destinationHash, rollbackId, rollbackPath, [sourcePath, destinationPath]);
  }
