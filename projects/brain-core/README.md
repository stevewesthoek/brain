...
Current `/execution/plans`, `/execution/plans/:kind`, and `/execution/readiness` expose a read-only execution-gate scaffold. The current candidates are `scheduler-run-mind-steward-dry-run`, `scheduler-run-mind-steward-inbox-dry-run`, and `scheduler-run-mind-steward-inbox-classifier-dry-run`, but Brain Core still reports `executionEnabled: false`, `wouldExecute: false`, and `executed: false`.
