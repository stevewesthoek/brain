...
Prompt-derived metadata with TTS narration, model-generated scene images, and a locally assembled slideshow MP4. Brain Core keeps the working FFmpeg slideshow assembly path, but swaps deterministic scene images for the configured image provider. Canonical thumbnail generation is automatic. When local FFmpeg motion generation succeeds, Brain Core also writes `metadata/motion-plan.json` and prefers motion clips during final assembly; otherwise it records a fallback and continues with the slideshow path.
...
```json
{
  ...
  "motionClipKeys": [
    "jobs/<jobId>/motion/scene-001.mp4"
  ],
  "overlayFrameKeys": [
  ...
  "warnings": ["Scene images are generated by an image model; final video prefers deterministic local motion clips and falls back to slideshow assembly when needed."]
}
```
