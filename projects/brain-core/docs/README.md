...
Brain Console calls Brain Core for AWS Video status and recent jobs. Never use `process.cwd()` to infer the jobs folder, never create jobs under `projects/brain-core/jobs`, and never create jobs under `projects/brain-console/jobs`.
