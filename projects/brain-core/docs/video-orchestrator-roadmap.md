...
- Read-only agent ledger and task graph snapshots are now exposed, and Brain Core can write and reload the snapshot JSON. The remaining gap is resumable task state, executor selection records, and approval-bound execution.
