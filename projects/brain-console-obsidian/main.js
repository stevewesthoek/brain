...
// Obsidian evaluates main.js from its renderer loader, so relative CommonJS
// requires are not available at runtime. Keep this small adapter self-contained
// and mirror the pure helpers in decision-center-core.cjs for contract tests.
...
function genericNotificationText(notification) {
  ...
  if (notification?.kind === 'daily-digest') return `Decision Center daily digest: ${pendingCount} pending item${pendingCount === 1 ? '' : 's'}.`;
  if (highCount > 0) return `Decision Center: ${pendingCount} pending, ${highCount} high-priority.`;
