...
export type RealUploadReadinessAssessmentMode = "real_upload_readiness_assessment_only" | "operator_review_readiness_assessment";
export type RealUploadReadinessAssessmentState = "draft" | "blocked" | "assessed" | "ready_for_operator_review" | "rejected" | "revoked";

export interface RealUploadReadinessRequiredArtifacts {
...
export interface RealUploadReadinessSummary {
  dry_run_upload_spike_passed: boolean;
  prior_artifacts_validated: boolean;
  blockers_remaining: boolean;
  ready_for_real_upload: false;
  ready_for_real_upload_execution_design_review: boolean;
  real_upload_requires_separate_explicit_phase: true;
  human_operator_required: true;
  credential_strategy_required: true;
  platform_api_strategy_required: true;
  network_execution_strategy_required: true;
  raw_media_path_strategy_required: true;
  rollback_strategy_required: true;
}

export interface RealUploadRemainingGate {
  gate_id: string;
  gate_kind: string;
  gate_state: "blocked" | "required" | "deferred" | "satisfied";
  safe_summary: string;
  blocking_reasons: string[];
  warnings: string[];
}

export interface RealUploadExecutionBoundary {
  upload_requested: false;
  upload_allowed: false;
  upload_execution_enabled: false;
  real_upload_execution_allowed: false;
  platform_api_calls_allowed: false;
  network_calls_allowed: false;
  resumable_upload_allowed: false;
  direct_upload_allowed: false;
...
export interface RealUploadReadinessCredentialBoundary {
  credentials_required: false;
  credentials_accessed: false;
  token_accessed: false;
  keychain_accessed: false;
...
export interface RealUploadReadinessNetworkBoundary {
  network_calls_allowed: false;
  network_calls_made: false;
  platform_api_calls_allowed: false;
  platform_api_calls_made: false;
...
export interface RealUploadReadinessPlatformBoundary {
  platform_endpoint_selected: false;
  platform_api_payload_created: false;
  platform_api_payload_stored: false;
  raw_account_ids_stored: false;
...
export interface RealUploadReadinessAssessmentValidationResult {
  ok: boolean;
  blocking_reasons: string[];
  warnings: string[];
}

export interface RealUploadReadinessAssessment {
  schema_version: "1.0";
  ...
  project_id: string;
  platform: string;
  assessment_state: RealUploadReadinessAssessmentState;
  created_at: string;
  assessment_mode: RealUploadReadinessAssessmentMode;
  required_artifacts: RealUploadReadinessRequiredArtifacts;
  readiness_summary: RealUploadReadinessSummary;
  remaining_gates: RealUploadRemainingGate[];
  upload_execution_boundary: RealUploadExecutionBoundary;
  credential_boundary: RealUploadReadinessCredentialBoundary;
  network_boundary: RealUploadReadinessNetworkBoundary;
  platform_boundary: RealUploadReadinessPlatformBoundary;
  validation: {
    real_upload_readiness_assessed: boolean;
    ready_for_real_upload: false;
    ready_for_real_upload_execution_design_review: boolean;
    upload_allowed: false;
    upload_execution_enabled: false;
  ...
  provenance: {
    generated_by: "createRealUploadReadinessAssessment" | "revokeRealUploadReadinessAssessment";
...
interface RealUploadReadinessAssessmentsStore {
  schema_version: "1.0";
  created_at: string;
  assessments: RealUploadReadinessAssessment[];
}

function getRealUploadReadinessAssessmentsPath(): string {
  return path.join(getRuntimeDir(), "real-upload-readiness-assessments.json");
}

function loadRealUploadReadinessAssessmentsStore(): RealUploadReadinessAssessmentsStore {
  try {
    const filePath = getRealUploadReadinessAssessmentsPath();
    if (fs.existsSync(filePath)) return JSON.parse(fs.readFileSync(filePath, "utf8")) as RealUploadReadinessAssessmentsStore;
  } catch {
    // start fresh
  }
  return { schema_version: "1.0", created_at: new Date().toISOString(), assessments: [] };
}

function saveRealUploadReadinessAssessmentsStore(store: RealUploadReadinessAssessmentsStore): void {
  const filePath = getRealUploadReadinessAssessmentsPath();
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  store.assessments.sort((a, b) => {
    const compare = new Date(a.created_at).getTime() - new Date(b.created_at).getTime();
    return compare !== 0 ? compare : a.real_upload_readiness_assessment_id.localeCompare(b.real_upload_readiness_assessment_id);
  });
  fs.writeFileSync(filePath, JSON.stringify(store, null, 2), "utf8");
}

function safeRealUploadReadinessString(value: unknown, fallback: string): string {
  return safeUploadExecutionDesignString(value, fallback);
}

function validateRealUploadReadinessAssessmentShape(assessment: unknown): RealUploadReadinessAssessmentValidationResult {
  const blocking_reasons: string[] = [];
  const warnings: string[] = [];
  if (typeof assessment !== "object" || assessment === null) return { ok: false, blocking_reasons: ["Real upload readiness assessment must be an object"], warnings };
  const a = assessment as Record<string, unknown>;
  const required = ["schema_version","real_upload_readiness_assessment_id","dry_run_upload_spike_result_id","upload_execution_design_id","upload_execution_approval_id","platform_upload_request_id","upload_package_design_id","local_output_review_id","production_render_spike_result_id","final_render_execution_request_id","render_plan_id","project_id","platform","assessment_state","created_at","assessment_mode","required_artifacts","readiness_summary","remaining_gates","upload_execution_boundary","credential_boundary","network_boundary","platform_boundary","validation","provenance"];
  for (const key of required) if (!(key in a)) blocking_reasons.push("Real upload readiness assessment is missing a required field");
  if (a.schema_version !== "1.0") blocking_reasons.push("schema_version must be 1.0");
  if (!["real_upload_readiness_assessment_only","operator_review_readiness_assessment"].includes(String(a.assessment_mode))) blocking_reasons.push("assessment_mode is invalid");
  if (!["draft","blocked","assessed","ready_for_operator_review","rejected","revoked"].includes(String(a.assessment_state))) blocking_reasons.push("assessment_state is invalid");
  const text = JSON.stringify(assessment);
  if (text.includes("videos.insert") || text.includes("youtube.videos().insert") || text.includes("fetch(") || text.includes("process.env") || text.includes("keychain://") || text.includes("stdout") || text.includes("stderr") || text.includes("ffmpeg -i") || text.includes("Bearer ") || text.includes("data:") || text.includes("base64,") || text.includes("/Users/") || text.includes("https://") || text.includes("http://") || text.includes("../")) blocking_reasons.push("Real upload readiness assessment contains forbidden payload content");
  const requiredArtifacts = a.required_artifacts as Record<string, unknown> | undefined;
  if (!requiredArtifacts || requiredArtifacts.dry_run_upload_spike_result_validated !== true || requiredArtifacts.upload_execution_design_validated !== true || requiredArtifacts.upload_execution_approval_validated !== true || requiredArtifacts.platform_upload_request_validated !== true || requiredArtifacts.upload_package_design_validated !== true || requiredArtifacts.local_output_review_validated !== true || requiredArtifacts.spike_result_validated !== true) blocking_reasons.push("Required artifacts are unsafe");
  const summary = a.readiness_summary as Record<string, unknown> | undefined;
  if (!summary || summary.dry_run_upload_spike_passed !== false && summary.dry_run_upload_spike_passed !== true || summary.prior_artifacts_validated !== false && summary.prior_artifacts_validated !== true || summary.blockers_remaining !== true || summary.ready_for_real_upload !== false || summary.ready_for_real_upload_execution_design_review !== false && summary.ready_for_real_upload_execution_design_review !== true || summary.real_upload_requires_separate_explicit_phase !== true || summary.human_operator_required !== true || summary.credential_strategy_required !== true || summary.platform_api_strategy_required !== true || summary.network_execution_strategy_required !== true || summary.raw_media_path_strategy_required !== true || summary.rollback_strategy_required !== true) blocking_reasons.push("Readiness summary is unsafe");
  const gates = a.remaining_gates as Record<string, unknown>[] | undefined;
  const requiredGateKinds = new Set([
    "explicit_real_upload_execution_phase_approval",
    "credential_access_strategy",
    "token_keychain_env_handling_strategy",
    "platform_api_endpoint_payload_strategy",
    "network_call_strategy",
    "raw_media_path_access_strategy",
    "upload_retry_rollback_idempotency_strategy",
    "post_upload_verification_strategy",
    "failure_handling_strategy",
    "manual_operator_confirmation",
  ]);
  if (!Array.isArray(gates) || gates.length < requiredGateKinds.size) blocking_reasons.push("Remaining gates are unsafe");
  else {
    const seen = new Set<string>();
    for (const gate of gates) {
      if (!gate || typeof gate.gate_id !== "string" || typeof gate.gate_kind !== "string" || !["blocked","required","deferred","satisfied"].includes(String(gate.gate_state)) || typeof gate.safe_summary !== "string" || !Array.isArray(gate.blocking_reasons) || !Array.isArray(gate.warnings)) {
        blocking_reasons.push("Remaining gate is unsafe");
        continue;
      }
      seen.add(String(gate.gate_kind));
      const gateSummaryLower = gate.safe_summary.toLowerCase();
      if (gate.safe_summary.length === 0 || gate.safe_summary.length > 160 || gateSummaryLower.includes("://") || gateSummaryLower.includes("stdout") || gateSummaryLower.includes("stderr") || gateSummaryLower.includes("process.env") || gateSummaryLower.includes("keychain://") || gateSummaryLower.includes("access_token") || gateSummaryLower.includes("refresh_token") || gateSummaryLower.includes("client_secret") || gateSummaryLower.includes("authorization_code") || gateSummaryLower.includes("code_verifier") || gateSummaryLower.includes("bearer") || gateSummaryLower.includes("../") || gateSummaryLower.includes("/users/")) blocking_reasons.push("Remaining gate summary is unsafe");
    }
    for (const kind of requiredGateKinds) if (!seen.has(kind)) blocking_reasons.push("Missing required remaining gate");
  }
  const execution = a.upload_execution_boundary as Record<string, unknown> | undefined;
  if (!execution || execution.upload_requested !== false || execution.upload_allowed !== false || execution.upload_execution_enabled !== false || execution.real_upload_execution_allowed !== false || execution.platform_api_calls_allowed !== false || execution.network_calls_allowed !== false || execution.resumable_upload_allowed !== false || execution.direct_upload_allowed !== false || execution.max_upload_attempts !== 0) blocking_reasons.push("Upload execution boundary is unsafe");
  const credential = a.credential_boundary as Record<string, unknown> | undefined;
  if (!credential || credential.credentials_required !== false || credential.credentials_accessed !== false || credential.token_accessed !== false || credential.keychain_accessed !== false || credential.env_accessed !== false || credential.credential_reference_stored !== false || credential.token_reference_stored !== false) blocking_reasons.push("Credential boundary is unsafe");
  const network = a.network_boundary as Record<string, unknown> | undefined;
  if (!network || network.network_calls_allowed !== false || network.network_calls_made !== false || network.platform_api_calls_allowed !== false || network.platform_api_calls_made !== false || network.external_side_effects_allowed !== false || network.external_side_effects_observed !== false) blocking_reasons.push("Network boundary is unsafe");
  const platform = a.platform_boundary as Record<string, unknown> | undefined;
  if (!platform || platform.platform_endpoint_selected !== false || platform.platform_api_payload_created !== false || platform.platform_api_payload_stored !== false || platform.raw_account_ids_stored !== false) blocking_reasons.push("Platform boundary is unsafe");
  if (looksLikeRawUploadPlatformId(platform?.account_reference_summary) || looksLikeRawUploadPlatformId(platform?.channel_or_profile_reference_summary)) blocking_reasons.push("Platform boundary contains unsafe account identifiers");
  const validation = a.validation as Record<string, unknown> | undefined;
  if (!validation || validation.real_upload_readiness_assessed !== true && validation.real_upload_readiness_assessed !== false || validation.ready_for_real_upload !== false || validation.ready_for_real_upload_execution_design_review !== false && validation.ready_for_real_upload_execution_design_review !== true || validation.upload_allowed !== false || validation.upload_execution_enabled !== false || validation.platform_api_calls_allowed !== false || validation.network_calls_allowed !== false || validation.credentials_accessed !== false || validation.token_accessed !== false) blocking_reasons.push("Validation state is unsafe");
  return { ok: blocking_reasons.length === 0, blocking_reasons, warnings };
}

export function createRealUploadReadinessAssessment(input: {
  ...
  spikeResult: ControlledProductionRenderSpikeResult;
  dryRun: true;
}): RealUploadReadinessAssessment {
  if (input.dryRun !== true) throw new Error("createRealUploadReadinessAssessment: dryRun=true required");
  if (!validateDryRunUploadSpikeResult(input.dryRunUploadSpikeResult).ok) throw new Error("createRealUploadReadinessAssessment: dryRunUploadSpikeResult must validate");
  if (!validateUploadExecutionDesign(input.uploadExecutionDesign).ok) throw new Error("createRealUploadReadinessAssessment: uploadExecutionDesign must validate");
  if (!validateUploadExecutionApproval(input.uploadExecutionApproval).ok) throw new Error("createRealUploadReadinessAssessment: uploadExecutionApproval must validate");
  if (!validatePlatformUploadRequest(input.platformUploadRequest).ok) throw new Error("createRealUploadReadinessAssessment: platformUploadRequest must validate");
  if (!validateUploadPackageDesign(input.uploadPackageDesign).ok) throw new Error("createRealUploadReadinessAssessment: uploadPackageDesign must validate");
  if (!validateLocalOutputOperatorReview(input.localOutputReview).ok) throw new Error("createRealUploadReadinessAssessment: localOutputReview must validate");
  if (!validateControlledProductionRenderSpikeResult(input.spikeResult).ok) throw new Error("createRealUploadReadinessAssessment: spikeResult must validate");
  if (input.uploadExecutionDesign.design_state !== "approved_for_future_dry_run_upload_spike") throw new Error("createRealUploadReadinessAssessment: uploadExecutionDesign must be approved_for_future_dry_run_upload_spike");
  if (input.uploadExecutionApproval.approval_state !== "approved_for_future_upload_execution_design" || input.platformUploadRequest.request_state !== "approved_for_future_upload_execution" || input.uploadPackageDesign.design_state !== "approved_for_upload_request_design" || input.localOutputReview.review_state !== "approved_for_upload_design" || input.spikeResult.validation.spike_passed !== true) throw new Error("createRealUploadReadinessAssessment: source artifacts must be approved");
  if (input.dryRunUploadSpikeResult.validation.dry_run_upload_spike_passed !== true) throw new Error("createRealUploadReadinessAssessment: dryRunUploadSpikeResult must have passed");
  if (input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id.length === 0 || input.dryRunUploadSpikeResult.upload_execution_design_id !== input.uploadExecutionDesign.upload_execution_design_id || input.dryRunUploadSpikeResult.upload_execution_approval_id !== input.uploadExecutionApproval.upload_execution_approval_id || input.dryRunUploadSpikeResult.platform_upload_request_id !== input.platformUploadRequest.platform_upload_request_id || input.dryRunUploadSpikeResult.upload_package_design_id !== input.uploadPackageDesign.upload_package_design_id || input.dryRunUploadSpikeResult.local_output_review_id !== input.localOutputReview.local_output_review_id || input.dryRunUploadSpikeResult.production_render_spike_result_id !== input.spikeResult.production_render_spike_result_id || input.dryRunUploadSpikeResult.final_render_execution_request_id !== input.spikeResult.final_render_execution_request_id || input.dryRunUploadSpikeResult.render_plan_id !== input.spikeResult.render_plan_id || input.dryRunUploadSpikeResult.project_id !== input.spikeResult.project_id || input.dryRunUploadSpikeResult.platform !== input.spikeResult.platform) throw new Error("createRealUploadReadinessAssessment: artifacts must match");
  const dryRunPassed = input.dryRunUploadSpikeResult.validation.dry_run_upload_spike_passed === true;
  const readReview = dryRunPassed;
  const gates: RealUploadRemainingGate[] = [
    { gate_id: "real-upload-phase-approval", gate_kind: "explicit_real_upload_execution_phase_approval", gate_state: "required", safe_summary: "A separate explicit real upload execution phase must be approved before any real upload can exist.", blocking_reasons: ["Separate explicit real upload execution phase not approved."], warnings: [] },
    { gate_id: "credential-strategy", gate_kind: "credential_access_strategy", gate_state: "required", safe_summary: "Credential access strategy is required.", blocking_reasons: ["Credential access strategy missing."], warnings: [] },
    { gate_id: "token-keychain-env-strategy", gate_kind: "token_keychain_env_handling_strategy", gate_state: "required", safe_summary: "Token, keychain, and env handling strategy is required.", blocking_reasons: ["Token/keychain/env handling strategy missing."], warnings: [] },
    { gate_id: "platform-api-strategy", gate_kind: "platform_api_endpoint_payload_strategy", gate_state: "required", safe_summary: "Platform API endpoint and payload strategy is required.", blocking_reasons: ["Platform API strategy missing."], warnings: [] },
    { gate_id: "network-strategy", gate_kind: "network_call_strategy", gate_state: "required", safe_summary: "Network call strategy is required.", blocking_reasons: ["Network call strategy missing."], warnings: [] },
    { gate_id: "raw-media-path-strategy", gate_kind: "raw_media_path_access_strategy", gate_state: "required", safe_summary: "Raw media path access strategy is required.", blocking_reasons: ["Raw media path access strategy missing."], warnings: [] },
    { gate_id: "retry-rollback-strategy", gate_kind: "upload_retry_rollback_idempotency_strategy", gate_state: "required", safe_summary: "Upload retry, rollback, and idempotency strategy is required.", blocking_reasons: ["Retry/rollback/idempotency strategy missing."], warnings: [] },
    { gate_id: "post-upload-verification", gate_kind: "post_upload_verification_strategy", gate_state: "required", safe_summary: "Post-upload verification strategy is required.", blocking_reasons: ["Post-upload verification strategy missing."], warnings: [] },
    { gate_id: "failure-handling", gate_kind: "failure_handling_strategy", gate_state: "required", safe_summary: "Failure handling strategy is required.", blocking_reasons: ["Failure handling strategy missing."], warnings: [] },
    { gate_id: "operator-confirmation", gate_kind: "manual_operator_confirmation", gate_state: "required", safe_summary: "Manual operator confirmation is required before any real upload execution can exist.", blocking_reasons: ["Manual operator confirmation missing."], warnings: [] },
  ];
  const assessment: RealUploadReadinessAssessment = {
    schema_version: "1.0",
    real_upload_readiness_assessment_id: `real-upload-readiness-${crypto.randomUUID()}`,
    ...
    project_id: input.spikeResult.project_id,
    platform: input.spikeResult.platform,
    assessment_state: dryRunPassed ? "assessed" : "blocked",
    created_at: new Date().toISOString(),
    assessment_mode: "real_upload_readiness_assessment_only",
    required_artifacts: {
    ...
    readiness_summary: {
      ...
      ready_for_real_upload: false,
      ready_for_real_upload_execution_design_review: readReview,
      real_upload_requires_separate_explicit_phase: true,
      human_operator_required: true,
      credential_strategy_required: true,
      platform_api_strategy_required: true,
    ...
    validation: {
      real_upload_readiness_assessed: true,
      ready_for_real_upload: false,
      ready_for_real_upload_execution_design_review: readReview,
      upload_allowed: false,
      upload_execution_enabled: false,
      ...
      credentials_accessed: false,
      token_accessed: false,
      blocking_reasons: dryRunPassed ? [] : ["Dry-run upload spike did not pass."],
      warnings: dryRunPassed ? ["Real upload readiness assessment completed safely without upload, network, or credential access."] : ["Real upload readiness assessment blocked."],
    },
    provenance: {
      generated_by: "createRealUploadReadinessAssessment",
...
export function validateRealUploadReadinessAssessment(assessment: unknown): RealUploadReadinessAssessmentValidationResult {
  const base = validateRealUploadReadinessAssessmentShape(assessment);
  return base;
}

interface RealUploadReadinessAssessmentsStoreSummaryItem {
  real_upload_readiness_assessment_id: string;
  project_id: string;
  platform: string;
...
function safeRealUploadReadinessSummary(assessment: RealUploadReadinessAssessment): RealUploadReadinessAssessmentsStoreSummaryItem {
  return {
    real_upload_readiness_assessment_id: safeRealUploadReadinessString(assessment.real_upload_readiness_assessment_id, "[unsafe-id]"),
    project_id: safeRealUploadReadinessString(assessment.project_id, "[unsafe-project]"),
    platform: safeRealUploadReadinessString(assessment.platform, "[unsafe-platform]"),
    assessment_state: assessment.assessment_state,
    created_at: assessment.created_at,
  };
}

export function saveRealUploadReadinessAssessment(assessment: RealUploadReadinessAssessment): void {
  const validation = validateRealUploadReadinessAssessment(assessment);
  if (!validation.ok) throw new Error("Unsafe real upload readiness assessment cannot be stored.");
  const store = loadRealUploadReadinessAssessmentsStore();
  const existing = store.assessments.findIndex((item) => item.real_upload_readiness_assessment_id === assessment.real_upload_readiness_assessment_id);
  if (existing >= 0) store.assessments[existing] = assessment;
  else store.assessments.push(assessment);
  saveRealUploadReadinessAssessmentsStore(store);
}

export function listRealUploadReadinessAssessments(options?: { project_id?: string; platform?: string; assessment_state?: string; dry_run_upload_spike_result_id?: string; upload_execution_design_id?: string }): RealUploadReadinessAssessment[] {
  const store = loadRealUploadReadinessAssessmentsStore();
  return store.assessments.filter((assessment) => {
    if (options?.project_id && assessment.project_id !== options.project_id) return false;
    if (options?.platform && assessment.platform !== options.platform) return false;
    if (options?.assessment_state && assessment.assessment_state !== options.assessment_state) return false;
    if (options?.dry_run_upload_spike_result_id && assessment.dry_run_upload_spike_result_id !== options.dry_run_upload_spike_result_id) return false;
    if (options?.upload_execution_design_id && assessment.upload_execution_design_id !== options.upload_execution_design_id) return false;
    return true;
  }).sort((a, b) => {
    const compare = new Date(a.created_at).getTime() - new Date(b.created_at).getTime();
    return compare !== 0 ? compare : a.real_upload_readiness_assessment_id.localeCompare(b.real_upload_readiness_assessment_id);
...
export function getRealUploadReadinessAssessment(real_upload_readiness_assessment_id: string): RealUploadReadinessAssessment | null {
  const store = loadRealUploadReadinessAssessmentsStore();
  return store.assessments.find((assessment) => assessment.real_upload_readiness_assessment_id === real_upload_readiness_assessment_id) ?? null;
}

export function revokeRealUploadReadinessAssessment(real_upload_readiness_assessment_id: string, reason: string): RealUploadReadinessAssessment {
  const assessment = getRealUploadReadinessAssessment(real_upload_readiness_assessment_id);
  if (!assessment) throw new Error("Real upload readiness assessment not found");
  const safeReason = safeRealUploadReadinessString(reason, "[unsafe-reason]");
  if (safeReason === "[unsafe-reason]") throw new Error("Unsafe real upload readiness reason cannot be stored.");
  const revoked: RealUploadReadinessAssessment = {
    ...assessment,
    assessment_state: "revoked",
    validation: { ...assessment.validation, blocking_reasons: [...assessment.validation.blocking_reasons, safeReason], warnings: [...assessment.validation.warnings, "Real upload readiness assessment revoked."] },
    provenance: { ...assessment.provenance, generated_by: "revokeRealUploadReadinessAssessment" },
  };
  saveRealUploadReadinessAssessment(revoked);
  return revoked;
}

export function getRealUploadReadinessAssessmentReport(options?: { project_id?: string; platform?: string }): {
  total: number;
  by_state: Record<string, number>;
  blocked: number;
  assessed: number;
  ...
  rejected: number;
  revoked: number;
  ready_for_real_upload_execution_design_review: number;
  ready_for_real_upload: 0;
  upload_allowed: 0;
  upload_execution_enabled: 0;
  platform_api_calls_allowed: 0;
  ...
  credentials_accessed: 0;
  token_accessed: 0;
  remaining_gates_by_state: Record<string, number>;
  assessments: Array<RealUploadReadinessAssessmentsStoreSummaryItem>;
} {
  const assessments = listRealUploadReadinessAssessments(options);
  const by_state: Record<string, number> = {};
  const remaining_gates_by_state: Record<string, number> = {};
  let blocked = 0;
  let assessed = 0;
  let ready_for_operator_review = 0;
  let rejected = 0;
  let revoked = 0;
  let ready_for_real_upload_execution_design_review = 0;
  const summaries = assessments.map((assessment) => {
    by_state[assessment.assessment_state] = (by_state[assessment.assessment_state] ?? 0) + 1;
    if (assessment.assessment_state === "blocked") blocked++;
    if (assessment.assessment_state === "assessed") assessed++;
    if (assessment.assessment_state === "ready_for_operator_review") ready_for_operator_review++;
    if (assessment.assessment_state === "rejected") rejected++;
    if (assessment.assessment_state === "revoked") revoked++;
    if (assessment.validation.ready_for_real_upload_execution_design_review) ready_for_real_upload_execution_design_review++;
    for (const gate of assessment.remaining_gates) {
      remaining_gates_by_state[gate.gate_state] = (remaining_gates_by_state[gate.gate_state] ?? 0) + 1;
    }
    return safeRealUploadReadinessSummary(assessment);
  });
  return {
    total: summaries.length,
    by_state,
    ...
    rejected,
    revoked,
    ready_for_real_upload_execution_design_review,
    ready_for_real_upload: 0,
    upload_allowed: 0,
    upload_execution_enabled: 0,
    platform_api_calls_allowed: 0,
    ...
    credentials_accessed: 0,
    token_accessed: 0,
    remaining_gates_by_state,
    assessments: summaries,
  };
}

export type RealUploadExecutionRequestMode = "real_upload_execution_request_only" | "operator_review_real_upload_execution_request";
export type RealUploadExecutionRequestState = "draft" | "blocked" | "ready_for_operator_review" | "approved_for_future_real_upload_design" | "rejected" | "revoked";

export interface RealUploadExecutionRequestRequiredArtifacts {
...
export interface RequestedFutureUploadCapabilities {
  requests_future_real_upload_design: boolean;
  requests_future_credential_strategy: boolean;
  requests_future_platform_api_strategy: boolean;
  requests_future_network_strategy: boolean;
  requests_future_media_path_access_strategy: boolean;
  requests_future_upload_retry_strategy: boolean;
  requests_future_post_upload_verification_strategy: boolean;
  requests_future_failure_handling_strategy: boolean;
  requests_current_upload_execution: false;
  requests_current_network_calls: false;
  requests_current_platform_api_calls: false;
  requests_current_credential_access: false;
}

export interface RealUploadExecutionRequestBoundary {
  upload_requested: false;
  upload_allowed: false;
  upload_execution_enabled: false;
  real_upload_execution_allowed: false;
...
export interface RealUploadExecutionRequestCredentialBoundary {
  credentials_required: false;
  credentials_accessed: false;
  token_accessed: false;
  keychain_accessed: false;
...
export interface RealUploadExecutionRequestNetworkBoundary {
  network_calls_allowed: false;
  network_calls_made: false;
  platform_api_calls_allowed: false;
  platform_api_calls_made: false;
...
export interface RealUploadExecutionRequestPlatformBoundary {
  platform_endpoint_selected: false;
  platform_api_payload_created: false;
  platform_api_payload_stored: false;
  raw_account_ids_stored: false;
...
export interface RealUploadExecutionRequestOperatorReview {
  reviewed_by_label?: string;
  checklist_acknowledged: boolean;
  readiness_assessment_acknowledged: boolean;
  remaining_gates_acknowledged: boolean;
  upload_risk_acknowledged: boolean;
  credential_risk_acknowledged: boolean;
  network_risk_acknowledged: boolean;
  platform_api_risk_acknowledged: boolean;
  understands_no_upload_enabled: boolean;
  understands_no_credentials_accessed: boolean;
  understands_no_network_calls: boolean;
  understands_future_design_phase_only: boolean;
  decision_note_summary?: string;
}

export interface RealUploadExecutionRequestValidationResult {
  ok: boolean;
  blocking_reasons: string[];
  warnings: string[];
}

export interface RealUploadExecutionRequest {
  schema_version: "1.0";
  ...
  project_id: string;
  platform: string;
  request_state: RealUploadExecutionRequestState;
  created_at: string;
  request_mode: RealUploadExecutionRequestMode;
  required_artifacts: RealUploadExecutionRequestRequiredArtifacts;
  requested_future_capabilities: RequestedFutureUploadCapabilities;
  upload_execution_boundary: RealUploadExecutionRequestBoundary;
  credential_boundary: RealUploadExecutionRequestCredentialBoundary;
  network_boundary: RealUploadExecutionRequestNetworkBoundary;
  platform_boundary: RealUploadExecutionRequestPlatformBoundary;
  operator_review: RealUploadExecutionRequestOperatorReview;
  validation: {
    ready_for_future_real_upload_design: boolean;
    ready_for_real_upload: false;
    upload_allowed: false;
    upload_execution_enabled: false;
    platform_api_calls_allowed: false;
  ...
  provenance: {
    generated_by: "createRealUploadExecutionRequest" | "revokeRealUploadExecutionRequest";
...
interface RealUploadExecutionRequestsStore {
  schema_version: "1.0";
  created_at: string;
  requests: RealUploadExecutionRequest[];
}

function getRealUploadExecutionRequestsPath(): string {
  return path.join(getRuntimeDir(), "real-upload-execution-requests.json");
}

function loadRealUploadExecutionRequestsStore(): RealUploadExecutionRequestsStore {
  try {
    const filePath = getRealUploadExecutionRequestsPath();
    if (fs.existsSync(filePath)) return JSON.parse(fs.readFileSync(filePath, "utf8")) as RealUploadExecutionRequestsStore;
  } catch {
    // start fresh
  }
  return { schema_version: "1.0", created_at: new Date().toISOString(), requests: [] };
...
function saveRealUploadExecutionRequestsStore(store: RealUploadExecutionRequestsStore): void {
  const filePath = getRealUploadExecutionRequestsPath();
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  store.requests.sort((a, b) => {
    const compare = new Date(a.created_at).getTime() - new Date(b.created_at).getTime();
    return compare !== 0 ? compare : a.real_upload_execution_request_id.localeCompare(b.real_upload_execution_request_id);
  });
  fs.writeFileSync(filePath, JSON.stringify(store, null, 2), "utf8");
}

function safeRealUploadExecutionRequestString(value: unknown, fallback: string): string {
  return safeRealUploadReadinessString(value, fallback);
}

function safeRealUploadExecutionRequestPlatformSummary(value: unknown, fallback: string): string {
  const text = safeRealUploadExecutionRequestString(value, fallback);
  if (text === fallback) return fallback;
  if (looksLikeRawUploadPlatformId(text)) return fallback;
  return text;
}

function validateRealUploadExecutionRequestShape(request: unknown): RealUploadExecutionRequestValidationResult {
  const blocking_reasons: string[] = [];
  const warnings: string[] = [];
  if (typeof request !== "object" || request === null) return { ok: false, blocking_reasons: ["Real upload execution request must be an object"], warnings };
  const r = request as Record<string, unknown>;
  const required = ["schema_version","real_upload_execution_request_id","real_upload_readiness_assessment_id","dry_run_upload_spike_result_id","upload_execution_design_id","upload_execution_approval_id","platform_upload_request_id","upload_package_design_id","local_output_review_id","production_render_spike_result_id","final_render_execution_request_id","render_plan_id","project_id","platform","request_state","created_at","request_mode","required_artifacts","requested_future_capabilities","upload_execution_boundary","credential_boundary","network_boundary","platform_boundary","operator_review","validation","provenance"];
  for (const key of required) if (!(key in r)) blocking_reasons.push("Real upload execution request is missing a required field");
  if (r.schema_version !== "1.0") blocking_reasons.push("schema_version must be 1.0");
  if (!["real_upload_execution_request_only", "operator_review_real_upload_execution_request"].includes(String(r.request_mode))) blocking_reasons.push("request_mode is invalid");
  if (!["draft", "blocked", "ready_for_operator_review", "approved_for_future_real_upload_design", "rejected", "revoked"].includes(String(r.request_state))) blocking_reasons.push("request_state is invalid");
  const text = JSON.stringify(request);
  if (text.includes("videos.insert") || text.includes("youtube.videos().insert") || text.includes("fetch(") || text.includes("process.env") || text.includes("keychain://") || text.includes("stdout") || text.includes("stderr") || text.includes("ffmpeg -i") || text.includes(" -i ") || text.includes("Bearer ") || text.includes("data:") || text.includes("base64,") || text.includes("/Users/") || text.includes("https://") || text.includes("http://") || text.includes("../")) blocking_reasons.push("Real upload execution request contains forbidden payload content");
  const artifacts = r.required_artifacts as Record<string, unknown> | undefined;
  if (!artifacts || artifacts.real_upload_readiness_assessment_validated !== true || artifacts.dry_run_upload_spike_result_validated !== true || artifacts.upload_execution_design_validated !== true || artifacts.upload_execution_approval_validated !== true || artifacts.platform_upload_request_validated !== true || artifacts.upload_package_design_validated !== true || artifacts.local_output_review_validated !== true || artifacts.spike_result_validated !== true) blocking_reasons.push("Required artifacts are unsafe");
  const requested = r.requested_future_capabilities as Record<string, unknown> | undefined;
  if (!requested || requested.requests_future_real_upload_design !== true || requested.requests_future_credential_strategy !== true || requested.requests_future_platform_api_strategy !== true || requested.requests_future_network_strategy !== true || requested.requests_future_media_path_access_strategy !== true || requested.requests_future_upload_retry_strategy !== true || requested.requests_future_post_upload_verification_strategy !== true || requested.requests_future_failure_handling_strategy !== true || requested.requests_current_upload_execution !== false || requested.requests_current_network_calls !== false || requested.requests_current_platform_api_calls !== false || requested.requests_current_credential_access !== false) blocking_reasons.push("Requested future capabilities are unsafe");
  const execution = r.upload_execution_boundary as Record<string, unknown> | undefined;
  if (!execution || execution.upload_requested !== false || execution.upload_allowed !== false || execution.upload_execution_enabled !== false || execution.real_upload_execution_allowed !== false || execution.platform_api_calls_allowed !== false || execution.network_calls_allowed !== false || execution.resumable_upload_allowed !== false || execution.direct_upload_allowed !== false || execution.max_upload_attempts !== 0) blocking_reasons.push("Upload execution boundary is unsafe");
  const credential = r.credential_boundary as Record<string, unknown> | undefined;
  if (!credential || credential.credentials_required !== false || credential.credentials_accessed !== false || credential.token_accessed !== false || credential.keychain_accessed !== false || credential.env_accessed !== false || credential.credential_reference_stored !== false || credential.token_reference_stored !== false) blocking_reasons.push("Credential boundary is unsafe");
  const network = r.network_boundary as Record<string, unknown> | undefined;
  ...
  if (looksLikeRawUploadPlatformId(platform?.account_reference_summary) || looksLikeRawUploadPlatformId(platform?.channel_or_profile_reference_summary)) blocking_reasons.push("Platform boundary contains unsafe account identifiers");
  const review = r.operator_review as Record<string, unknown> | undefined;
  if (!review || review.checklist_acknowledged !== true || review.readiness_assessment_acknowledged !== true || review.remaining_gates_acknowledged !== true || review.upload_risk_acknowledged !== true || review.credential_risk_acknowledged !== true || review.network_risk_acknowledged !== true || review.platform_api_risk_acknowledged !== true || review.understands_no_upload_enabled !== true || review.understands_no_credentials_accessed !== true || review.understands_no_network_calls !== true || review.understands_future_design_phase_only !== true) blocking_reasons.push("Operator review is unsafe");
  const validation = r.validation as Record<string, unknown> | undefined;
  if (!validation || validation.ready_for_future_real_upload_design !== false && validation.ready_for_future_real_upload_design !== true || validation.ready_for_real_upload !== false || validation.upload_allowed !== false || validation.upload_execution_enabled !== false || validation.platform_api_calls_allowed !== false || validation.network_calls_allowed !== false || validation.credentials_accessed !== false || validation.token_accessed !== false) blocking_reasons.push("Validation state is unsafe");
  return { ok: blocking_reasons.length === 0, blocking_reasons, warnings };
}

export function createRealUploadExecutionRequest(input: {
  ...
  localOutputReview: LocalOutputOperatorReview;
  spikeResult: ControlledProductionRenderSpikeResult;
  decision?: "draft" | "approved_for_future_real_upload_design" | "rejected";
  reviewed_by_label?: string;
  checklist_acknowledged?: boolean;
  readiness_assessment_acknowledged?: boolean;
  remaining_gates_acknowledged?: boolean;
  ...
  if (input.dryRun !== true) throw new Error("createRealUploadExecutionRequest: dryRun=true required");
  if (!validateRealUploadReadinessAssessment(input.readinessAssessment).ok) throw new Error("createRealUploadExecutionRequest: readinessAssessment must validate");
  if (!validateDryRunUploadSpikeResult(input.dryRunUploadSpikeResult).ok) throw new Error("createRealUploadExecutionRequest: dryRunUploadSpikeResult must validate");
  if (!validateUploadExecutionDesign(input.uploadExecutionDesign).ok) throw new Error("createRealUploadExecutionRequest: uploadExecutionDesign must validate");
  if (!validateUploadExecutionApproval(input.uploadExecutionApproval).ok) throw new Error("createRealUploadExecutionRequest: uploadExecutionApproval must validate");
  if (!validatePlatformUploadRequest(input.platformUploadRequest).ok) throw new Error("createRealUploadExecutionRequest: platformUploadRequest must validate");
  if (!validateUploadPackageDesign(input.uploadPackageDesign).ok) throw new Error("createRealUploadExecutionRequest: uploadPackageDesign must validate");
  if (!validateLocalOutputOperatorReview(input.localOutputReview).ok) throw new Error("createRealUploadExecutionRequest: localOutputReview must validate");
  if (!validateControlledProductionRenderSpikeResult(input.spikeResult).ok) throw new Error("createRealUploadExecutionRequest: spikeResult must validate");
  if (input.readinessAssessment.validation.ready_for_real_upload !== false) throw new Error("createRealUploadExecutionRequest: readinessAssessment must not be ready for real upload");
  if (input.uploadExecutionDesign.design_state !== "approved_for_future_dry_run_upload_spike") throw new Error("createRealUploadExecutionRequest: uploadExecutionDesign must be approved_for_future_dry_run_upload_spike");
  if (input.uploadExecutionApproval.approval_state !== "approved_for_future_upload_execution_design" || input.platformUploadRequest.request_state !== "approved_for_future_upload_execution" || input.uploadPackageDesign.design_state !== "approved_for_upload_request_design" || input.localOutputReview.review_state !== "approved_for_upload_design" || input.spikeResult.validation.spike_passed !== true || input.dryRunUploadSpikeResult.validation.dry_run_upload_spike_passed !== true) throw new Error("createRealUploadExecutionRequest: source artifacts must be approved");
  if (input.readinessAssessment.validation.ready_for_real_upload_execution_design_review !== true) throw new Error("createRealUploadExecutionRequest: readiness assessment must support future design review");
  if (input.readinessAssessment.dry_run_upload_spike_result_id !== input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id || input.readinessAssessment.upload_execution_design_id !== input.uploadExecutionDesign.upload_execution_design_id || input.readinessAssessment.upload_execution_approval_id !== input.uploadExecutionApproval.upload_execution_approval_id || input.readinessAssessment.platform_upload_request_id !== input.platformUploadRequest.platform_upload_request_id || input.readinessAssessment.upload_package_design_id !== input.uploadPackageDesign.upload_package_design_id || input.readinessAssessment.local_output_review_id !== input.localOutputReview.local_output_review_id || input.readinessAssessment.production_render_spike_result_id !== input.spikeResult.production_render_spike_result_id || input.readinessAssessment.final_render_execution_request_id !== input.spikeResult.final_render_execution_request_id || input.readinessAssessment.render_plan_id !== input.spikeResult.render_plan_id || input.readinessAssessment.project_id !== input.spikeResult.project_id || input.readinessAssessment.platform !== input.spikeResult.platform) throw new Error("createRealUploadExecutionRequest: assessment artifacts must match");
  if (input.dryRunUploadSpikeResult.upload_execution_design_id !== input.uploadExecutionDesign.upload_execution_design_id || input.dryRunUploadSpikeResult.upload_execution_approval_id !== input.uploadExecutionApproval.upload_execution_approval_id || input.dryRunUploadSpikeResult.platform_upload_request_id !== input.platformUploadRequest.platform_upload_request_id || input.dryRunUploadSpikeResult.upload_package_design_id !== input.uploadPackageDesign.upload_package_design_id || input.dryRunUploadSpikeResult.local_output_review_id !== input.localOutputReview.local_output_review_id || input.dryRunUploadSpikeResult.production_render_spike_result_id !== input.spikeResult.production_render_spike_result_id || input.dryRunUploadSpikeResult.final_render_execution_request_id !== input.spikeResult.final_render_execution_request_id || input.dryRunUploadSpikeResult.render_plan_id !== input.spikeResult.render_plan_id || input.dryRunUploadSpikeResult.project_id !== input.spikeResult.project_id || input.dryRunUploadSpikeResult.platform !== input.spikeResult.platform) throw new Error("createRealUploadExecutionRequest: dry-run spike artifacts must match");
  if (input.spikeResult.production_render_spike_result_id !== input.uploadExecutionDesign.production_render_spike_result_id || input.spikeResult.final_render_execution_request_id !== input.uploadExecutionDesign.final_render_execution_request_id || input.spikeResult.render_plan_id !== input.uploadExecutionDesign.render_plan_id || input.spikeResult.project_id !== input.uploadExecutionDesign.project_id || input.spikeResult.platform !== input.uploadExecutionDesign.platform) throw new Error("createRealUploadExecutionRequest: uploadExecutionDesign artifacts must match");
  if (input.spikeResult.production_render_spike_result_id !== input.uploadExecutionApproval.production_render_spike_result_id || input.spikeResult.final_render_execution_request_id !== input.uploadExecutionApproval.final_render_execution_request_id || input.spikeResult.render_plan_id !== input.uploadExecutionApproval.render_plan_id || input.spikeResult.project_id !== input.uploadExecutionApproval.project_id || input.spikeResult.platform !== input.uploadExecutionApproval.platform) throw new Error("createRealUploadExecutionRequest: uploadExecutionApproval artifacts must match");
  if (input.spikeResult.production_render_spike_result_id !== input.platformUploadRequest.production_render_spike_result_id || input.spikeResult.final_render_execution_request_id !== input.platformUploadRequest.final_render_execution_request_id || input.spikeResult.render_plan_id !== input.platformUploadRequest.render_plan_id || input.spikeResult.project_id !== input.platformUploadRequest.project_id || input.spikeResult.platform !== input.platformUploadRequest.platform) throw new Error("createRealUploadExecutionRequest: platformUploadRequest artifacts must match");
  if (input.spikeResult.production_render_spike_result_id !== input.uploadPackageDesign.production_render_spike_result_id || input.spikeResult.final_render_execution_request_id !== input.uploadPackageDesign.final_render_execution_request_id || input.spikeResult.render_plan_id !== input.uploadPackageDesign.render_plan_id || input.spikeResult.project_id !== input.uploadPackageDesign.project_id || input.spikeResult.platform !== input.uploadPackageDesign.platform) throw new Error("createRealUploadExecutionRequest: uploadPackageDesign artifacts must match");
  if (input.spikeResult.production_render_spike_result_id !== input.localOutputReview.production_render_spike_result_id || input.spikeResult.final_render_execution_request_id !== input.localOutputReview.final_render_execution_request_id || input.spikeResult.render_plan_id !== input.localOutputReview.render_plan_id || input.spikeResult.project_id !== input.localOutputReview.project_id || input.spikeResult.platform !== input.localOutputReview.platform) throw new Error("createRealUploadExecutionRequest: localOutputReview artifacts must match");
  const decision = input.decision ?? "draft";
  const approved = decision === "approved_for_future_real_upload_design";
  if (approved && (!input.readinessAssessment.validation.ready_for_real_upload_execution_design_review || !input.checklist_acknowledged || !input.readiness_assessment_acknowledged || !input.remaining_gates_acknowledged || !input.upload_risk_acknowledged || !input.credential_risk_acknowledged || !input.network_risk_acknowledged || !input.platform_api_risk_acknowledged || !input.understands_no_upload_enabled || !input.understands_no_credentials_accessed || !input.understands_no_network_calls || !input.understands_future_design_phase_only)) {
    throw new Error("createRealUploadExecutionRequest: approved decision requires acknowledgements");
  }
  const reviewedBy = safeRealUploadExecutionRequestString(input.reviewed_by_label ?? "", "[unsafe-reviewer]");
  const note = safeRealUploadExecutionRequestString(input.decision_note_summary ?? "", "[unsafe-review-note]");
  const request: RealUploadExecutionRequest = {
    schema_version: "1.0",
    real_upload_execution_request_id: `real-upload-execution-request-${crypto.randomUUID()}`,
    ...
    project_id: input.spikeResult.project_id,
    platform: input.spikeResult.platform,
    request_state: approved ? "approved_for_future_real_upload_design" : "ready_for_operator_review",
    created_at: new Date().toISOString(),
    request_mode: "real_upload_execution_request_only",
    required_artifacts: {
    ...
    requested_future_capabilities: {
      requests_future_real_upload_design: true,
      requests_future_credential_strategy: true,
      requests_future_platform_api_strategy: true,
      requests_future_network_strategy: true,
      requests_future_media_path_access_strategy: true,
      requests_future_upload_retry_strategy: true,
      requests_future_post_upload_verification_strategy: true,
      requests_future_failure_handling_strategy: true,
      requests_current_upload_execution: false,
      requests_current_network_calls: false,
      requests_current_platform_api_calls: false,
      requests_current_credential_access: false,
    ...
    operator_review: {
      ...(reviewedBy ? { reviewed_by_label: reviewedBy } : {}),
      checklist_acknowledged: input.checklist_acknowledged === true,
      readiness_assessment_acknowledged: input.readiness_assessment_acknowledged === true,
      remaining_gates_acknowledged: input.remaining_gates_acknowledged === true,
      upload_risk_acknowledged: input.upload_risk_acknowledged === true,
      credential_risk_acknowledged: input.credential_risk_acknowledged === true,
      network_risk_acknowledged: input.network_risk_acknowledged === true,
      platform_api_risk_acknowledged: input.platform_api_risk_acknowledged === true,
      understands_no_upload_enabled: input.understands_no_upload_enabled === true,
      understands_no_credentials_accessed: input.understands_no_credentials_accessed === true,
      understands_no_network_calls: input.understands_no_network_calls === true,
      understands_future_design_phase_only: input.understands_future_design_phase_only === true,
      ...(note ? { decision_note_summary: note } : {}),
    },
    validation: {
      ready_for_future_real_upload_design: approved && input.readinessAssessment.validation.ready_for_real_upload_execution_design_review,
      ready_for_real_upload: false,
      upload_allowed: false,
      upload_execution_enabled: false,
      platform_api_calls_allowed: false,
      ...
      token_accessed: false,
      blocking_reasons: [],
      warnings: approved ? ["No real upload, network call, or platform API call is enabled."] : [],
    },
    provenance: {
      generated_by: "createRealUploadExecutionRequest",
  ...
  if (!validateRealUploadExecutionRequest(request).ok) throw new Error("Unsafe real upload execution request cannot be created.");
  return request;
}

export function validateRealUploadExecutionRequest(request: unknown): RealUploadExecutionRequestValidationResult {
  return validateRealUploadExecutionRequestShape(request);
}

interface RealUploadExecutionRequestsStoreSummaryItem {
  real_upload_execution_request_id: string;
  project_id: string;
  platform: string;
  request_state: string;
...
function safeRealUploadExecutionRequestSummary(request: RealUploadExecutionRequest): RealUploadExecutionRequestsStoreSummaryItem {
  return {
    real_upload_execution_request_id: safeRealUploadExecutionRequestString(request.real_upload_execution_request_id, "[unsafe-id]"),
    project_id: safeRealUploadExecutionRequestString(request.project_id, "[unsafe-project]"),
    platform: safeRealUploadExecutionRequestString(request.platform, "[unsafe-platform]"),
    request_state: request.request_state,
    created_at: request.created_at,
  };
}

export function saveRealUploadExecutionRequest(request: RealUploadExecutionRequest): void {
  const validation = validateRealUploadExecutionRequest(request);
  if (!validation.ok) throw new Error("Unsafe real upload execution request cannot be stored.");
  const store = loadRealUploadExecutionRequestsStore();
  const existing = store.requests.findIndex((item) => item.real_upload_execution_request_id === request.real_upload_execution_request_id);
  if (existing >= 0) store.requests[existing] = request;
  else store.requests.push(request);
  saveRealUploadExecutionRequestsStore(store);
}

export function listRealUploadExecutionRequests(options?: { project_id?: string; platform?: string; request_state?: string; real_upload_readiness_assessment_id?: string; dry_run_upload_spike_result_id?: string }): RealUploadExecutionRequest[] {
  const store = loadRealUploadExecutionRequestsStore();
  return store.requests.filter((request) => {
    if (options?.project_id && request.project_id !== options.project_id) return false;
    if (options?.platform && request.platform !== options.platform) return false;
    if (options?.request_state && request.request_state !== options.request_state) return false;
    if (options?.real_upload_readiness_assessment_id && request.real_upload_readiness_assessment_id !== options.real_upload_readiness_assessment_id) return false;
    if (options?.dry_run_upload_spike_result_id && request.dry_run_upload_spike_result_id !== options.dry_run_upload_spike_result_id) return false;
    return true;
  }).sort((a, b) => {
    const compare = new Date(a.created_at).getTime() - new Date(b.created_at).getTime();
    return compare !== 0 ? compare : a.real_upload_execution_request_id.localeCompare(b.real_upload_execution_request_id);
...
export function getRealUploadExecutionRequest(real_upload_execution_request_id: string): RealUploadExecutionRequest | null {
  const store = loadRealUploadExecutionRequestsStore();
  return store.requests.find((request) => request.real_upload_execution_request_id === real_upload_execution_request_id) ?? null;
}

export function revokeRealUploadExecutionRequest(real_upload_execution_request_id: string, reason: string): RealUploadExecutionRequest {
  const request = getRealUploadExecutionRequest(real_upload_execution_request_id);
  if (!request) throw new Error("Real upload execution request not found");
  const safeReason = safeRealUploadExecutionRequestString(reason, "[unsafe-reason]");
  if (safeReason === "[unsafe-reason]") throw new Error("Unsafe real upload execution request reason cannot be stored.");
  const revoked: RealUploadExecutionRequest = {
    ...request,
    request_state: "revoked",
    validation: { ...request.validation, blocking_reasons: [...request.validation.blocking_reasons, safeReason], warnings: [...request.validation.warnings, "Real upload execution request revoked."] },
    provenance: { ...request.provenance, generated_by: "revokeRealUploadExecutionRequest" },
  };
  saveRealUploadExecutionRequest(revoked);
  return revoked;
}

export function getRealUploadExecutionRequestReport(options?: { project_id?: string; platform?: string }): {
  total: number;
  by_state: Record<string, number>;
  blocked: number;
  ready_for_operator_review: number;
  approved_for_future_real_upload_design: number;
  rejected: number;
  revoked: number;
  ready_for_future_real_upload_design: number;
  ready_for_real_upload: 0;
  upload_allowed: 0;
  upload_execution_enabled: 0;
  platform_api_calls_allowed: 0;
  ...
  credentials_accessed: 0;
  token_accessed: 0;
  requests: Array<RealUploadExecutionRequestsStoreSummaryItem>;
} {
  const requests = listRealUploadExecutionRequests(options);
  const by_state: Record<string, number> = {};
  let blocked = 0;
  let ready = 0;
  let approved = 0;
  ...
  const summaries = requests.map((request) => {
    ...
    if (request.request_state === "blocked") blocked++;
    if (request.request_state === "ready_for_operator_review") ready++;
    if (request.request_state === "approved_for_future_real_upload_design") approved++;
    if (request.request_state === "rejected") rejected++;
    if (request.request_state === "revoked") revoked++;
    if (request.validation.ready_for_future_real_upload_design) readyFuture++;
    return safeRealUploadExecutionRequestSummary(request);
  });
  return {
    total: summaries.length,
    by_state,
    blocked,
    ready_for_operator_review: ready,
    approved_for_future_real_upload_design: approved,
    rejected,
    revoked,
    ready_for_future_real_upload_design: readyFuture,
    ready_for_real_upload: 0,
    upload_allowed: 0,
    upload_execution_enabled: 0,
    platform_api_calls_allowed: 0,
...
export type FinalRealUploadPreflightGateMode = "final_real_upload_preflight_gate_only" | "operator_review_final_real_upload_preflight_gate";
export type FinalRealUploadPreflightGateState = "draft" | "blocked" | "ready_for_operator_review" | "approved_for_future_real_upload_implementation_design" | "rejected" | "revoked";

export interface FinalRealUploadPreflightRequiredArtifacts {
  real_upload_dry_run_execution_result_validated: boolean;
  real_upload_execution_plan_validated: boolean;
  real_upload_strategy_design_validated: boolean;
  real_upload_execution_request_validated: boolean;
  real_upload_readiness_assessment_validated: boolean;
...
export interface FinalRealUploadPreflightScope { future_real_upload_implementation_design_requested: boolean; current_real_upload_requested: false; current_network_calls_requested: false; current_platform_api_calls_requested: false; current_credential_access_requested: false; current_media_read_requested: false; preflight_gate_only: true; }
export interface FinalRealUploadArtifactChainAssessment { all_required_artifacts_present: boolean; all_required_artifacts_validated: boolean; dry_run_execution_passed: boolean; prior_ready_for_real_upload_flags_false: boolean; prior_upload_allowed_flags_false: boolean; prior_network_api_credential_flags_false: boolean; prior_media_read_flags_false: boolean; chain_safe_for_future_implementation_design: boolean; chain_safe_for_real_upload_now: false; blocking_reasons: string[]; warnings: string[]; }
export interface FinalRealUploadCriticalRiskAcknowledgements { upload_side_effect_risk_acknowledged: boolean; platform_policy_risk_acknowledged: boolean; credential_handling_risk_acknowledged: boolean; token_scope_risk_acknowledged: boolean; network_side_effect_risk_acknowledged: boolean; media_access_risk_acknowledged: boolean; retry_idempotency_risk_acknowledged: boolean; rollback_limitations_acknowledged: boolean; post_upload_visibility_risk_acknowledged: boolean; operator_accountability_acknowledged: boolean; }
export interface FinalRealUploadUnresolvedExecutionRequirement { requirement_id: string; requirement_kind: "credential_storage_policy" | "credential_scope_policy" | "platform_api_endpoint_policy" | "platform_api_payload_policy" | "network_execution_policy" | "media_path_access_policy" | "upload_retry_policy" | "idempotency_policy" | "rollback_policy" | "post_upload_verification_policy" | "audit_logging_policy" | "operator_runbook_policy" | "emergency_stop_policy" | "manual_final_confirmation_policy"; requirement_state: "required" | "deferred" | "blocked" | "acknowledged"; safe_summary: string; blocking_reasons: string[]; warnings: string[]; }
export interface FinalRealUploadOperatorPreflightReview { reviewed_by_label?: string; checklist_acknowledged: boolean; artifact_chain_acknowledged: boolean; unresolved_requirements_acknowledged: boolean; critical_risks_acknowledged: boolean; understands_preflight_gate_only: boolean; understands_no_upload_enabled: boolean; understands_no_credentials_accessed: boolean; understands_no_network_calls: boolean; understands_no_media_reads: boolean; understands_future_implementation_design_required: boolean; decision_note_summary?: string; }
export interface FinalRealUploadPreflightCredentialBoundary { credentials_required: false; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; credential_reference_stored: false; token_reference_stored: false; secret_material_stored: false; }
export interface FinalRealUploadPreflightNetworkBoundary { network_calls_allowed: false; network_calls_made: false; external_side_effects_allowed: false; external_side_effects_observed: false; }
export interface FinalRealUploadPreflightPlatformApiBoundary { platform_api_calls_allowed: false; platform_api_calls_made: false; platform_endpoint_selected: false; platform_api_payload_created: false; platform_api_payload_stored: false; raw_account_ids_stored: false; upload_payload_created: false; response_received: false; raw_response_stored: false; }
export interface FinalRealUploadPreflightMediaBoundary { media_file_read: false; raw_media_path_stored: false; media_file_modified: false; media_file_copied: false; media_file_moved: false; media_file_deleted: false; }
export interface FinalRealUploadPreflightExecutionBoundary { upload_allowed: false; upload_execution_enabled: false; real_upload_execution_allowed: false; platform_api_calls_allowed: false; network_calls_allowed: false; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; media_file_read: false; ready_for_real_upload: false; }
export interface FinalRealUploadPreflightGateValidationResult { ok: boolean; blocking_reasons: string[]; warnings: string[]; }
export interface FinalRealUploadPreflightGate {
  schema_version: "1.0";
  final_real_upload_preflight_gate_id: string;
  real_upload_dry_run_execution_result_id: string;
  real_upload_execution_plan_id: string;
  real_upload_strategy_design_id: string;
  real_upload_execution_request_id: string;
  ...
  project_id: string;
  platform: string;
  gate_state: FinalRealUploadPreflightGateState;
  created_at: string;
  gate_mode: FinalRealUploadPreflightGateMode;
  required_artifacts: FinalRealUploadPreflightRequiredArtifacts;
  preflight_scope: FinalRealUploadPreflightScope;
  artifact_chain_assessment: FinalRealUploadArtifactChainAssessment;
  critical_risk_acknowledgements: FinalRealUploadCriticalRiskAcknowledgements;
  unresolved_execution_requirements: FinalRealUploadUnresolvedExecutionRequirement[];
  operator_preflight_review: FinalRealUploadOperatorPreflightReview;
  credential_boundary: FinalRealUploadPreflightCredentialBoundary;
  network_boundary: FinalRealUploadPreflightNetworkBoundary;
  platform_api_boundary: FinalRealUploadPreflightPlatformApiBoundary;
  media_boundary: FinalRealUploadPreflightMediaBoundary;
  execution_boundary: FinalRealUploadPreflightExecutionBoundary;
  validation: {
    final_real_upload_preflight_passed: boolean;
    ready_for_future_real_upload_implementation_design: boolean;
    ready_for_real_upload: false;
    upload_allowed: false;
    upload_execution_enabled: false;
    platform_api_calls_allowed: false;
  ...
  provenance: {
    generated_by: "createFinalRealUploadPreflightGate" | "revokeFinalRealUploadPreflightGate";
    source_real_upload_dry_run_execution_result_id: string;
    source_real_upload_execution_plan_id: string;
    source_real_upload_strategy_design_id: string;
    source_real_upload_execution_request_id: string;
    source_real_upload_readiness_assessment_id: string;
...
interface FinalRealUploadPreflightGatesStore { schema_version: "1.0"; created_at: string; gates: FinalRealUploadPreflightGate[]; }
function getFinalRealUploadPreflightGatesPath(): string { return path.join(getRuntimeDir(), "final-real-upload-preflight-gates.json"); }
function loadFinalRealUploadPreflightGatesStore(): FinalRealUploadPreflightGatesStore { try { const filePath = getFinalRealUploadPreflightGatesPath(); if (fs.existsSync(filePath)) return JSON.parse(fs.readFileSync(filePath, "utf8")) as FinalRealUploadPreflightGatesStore; } catch {} return { schema_version: "1.0", created_at: new Date().toISOString(), gates: [] }; }
function saveFinalRealUploadPreflightGatesStore(store: FinalRealUploadPreflightGatesStore): void { const filePath = getFinalRealUploadPreflightGatesPath(); fs.mkdirSync(path.dirname(filePath), { recursive: true }); store.gates.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.final_real_upload_preflight_gate_id.localeCompare(b.final_real_upload_preflight_gate_id)); fs.writeFileSync(filePath, JSON.stringify(store, null, 2), "utf8"); }
function safeFinalRealUploadPreflightString(value: unknown, fallback: string): string { return safeRealUploadExecutionPlanString(value, fallback); }
function validateFinalRealUploadPreflightGateShape(gate: unknown): FinalRealUploadPreflightGateValidationResult {
  const blocking_reasons: string[] = [];
  const warnings: string[] = [];
  if (typeof gate !== "object" || gate === null) return { ok: false, blocking_reasons: ["Final real upload preflight gate must be an object"], warnings };
  const g = gate as Record<string, unknown>;
  const required = ["schema_version","final_real_upload_preflight_gate_id","real_upload_dry_run_execution_result_id","real_upload_execution_plan_id","real_upload_strategy_design_id","real_upload_execution_request_id","real_upload_readiness_assessment_id","dry_run_upload_spike_result_id","upload_execution_design_id","upload_execution_approval_id","platform_upload_request_id","upload_package_design_id","local_output_review_id","production_render_spike_result_id","final_render_execution_request_id","render_plan_id","project_id","platform","gate_state","created_at","gate_mode","required_artifacts","preflight_scope","artifact_chain_assessment","critical_risk_acknowledgements","unresolved_execution_requirements","operator_preflight_review","credential_boundary","network_boundary","platform_api_boundary","media_boundary","execution_boundary","validation","provenance"];
  for (const key of required) if (!(key in g)) blocking_reasons.push("Final real upload preflight gate is missing a required field");
  if (g.schema_version !== "1.0") blocking_reasons.push("schema_version must be 1.0");
  if (!["final_real_upload_preflight_gate_only","operator_review_final_real_upload_preflight_gate"].includes(String(g.gate_mode))) blocking_reasons.push("gate_mode is invalid");
  if (!["draft","blocked","ready_for_operator_review","approved_for_future_real_upload_implementation_design","rejected","revoked"].includes(String(g.gate_state))) blocking_reasons.push("gate_state is invalid");
  const text = JSON.stringify(gate);
  if (text.includes("videos.insert") || text.includes("youtube.videos().insert") || text.includes("fetch(") || text.includes("process.env") || text.includes("keychain://") || text.includes("stdout") || text.includes("stderr") || text.includes("ffmpeg -i") || text.includes("Bearer ") || text.includes("data:") || text.includes("base64,") || text.includes("/Users/") || text.includes("https://") || text.includes("http://") || text.includes("../")) blocking_reasons.push("Final real upload preflight gate contains forbidden payload content");
  const requiredArtifacts = g.required_artifacts as Record<string, unknown> | undefined;
  if (!requiredArtifacts || requiredArtifacts.real_upload_dry_run_execution_result_validated !== true || requiredArtifacts.real_upload_execution_plan_validated !== true || requiredArtifacts.real_upload_strategy_design_validated !== true || requiredArtifacts.real_upload_execution_request_validated !== true || requiredArtifacts.real_upload_readiness_assessment_validated !== true || requiredArtifacts.dry_run_upload_spike_result_validated !== true || requiredArtifacts.upload_execution_design_validated !== true || requiredArtifacts.upload_execution_approval_validated !== true || requiredArtifacts.platform_upload_request_validated !== true || requiredArtifacts.upload_package_design_validated !== true || requiredArtifacts.local_output_review_validated !== true || requiredArtifacts.spike_result_validated !== true) blocking_reasons.push("Required artifacts are unsafe");
  const scope = g.preflight_scope as Record<string, unknown> | undefined;
  if (!scope || scope.future_real_upload_implementation_design_requested !== true || scope.current_real_upload_requested !== false || scope.current_network_calls_requested !== false || scope.current_platform_api_calls_requested !== false || scope.current_credential_access_requested !== false || scope.current_media_read_requested !== false || scope.preflight_gate_only !== true) blocking_reasons.push("Preflight scope is unsafe");
  const requiredKinds = new Set(["credential_storage_policy","credential_scope_policy","platform_api_endpoint_policy","platform_api_payload_policy","network_execution_policy","media_path_access_policy","upload_retry_policy","idempotency_policy","rollback_policy","post_upload_verification_policy","audit_logging_policy","operator_runbook_policy","emergency_stop_policy","manual_final_confirmation_policy"]);
  const requirements = g.unresolved_execution_requirements as Record<string, unknown>[] | undefined;
  if (!Array.isArray(requirements) || requirements.length < requiredKinds.size) blocking_reasons.push("Unresolved execution requirements are unsafe");
  else {
    const seen = new Set<string>();
    for (const requirement of requirements) {
      if (!requirement || typeof requirement.requirement_id !== "string" || typeof requirement.requirement_kind !== "string" || !["required","deferred","blocked","acknowledged"].includes(String(requirement.requirement_state)) || typeof requirement.safe_summary !== "string" || !Array.isArray(requirement.blocking_reasons) || !Array.isArray(requirement.warnings)) { blocking_reasons.push("Unresolved execution requirement is unsafe"); continue; }
      seen.add(String(requirement.requirement_kind));
      const summary = requirement.safe_summary.toLowerCase();
      if (summary.length === 0 || summary.length > 200 || summary.includes("://") || summary.includes("stdout") || summary.includes("stderr") || summary.includes("process.env") || summary.includes("keychain://") || summary.includes("access_token") || summary.includes("refresh_token") || summary.includes("client_secret") || summary.includes("authorization_code") || summary.includes("code_verifier") || summary.includes("bearer") || summary.includes("/users/") || summary.includes("../")) blocking_reasons.push("Unresolved execution requirement summary is unsafe");
    }
    for (const kind of requiredKinds) if (!seen.has(kind)) blocking_reasons.push("Missing required unresolved execution requirement kind");
  }
  const chain = g.artifact_chain_assessment as Record<string, unknown> | undefined;
  if (!chain || chain.all_required_artifacts_present !== true || chain.all_required_artifacts_validated !== true || chain.dry_run_execution_passed !== true || chain.prior_ready_for_real_upload_flags_false !== true || chain.prior_upload_allowed_flags_false !== true || chain.prior_network_api_credential_flags_false !== true || chain.prior_media_read_flags_false !== true || chain.chain_safe_for_future_implementation_design !== true || chain.chain_safe_for_real_upload_now !== false || !Array.isArray(chain.blocking_reasons) || !Array.isArray(chain.warnings)) blocking_reasons.push("Artifact chain assessment is unsafe");
  const risks = g.critical_risk_acknowledgements as Record<string, unknown> | undefined;
  if (!risks || risks.upload_side_effect_risk_acknowledged !== true || risks.platform_policy_risk_acknowledged !== true || risks.credential_handling_risk_acknowledged !== true || risks.token_scope_risk_acknowledged !== true || risks.network_side_effect_risk_acknowledged !== true || risks.media_access_risk_acknowledged !== true || risks.retry_idempotency_risk_acknowledged !== true || risks.rollback_limitations_acknowledged !== true || risks.post_upload_visibility_risk_acknowledged !== true || risks.operator_accountability_acknowledged !== true) blocking_reasons.push("Critical risk acknowledgements are unsafe");
  const review = g.operator_preflight_review as Record<string, unknown> | undefined;
  if (!review || review.checklist_acknowledged !== true || review.artifact_chain_acknowledged !== true || review.unresolved_requirements_acknowledged !== true || review.critical_risks_acknowledged !== true || review.understands_preflight_gate_only !== true || review.understands_no_upload_enabled !== true || review.understands_no_credentials_accessed !== true || review.understands_no_network_calls !== true || review.understands_no_media_reads !== true || review.understands_future_implementation_design_required !== true) blocking_reasons.push("Operator preflight review is unsafe");
  const credential = g.credential_boundary as Record<string, unknown> | undefined; if (!credential || credential.credentials_required !== false || credential.credentials_accessed !== false || credential.token_accessed !== false || credential.keychain_accessed !== false || credential.env_accessed !== false || credential.credential_reference_stored !== false || credential.token_reference_stored !== false || credential.secret_material_stored !== false) blocking_reasons.push("Credential boundary is unsafe");
  const network = g.network_boundary as Record<string, unknown> | undefined; if (!network || network.network_calls_allowed !== false || network.network_calls_made !== false || network.external_side_effects_allowed !== false || network.external_side_effects_observed !== false) blocking_reasons.push("Network boundary is unsafe");
  const platform = g.platform_api_boundary as Record<string, unknown> | undefined; if (!platform || platform.platform_api_calls_allowed !== false || platform.platform_api_calls_made !== false || platform.platform_endpoint_selected !== false || platform.platform_api_payload_created !== false || platform.platform_api_payload_stored !== false || platform.raw_account_ids_stored !== false || platform.upload_payload_created !== false || platform.response_received !== false || platform.raw_response_stored !== false) blocking_reasons.push("Platform API boundary is unsafe");
  const media = g.media_boundary as Record<string, unknown> | undefined; if (!media || media.media_file_read !== false || media.raw_media_path_stored !== false || media.media_file_modified !== false || media.media_file_copied !== false || media.media_file_moved !== false || media.media_file_deleted !== false) blocking_reasons.push("Media boundary is unsafe");
  const execution = g.execution_boundary as Record<string, unknown> | undefined; if (!execution || execution.upload_allowed !== false || execution.upload_execution_enabled !== false || execution.real_upload_execution_allowed !== false || execution.platform_api_calls_allowed !== false || execution.network_calls_allowed !== false || execution.credentials_accessed !== false || execution.token_accessed !== false || execution.keychain_accessed !== false || execution.env_accessed !== false || execution.media_file_read !== false || execution.ready_for_real_upload !== false) blocking_reasons.push("Execution boundary is unsafe");
  const validation = g.validation as Record<string, unknown> | undefined; if (!validation || validation.final_real_upload_preflight_passed !== false && validation.final_real_upload_preflight_passed !== true || validation.ready_for_future_real_upload_implementation_design !== false && validation.ready_for_future_real_upload_implementation_design !== true || validation.ready_for_real_upload !== false || validation.upload_allowed !== false || validation.upload_execution_enabled !== false || validation.platform_api_calls_allowed !== false || validation.network_calls_allowed !== false || validation.credentials_accessed !== false || validation.token_accessed !== false || validation.media_file_read !== false) blocking_reasons.push("Validation is unsafe");
  return { ok: blocking_reasons.length === 0, blocking_reasons, warnings };
}
export function createFinalRealUploadPreflightGate(input: {
  ...
  localOutputReview: LocalOutputOperatorReview;
  spikeResult: ControlledProductionRenderSpikeResult;
  decision?: "draft" | "approved_for_future_real_upload_implementation_design" | "rejected";
  reviewed_by_label?: string;
  checklist_acknowledged?: boolean;
  ...
  if (input.dryRun !== true) throw new Error("createFinalRealUploadPreflightGate: dryRun=true required");
  if (!validateRealUploadDryRunExecutionResult(input.dryRunExecutionResult).ok) throw new Error("createFinalRealUploadPreflightGate: dryRunExecutionResult must validate");
  if (!validateRealUploadExecutionPlan(input.executionPlan).ok) throw new Error("createFinalRealUploadPreflightGate: executionPlan must validate");
  if (!validateRealUploadStrategyDesign(input.strategyDesign).ok) throw new Error("createFinalRealUploadPreflightGate: strategyDesign must validate");
  if (!validateRealUploadExecutionRequest(input.realUploadExecutionRequest).ok) throw new Error("createFinalRealUploadPreflightGate: realUploadExecutionRequest must validate");
  if (!validateRealUploadReadinessAssessment(input.readinessAssessment).ok) throw new Error("createFinalRealUploadPreflightGate: readinessAssessment must validate");
  if (!validateDryRunUploadSpikeResult(input.dryRunUploadSpikeResult).ok) throw new Error("createFinalRealUploadPreflightGate: dryRunUploadSpikeResult must validate");
  if (!validateUploadExecutionDesign(input.uploadExecutionDesign).ok) throw new Error("createFinalRealUploadPreflightGate: uploadExecutionDesign must validate");
  if (!validateUploadExecutionApproval(input.uploadExecutionApproval).ok) throw new Error("createFinalRealUploadPreflightGate: uploadExecutionApproval must validate");
  if (!validatePlatformUploadRequest(input.platformUploadRequest).ok) throw new Error("createFinalRealUploadPreflightGate: platformUploadRequest must validate");
  if (!validateUploadPackageDesign(input.uploadPackageDesign).ok) throw new Error("createFinalRealUploadPreflightGate: uploadPackageDesign must validate");
  if (!validateLocalOutputOperatorReview(input.localOutputReview).ok) throw new Error("createFinalRealUploadPreflightGate: localOutputReview must validate");
  if (!validateControlledProductionRenderSpikeResult(input.spikeResult).ok) throw new Error("createFinalRealUploadPreflightGate: spikeResult must validate");
  if (input.dryRunExecutionResult.validation.ready_for_real_upload !== false || input.executionPlan.validation.ready_for_real_upload !== false || input.strategyDesign.validation.ready_for_real_upload !== false || input.realUploadExecutionRequest.validation.ready_for_real_upload !== false || input.readinessAssessment.validation.ready_for_real_upload !== false) throw new Error("createFinalRealUploadPreflightGate: ready_for_real_upload must remain false");
  if (input.dryRunExecutionResult.validation.real_upload_dry_run_execution_passed !== true || input.dryRunExecutionResult.validation.ready_for_future_real_upload_preflight !== true) throw new Error("createFinalRealUploadPreflightGate: dry-run execution must have passed");
  if (input.dryRunExecutionResult.dry_run_upload_spike_result_id !== input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id) throw new Error("createFinalRealUploadPreflightGate: dry-run execution result must match dry-run spike");
  if (input.executionPlan.real_upload_strategy_design_id !== input.strategyDesign.real_upload_strategy_design_id || input.executionPlan.real_upload_execution_request_id !== input.realUploadExecutionRequest.real_upload_execution_request_id || input.executionPlan.real_upload_readiness_assessment_id !== input.readinessAssessment.real_upload_readiness_assessment_id || input.executionPlan.dry_run_upload_spike_result_id !== input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id || input.executionPlan.upload_execution_design_id !== input.uploadExecutionDesign.upload_execution_design_id || input.executionPlan.upload_execution_approval_id !== input.uploadExecutionApproval.upload_execution_approval_id || input.executionPlan.platform_upload_request_id !== input.platformUploadRequest.platform_upload_request_id || input.executionPlan.upload_package_design_id !== input.uploadPackageDesign.upload_package_design_id || input.executionPlan.local_output_review_id !== input.localOutputReview.local_output_review_id || input.executionPlan.production_render_spike_result_id !== input.spikeResult.production_render_spike_result_id || input.executionPlan.final_render_execution_request_id !== input.spikeResult.final_render_execution_request_id || input.executionPlan.render_plan_id !== input.spikeResult.render_plan_id || input.executionPlan.project_id !== input.spikeResult.project_id || input.executionPlan.platform !== input.spikeResult.platform) throw new Error("createFinalRealUploadPreflightGate: executionPlan inputs must match");
  if (input.strategyDesign.real_upload_execution_request_id !== input.realUploadExecutionRequest.real_upload_execution_request_id || input.strategyDesign.real_upload_readiness_assessment_id !== input.readinessAssessment.real_upload_readiness_assessment_id || input.strategyDesign.dry_run_upload_spike_result_id !== input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id || input.strategyDesign.upload_execution_design_id !== input.uploadExecutionDesign.upload_execution_design_id || input.strategyDesign.upload_execution_approval_id !== input.uploadExecutionApproval.upload_execution_approval_id || input.strategyDesign.platform_upload_request_id !== input.platformUploadRequest.platform_upload_request_id || input.strategyDesign.upload_package_design_id !== input.uploadPackageDesign.upload_package_design_id || input.strategyDesign.local_output_review_id !== input.localOutputReview.local_output_review_id || input.strategyDesign.production_render_spike_result_id !== input.spikeResult.production_render_spike_result_id || input.strategyDesign.final_render_execution_request_id !== input.spikeResult.final_render_execution_request_id || input.strategyDesign.render_plan_id !== input.spikeResult.render_plan_id || input.strategyDesign.project_id !== input.spikeResult.project_id || input.strategyDesign.platform !== input.spikeResult.platform) throw new Error("createFinalRealUploadPreflightGate: strategyDesign inputs must match");
  if (input.realUploadExecutionRequest.real_upload_readiness_assessment_id !== input.readinessAssessment.real_upload_readiness_assessment_id || input.realUploadExecutionRequest.dry_run_upload_spike_result_id !== input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id || input.realUploadExecutionRequest.upload_execution_design_id !== input.uploadExecutionDesign.upload_execution_design_id || input.realUploadExecutionRequest.upload_execution_approval_id !== input.uploadExecutionApproval.upload_execution_approval_id || input.realUploadExecutionRequest.platform_upload_request_id !== input.platformUploadRequest.platform_upload_request_id || input.realUploadExecutionRequest.upload_package_design_id !== input.uploadPackageDesign.upload_package_design_id || input.realUploadExecutionRequest.local_output_review_id !== input.localOutputReview.local_output_review_id || input.realUploadExecutionRequest.production_render_spike_result_id !== input.spikeResult.production_render_spike_result_id || input.realUploadExecutionRequest.final_render_execution_request_id !== input.spikeResult.final_render_execution_request_id || input.realUploadExecutionRequest.render_plan_id !== input.spikeResult.render_plan_id || input.realUploadExecutionRequest.project_id !== input.spikeResult.project_id || input.realUploadExecutionRequest.platform !== input.spikeResult.platform) throw new Error("createFinalRealUploadPreflightGate: request inputs must match");
  if (input.readinessAssessment.dry_run_upload_spike_result_id !== input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id || input.readinessAssessment.upload_execution_design_id !== input.uploadExecutionDesign.upload_execution_design_id || input.readinessAssessment.upload_execution_approval_id !== input.uploadExecutionApproval.upload_execution_approval_id || input.readinessAssessment.platform_upload_request_id !== input.platformUploadRequest.platform_upload_request_id || input.readinessAssessment.upload_package_design_id !== input.uploadPackageDesign.upload_package_design_id || input.readinessAssessment.local_output_review_id !== input.localOutputReview.local_output_review_id || input.readinessAssessment.production_render_spike_result_id !== input.spikeResult.production_render_spike_result_id || input.readinessAssessment.final_render_execution_request_id !== input.spikeResult.final_render_execution_request_id || input.readinessAssessment.render_plan_id !== input.spikeResult.render_plan_id || input.readinessAssessment.project_id !== input.spikeResult.project_id || input.readinessAssessment.platform !== input.spikeResult.platform) throw new Error("createFinalRealUploadPreflightGate: assessment inputs must match");
  if (input.dryRunUploadSpikeResult.upload_execution_design_id !== input.uploadExecutionDesign.upload_execution_design_id || input.dryRunUploadSpikeResult.upload_execution_approval_id !== input.uploadExecutionApproval.upload_execution_approval_id || input.dryRunUploadSpikeResult.platform_upload_request_id !== input.platformUploadRequest.platform_upload_request_id || input.dryRunUploadSpikeResult.upload_package_design_id !== input.uploadPackageDesign.upload_package_design_id || input.dryRunUploadSpikeResult.local_output_review_id !== input.localOutputReview.local_output_review_id || input.dryRunUploadSpikeResult.production_render_spike_result_id !== input.spikeResult.production_render_spike_result_id || input.dryRunUploadSpikeResult.final_render_execution_request_id !== input.spikeResult.final_render_execution_request_id || input.dryRunUploadSpikeResult.render_plan_id !== input.spikeResult.render_plan_id || input.dryRunUploadSpikeResult.project_id !== input.spikeResult.project_id || input.dryRunUploadSpikeResult.platform !== input.spikeResult.platform) throw new Error("createFinalRealUploadPreflightGate: dryRunUploadSpikeResult inputs must match");
  if (input.spikeResult.production_render_spike_result_id !== input.uploadExecutionDesign.production_render_spike_result_id || input.spikeResult.final_render_execution_request_id !== input.uploadExecutionDesign.final_render_execution_request_id || input.spikeResult.render_plan_id !== input.uploadExecutionDesign.render_plan_id || input.spikeResult.project_id !== input.uploadExecutionDesign.project_id || input.spikeResult.platform !== input.uploadExecutionDesign.platform) throw new Error("createFinalRealUploadPreflightGate: uploadExecutionDesign inputs must match");
  if (input.spikeResult.production_render_spike_result_id !== input.uploadExecutionApproval.production_render_spike_result_id || input.spikeResult.final_render_execution_request_id !== input.uploadExecutionApproval.final_render_execution_request_id || input.spikeResult.render_plan_id !== input.uploadExecutionApproval.render_plan_id || input.spikeResult.project_id !== input.uploadExecutionApproval.project_id || input.spikeResult.platform !== input.uploadExecutionApproval.platform) throw new Error("createFinalRealUploadPreflightGate: uploadExecutionApproval inputs must match");
  if (input.spikeResult.production_render_spike_result_id !== input.platformUploadRequest.production_render_spike_result_id || input.spikeResult.final_render_execution_request_id !== input.platformUploadRequest.final_render_execution_request_id || input.spikeResult.render_plan_id !== input.platformUploadRequest.render_plan_id || input.spikeResult.project_id !== input.platformUploadRequest.project_id || input.spikeResult.platform !== input.platformUploadRequest.platform) throw new Error("createFinalRealUploadPreflightGate: platformUploadRequest inputs must match");
  if (input.spikeResult.production_render_spike_result_id !== input.uploadPackageDesign.production_render_spike_result_id || input.spikeResult.final_render_execution_request_id !== input.uploadPackageDesign.final_render_execution_request_id || input.spikeResult.render_plan_id !== input.uploadPackageDesign.render_plan_id || input.spikeResult.project_id !== input.uploadPackageDesign.project_id || input.spikeResult.platform !== input.uploadPackageDesign.platform) throw new Error("createFinalRealUploadPreflightGate: uploadPackageDesign inputs must match");
  if (input.spikeResult.production_render_spike_result_id !== input.localOutputReview.production_render_spike_result_id || input.spikeResult.final_render_execution_request_id !== input.localOutputReview.final_render_execution_request_id || input.spikeResult.render_plan_id !== input.localOutputReview.render_plan_id || input.spikeResult.project_id !== input.localOutputReview.project_id || input.spikeResult.platform !== input.localOutputReview.platform) throw new Error("createFinalRealUploadPreflightGate: localOutputReview inputs must match");
  if (input.strategyDesign.validation.ready_for_real_upload !== false || input.realUploadExecutionRequest.validation.ready_for_real_upload !== false || input.readinessAssessment.validation.ready_for_real_upload !== false || input.executionPlan.validation.ready_for_real_upload !== false) throw new Error("createFinalRealUploadPreflightGate: prior ready_for_real_upload flags must remain false");
  if (input.executionPlan.plan_state !== "approved_for_future_upload_execution_dry_run" || input.strategyDesign.strategy_state !== "approved_for_future_upload_execution_plan" || input.realUploadExecutionRequest.request_state !== "approved_for_future_real_upload_design" || !["assessed","ready_for_operator_review"].includes(input.readinessAssessment.assessment_state)) throw new Error("createFinalRealUploadPreflightGate: prior approvals are required");
  const requiredKinds: FinalRealUploadUnresolvedExecutionRequirement["requirement_kind"][] = ["credential_storage_policy","credential_scope_policy","platform_api_endpoint_policy","platform_api_payload_policy","network_execution_policy","media_path_access_policy","upload_retry_policy","idempotency_policy","rollback_policy","post_upload_verification_policy","audit_logging_policy","operator_runbook_policy","emergency_stop_policy","manual_final_confirmation_policy"];
  const requirements: FinalRealUploadUnresolvedExecutionRequirement[] = requiredKinds.map((kind, index) => ({ requirement_id: `final-real-upload-preflight-gate-requirement-${index + 1}`, requirement_kind: kind, requirement_state: "required", safe_summary: `Future ${kind.replace(/_/g, " ")} is required.`, blocking_reasons: ["Future implementation design is still required."], warnings: [] }));
  const approved = input.dryRunExecutionResult.validation.real_upload_dry_run_execution_passed === true && input.dryRunExecutionResult.validation.ready_for_future_real_upload_preflight === true;
  const chainSafe = approved && requirements.length === requiredKinds.length;
  const reviewedBy = safeFinalRealUploadPreflightString(input.reviewed_by_label ?? "", "[unsafe-reviewer]");
  const note = safeFinalRealUploadPreflightString(input.decision_note_summary ?? "", "[unsafe-preflight-note]");
  const decision = input.decision ?? "draft";
  if (decision === "approved_for_future_real_upload_implementation_design" && (!approved || input.checklist_acknowledged !== true || input.artifact_chain_acknowledged !== true || input.unresolved_requirements_acknowledged !== true || input.critical_risks_acknowledged !== true || input.upload_side_effect_risk_acknowledged !== true || input.platform_policy_risk_acknowledged !== true || input.credential_handling_risk_acknowledged !== true || input.token_scope_risk_acknowledged !== true || input.network_side_effect_risk_acknowledged !== true || input.media_access_risk_acknowledged !== true || input.retry_idempotency_risk_acknowledged !== true || input.rollback_limitations_acknowledged !== true || input.post_upload_visibility_risk_acknowledged !== true || input.operator_accountability_acknowledged !== true || input.understands_preflight_gate_only !== true || input.understands_no_upload_enabled !== true || input.understands_no_credentials_accessed !== true || input.understands_no_network_calls !== true || input.understands_no_media_reads !== true || input.understands_future_implementation_design_required !== true)) throw new Error("createFinalRealUploadPreflightGate: approved decision requires acknowledgements");
  const gate: FinalRealUploadPreflightGate = {
    schema_version: "1.0",
    final_real_upload_preflight_gate_id: `final-real-upload-preflight-gate-${crypto.randomUUID()}`,
    ...
    artifact_chain_assessment: { all_required_artifacts_present: true, all_required_artifacts_validated: true, dry_run_execution_passed: approved, prior_ready_for_real_upload_flags_false: true, prior_upload_allowed_flags_false: true, prior_network_api_credential_flags_false: true, prior_media_read_flags_false: true, chain_safe_for_future_implementation_design: chainSafe, chain_safe_for_real_upload_now: false, blocking_reasons: approved ? [] : ["Dry-run execution did not fully pass."], warnings: approved ? ["All prior artifacts validated; future implementation design may proceed safely."] : ["Dry-run execution did not fully pass."] },
    critical_risk_acknowledgements: { upload_side_effect_risk_acknowledged: input.upload_side_effect_risk_acknowledged === true, platform_policy_risk_acknowledged: input.platform_policy_risk_acknowledged === true, credential_handling_risk_acknowledged: input.credential_handling_risk_acknowledged === true, token_scope_risk_acknowledged: input.token_scope_risk_acknowledged === true, network_side_effect_risk_acknowledged: input.network_side_effect_risk_acknowledged === true, media_access_risk_acknowledged: input.media_access_risk_acknowledged === true, retry_idempotency_risk_acknowledged: input.retry_idempotency_risk_acknowledged === true, rollback_limitations_acknowledged: input.rollback_limitations_acknowledged === true, post_upload_visibility_risk_acknowledged: input.post_upload_visibility_risk_acknowledged === true, operator_accountability_acknowledged: input.operator_accountability_acknowledged === true },
    unresolved_execution_requirements: requirements,
    operator_preflight_review: { reviewed_by_label: reviewedBy, checklist_acknowledged: input.checklist_acknowledged === true, artifact_chain_acknowledged: input.artifact_chain_acknowledged === true, unresolved_requirements_acknowledged: input.unresolved_requirements_acknowledged === true, critical_risks_acknowledged: input.critical_risks_acknowledged === true, understands_preflight_gate_only: input.understands_preflight_gate_only === true, understands_no_upload_enabled: input.understands_no_upload_enabled === true, understands_no_credentials_accessed: input.understands_no_credentials_accessed === true, understands_no_network_calls: input.understands_no_network_calls === true, understands_no_media_reads: input.understands_no_media_reads === true, understands_future_implementation_design_required: input.understands_future_implementation_design_required === true, decision_note_summary: note },
    credential_boundary: { credentials_required: false, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, credential_reference_stored: false, token_reference_stored: false, secret_material_stored: false },
    network_boundary: { network_calls_allowed: false, network_calls_made: false, external_side_effects_allowed: false, external_side_effects_observed: false },
    platform_api_boundary: { platform_api_calls_allowed: false, platform_api_calls_made: false, platform_endpoint_selected: false, platform_api_payload_created: false, platform_api_payload_stored: false, raw_account_ids_stored: false, upload_payload_created: false, response_received: false, raw_response_stored: false },
    media_boundary: { media_file_read: false, raw_media_path_stored: false, media_file_modified: false, media_file_copied: false, media_file_moved: false, media_file_deleted: false },
    execution_boundary: { upload_allowed: false, upload_execution_enabled: false, real_upload_execution_allowed: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, media_file_read: false, ready_for_real_upload: false },
    validation: { final_real_upload_preflight_passed: approved && input.checklist_acknowledged === true && input.artifact_chain_acknowledged === true && input.unresolved_requirements_acknowledged === true && input.critical_risks_acknowledged === true && input.upload_side_effect_risk_acknowledged === true && input.platform_policy_risk_acknowledged === true && input.credential_handling_risk_acknowledged === true && input.token_scope_risk_acknowledged === true && input.network_side_effect_risk_acknowledged === true && input.media_access_risk_acknowledged === true && input.retry_idempotency_risk_acknowledged === true && input.rollback_limitations_acknowledged === true && input.post_upload_visibility_risk_acknowledged === true && input.operator_accountability_acknowledged === true && input.understands_preflight_gate_only === true && input.understands_no_upload_enabled === true && input.understands_no_credentials_accessed === true && input.understands_no_network_calls === true && input.understands_no_media_reads === true && input.understands_future_implementation_design_required === true, ready_for_future_real_upload_implementation_design: approved && input.checklist_acknowledged === true && input.artifact_chain_acknowledged === true && input.unresolved_requirements_acknowledged === true && input.critical_risks_acknowledged === true && input.upload_side_effect_risk_acknowledged === true && input.platform_policy_risk_acknowledged === true && input.credential_handling_risk_acknowledged === true && input.token_scope_risk_acknowledged === true && input.network_side_effect_risk_acknowledged === true && input.media_access_risk_acknowledged === true && input.retry_idempotency_risk_acknowledged === true && input.rollback_limitations_acknowledged === true && input.post_upload_visibility_risk_acknowledged === true && input.operator_accountability_acknowledged === true && input.understands_preflight_gate_only === true && input.understands_no_upload_enabled === true && input.understands_no_credentials_accessed === true && input.understands_no_network_calls === true && input.understands_no_media_reads === true && input.understands_future_implementation_design_required === true, ready_for_real_upload: false, upload_allowed: false, upload_execution_enabled: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, media_file_read: false, blocking_reasons: approved ? [] : ["Preflight gate requires a passed dry-run execution result."], warnings: approved ? ["Final preflight gate is ready for future implementation design review only."] : ["Dry-run execution has not yet passed."] },
    provenance: { generated_by: "createFinalRealUploadPreflightGate", source_real_upload_dry_run_execution_result_id: input.dryRunExecutionResult.real_upload_dry_run_execution_result_id, source_real_upload_execution_plan_id: input.executionPlan.real_upload_execution_plan_id, source_real_upload_strategy_design_id: input.strategyDesign.real_upload_strategy_design_id, source_real_upload_execution_request_id: input.realUploadExecutionRequest.real_upload_execution_request_id, source_real_upload_readiness_assessment_id: input.readinessAssessment.real_upload_readiness_assessment_id, source_dry_run_upload_spike_result_id: input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id, source_upload_execution_design_id: input.uploadExecutionDesign.upload_execution_design_id, source_upload_execution_approval_id: input.uploadExecutionApproval.upload_execution_approval_id, source_platform_upload_request_id: input.platformUploadRequest.platform_upload_request_id, source_upload_package_design_id: input.uploadPackageDesign.upload_package_design_id, source_local_output_review_id: input.localOutputReview.local_output_review_id, source_production_render_spike_result_id: input.spikeResult.production_render_spike_result_id, source_final_render_execution_request_id: input.spikeResult.final_render_execution_request_id, source_render_plan_id: input.spikeResult.render_plan_id },
  };
  if (!validateFinalRealUploadPreflightGate(gate).ok) throw new Error("Unsafe final real upload preflight gate cannot be created.");
  return gate;
}
export function validateFinalRealUploadPreflightGate(gate: unknown): FinalRealUploadPreflightGateValidationResult { return validateFinalRealUploadPreflightGateShape(gate); }
interface FinalRealUploadPreflightGatesStoreSummaryItem { final_real_upload_preflight_gate_id: string; project_id: string; platform: string; gate_state: string; created_at: string; }
function safeFinalRealUploadPreflightGateSummary(gate: FinalRealUploadPreflightGate): FinalRealUploadPreflightGatesStoreSummaryItem { return { final_real_upload_preflight_gate_id: safeFinalRealUploadPreflightString(gate.final_real_upload_preflight_gate_id, "[unsafe-id]"), project_id: safeFinalRealUploadPreflightString(gate.project_id, "[unsafe-project]"), platform: safeFinalRealUploadPreflightString(gate.platform, "[unsafe-platform]"), gate_state: gate.gate_state, created_at: gate.created_at }; }
export function saveFinalRealUploadPreflightGate(gate: FinalRealUploadPreflightGate): void { if (!validateFinalRealUploadPreflightGate(gate).ok) throw new Error("Unsafe final real upload preflight gate cannot be stored."); const store = loadFinalRealUploadPreflightGatesStore(); const existing = store.gates.findIndex((item) => item.final_real_upload_preflight_gate_id === gate.final_real_upload_preflight_gate_id); if (existing >= 0) store.gates[existing] = gate; else store.gates.push(gate); saveFinalRealUploadPreflightGatesStore(store); }
export function listFinalRealUploadPreflightGates(options?: { project_id?: string; platform?: string; gate_state?: string; real_upload_dry_run_execution_result_id?: string; real_upload_execution_plan_id?: string }): FinalRealUploadPreflightGate[] { const store = loadFinalRealUploadPreflightGatesStore(); return store.gates.filter((gate) => (!options?.project_id || gate.project_id === options.project_id) && (!options?.platform || gate.platform === options.platform) && (!options?.gate_state || gate.gate_state === options.gate_state) && (!options?.real_upload_dry_run_execution_result_id || gate.real_upload_dry_run_execution_result_id === options.real_upload_dry_run_execution_result_id) && (!options?.real_upload_execution_plan_id || gate.real_upload_execution_plan_id === options.real_upload_execution_plan_id)).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.final_real_upload_preflight_gate_id.localeCompare(b.final_real_upload_preflight_gate_id)); }
export function getFinalRealUploadPreflightGate(final_real_upload_preflight_gate_id: string): FinalRealUploadPreflightGate | null { return loadFinalRealUploadPreflightGatesStore().gates.find((gate) => gate.final_real_upload_preflight_gate_id === final_real_upload_preflight_gate_id) ?? null; }
export function revokeFinalRealUploadPreflightGate(final_real_upload_preflight_gate_id: string, reason: string): FinalRealUploadPreflightGate { const gate = getFinalRealUploadPreflightGate(final_real_upload_preflight_gate_id); if (!gate) throw new Error("Final real upload preflight gate not found"); const safeReason = safeFinalRealUploadPreflightString(reason, "[unsafe-reason]"); if (safeReason === "[unsafe-reason]") throw new Error("Unsafe final real upload preflight gate reason cannot be stored."); const revoked: FinalRealUploadPreflightGate = { ...gate, gate_state: "revoked", validation: { ...gate.validation, blocking_reasons: [...gate.validation.blocking_reasons, safeReason], warnings: [...gate.validation.warnings, "Final real upload preflight gate revoked."] }, provenance: { ...gate.provenance, generated_by: "revokeFinalRealUploadPreflightGate" } }; saveFinalRealUploadPreflightGate(revoked); return revoked; }
export function getFinalRealUploadPreflightGateReport(options?: { project_id?: string; platform?: string }): { total: number; by_state: Record<string, number>; blocked: number; ready_for_operator_review: number; approved_for_future_real_upload_implementation_design: number; rejected: number; revoked: number; final_real_upload_preflight_passed: number; ready_for_future_real_upload_implementation_design: number; ready_for_real_upload: 0; upload_allowed: 0; upload_execution_enabled: 0; platform_api_calls_allowed: 0; network_calls_allowed: 0; credentials_accessed: 0; token_accessed: 0; media_file_read: 0; unresolved_requirements_total: number; unresolved_requirements_blocking: number; gates: Array<FinalRealUploadPreflightGatesStoreSummaryItem>; } { const gates = listFinalRealUploadPreflightGates(options); const by_state: Record<string, number> = {}; let blocked = 0, ready = 0, approved = 0, rejected = 0, revoked = 0, passed = 0, reqTotal = 0, reqBlocking = 0; const summaries = gates.map((gate) => { by_state[gate.gate_state] = (by_state[gate.gate_state] ?? 0) + 1; if (gate.gate_state === "blocked") blocked++; if (gate.gate_state === "ready_for_operator_review") ready++; if (gate.gate_state === "approved_for_future_real_upload_implementation_design") approved++; if (gate.gate_state === "rejected") rejected++; if (gate.gate_state === "revoked") revoked++; if (gate.validation.final_real_upload_preflight_passed) passed++; reqTotal += gate.unresolved_execution_requirements.length; reqBlocking += gate.unresolved_execution_requirements.filter((req) => req.blocking_reasons.length > 0).length; return safeFinalRealUploadPreflightGateSummary(gate); }); return { total: summaries.length, by_state, blocked, ready_for_operator_review: ready, approved_for_future_real_upload_implementation_design: approved, rejected, revoked, final_real_upload_preflight_passed: passed, ready_for_future_real_upload_implementation_design: passed, ready_for_real_upload: 0, upload_allowed: 0, upload_execution_enabled: 0, platform_api_calls_allowed: 0, network_calls_allowed: 0, credentials_accessed: 0, token_accessed: 0, media_file_read: 0, unresolved_requirements_total: reqTotal, unresolved_requirements_blocking: reqBlocking, gates: summaries }; }

// ─── VO-7N: Real Upload Implementation Design ────────────────────────────

export type RealUploadImplementationDesignMode = "real_upload_implementation_design_only" | "operator_review_real_upload_implementation_design";
export type RealUploadImplementationDesignState = "draft" | "blocked" | "ready_for_operator_review" | "approved_for_future_scaffold_design" | "rejected" | "revoked";

export interface RealUploadImplementationRequiredArtifacts {
  final_real_upload_preflight_gate_validated: boolean;
  real_upload_dry_run_execution_result_validated: boolean;
  real_upload_execution_plan_validated: boolean;
  real_upload_strategy_design_validated: boolean;
  real_upload_execution_request_validated: boolean;
...
export interface RealUploadImplementationScope {
  future_scaffold_design_requested: boolean;
  current_real_upload_requested: false;
  current_network_calls_requested: false;
  current_platform_api_calls_requested: false;
  current_credential_access_requested: false;
...
export type RealUploadImplementationComponentState = "planned" | "blocked" | "deferred" | "ready_for_future_scaffold";

export interface RealUploadImplementationComponentDesign {
  component_id: string;
  component_kind:
    | "credential_provider_boundary"
    | "media_reference_resolver"
    | "platform_adapter_interface"
    | "payload_builder_boundary"
    | "network_client_boundary"
    | "upload_executor_boundary"
    | "retry_idempotency_controller"
    | "rollback_controller"
    | "post_upload_verifier"
    | "audit_event_writer"
    | "operator_confirmation_gate"
    | "emergency_stop_controller"
    | "status_reporter";
  component_state: RealUploadImplementationComponentState;
  safe_summary: string;
  execution_enabled: false;
...
export interface RealUploadCredentialHandlingDesign {
...
export interface RealUploadMediaResolutionDesign {
  design_required: true;
  media_file_read: false;
  raw_media_path_stored: false;
...
export interface RealUploadPlatformAdapterDesign {
  design_required: true;
...
export interface RealUploadPayloadConstructionDesign {
  design_required: true;
  platform_api_payload_created: false;
  platform_api_payload_stored: false;
...
export interface RealUploadNetworkExecutionDesign {
  design_required: true;
...
export interface RealUploadRetryIdempotencyDesign {
  design_required: true;
  retry_execution_enabled: false;
  max_upload_attempts: 0;
  idempotency_design_defined: boolean;
  safe_summary: string;
  blocking_reasons: string[];
  warnings: string[];
}

export interface RealUploadRollbackDesign {
  design_required: true;
...
export interface RealUploadPostUploadVerificationDesign {
  design_required: true;
...
export interface RealUploadAuditLoggingDesign {
  design_required: true;
  audit_log_write_enabled: false;
...
export interface RealUploadOperatorControlDesign {
  design_required: true;
...
export interface RealUploadEmergencyStopDesign {
  design_required: true;
  emergency_stop_required: true;
  emergency_stop_implemented: false;
  automatic_kill_switch_enabled: false;
  safe_summary: string;
  blocking_reasons: string[];
  warnings: string[];
}

export interface RealUploadImplementationBoundary {
  upload_allowed: false;
  upload_execution_enabled: false;
  real_upload_execution_allowed: false;
  scaffold_code_allowed: false;
  platform_api_calls_allowed: false;
  network_calls_allowed: false;
  credentials_accessed: false;
  token_accessed: false;
...
export interface RealUploadImplementationDesignValidationResult { ok: boolean; blocking_reasons: string[]; warnings: string[]; }

export interface RealUploadImplementationDesign {
  schema_version: "1.0";
  real_upload_implementation_design_id: string;
  final_real_upload_preflight_gate_id: string;
  real_upload_dry_run_execution_result_id: string;
  real_upload_execution_plan_id: string;
  real_upload_strategy_design_id: string;
  ...
  project_id: string;
  platform: string;
  design_state: RealUploadImplementationDesignState;
  created_at: string;
  design_mode: RealUploadImplementationDesignMode;
  required_artifacts: RealUploadImplementationRequiredArtifacts;
  implementation_scope: RealUploadImplementationScope;
  component_designs: RealUploadImplementationComponentDesign[];
  credential_handling_design: RealUploadCredentialHandlingDesign;
  media_resolution_design: RealUploadMediaResolutionDesign;
  platform_adapter_design: RealUploadPlatformAdapterDesign;
  payload_construction_design: RealUploadPayloadConstructionDesign;
  network_execution_design: RealUploadNetworkExecutionDesign;
  retry_idempotency_design: RealUploadRetryIdempotencyDesign;
  rollback_design: RealUploadRollbackDesign;
  post_upload_verification_design: RealUploadPostUploadVerificationDesign;
  audit_logging_design: RealUploadAuditLoggingDesign;
  operator_control_design: RealUploadOperatorControlDesign;
  emergency_stop_design: RealUploadEmergencyStopDesign;
  implementation_boundary: RealUploadImplementationBoundary;
  validation: {
    implementation_design_complete: boolean;
    ready_for_future_scaffold_design: boolean;
    ready_for_real_upload: false;
    upload_allowed: false;
    upload_execution_enabled: false;
    platform_api_calls_allowed: false;
  ...
  provenance: {
    generated_by: "createRealUploadImplementationDesign" | "revokeRealUploadImplementationDesign";
    source_final_real_upload_preflight_gate_id: string;
    source_real_upload_dry_run_execution_result_id: string;
    source_real_upload_execution_plan_id: string;
    source_real_upload_strategy_design_id: string;
    source_real_upload_execution_request_id: string;
...
interface RealUploadImplementationDesignsStore { schema_version: "1.0"; created_at: string; designs: RealUploadImplementationDesign[]; }
function getRealUploadImplementationDesignsPath(): string { return path.join(getRuntimeDir(), "real-upload-implementation-designs.json"); }
function loadRealUploadImplementationDesignsStore(): RealUploadImplementationDesignsStore { try { const filePath = getRealUploadImplementationDesignsPath(); if (fs.existsSync(filePath)) return JSON.parse(fs.readFileSync(filePath, "utf8")) as RealUploadImplementationDesignsStore; } catch {} return { schema_version: "1.0", created_at: new Date().toISOString(), designs: [] }; }
function saveRealUploadImplementationDesignsStore(store: RealUploadImplementationDesignsStore): void { const filePath = getRealUploadImplementationDesignsPath(); fs.mkdirSync(path.dirname(filePath), { recursive: true }); store.designs.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_implementation_design_id.localeCompare(b.real_upload_implementation_design_id)); fs.writeFileSync(filePath, JSON.stringify(store, null, 2), "utf8"); }
function safeRealUploadImplementationDesignString(value: unknown, fallback: string): string { return safeRealUploadDryRunExecutionResultString(value, fallback); }
function validateRealUploadImplementationDesignShape(design: unknown): RealUploadImplementationDesignValidationResult {
  const blocking_reasons: string[] = [];
  const warnings: string[] = [];
  if (!design || typeof design !== "object" || Array.isArray(design)) {
    return { ok: false, blocking_reasons: ["Design must be an object"], warnings };
  }
  const a = design as Record<string, unknown>;
  const requiredFields = ["schema_version","real_upload_implementation_design_id","final_real_upload_preflight_gate_id","real_upload_dry_run_execution_result_id","real_upload_execution_plan_id","real_upload_strategy_design_id","real_upload_execution_request_id","real_upload_readiness_assessment_id","dry_run_upload_spike_result_id","upload_execution_design_id","upload_execution_approval_id","platform_upload_request_id","upload_package_design_id","local_output_review_id","production_render_spike_result_id","final_render_execution_request_id","render_plan_id","project_id","platform","design_state","created_at","design_mode","required_artifacts","implementation_scope","component_designs","credential_handling_design","media_resolution_design","platform_adapter_design","payload_construction_design","network_execution_design","retry_idempotency_design","rollback_design","post_upload_verification_design","audit_logging_design","operator_control_design","emergency_stop_design","implementation_boundary","validation","provenance"];
  for (const key of requiredFields) {
    if (!(key in a)) blocking_reasons.push(`Missing required field: ${key}`);
  }
  const json = JSON.stringify(a);
  if (json.includes("videos.insert") || json.includes("youtube.videos().insert") || json.includes("fetch(") || json.includes("process.env") || json.includes("keychain://") || json.includes("stdout") || json.includes("stderr") || json.includes("ffmpeg") || json.includes("Bearer ") || json.includes("data:") || json.includes("base64,") || json.includes("/Users/") || json.includes("https://") || json.includes("http://") || json.includes("../")) blocking_reasons.push("Real upload implementation design contains forbidden payload content");
  return { ok: blocking_reasons.length === 0, blocking_reasons, warnings };
}

export function validateRealUploadImplementationDesign(design: unknown): RealUploadImplementationDesignValidationResult {
  const validation = validateRealUploadImplementationDesignShape(design);
  if (!validation.ok) return validation;
  const d = design as RealUploadImplementationDesign;
  const requiredKinds: RealUploadImplementationDesign["component_designs"][number]["component_kind"][] = [
    "credential_provider_boundary",
    "media_reference_resolver",
    "platform_adapter_interface",
    "payload_builder_boundary",
  ...
  if (d.schema_version !== "1.0") validation.blocking_reasons.push("schema_version must be 1.0");
  if (d.design_mode !== "real_upload_implementation_design_only" && d.design_mode !== "operator_review_real_upload_implementation_design") validation.blocking_reasons.push("Invalid design_mode");
  if (d.design_state === "approved_for_future_scaffold_design") {
    // allowed
  } else if (!["draft", "blocked", "ready_for_operator_review", "approved_for_future_scaffold_design", "rejected", "revoked"].includes(d.design_state)) {
    validation.blocking_reasons.push("Invalid design_state");
  }
  const scope = d.implementation_scope;
  if (!scope || scope.implementation_design_only !== true || scope.current_real_upload_requested !== false || scope.current_network_calls_requested !== false || scope.current_platform_api_calls_requested !== false || scope.current_credential_access_requested !== false || scope.current_media_read_requested !== false) validation.blocking_reasons.push("Implementation scope is unsafe");
  const componentKinds = new Set(d.component_designs.map((component) => component.component_kind));
  for (const kind of requiredKinds) if (!componentKinds.has(kind)) validation.blocking_reasons.push(`Missing component kind: ${kind}`);
  for (const component of d.component_designs) {
    if (component.execution_enabled !== false || component.network_enabled !== false || component.platform_api_enabled !== false || component.credential_access_enabled !== false || component.media_read_enabled !== false || component.file_mutation_enabled !== false) validation.blocking_reasons.push("Component design is unsafe");
  }
  const boundary = d.implementation_boundary;
  if (!boundary || boundary.upload_allowed !== false || boundary.upload_execution_enabled !== false || boundary.real_upload_execution_allowed !== false || boundary.scaffold_code_allowed !== false || boundary.platform_api_calls_allowed !== false || boundary.network_calls_allowed !== false || boundary.credentials_accessed !== false || boundary.token_accessed !== false || boundary.keychain_accessed !== false || boundary.env_accessed !== false || boundary.media_file_read !== false || boundary.file_mutation_allowed !== false || boundary.ready_for_real_upload !== false) validation.blocking_reasons.push("Implementation boundary is unsafe");
  if (d.credential_handling_design.credentials_required !== false || d.credential_handling_design.credentials_accessed !== false || d.credential_handling_design.token_accessed !== false || d.credential_handling_design.keychain_accessed !== false || d.credential_handling_design.env_accessed !== false || d.credential_handling_design.credential_reference_stored !== false || d.credential_handling_design.token_reference_stored !== false || d.credential_handling_design.secret_material_stored !== false) validation.blocking_reasons.push("Credential handling design is unsafe");
  if (d.media_resolution_design.media_file_read !== false || d.media_resolution_design.raw_media_path_stored !== false || d.media_resolution_design.media_file_modified !== false || d.media_resolution_design.media_file_copied !== false || d.media_resolution_design.media_file_moved !== false || d.media_resolution_design.media_file_deleted !== false) validation.blocking_reasons.push("Media resolution design is unsafe");
  if (d.platform_adapter_design.platform_adapter_implemented !== false || d.platform_adapter_design.platform_endpoint_selected !== false || d.platform_adapter_design.platform_api_calls_allowed !== false || d.platform_adapter_design.platform_api_calls_made !== false || d.platform_adapter_design.raw_account_ids_stored !== false) validation.blocking_reasons.push("Platform adapter design is unsafe");
  if (d.payload_construction_design.platform_api_payload_created !== false || d.payload_construction_design.platform_api_payload_stored !== false || d.payload_construction_design.upload_payload_created !== false || d.payload_construction_design.raw_payload_stored !== false) validation.blocking_reasons.push("Payload construction design is unsafe");
  if (d.network_execution_design.network_client_implemented !== false || d.network_execution_design.network_calls_allowed !== false || d.network_execution_design.network_calls_made !== false || d.network_execution_design.external_side_effects_allowed !== false || d.network_execution_design.external_side_effects_observed !== false) validation.blocking_reasons.push("Network execution design is unsafe");
  if (d.retry_idempotency_design.retry_execution_enabled !== false || d.retry_idempotency_design.max_upload_attempts !== 0) validation.blocking_reasons.push("Retry design is unsafe");
  if (d.rollback_design.rollback_execution_enabled !== false || d.rollback_design.destructive_action_allowed !== false) validation.blocking_reasons.push("Rollback design is unsafe");
  if (d.post_upload_verification_design.verification_execution_enabled !== false || d.post_upload_verification_design.platform_api_calls_allowed !== false || d.post_upload_verification_design.network_calls_allowed !== false) validation.blocking_reasons.push("Verification design is unsafe");
  if (d.audit_logging_design.audit_log_write_enabled !== false || d.audit_logging_design.raw_payload_logging_allowed !== false || d.audit_logging_design.raw_response_logging_allowed !== false || d.audit_logging_design.secret_logging_allowed !== false) validation.blocking_reasons.push("Audit logging design is unsafe");
  if (d.operator_control_design.operator_confirmation_required !== true || d.operator_control_design.automatic_execution_enabled !== false || d.operator_control_design.manual_go_no_go_required !== true) validation.blocking_reasons.push("Operator control design is unsafe");
  if (d.emergency_stop_design.emergency_stop_required !== true || d.emergency_stop_design.emergency_stop_implemented !== false || d.emergency_stop_design.automatic_kill_switch_enabled !== false) validation.blocking_reasons.push("Emergency stop design is unsafe");
  if (d.validation.ready_for_real_upload !== false || d.validation.upload_allowed !== false || d.validation.upload_execution_enabled !== false || d.validation.platform_api_calls_allowed !== false || d.validation.network_calls_allowed !== false || d.validation.credentials_accessed !== false || d.validation.token_accessed !== false || d.validation.media_file_read !== false) validation.blocking_reasons.push("Validation flags are unsafe");
  if (d.required_artifacts.final_real_upload_preflight_gate_validated !== true || d.required_artifacts.real_upload_dry_run_execution_result_validated !== true || d.required_artifacts.real_upload_execution_plan_validated !== true || d.required_artifacts.real_upload_strategy_design_validated !== true || d.required_artifacts.real_upload_execution_request_validated !== true || d.required_artifacts.real_upload_readiness_assessment_validated !== true || d.required_artifacts.dry_run_upload_spike_result_validated !== true || d.required_artifacts.upload_execution_design_validated !== true || d.required_artifacts.upload_execution_approval_validated !== true || d.required_artifacts.platform_upload_request_validated !== true || d.required_artifacts.upload_package_design_validated !== true || d.required_artifacts.local_output_review_validated !== true || d.required_artifacts.spike_result_validated !== true) validation.blocking_reasons.push("Required artifacts are unsafe");
  if (d.implementation_scope.future_scaffold_design_requested !== true) validation.blocking_reasons.push("future_scaffold_design_requested must be true");
  if (validation.blocking_reasons.length === 0) validation.warnings.push("Design is future-scaffold only.");
  return { ok: validation.blocking_reasons.length === 0, blocking_reasons: validation.blocking_reasons, warnings: validation.warnings };
}

export function createRealUploadImplementationDesign(input: {
  ...
  localOutputReview: LocalOutputOperatorReview;
  spikeResult: ControlledProductionRenderSpikeResult;
  decision?: "draft" | "approved_for_future_scaffold_design" | "rejected";
  reviewed_by_label?: string;
  checklist_acknowledged?: boolean;
  understands_implementation_design_only?: boolean;
  understands_no_upload_enabled?: boolean;
  ...
  if (input.dryRun !== true) throw new Error("createRealUploadImplementationDesign: dryRun=true required");
  if (!validateFinalRealUploadPreflightGate(input.finalPreflightGate).ok) throw new Error("createRealUploadImplementationDesign: finalPreflightGate must validate");
  if (!validateRealUploadDryRunExecutionResult(input.dryRunExecutionResult).ok) throw new Error("createRealUploadImplementationDesign: dryRunExecutionResult must validate");
  if (!validateRealUploadExecutionPlan(input.executionPlan).ok) throw new Error("createRealUploadImplementationDesign: executionPlan must validate");
  if (!validateRealUploadStrategyDesign(input.strategyDesign).ok) throw new Error("createRealUploadImplementationDesign: strategyDesign must validate");
  if (!validateRealUploadExecutionRequest(input.realUploadExecutionRequest).ok) throw new Error("createRealUploadImplementationDesign: realUploadExecutionRequest must validate");
  if (!validateRealUploadReadinessAssessment(input.readinessAssessment).ok) throw new Error("createRealUploadImplementationDesign: readinessAssessment must validate");
  if (!validateDryRunUploadSpikeResult(input.dryRunUploadSpikeResult).ok) throw new Error("createRealUploadImplementationDesign: dryRunUploadSpikeResult must validate");
  if (!validateUploadExecutionDesign(input.uploadExecutionDesign).ok) throw new Error("createRealUploadImplementationDesign: uploadExecutionDesign must validate");
  if (!validateUploadExecutionApproval(input.uploadExecutionApproval).ok) throw new Error("createRealUploadImplementationDesign: uploadExecutionApproval must validate");
  if (!validatePlatformUploadRequest(input.platformUploadRequest).ok) throw new Error("createRealUploadImplementationDesign: platformUploadRequest must validate");
  if (!validateUploadPackageDesign(input.uploadPackageDesign).ok) throw new Error("createRealUploadImplementationDesign: uploadPackageDesign must validate");
  if (!validateLocalOutputOperatorReview(input.localOutputReview).ok) throw new Error("createRealUploadImplementationDesign: localOutputReview must validate");
  if (!validateControlledProductionRenderSpikeResult(input.spikeResult).ok) throw new Error("createRealUploadImplementationDesign: spikeResult must validate");
  if (input.finalPreflightGate.gate_state !== "approved_for_future_real_upload_implementation_design") throw new Error("createRealUploadImplementationDesign: finalPreflightGate must be approved_for_future_real_upload_implementation_design");
  if (input.finalPreflightGate.validation.ready_for_real_upload !== false || input.dryRunExecutionResult.validation.ready_for_real_upload !== false || input.executionPlan.validation.ready_for_real_upload !== false) throw new Error("createRealUploadImplementationDesign: ready_for_real_upload must remain false");
  if (input.finalPreflightGate.validation.ready_for_future_real_upload_implementation_design !== true || input.dryRunExecutionResult.validation.ready_for_future_real_upload_preflight !== true) throw new Error("createRealUploadImplementationDesign: future design prerequisites must be ready");
  if (input.finalPreflightGate.real_upload_dry_run_execution_result_id !== input.dryRunExecutionResult.real_upload_dry_run_execution_result_id || input.finalPreflightGate.real_upload_execution_plan_id !== input.executionPlan.real_upload_execution_plan_id || input.finalPreflightGate.real_upload_strategy_design_id !== input.strategyDesign.real_upload_strategy_design_id || input.finalPreflightGate.real_upload_execution_request_id !== input.realUploadExecutionRequest.real_upload_execution_request_id || input.finalPreflightGate.real_upload_readiness_assessment_id !== input.readinessAssessment.real_upload_readiness_assessment_id || input.finalPreflightGate.dry_run_upload_spike_result_id !== input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id || input.finalPreflightGate.upload_execution_design_id !== input.uploadExecutionDesign.upload_execution_design_id || input.finalPreflightGate.upload_execution_approval_id !== input.uploadExecutionApproval.upload_execution_approval_id || input.finalPreflightGate.platform_upload_request_id !== input.platformUploadRequest.platform_upload_request_id || input.finalPreflightGate.upload_package_design_id !== input.uploadPackageDesign.upload_package_design_id || input.finalPreflightGate.local_output_review_id !== input.localOutputReview.local_output_review_id || input.finalPreflightGate.production_render_spike_result_id !== input.spikeResult.production_render_spike_result_id || input.finalPreflightGate.final_render_execution_request_id !== input.spikeResult.final_render_execution_request_id || input.finalPreflightGate.render_plan_id !== input.spikeResult.render_plan_id || input.finalPreflightGate.project_id !== input.spikeResult.project_id || input.finalPreflightGate.platform !== input.spikeResult.platform) throw new Error("createRealUploadImplementationDesign: finalPreflightGate inputs must match");
  if (input.executionPlan.plan_state !== "approved_for_future_upload_execution_dry_run" || input.strategyDesign.strategy_state !== "approved_for_future_upload_execution_plan" || input.realUploadExecutionRequest.request_state !== "approved_for_future_real_upload_design" || !["assessed", "ready_for_operator_review"].includes(input.readinessAssessment.assessment_state) || input.finalPreflightGate.gate_state !== "approved_for_future_real_upload_implementation_design") throw new Error("createRealUploadImplementationDesign: prior approvals are required");
  const requiredKinds: RealUploadImplementationComponentDesign["component_kind"][] = ["credential_provider_boundary","media_reference_resolver","platform_adapter_interface","payload_builder_boundary","network_client_boundary","upload_executor_boundary","retry_idempotency_controller","rollback_controller","post_upload_verifier","audit_event_writer","operator_confirmation_gate","emergency_stop_controller","status_reporter"];
  const components: RealUploadImplementationComponentDesign[] = requiredKinds.map((kind, index) => ({ component_id: `real-upload-implementation-design-component-${index + 1}`, component_kind: kind, component_state: "planned", safe_summary: `Future ${kind.replace(/_/g, " ")} design only.`, execution_enabled: false, network_enabled: false, platform_api_enabled: false, credential_access_enabled: false, media_read_enabled: false, file_mutation_enabled: false, blocking_reasons: ["Future scaffold design only."], warnings: [] }));
  const reviewedBy = safeRealUploadImplementationDesignString(input.reviewed_by_label ?? "", "[unsafe-reviewer]");
  const note = safeRealUploadImplementationDesignString(input.decision_note_summary ?? "", "[unsafe-implementation-note]");
  const decision = input.decision ?? "draft";
  if (decision === "approved_for_future_scaffold_design" && (!input.checklist_acknowledged || input.understands_implementation_design_only !== true || input.understands_no_upload_enabled !== true || input.understands_no_credentials_accessed !== true || input.understands_no_network_calls !== true || input.understands_no_media_reads !== true || input.understands_future_scaffold_phase_required !== true)) throw new Error("createRealUploadImplementationDesign: approved decision requires acknowledgements");
  const design: RealUploadImplementationDesign = {
    schema_version: "1.0",
    real_upload_implementation_design_id: `real-upload-implementation-design-${crypto.randomUUID()}`,
    ...
    project_id: input.spikeResult.project_id,
    platform: input.spikeResult.platform,
    design_state: decision === "approved_for_future_scaffold_design" ? "approved_for_future_scaffold_design" : decision === "rejected" ? "rejected" : "draft",
    created_at: new Date().toISOString(),
    design_mode: "real_upload_implementation_design_only",
    required_artifacts: { final_real_upload_preflight_gate_validated: true, real_upload_dry_run_execution_result_validated: true, real_upload_execution_plan_validated: true, real_upload_strategy_design_validated: true, real_upload_execution_request_validated: true, real_upload_readiness_assessment_validated: true, dry_run_upload_spike_result_validated: true, upload_execution_design_validated: true, upload_execution_approval_validated: true, platform_upload_request_validated: true, upload_package_design_validated: true, local_output_review_validated: true, spike_result_validated: true },
    implementation_scope: { future_scaffold_design_requested: true, current_real_upload_requested: false, current_network_calls_requested: false, current_platform_api_calls_requested: false, current_credential_access_requested: false, current_media_read_requested: false, implementation_design_only: true },
    component_designs: components,
    credential_handling_design: { design_required: true, credentials_required: false, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, credential_reference_stored: false, token_reference_stored: false, secret_material_stored: false, safe_summary: "Credential handling design is future scaffold only.", blocking_reasons: ["Credential access remains disabled."], warnings: [] },
    media_resolution_design: { design_required: true, media_file_read: false, raw_media_path_stored: false, media_file_modified: false, media_file_copied: false, media_file_moved: false, media_file_deleted: false, safe_summary: "Media resolution design is future scaffold only.", blocking_reasons: ["Media reads remain disabled."], warnings: [] },
    platform_adapter_design: { design_required: true, platform_adapter_implemented: false, platform_endpoint_selected: false, platform_api_calls_allowed: false, platform_api_calls_made: false, raw_account_ids_stored: false, safe_summary: "Platform adapter design is future scaffold only.", blocking_reasons: ["Platform APIs remain disabled."], warnings: [] },
    payload_construction_design: { design_required: true, platform_api_payload_created: false, platform_api_payload_stored: false, upload_payload_created: false, raw_payload_stored: false, safe_summary: "Payload construction design is future scaffold only.", blocking_reasons: ["Payload construction remains disabled."], warnings: [] },
    network_execution_design: { design_required: true, network_client_implemented: false, network_calls_allowed: false, network_calls_made: false, external_side_effects_allowed: false, external_side_effects_observed: false, safe_summary: "Network execution design is future scaffold only.", blocking_reasons: ["Network execution remains disabled."], warnings: [] },
    retry_idempotency_design: { design_required: true, retry_execution_enabled: false, max_upload_attempts: 0, idempotency_design_defined: true, safe_summary: "Retry design is future scaffold only.", blocking_reasons: ["Retry execution remains disabled."], warnings: [] },
    rollback_design: { design_required: true, rollback_execution_enabled: false, destructive_action_allowed: false, safe_summary: "Rollback design is future scaffold only.", blocking_reasons: ["Rollback execution remains disabled."], warnings: [] },
    post_upload_verification_design: { design_required: true, verification_execution_enabled: false, platform_api_calls_allowed: false, network_calls_allowed: false, safe_summary: "Verification design is future scaffold only.", blocking_reasons: ["Verification execution remains disabled."], warnings: [] },
    audit_logging_design: { design_required: true, audit_log_write_enabled: false, raw_payload_logging_allowed: false, raw_response_logging_allowed: false, secret_logging_allowed: false, safe_summary: "Audit logging design is future scaffold only.", blocking_reasons: ["Raw logging remains disabled."], warnings: [] },
    operator_control_design: { design_required: true, operator_confirmation_required: true, automatic_execution_enabled: false, manual_go_no_go_required: true, reviewed_by_label: reviewedBy, checklist_acknowledged: input.checklist_acknowledged === true, understands_implementation_design_only: input.understands_implementation_design_only === true, understands_no_upload_enabled: input.understands_no_upload_enabled === true, understands_no_credentials_accessed: input.understands_no_credentials_accessed === true, understands_no_network_calls: input.understands_no_network_calls === true, understands_no_media_reads: input.understands_no_media_reads === true, understands_future_scaffold_phase_required: input.understands_future_scaffold_phase_required === true, decision_note_summary: note },
    emergency_stop_design: { design_required: true, emergency_stop_required: true, emergency_stop_implemented: false, automatic_kill_switch_enabled: false, safe_summary: "Emergency stop design is future scaffold only.", blocking_reasons: ["Emergency stop remains a design-only boundary."], warnings: [] },
    implementation_boundary: { upload_allowed: false, upload_execution_enabled: false, real_upload_execution_allowed: false, scaffold_code_allowed: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, media_file_read: false, file_mutation_allowed: false, ready_for_real_upload: false },
    validation: { implementation_design_complete: decision === "approved_for_future_scaffold_design" && input.checklist_acknowledged === true && input.understands_implementation_design_only === true && input.understands_no_upload_enabled === true && input.understands_no_credentials_accessed === true && input.understands_no_network_calls === true && input.understands_no_media_reads === true && input.understands_future_scaffold_phase_required === true, ready_for_future_scaffold_design: decision === "approved_for_future_scaffold_design" && input.checklist_acknowledged === true && input.understands_implementation_design_only === true && input.understands_no_upload_enabled === true && input.understands_no_credentials_accessed === true && input.understands_no_network_calls === true && input.understands_no_media_reads === true && input.understands_future_scaffold_phase_required === true, ready_for_real_upload: false, upload_allowed: false, upload_execution_enabled: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, media_file_read: false, blocking_reasons: [], warnings: [] },
    provenance: { generated_by: "createRealUploadImplementationDesign", source_final_real_upload_preflight_gate_id: input.finalPreflightGate.final_real_upload_preflight_gate_id, source_real_upload_dry_run_execution_result_id: input.dryRunExecutionResult.real_upload_dry_run_execution_result_id, source_real_upload_execution_plan_id: input.executionPlan.real_upload_execution_plan_id, source_real_upload_strategy_design_id: input.strategyDesign.real_upload_strategy_design_id, source_real_upload_execution_request_id: input.realUploadExecutionRequest.real_upload_execution_request_id, source_real_upload_readiness_assessment_id: input.readinessAssessment.real_upload_readiness_assessment_id, source_dry_run_upload_spike_result_id: input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id, source_upload_execution_design_id: input.uploadExecutionDesign.upload_execution_design_id, source_upload_execution_approval_id: input.uploadExecutionApproval.upload_execution_approval_id, source_platform_upload_request_id: input.platformUploadRequest.platform_upload_request_id, source_upload_package_design_id: input.uploadPackageDesign.upload_package_design_id, source_local_output_review_id: input.localOutputReview.local_output_review_id, source_production_render_spike_result_id: input.spikeResult.production_render_spike_result_id, source_final_render_execution_request_id: input.spikeResult.final_render_execution_request_id, source_render_plan_id: input.spikeResult.render_plan_id },
  };
  if (!validateRealUploadImplementationDesign(design).ok) throw new Error("Unsafe real upload implementation design cannot be created.");
  return design;
}

export function saveRealUploadImplementationDesign(design: RealUploadImplementationDesign): void { if (!validateRealUploadImplementationDesign(design).ok) throw new Error("Unsafe real upload implementation design cannot be stored."); const store = loadRealUploadImplementationDesignsStore(); const existingIndex = store.designs.findIndex((item) => item.real_upload_implementation_design_id === design.real_upload_implementation_design_id); if (existingIndex >= 0) store.designs[existingIndex] = design; else store.designs.push(design); saveRealUploadImplementationDesignsStore(store); }
export function listRealUploadImplementationDesigns(options?: { project_id?: string; platform?: string; design_state?: string; final_real_upload_preflight_gate_id?: string; real_upload_dry_run_execution_result_id?: string }): RealUploadImplementationDesign[] { const store = loadRealUploadImplementationDesignsStore(); return store.designs.filter((design) => (!options?.project_id || design.project_id === options.project_id) && (!options?.platform || design.platform === options.platform) && (!options?.design_state || design.design_state === options.design_state) && (!options?.final_real_upload_preflight_gate_id || design.final_real_upload_preflight_gate_id === options.final_real_upload_preflight_gate_id) && (!options?.real_upload_dry_run_execution_result_id || design.real_upload_dry_run_execution_result_id === options.real_upload_dry_run_execution_result_id)).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_implementation_design_id.localeCompare(b.real_upload_implementation_design_id)); }
export function getRealUploadImplementationDesign(real_upload_implementation_design_id: string): RealUploadImplementationDesign | null { return loadRealUploadImplementationDesignsStore().designs.find((design) => design.real_upload_implementation_design_id === real_upload_implementation_design_id) ?? null; }
export function revokeRealUploadImplementationDesign(real_upload_implementation_design_id: string, reason: string): RealUploadImplementationDesign { const design = getRealUploadImplementationDesign(real_upload_implementation_design_id); if (!design) throw new Error("Real upload implementation design not found"); const safeReason = safeRealUploadImplementationDesignString(reason, "[unsafe-reason]"); if (safeReason === "[unsafe-reason]") throw new Error("Unsafe real upload implementation design reason cannot be stored."); const revoked: RealUploadImplementationDesign = { ...design, design_state: "revoked", validation: { ...design.validation, blocking_reasons: [...design.validation.blocking_reasons, safeReason], warnings: [...design.validation.warnings, "Real upload implementation design revoked."] }, provenance: { ...design.provenance, generated_by: "revokeRealUploadImplementationDesign" } }; saveRealUploadImplementationDesign(revoked); return revoked; }
export function getRealUploadImplementationDesignReport(options?: { project_id?: string; platform?: string }): { total: number; by_state: Record<string, number>; blocked: number; ready_for_operator_review: number; approved_for_future_scaffold_design: number; rejected: number; revoked: number; implementation_design_complete: number; ready_for_future_scaffold_design: number; ready_for_real_upload: 0; upload_allowed: 0; upload_execution_enabled: 0; scaffold_code_allowed: 0; platform_api_calls_allowed: 0; network_calls_allowed: 0; credentials_accessed: 0; token_accessed: 0; media_file_read: 0; file_mutation_allowed: 0; components_total: number; components_with_blockers: number; designs: Array<{ real_upload_implementation_design_id: string; project_id: string; platform: string; design_state: string; created_at: string; }>; } { const designs = listRealUploadImplementationDesigns(options); const by_state: Record<string, number> = {}; let blocked = 0, ready = 0, approved = 0, rejected = 0, revoked = 0, complete = 0, readyFuture = 0, componentsTotal = 0, componentsBlocked = 0; const summaries = designs.map((design) => { by_state[design.design_state] = (by_state[design.design_state] ?? 0) + 1; if (design.design_state === "blocked") blocked++; if (design.design_state === "ready_for_operator_review") ready++; if (design.design_state === "approved_for_future_scaffold_design") approved++; if (design.design_state === "rejected") rejected++; if (design.design_state === "revoked") revoked++; if (design.validation.implementation_design_complete) complete++; if (design.validation.ready_for_future_scaffold_design) readyFuture++; componentsTotal += design.component_designs.length; componentsBlocked += design.component_designs.filter((component) => component.blocking_reasons.length > 0).length; return { real_upload_implementation_design_id: safeRealUploadImplementationDesignString(design.real_upload_implementation_design_id, "[unsafe-id]"), project_id: safeRealUploadImplementationDesignString(design.project_id, "[unsafe-project]"), platform: safeRealUploadImplementationDesignString(design.platform, "[unsafe-platform]"), design_state: design.design_state, created_at: design.created_at }; }); return { total: summaries.length, by_state, blocked, ready_for_operator_review: ready, approved_for_future_scaffold_design: approved, rejected, revoked, implementation_design_complete: complete, ready_for_future_scaffold_design: readyFuture, ready_for_real_upload: 0, upload_allowed: 0, upload_execution_enabled: 0, scaffold_code_allowed: 0, platform_api_calls_allowed: 0, network_calls_allowed: 0, credentials_accessed: 0, token_accessed: 0, media_file_read: 0, file_mutation_allowed: 0, components_total: componentsTotal, components_with_blockers: componentsBlocked, designs: summaries }; }
...
// ─── VO-7O: Real Upload Scaffold Design ───────────────────────────────────

export type RealUploadScaffoldDesignMode = "real_upload_scaffold_design_only" | "operator_review_real_upload_scaffold_design";
export type RealUploadScaffoldDesignState = "draft" | "blocked" | "ready_for_operator_review" | "approved_for_future_scaffold_contracts" | "rejected" | "revoked";
export interface RealUploadScaffoldRequiredArtifacts { real_upload_implementation_design_validated: boolean; final_real_upload_preflight_gate_validated: boolean; real_upload_dry_run_execution_result_validated: boolean; real_upload_execution_plan_validated: boolean; real_upload_strategy_design_validated: boolean; real_upload_execution_request_validated: boolean; real_upload_readiness_assessment_validated: boolean; dry_run_upload_spike_result_validated: boolean; upload_execution_design_validated: boolean; upload_execution_approval_validated: boolean; platform_upload_request_validated: boolean; upload_package_design_validated: boolean; local_output_review_validated: boolean; spike_result_validated: boolean; }
export interface RealUploadScaffoldScope { future_scaffold_contracts_requested: boolean; scaffold_design_only: true; scaffold_code_created: false; implementation_files_created: false; current_real_upload_requested: false; current_network_calls_requested: false; current_platform_api_calls_requested: false; current_credential_access_requested: false; current_media_read_requested: false; dependencies_requested: false; }
export interface RealUploadScaffoldProposedModule { module_id: string; module_kind: "credential_provider_contract" | "media_reference_resolver_contract" | "platform_adapter_contract" | "payload_builder_contract" | "network_client_contract" | "upload_executor_contract" | "retry_idempotency_contract" | "rollback_contract" | "post_upload_verification_contract" | "audit_event_contract" | "operator_confirmation_contract" | "emergency_stop_contract" | "status_reporter_contract" | "upload_scaffold_index_contract"; module_state: "proposed" | "blocked" | "deferred" | "ready_for_future_contract"; safe_summary: string; file_created: false; code_created: false; execution_enabled: false; network_enabled: false; platform_api_enabled: false; credential_access_enabled: false; media_read_enabled: false; file_mutation_enabled: false; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadScaffoldProposedInterface { interface_id: string; interface_kind: string; interface_state: "proposed" | "blocked" | "deferred" | "ready_for_future_contract"; safe_summary: string; runtime_enabled: false; external_effects_allowed: false; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadScaffoldProposedFileBoundaries { files_may_be_created_now: false; implementation_files_allowed: false; scaffold_files_allowed: false; package_files_allowed: false; dependency_files_allowed: false; runtime_files_allowed: false; safe_summary: string; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadCredentialScaffoldBoundary { credential_contract_designed: boolean; credentials_required: false; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; credential_reference_stored: false; token_reference_stored: false; secret_material_stored: false; }
export interface RealUploadMediaScaffoldBoundary { media_contract_designed: boolean; media_file_read: false; raw_media_path_stored: false; media_file_modified: false; media_file_copied: false; media_file_moved: false; media_file_deleted: false; }
export interface RealUploadPlatformAdapterScaffoldBoundary { adapter_contract_designed: boolean; platform_adapter_implemented: false; platform_endpoint_selected: false; platform_api_calls_allowed: false; platform_api_calls_made: false; raw_account_ids_stored: false; }
export interface RealUploadPayloadScaffoldBoundary { payload_contract_designed: boolean; platform_api_payload_created: false; platform_api_payload_stored: false; upload_payload_created: false; raw_payload_stored: false; }
export interface RealUploadNetworkScaffoldBoundary { network_contract_designed: boolean; network_client_implemented: false; network_calls_allowed: false; network_calls_made: false; external_side_effects_allowed: false; external_side_effects_observed: false; }
export interface RealUploadRuntimeConfigScaffoldBoundary { runtime_config_contract_designed: boolean; runtime_config_created: false; env_files_created: false; secrets_config_created: false; credential_config_created: false; package_dependency_changes: false; }
export interface RealUploadTestScaffoldBoundary { test_contract_designed: boolean; test_files_created: false; network_tests_enabled: false; credential_tests_enabled: false; media_read_tests_enabled: false; upload_tests_enabled: false; }
export interface RealUploadScaffoldOperatorReview { reviewed_by_label?: string | undefined; checklist_acknowledged: boolean; understands_scaffold_design_only: boolean; understands_no_scaffold_code_created: boolean; understands_no_upload_enabled: boolean; understands_no_credentials_accessed: boolean; understands_no_network_calls: boolean; understands_no_media_reads: boolean; understands_no_dependencies_added: boolean; understands_future_contract_phase_required: boolean; decision_note_summary?: string | undefined; }
export interface RealUploadScaffoldBoundary { scaffold_code_allowed: false; scaffold_code_created: false; implementation_files_created: false; dependencies_added: false; upload_allowed: false; upload_execution_enabled: false; real_upload_execution_allowed: false; platform_api_calls_allowed: false; network_calls_allowed: false; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; media_file_read: false; file_mutation_allowed: false; ready_for_real_upload: false; }
export interface RealUploadScaffoldDesignValidationResult { ok: boolean; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadScaffoldDesign { schema_version: "1.0"; real_upload_scaffold_design_id: string; real_upload_implementation_design_id: string; final_real_upload_preflight_gate_id: string; real_upload_dry_run_execution_result_id: string; real_upload_execution_plan_id: string; real_upload_strategy_design_id: string; real_upload_execution_request_id: string; real_upload_readiness_assessment_id: string; dry_run_upload_spike_result_id: string; upload_execution_design_id: string; upload_execution_approval_id: string; platform_upload_request_id: string; upload_package_design_id: string; local_output_review_id: string; production_render_spike_result_id: string; final_render_execution_request_id: string; render_plan_id: string; project_id: string; platform: string; scaffold_design_state: RealUploadScaffoldDesignState; created_at: string; scaffold_design_mode: RealUploadScaffoldDesignMode; required_artifacts: RealUploadScaffoldRequiredArtifacts; scaffold_scope: RealUploadScaffoldScope; proposed_modules: RealUploadScaffoldProposedModule[]; proposed_interfaces: RealUploadScaffoldProposedInterface[]; proposed_file_boundaries: RealUploadScaffoldProposedFileBoundaries; credential_scaffold_boundary: RealUploadCredentialScaffoldBoundary; media_scaffold_boundary: RealUploadMediaScaffoldBoundary; platform_adapter_scaffold_boundary: RealUploadPlatformAdapterScaffoldBoundary; payload_scaffold_boundary: RealUploadPayloadScaffoldBoundary; network_scaffold_boundary: RealUploadNetworkScaffoldBoundary; runtime_config_scaffold_boundary: RealUploadRuntimeConfigScaffoldBoundary; test_scaffold_boundary: RealUploadTestScaffoldBoundary; operator_review: RealUploadScaffoldOperatorReview; scaffold_boundary: RealUploadScaffoldBoundary; validation: { scaffold_design_complete: boolean; ready_for_future_scaffold_contracts: boolean; ready_for_real_upload: false; scaffold_code_created: false; dependencies_added: false; upload_allowed: false; upload_execution_enabled: false; platform_api_calls_allowed: false; network_calls_allowed: false; credentials_accessed: false; token_accessed: false; media_file_read: false; blocking_reasons: string[]; warnings: string[]; }; provenance: { generated_by: "createRealUploadScaffoldDesign" | "revokeRealUploadScaffoldDesign"; source_real_upload_implementation_design_id: string; source_final_real_upload_preflight_gate_id: string; source_real_upload_dry_run_execution_result_id: string; source_real_upload_execution_plan_id: string; source_real_upload_strategy_design_id: string; source_real_upload_execution_request_id: string; source_real_upload_readiness_assessment_id: string; source_dry_run_upload_spike_result_id: string; source_upload_execution_design_id: string; source_upload_execution_approval_id: string; source_platform_upload_request_id: string; source_upload_package_design_id: string; source_local_output_review_id: string; source_production_render_spike_result_id: string; source_final_render_execution_request_id: string; source_render_plan_id: string; }; }

function safeRealUploadScaffoldDesignString(value: unknown, fallback: string): string { return safeRealUploadImplementationDesignString(value, fallback); }
interface RealUploadScaffoldDesignsStore { schema_version: "1.0"; created_at: string; designs: RealUploadScaffoldDesign[]; }
function getRealUploadScaffoldDesignsPath(): string { return path.join(getRuntimeDir(), "real-upload-scaffold-designs.json"); }
function loadRealUploadScaffoldDesignsStore(): RealUploadScaffoldDesignsStore { try { const filePath = getRealUploadScaffoldDesignsPath(); if (fs.existsSync(filePath)) return JSON.parse(fs.readFileSync(filePath, "utf8")) as RealUploadScaffoldDesignsStore; } catch {} return { schema_version: "1.0", created_at: new Date().toISOString(), designs: [] }; }
function saveRealUploadScaffoldDesignsStore(store: RealUploadScaffoldDesignsStore): void { const filePath = getRealUploadScaffoldDesignsPath(); fs.mkdirSync(path.dirname(filePath), { recursive: true }); store.designs.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_scaffold_design_id.localeCompare(b.real_upload_scaffold_design_id)); fs.writeFileSync(filePath, JSON.stringify(store, null, 2), "utf8"); }
function validateRealUploadScaffoldDesignShape(design: unknown): RealUploadScaffoldDesignValidationResult { const blocking_reasons: string[] = []; const warnings: string[] = []; const fail = (reason: string) => blocking_reasons.push(reason); if (!design || typeof design !== "object" || Array.isArray(design)) return { ok: false, blocking_reasons: ["Invalid scaffold design"], warnings }; const d = design as RealUploadScaffoldDesign; if (d.schema_version !== "1.0") fail("Invalid schema version"); if (d.scaffold_design_mode !== "real_upload_scaffold_design_only" && d.scaffold_design_mode !== "operator_review_real_upload_scaffold_design") fail("Invalid scaffold design mode"); if (d.scaffold_scope?.scaffold_design_only !== true || d.scaffold_scope?.scaffold_code_created !== false || d.scaffold_scope?.implementation_files_created !== false || d.scaffold_scope?.current_real_upload_requested !== false || d.scaffold_scope?.current_network_calls_requested !== false || d.scaffold_scope?.current_platform_api_calls_requested !== false || d.scaffold_scope?.current_credential_access_requested !== false || d.scaffold_scope?.current_media_read_requested !== false || d.scaffold_scope?.dependencies_requested !== false) fail("Unsafe scaffold scope"); const requiredKinds = ["credential_provider_contract","media_reference_resolver_contract","platform_adapter_contract","payload_builder_contract","network_client_contract","upload_executor_contract","retry_idempotency_contract","rollback_contract","post_upload_verification_contract","audit_event_contract","operator_confirmation_contract","emergency_stop_contract","status_reporter_contract","upload_scaffold_index_contract"] as const; if (!Array.isArray(d.proposed_modules) || requiredKinds.some((kind) => !d.proposed_modules.some((module) => module.module_kind === kind))) fail("Missing required proposed modules"); if (Array.isArray(d.proposed_modules)) { const kinds = d.proposed_modules.map((module) => module.module_kind); if (new Set(kinds).size !== kinds.length) fail("Duplicate proposed module kinds"); if (requiredKinds.some((kind) => !kinds.includes(kind))) fail("Missing required proposed modules"); if (d.proposed_modules.some((module) => module.file_created !== false || module.code_created !== false || module.execution_enabled !== false || module.network_enabled !== false || module.platform_api_enabled !== false || module.credential_access_enabled !== false || module.media_read_enabled !== false || module.file_mutation_enabled !== false)) fail("Unsafe proposed module flags"); } if (!d.required_artifacts || d.required_artifacts.real_upload_implementation_design_validated !== true || d.required_artifacts.final_real_upload_preflight_gate_validated !== true || d.required_artifacts.real_upload_dry_run_execution_result_validated !== true || d.required_artifacts.real_upload_execution_plan_validated !== true || d.required_artifacts.real_upload_strategy_design_validated !== true || d.required_artifacts.real_upload_execution_request_validated !== true || d.required_artifacts.real_upload_readiness_assessment_validated !== true || d.required_artifacts.dry_run_upload_spike_result_validated !== true || d.required_artifacts.upload_execution_design_validated !== true || d.required_artifacts.upload_execution_approval_validated !== true || d.required_artifacts.platform_upload_request_validated !== true || d.required_artifacts.upload_package_design_validated !== true || d.required_artifacts.local_output_review_validated !== true || d.required_artifacts.spike_result_validated !== true) fail("Unsafe required artifacts"); if (!d.proposed_file_boundaries || d.proposed_file_boundaries.files_may_be_created_now !== false || d.proposed_file_boundaries.implementation_files_allowed !== false || d.proposed_file_boundaries.scaffold_files_allowed !== false || d.proposed_file_boundaries.package_files_allowed !== false || d.proposed_file_boundaries.dependency_files_allowed !== false || d.proposed_file_boundaries.runtime_files_allowed !== false) fail("Unsafe file boundaries"); if (!d.credential_scaffold_boundary || d.credential_scaffold_boundary.credentials_required !== false || d.credential_scaffold_boundary.credentials_accessed !== false || d.credential_scaffold_boundary.token_accessed !== false || d.credential_scaffold_boundary.keychain_accessed !== false || d.credential_scaffold_boundary.env_accessed !== false || d.credential_scaffold_boundary.credential_reference_stored !== false || d.credential_scaffold_boundary.token_reference_stored !== false || d.credential_scaffold_boundary.secret_material_stored !== false) fail("Unsafe credential boundary"); if (!d.media_scaffold_boundary || d.media_scaffold_boundary.media_file_read !== false || d.media_scaffold_boundary.raw_media_path_stored !== false || d.media_scaffold_boundary.media_file_modified !== false || d.media_scaffold_boundary.media_file_copied !== false || d.media_scaffold_boundary.media_file_moved !== false || d.media_scaffold_boundary.media_file_deleted !== false) fail("Unsafe media boundary"); if (!d.platform_adapter_scaffold_boundary || d.platform_adapter_scaffold_boundary.platform_adapter_implemented !== false || d.platform_adapter_scaffold_boundary.platform_endpoint_selected !== false || d.platform_adapter_scaffold_boundary.platform_api_calls_allowed !== false || d.platform_adapter_scaffold_boundary.platform_api_calls_made !== false || d.platform_adapter_scaffold_boundary.raw_account_ids_stored !== false) fail("Unsafe platform adapter boundary"); if (!d.payload_scaffold_boundary || d.payload_scaffold_boundary.platform_api_payload_created !== false || d.payload_scaffold_boundary.platform_api_payload_stored !== false || d.payload_scaffold_boundary.upload_payload_created !== false || d.payload_scaffold_boundary.raw_payload_stored !== false) fail("Unsafe payload boundary"); if (!d.network_scaffold_boundary || d.network_scaffold_boundary.network_client_implemented !== false || d.network_scaffold_boundary.network_calls_allowed !== false || d.network_scaffold_boundary.network_calls_made !== false || d.network_scaffold_boundary.external_side_effects_allowed !== false || d.network_scaffold_boundary.external_side_effects_observed !== false) fail("Unsafe network boundary"); if (!d.runtime_config_scaffold_boundary || d.runtime_config_scaffold_boundary.runtime_config_created !== false || d.runtime_config_scaffold_boundary.env_files_created !== false || d.runtime_config_scaffold_boundary.secrets_config_created !== false || d.runtime_config_scaffold_boundary.credential_config_created !== false || d.runtime_config_scaffold_boundary.package_dependency_changes !== false) fail("Unsafe runtime config boundary"); if (!d.test_scaffold_boundary || d.test_scaffold_boundary.test_files_created !== false || d.test_scaffold_boundary.network_tests_enabled !== false || d.test_scaffold_boundary.credential_tests_enabled !== false || d.test_scaffold_boundary.media_read_tests_enabled !== false || d.test_scaffold_boundary.upload_tests_enabled !== false) fail("Unsafe test boundary"); if (!d.scaffold_boundary || d.scaffold_boundary.scaffold_code_allowed !== false || d.scaffold_boundary.scaffold_code_created !== false || d.scaffold_boundary.implementation_files_created !== false || d.scaffold_boundary.dependencies_added !== false || d.scaffold_boundary.upload_allowed !== false || d.scaffold_boundary.upload_execution_enabled !== false || d.scaffold_boundary.real_upload_execution_allowed !== false || d.scaffold_boundary.platform_api_calls_allowed !== false || d.scaffold_boundary.network_calls_allowed !== false || d.scaffold_boundary.credentials_accessed !== false || d.scaffold_boundary.token_accessed !== false || d.scaffold_boundary.keychain_accessed !== false || d.scaffold_boundary.env_accessed !== false || d.scaffold_boundary.media_file_read !== false || d.scaffold_boundary.file_mutation_allowed !== false || d.scaffold_boundary.ready_for_real_upload !== false) fail("Unsafe scaffold boundary"); if (!d.validation || d.validation.ready_for_real_upload !== false || d.validation.scaffold_code_created !== false || d.validation.dependencies_added !== false || d.validation.upload_allowed !== false || d.validation.upload_execution_enabled !== false || d.validation.platform_api_calls_allowed !== false || d.validation.network_calls_allowed !== false || d.validation.credentials_accessed !== false || d.validation.token_accessed !== false || d.validation.media_file_read !== false) fail("Unsafe validation"); const text = JSON.stringify(design); if (text.includes("videos.insert") || text.includes("youtube.videos().insert") || text.includes("fetch(") || text.includes("Bearer ") || text.includes("access_token") || text.includes("refresh_token") || text.includes("client_secret") || text.includes("keychain://") || text.includes("/Users/") || text.includes("http://") || text.includes("https://") || text.includes("../") || text.includes("stdout") || text.includes("stderr")) fail("Unsafe text content"); return { ok: blocking_reasons.length === 0, blocking_reasons, warnings }; }
export function validateRealUploadScaffoldDesign(design: unknown): RealUploadScaffoldDesignValidationResult { return validateRealUploadScaffoldDesignShape(design); }
export function createRealUploadScaffoldDesign(input: { implementationDesign: RealUploadImplementationDesign; finalPreflightGate: FinalRealUploadPreflightGate; dryRunExecutionResult: RealUploadDryRunExecutionResult; executionPlan: RealUploadExecutionPlan; strategyDesign: RealUploadStrategyDesign; realUploadExecutionRequest: RealUploadExecutionRequest; readinessAssessment: RealUploadReadinessAssessment; dryRunUploadSpikeResult: DryRunUploadSpikeResult; uploadExecutionDesign: UploadExecutionDesign; uploadExecutionApproval: UploadExecutionApproval; platformUploadRequest: PlatformUploadRequest; uploadPackageDesign: UploadPackageDesign; localOutputReview: LocalOutputOperatorReview; spikeResult: ControlledProductionRenderSpikeResult; decision?: "draft" | "approved_for_future_scaffold_contracts" | "rejected"; reviewed_by_label?: string; checklist_acknowledged?: boolean; understands_scaffold_design_only?: boolean; understands_no_scaffold_code_created?: boolean; understands_no_upload_enabled?: boolean; understands_no_credentials_accessed?: boolean; understands_no_network_calls?: boolean; understands_no_media_reads?: boolean; understands_no_dependencies_added?: boolean; understands_future_contract_phase_required?: boolean; decision_note_summary?: string; dryRun: true; }): RealUploadScaffoldDesign { if (input.dryRun !== true) throw new Error("createRealUploadScaffoldDesign: dryRun=true required"); if (!validateRealUploadImplementationDesign(input.implementationDesign).ok || !validateFinalRealUploadPreflightGate(input.finalPreflightGate).ok || !validateRealUploadDryRunExecutionResult(input.dryRunExecutionResult).ok || !validateRealUploadExecutionPlan(input.executionPlan).ok || !validateRealUploadStrategyDesign(input.strategyDesign).ok || !validateRealUploadExecutionRequest(input.realUploadExecutionRequest).ok || !validateRealUploadReadinessAssessment(input.readinessAssessment).ok || !validateDryRunUploadSpikeResult(input.dryRunUploadSpikeResult).ok || !validateUploadExecutionDesign(input.uploadExecutionDesign).ok || !validateUploadExecutionApproval(input.uploadExecutionApproval).ok || !validatePlatformUploadRequest(input.platformUploadRequest).ok || !validateUploadPackageDesign(input.uploadPackageDesign).ok || !validateLocalOutputOperatorReview(input.localOutputReview).ok || !validateControlledProductionRenderSpikeResult(input.spikeResult).ok) throw new Error("createRealUploadScaffoldDesign: all input artifacts must validate"); if (input.implementationDesign.design_state !== "approved_for_future_scaffold_design") throw new Error("createRealUploadScaffoldDesign: implementationDesign must be approved_for_future_scaffold_design"); if (input.implementationDesign.validation.ready_for_real_upload !== false || input.implementationDesign.validation.ready_for_future_scaffold_design !== true) throw new Error("createRealUploadScaffoldDesign: implementationDesign readiness must remain safe"); const approved = input.decision === "approved_for_future_scaffold_contracts"; if (approved && (!input.checklist_acknowledged || !input.understands_scaffold_design_only || !input.understands_no_scaffold_code_created || !input.understands_no_upload_enabled || !input.understands_no_credentials_accessed || !input.understands_no_network_calls || !input.understands_no_media_reads || !input.understands_no_dependencies_added || !input.understands_future_contract_phase_required)) throw new Error("createRealUploadScaffoldDesign: acknowledgements required"); const scaffold: RealUploadScaffoldDesign = { schema_version: "1.0", real_upload_scaffold_design_id: crypto.randomUUID(), real_upload_implementation_design_id: input.implementationDesign.real_upload_implementation_design_id, final_real_upload_preflight_gate_id: input.finalPreflightGate.final_real_upload_preflight_gate_id, real_upload_dry_run_execution_result_id: input.dryRunExecutionResult.real_upload_dry_run_execution_result_id, real_upload_execution_plan_id: input.executionPlan.real_upload_execution_plan_id, real_upload_strategy_design_id: input.strategyDesign.real_upload_strategy_design_id, real_upload_execution_request_id: input.realUploadExecutionRequest.real_upload_execution_request_id, real_upload_readiness_assessment_id: input.readinessAssessment.real_upload_readiness_assessment_id, dry_run_upload_spike_result_id: input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id, upload_execution_design_id: input.uploadExecutionDesign.upload_execution_design_id, upload_execution_approval_id: input.uploadExecutionApproval.upload_execution_approval_id, platform_upload_request_id: input.platformUploadRequest.platform_upload_request_id, upload_package_design_id: input.uploadPackageDesign.upload_package_design_id, local_output_review_id: input.localOutputReview.local_output_review_id, production_render_spike_result_id: input.spikeResult.production_render_spike_result_id, final_render_execution_request_id: input.spikeResult.final_render_execution_request_id, render_plan_id: input.spikeResult.render_plan_id, project_id: input.implementationDesign.project_id, platform: input.implementationDesign.platform, scaffold_design_state: input.decision === "rejected" ? "rejected" : approved ? "approved_for_future_scaffold_contracts" : "draft", created_at: new Date().toISOString(), scaffold_design_mode: approved ? "operator_review_real_upload_scaffold_design" : "real_upload_scaffold_design_only", required_artifacts: { real_upload_implementation_design_validated: true, final_real_upload_preflight_gate_validated: true, real_upload_dry_run_execution_result_validated: true, real_upload_execution_plan_validated: true, real_upload_strategy_design_validated: true, real_upload_execution_request_validated: true, real_upload_readiness_assessment_validated: true, dry_run_upload_spike_result_validated: true, upload_execution_design_validated: true, upload_execution_approval_validated: true, platform_upload_request_validated: true, upload_package_design_validated: true, local_output_review_validated: true, spike_result_validated: true }, scaffold_scope: { future_scaffold_contracts_requested: true, scaffold_design_only: true, scaffold_code_created: false, implementation_files_created: false, current_real_upload_requested: false, current_network_calls_requested: false, current_platform_api_calls_requested: false, current_credential_access_requested: false, current_media_read_requested: false, dependencies_requested: false }, proposed_modules: ["credential_provider_contract","media_reference_resolver_contract","platform_adapter_contract","payload_builder_contract","network_client_contract","upload_executor_contract","retry_idempotency_contract","rollback_contract","post_upload_verification_contract","audit_event_contract","operator_confirmation_contract","emergency_stop_contract","status_reporter_contract","upload_scaffold_index_contract"].map((kind, index) => ({ module_id: `module-${index + 1}`, module_kind: kind as RealUploadScaffoldProposedModule["module_kind"], module_state: kind === "operator_confirmation_contract" ? "ready_for_future_contract" : "proposed", safe_summary: `Future ${kind.replaceAll("_", " ")} only.`, file_created: false, code_created: false, execution_enabled: false, network_enabled: false, platform_api_enabled: false, credential_access_enabled: false, media_read_enabled: false, file_mutation_enabled: false, blocking_reasons: [kind === "credential_provider_contract" ? "Credential access remains disabled." : kind === "media_reference_resolver_contract" ? "Media reads remain disabled." : kind === "platform_adapter_contract" ? "Platform APIs remain disabled." : kind === "payload_builder_contract" ? "Payload construction remains disabled." : kind === "network_client_contract" ? "Network calls remain disabled." : kind === "upload_executor_contract" ? "Upload execution remains disabled." : kind === "retry_idempotency_contract" ? "Retry execution remains disabled." : kind === "rollback_contract" ? "Rollback execution remains disabled." : kind === "post_upload_verification_contract" ? "Verification execution remains disabled." : kind === "audit_event_contract" ? "Raw logging remains disabled." : kind === "operator_confirmation_contract" ? "Operator approval remains required." : kind === "emergency_stop_contract" ? "Emergency stop is design-only." : "Contract remains safe and local-only."], warnings: [] })), proposed_interfaces: [{ interface_id: "interface-1", interface_kind: "credential-provider", interface_state: "proposed", safe_summary: "Credential provider interface only.", runtime_enabled: false, external_effects_allowed: false, blocking_reasons: ["Credential access remains disabled."], warnings: [] }], proposed_file_boundaries: { files_may_be_created_now: false, implementation_files_allowed: false, scaffold_files_allowed: false, package_files_allowed: false, dependency_files_allowed: false, runtime_files_allowed: false, safe_summary: "No files may be created now.", blocking_reasons: ["File creation remains disabled."], warnings: [] }, credential_scaffold_boundary: { credential_contract_designed: true, credentials_required: false, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, credential_reference_stored: false, token_reference_stored: false, secret_material_stored: false }, media_scaffold_boundary: { media_contract_designed: true, media_file_read: false, raw_media_path_stored: false, media_file_modified: false, media_file_copied: false, media_file_moved: false, media_file_deleted: false }, platform_adapter_scaffold_boundary: { adapter_contract_designed: true, platform_adapter_implemented: false, platform_endpoint_selected: false, platform_api_calls_allowed: false, platform_api_calls_made: false, raw_account_ids_stored: false }, payload_scaffold_boundary: { payload_contract_designed: true, platform_api_payload_created: false, platform_api_payload_stored: false, upload_payload_created: false, raw_payload_stored: false }, network_scaffold_boundary: { network_contract_designed: true, network_client_implemented: false, network_calls_allowed: false, network_calls_made: false, external_side_effects_allowed: false, external_side_effects_observed: false }, runtime_config_scaffold_boundary: { runtime_config_contract_designed: true, runtime_config_created: false, env_files_created: false, secrets_config_created: false, credential_config_created: false, package_dependency_changes: false }, test_scaffold_boundary: { test_contract_designed: true, test_files_created: false, network_tests_enabled: false, credential_tests_enabled: false, media_read_tests_enabled: false, upload_tests_enabled: false }, operator_review: { reviewed_by_label: input.reviewed_by_label, checklist_acknowledged: !!input.checklist_acknowledged, understands_scaffold_design_only: !!input.understands_scaffold_design_only, understands_no_scaffold_code_created: !!input.understands_no_scaffold_code_created, understands_no_upload_enabled: !!input.understands_no_upload_enabled, understands_no_credentials_accessed: !!input.understands_no_credentials_accessed, understands_no_network_calls: !!input.understands_no_network_calls, understands_no_media_reads: !!input.understands_no_media_reads, understands_no_dependencies_added: !!input.understands_no_dependencies_added, understands_future_contract_phase_required: !!input.understands_future_contract_phase_required, decision_note_summary: input.decision_note_summary }, scaffold_boundary: { scaffold_code_allowed: false, scaffold_code_created: false, implementation_files_created: false, dependencies_added: false, upload_allowed: false, upload_execution_enabled: false, real_upload_execution_allowed: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, media_file_read: false, file_mutation_allowed: false, ready_for_real_upload: false }, validation: { scaffold_design_complete: true, ready_for_future_scaffold_contracts: approved, ready_for_real_upload: false, scaffold_code_created: false, dependencies_added: false, upload_allowed: false, upload_execution_enabled: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, media_file_read: false, blocking_reasons: [], warnings: [] }, provenance: { generated_by: "createRealUploadScaffoldDesign", source_real_upload_implementation_design_id: input.implementationDesign.real_upload_implementation_design_id, source_final_real_upload_preflight_gate_id: input.finalPreflightGate.final_real_upload_preflight_gate_id, source_real_upload_dry_run_execution_result_id: input.dryRunExecutionResult.real_upload_dry_run_execution_result_id, source_real_upload_execution_plan_id: input.executionPlan.real_upload_execution_plan_id, source_real_upload_strategy_design_id: input.strategyDesign.real_upload_strategy_design_id, source_real_upload_execution_request_id: input.realUploadExecutionRequest.real_upload_execution_request_id, source_real_upload_readiness_assessment_id: input.readinessAssessment.real_upload_readiness_assessment_id, source_dry_run_upload_spike_result_id: input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id, source_upload_execution_design_id: input.uploadExecutionDesign.upload_execution_design_id, source_upload_execution_approval_id: input.uploadExecutionApproval.upload_execution_approval_id, source_platform_upload_request_id: input.platformUploadRequest.platform_upload_request_id, source_upload_package_design_id: input.uploadPackageDesign.upload_package_design_id, source_local_output_review_id: input.localOutputReview.local_output_review_id, source_production_render_spike_result_id: input.spikeResult.production_render_spike_result_id, source_final_render_execution_request_id: input.spikeResult.final_render_execution_request_id, source_render_plan_id: input.spikeResult.render_plan_id } }; if (!validateRealUploadScaffoldDesign(scaffold).ok) throw new Error("Unsafe real upload scaffold design cannot be created."); return scaffold; }
interface RealUploadScaffoldDesignsStoreSummaryItem { real_upload_scaffold_design_id: string; project_id: string; platform: string; scaffold_design_state: string; created_at: string; }
function safeRealUploadScaffoldDesignSummary(design: RealUploadScaffoldDesign): RealUploadScaffoldDesignsStoreSummaryItem { return { real_upload_scaffold_design_id: safeRealUploadScaffoldDesignString(design.real_upload_scaffold_design_id, "[unsafe-id]"), project_id: safeRealUploadScaffoldDesignString(design.project_id, "[unsafe-project]"), platform: safeRealUploadScaffoldDesignString(design.platform, "[unsafe-platform]"), scaffold_design_state: design.scaffold_design_state, created_at: design.created_at }; }
export function saveRealUploadScaffoldDesign(design: RealUploadScaffoldDesign): void { if (!validateRealUploadScaffoldDesign(design).ok) throw new Error("Unsafe real upload scaffold design cannot be stored."); const store = loadRealUploadScaffoldDesignsStore(); const index = store.designs.findIndex((item) => item.real_upload_scaffold_design_id === design.real_upload_scaffold_design_id); if (index >= 0) store.designs[index] = design; else store.designs.push(design); saveRealUploadScaffoldDesignsStore(store); }
export function listRealUploadScaffoldDesigns(options?: { project_id?: string; platform?: string; scaffold_design_state?: string; real_upload_implementation_design_id?: string; final_real_upload_preflight_gate_id?: string }): RealUploadScaffoldDesign[] { return loadRealUploadScaffoldDesignsStore().designs.filter((design) => (!options?.project_id || design.project_id === options.project_id) && (!options?.platform || design.platform === options.platform) && (!options?.scaffold_design_state || design.scaffold_design_state === options.scaffold_design_state) && (!options?.real_upload_implementation_design_id || design.real_upload_implementation_design_id === options.real_upload_implementation_design_id) && (!options?.final_real_upload_preflight_gate_id || design.final_real_upload_preflight_gate_id === options.final_real_upload_preflight_gate_id)).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_scaffold_design_id.localeCompare(b.real_upload_scaffold_design_id)); }
export function getRealUploadScaffoldDesign(real_upload_scaffold_design_id: string): RealUploadScaffoldDesign | null { return loadRealUploadScaffoldDesignsStore().designs.find((design) => design.real_upload_scaffold_design_id === real_upload_scaffold_design_id) ?? null; }
export function revokeRealUploadScaffoldDesign(real_upload_scaffold_design_id: string, reason: string): RealUploadScaffoldDesign { const design = getRealUploadScaffoldDesign(real_upload_scaffold_design_id); if (!design) throw new Error("Real upload scaffold design not found"); const safeReason = safeRealUploadScaffoldDesignString(reason, "[unsafe-reason]"); if (safeReason === "[unsafe-reason]") throw new Error("Unsafe real upload scaffold design reason cannot be stored."); const revoked: RealUploadScaffoldDesign = { ...design, scaffold_design_state: "revoked", validation: { ...design.validation, blocking_reasons: [...design.validation.blocking_reasons, safeReason], warnings: [...design.validation.warnings, "Real upload scaffold design revoked."] }, provenance: { ...design.provenance, generated_by: "revokeRealUploadScaffoldDesign" } }; saveRealUploadScaffoldDesign(revoked); return revoked; }
export function getRealUploadScaffoldDesignReport(options?: { project_id?: string; platform?: string }): { total: number; by_state: Record<string, number>; blocked: number; ready_for_operator_review: number; approved_for_future_scaffold_contracts: number; rejected: number; revoked: number; scaffold_design_complete: number; ready_for_future_scaffold_contracts: number; ready_for_real_upload: 0; scaffold_code_created: 0; implementation_files_created: 0; dependencies_added: 0; upload_allowed: 0; upload_execution_enabled: 0; platform_api_calls_allowed: 0; network_calls_allowed: 0; credentials_accessed: 0; token_accessed: 0; media_file_read: 0; file_mutation_allowed: 0; modules_total: number; modules_with_blockers: number; designs: Array<RealUploadScaffoldDesignsStoreSummaryItem>; } { const designs = listRealUploadScaffoldDesigns(options); const by_state: Record<string, number> = {}; let blocked = 0, ready = 0, approved = 0, rejected = 0, revoked = 0, complete = 0, readyFuture = 0, modulesTotal = 0, modulesBlocked = 0; const summaries = designs.map((design) => { by_state[design.scaffold_design_state] = (by_state[design.scaffold_design_state] ?? 0) + 1; if (design.scaffold_design_state === "blocked") blocked++; if (design.scaffold_design_state === "ready_for_operator_review") ready++; if (design.scaffold_design_state === "approved_for_future_scaffold_contracts") approved++; if (design.scaffold_design_state === "rejected") rejected++; if (design.scaffold_design_state === "revoked") revoked++; if (design.validation.scaffold_design_complete) complete++; if (design.validation.ready_for_future_scaffold_contracts) readyFuture++; modulesTotal += design.proposed_modules.length; modulesBlocked += design.proposed_modules.filter((module) => module.blocking_reasons.length > 0).length; return safeRealUploadScaffoldDesignSummary(design); }); return { total: summaries.length, by_state, blocked, ready_for_operator_review: ready, approved_for_future_scaffold_contracts: approved, rejected, revoked, scaffold_design_complete: complete, ready_for_future_scaffold_contracts: readyFuture, ready_for_real_upload: 0, scaffold_code_created: 0, implementation_files_created: 0, dependencies_added: 0, upload_allowed: 0, upload_execution_enabled: 0, platform_api_calls_allowed: 0, network_calls_allowed: 0, credentials_accessed: 0, token_accessed: 0, media_file_read: 0, file_mutation_allowed: 0, modules_total: modulesTotal, modules_with_blockers: modulesBlocked, designs: summaries }; }

// ─── VO-7P: Real Upload Scaffold Contracts ───────────────────────────────

export type RealUploadScaffoldContractsMode = "real_upload_scaffold_contracts_only" | "operator_review_real_upload_scaffold_contracts";
export type RealUploadScaffoldContractsState = "draft" | "blocked" | "ready_for_operator_review" | "approved_for_future_contract_tests" | "rejected" | "revoked";
export interface RealUploadScaffoldContractsRequiredArtifacts { real_upload_scaffold_design_validated: true; real_upload_implementation_design_validated: true; final_real_upload_preflight_gate_validated: true; real_upload_dry_run_execution_result_validated: true; real_upload_execution_plan_validated: true; real_upload_strategy_design_validated: true; real_upload_execution_request_validated: true; real_upload_readiness_assessment_validated: true; dry_run_upload_spike_result_validated: true; upload_execution_design_validated: true; upload_execution_approval_validated: true; platform_upload_request_validated: true; upload_package_design_validated: true; local_output_review_validated: true; spike_result_validated: true; }
export interface RealUploadScaffoldContractsScope { future_contract_tests_requested: boolean; contracts_only: true; implementation_files_created: false; scaffold_code_created: false; runtime_enabled: false; current_real_upload_requested: false; current_network_calls_requested: false; current_platform_api_calls_requested: false; current_credential_access_requested: false; current_media_read_requested: false; dependencies_requested: false; }
export interface RealUploadModuleContract { contract_id: string; module_kind: "credential_provider_contract" | "media_reference_resolver_contract" | "platform_adapter_contract" | "payload_builder_contract" | "network_client_contract" | "upload_executor_contract" | "retry_idempotency_contract" | "rollback_contract" | "post_upload_verification_contract" | "audit_event_contract" | "operator_confirmation_contract" | "emergency_stop_contract" | "status_reporter_contract" | "upload_scaffold_index_contract"; contract_state: "proposed" | "blocked" | "deferred" | "ready_for_future_contract_tests"; safe_summary: string; interface_defined: boolean; implementation_created: false; runtime_enabled: false; upload_enabled: false; network_enabled: false; platform_api_enabled: false; credential_access_enabled: false; media_read_enabled: false; file_mutation_enabled: false; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadInterfaceContract { interface_id: string; interface_kind: string; interface_state: "proposed" | "blocked" | "deferred" | "ready_for_future_contract_tests"; safe_summary: string; type_only: true; runtime_enabled: false; external_effects_allowed: false; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadDataContracts { safe_id_contract_defined: boolean; safe_summary_contract_defined: boolean; raw_path_contract_defined: false; raw_account_id_contract_defined: false; raw_payload_contract_defined: false; raw_response_contract_defined: false; secret_contract_defined: false; safe_summary: string; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadErrorContracts { safe_error_contract_defined: boolean; raw_error_payload_allowed: false; raw_stdout_allowed: false; raw_stderr_allowed: false; raw_response_allowed: false; secret_echo_allowed: false; safe_summary: string; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadAuditContracts { audit_contract_defined: boolean; audit_write_enabled: false; raw_payload_logging_allowed: false; raw_response_logging_allowed: false; secret_logging_allowed: false; raw_path_logging_allowed: false; raw_account_id_logging_allowed: false; safe_summary: string; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadOperatorControlContracts { operator_confirmation_contract_defined: boolean; manual_go_no_go_required: true; automatic_execution_allowed: false; emergency_stop_contract_defined: boolean; reviewed_by_label?: string | undefined; checklist_acknowledged: boolean; understands_contracts_only: boolean; understands_no_implementation_files_created: boolean; understands_no_upload_enabled: boolean; understands_no_credentials_accessed: boolean; understands_no_network_calls: boolean; understands_no_media_reads: boolean; understands_no_dependencies_added: boolean; understands_future_contract_test_phase_required: boolean; decision_note_summary?: string | undefined; }
export interface RealUploadCredentialContractBoundary { credential_contract_defined: boolean; credentials_required: false; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; credential_reference_stored: false; token_reference_stored: false; secret_material_stored: false; }
export interface RealUploadMediaContractBoundary { media_contract_defined: boolean; media_file_read: false; raw_media_path_stored: false; media_file_modified: false; media_file_copied: false; media_file_moved: false; media_file_deleted: false; }
export interface RealUploadPlatformApiContractBoundary { platform_api_contract_defined: boolean; platform_adapter_implemented: false; platform_endpoint_selected: false; platform_api_calls_allowed: false; platform_api_calls_made: false; raw_account_ids_stored: false; }
export interface RealUploadPayloadContractBoundary { payload_contract_defined: boolean; platform_api_payload_created: false; platform_api_payload_stored: false; upload_payload_created: false; raw_payload_stored: false; }
export interface RealUploadNetworkContractBoundary { network_contract_defined: boolean; network_client_implemented: false; network_calls_allowed: false; network_calls_made: false; external_side_effects_allowed: false; external_side_effects_observed: false; }
export interface RealUploadRuntimeContractBoundary { runtime_contract_defined: boolean; runtime_config_created: false; env_files_created: false; secrets_config_created: false; credential_config_created: false; package_dependency_changes: false; }
export interface RealUploadFileContractBoundary { files_created_now: false; implementation_files_created: false; scaffold_files_created: false; package_files_changed: false; runtime_files_created: false; file_mutation_allowed: false; }
export interface RealUploadScaffoldContractsValidationResult { ok: boolean; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadScaffoldContracts {
  schema_version: "1.0";
  real_upload_scaffold_contracts_id: string;
  real_upload_scaffold_design_id: string;
  real_upload_implementation_design_id: string;
  final_real_upload_preflight_gate_id: string;
  real_upload_dry_run_execution_result_id: string;
  real_upload_execution_plan_id: string;
  ...
  project_id: string;
  platform: string;
  contracts_state: RealUploadScaffoldContractsState;
  created_at: string;
  contracts_mode: RealUploadScaffoldContractsMode;
  required_artifacts: RealUploadScaffoldContractsRequiredArtifacts;
  contracts_scope: RealUploadScaffoldContractsScope;
  module_contracts: RealUploadModuleContract[];
  interface_contracts: RealUploadInterfaceContract[];
  data_contracts: RealUploadDataContracts;
  error_contracts: RealUploadErrorContracts;
  audit_contracts: RealUploadAuditContracts;
  operator_control_contracts: RealUploadOperatorControlContracts;
  credential_contract_boundary: RealUploadCredentialContractBoundary;
  media_contract_boundary: RealUploadMediaContractBoundary;
  platform_api_contract_boundary: RealUploadPlatformApiContractBoundary;
  payload_contract_boundary: RealUploadPayloadContractBoundary;
  network_contract_boundary: RealUploadNetworkContractBoundary;
  runtime_contract_boundary: RealUploadRuntimeContractBoundary;
  file_contract_boundary: RealUploadFileContractBoundary;
  validation: { scaffold_contracts_complete: boolean; ready_for_future_contract_tests: boolean; ready_for_real_upload: false; implementation_files_created: false; scaffold_code_created: false; dependencies_added: false; upload_allowed: false; upload_execution_enabled: false; platform_api_calls_allowed: false; network_calls_allowed: false; credentials_accessed: false; token_accessed: false; media_file_read: false; file_mutation_allowed: false; blocking_reasons: string[]; warnings: string[]; };
  provenance: { generated_by: "createRealUploadScaffoldContracts" | "revokeRealUploadScaffoldContracts"; source_real_upload_scaffold_design_id: string; source_real_upload_implementation_design_id: string; source_final_real_upload_preflight_gate_id: string; source_real_upload_dry_run_execution_result_id: string; source_real_upload_execution_plan_id: string; source_real_upload_strategy_design_id: string; source_real_upload_execution_request_id: string; source_real_upload_readiness_assessment_id: string; source_dry_run_upload_spike_result_id: string; source_upload_execution_design_id: string; source_upload_execution_approval_id: string; source_platform_upload_request_id: string; source_upload_package_design_id: string; source_local_output_review_id: string; source_production_render_spike_result_id: string; source_final_render_execution_request_id: string; source_render_plan_id: string; };
}

function safeRealUploadScaffoldContractsString(value: unknown, fallback: string): string { return safeRealUploadScaffoldDesignString(value, fallback); }
interface RealUploadScaffoldContractsStore { schema_version: "1.0"; created_at: string; contracts: RealUploadScaffoldContracts[]; }
function getRealUploadScaffoldContractsPath(): string { return path.join(getRuntimeDir(), "real-upload-scaffold-contracts.json"); }
function loadRealUploadScaffoldContractsStore(): RealUploadScaffoldContractsStore { try { const filePath = getRealUploadScaffoldContractsPath(); if (fs.existsSync(filePath)) return JSON.parse(fs.readFileSync(filePath, "utf8")) as RealUploadScaffoldContractsStore; } catch {} return { schema_version: "1.0", created_at: new Date().toISOString(), contracts: [] }; }
function saveRealUploadScaffoldContractsStore(store: RealUploadScaffoldContractsStore): void { const filePath = getRealUploadScaffoldContractsPath(); fs.mkdirSync(path.dirname(filePath), { recursive: true }); store.contracts.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_scaffold_contracts_id.localeCompare(b.real_upload_scaffold_contracts_id)); fs.writeFileSync(filePath, JSON.stringify(store, null, 2), "utf8"); }
function validateRealUploadScaffoldContractsShape(contracts: unknown): RealUploadScaffoldContractsValidationResult { const blocking_reasons: string[] = []; const warnings: string[] = []; const fail = (reason: string) => blocking_reasons.push(reason); if (!contracts || typeof contracts !== "object" || Array.isArray(contracts)) return { ok: false, blocking_reasons: ["Invalid scaffold contracts"], warnings }; const c = contracts as RealUploadScaffoldContracts; const kinds = ["credential_provider_contract","media_reference_resolver_contract","platform_adapter_contract","payload_builder_contract","network_client_contract","upload_executor_contract","retry_idempotency_contract","rollback_contract","post_upload_verification_contract","audit_event_contract","operator_confirmation_contract","emergency_stop_contract","status_reporter_contract","upload_scaffold_index_contract"] as const; const requiredArtifactsOk = !!c.required_artifacts && c.required_artifacts.real_upload_scaffold_design_validated === true && c.required_artifacts.real_upload_implementation_design_validated === true && c.required_artifacts.final_real_upload_preflight_gate_validated === true && c.required_artifacts.real_upload_dry_run_execution_result_validated === true && c.required_artifacts.real_upload_execution_plan_validated === true && c.required_artifacts.real_upload_strategy_design_validated === true && c.required_artifacts.real_upload_execution_request_validated === true && c.required_artifacts.real_upload_readiness_assessment_validated === true && c.required_artifacts.dry_run_upload_spike_result_validated === true && c.required_artifacts.upload_execution_design_validated === true && c.required_artifacts.upload_execution_approval_validated === true && c.required_artifacts.platform_upload_request_validated === true && c.required_artifacts.upload_package_design_validated === true && c.required_artifacts.local_output_review_validated === true && c.required_artifacts.spike_result_validated === true; if (c.schema_version !== "1.0") fail("Invalid schema version"); if (!requiredArtifactsOk) fail("Unsafe required artifacts"); if (c.contracts_scope?.contracts_only !== true || c.contracts_scope?.implementation_files_created !== false || c.contracts_scope?.scaffold_code_created !== false || c.contracts_scope?.runtime_enabled !== false || c.contracts_scope?.current_real_upload_requested !== false || c.contracts_scope?.current_network_calls_requested !== false || c.contracts_scope?.current_platform_api_calls_requested !== false || c.contracts_scope?.current_credential_access_requested !== false || c.contracts_scope?.current_media_read_requested !== false || c.contracts_scope?.dependencies_requested !== false) fail("Unsafe contracts scope"); if (!Array.isArray(c.module_contracts) || new Set(c.module_contracts.map((m) => m.module_kind)).size !== c.module_contracts.length || kinds.some((kind) => !c.module_contracts.some((m) => m.module_kind === kind))) fail("Missing or duplicate module kinds"); if (Array.isArray(c.module_contracts) && c.module_contracts.some((m) => m.implementation_created !== false || m.runtime_enabled !== false || m.upload_enabled !== false || m.network_enabled !== false || m.platform_api_enabled !== false || m.credential_access_enabled !== false || m.media_read_enabled !== false || m.file_mutation_enabled !== false)) fail("Unsafe module contracts"); if (!c.interface_contracts || c.interface_contracts.some((i) => i.type_only !== true || i.runtime_enabled !== false || i.external_effects_allowed !== false)) fail("Unsafe interface contracts"); if (!c.data_contracts || c.data_contracts.raw_path_contract_defined !== false || c.data_contracts.raw_account_id_contract_defined !== false || c.data_contracts.raw_payload_contract_defined !== false || c.data_contracts.raw_response_contract_defined !== false || c.data_contracts.secret_contract_defined !== false) fail("Unsafe data contracts"); if (!c.error_contracts || c.error_contracts.raw_error_payload_allowed !== false || c.error_contracts.raw_stdout_allowed !== false || c.error_contracts.raw_stderr_allowed !== false || c.error_contracts.raw_response_allowed !== false || c.error_contracts.secret_echo_allowed !== false) fail("Unsafe error contracts"); if (!c.audit_contracts || c.audit_contracts.audit_write_enabled !== false || c.audit_contracts.raw_payload_logging_allowed !== false || c.audit_contracts.raw_response_logging_allowed !== false || c.audit_contracts.secret_logging_allowed !== false || c.audit_contracts.raw_path_logging_allowed !== false || c.audit_contracts.raw_account_id_logging_allowed !== false) fail("Unsafe audit contracts"); if (!c.operator_control_contracts || c.operator_control_contracts.manual_go_no_go_required !== true || c.operator_control_contracts.automatic_execution_allowed !== false) fail("Unsafe operator control contracts"); if (!c.credential_contract_boundary || c.credential_contract_boundary.credentials_required !== false || c.credential_contract_boundary.credentials_accessed !== false || c.credential_contract_boundary.token_accessed !== false || c.credential_contract_boundary.keychain_accessed !== false || c.credential_contract_boundary.env_accessed !== false || c.credential_contract_boundary.credential_reference_stored !== false || c.credential_contract_boundary.token_reference_stored !== false || c.credential_contract_boundary.secret_material_stored !== false) fail("Unsafe credential boundary"); if (!c.media_contract_boundary || c.media_contract_boundary.media_file_read !== false || c.media_contract_boundary.raw_media_path_stored !== false || c.media_contract_boundary.media_file_modified !== false || c.media_contract_boundary.media_file_copied !== false || c.media_contract_boundary.media_file_moved !== false || c.media_contract_boundary.media_file_deleted !== false) fail("Unsafe media boundary"); if (!c.platform_api_contract_boundary || c.platform_api_contract_boundary.platform_adapter_implemented !== false || c.platform_api_contract_boundary.platform_endpoint_selected !== false || c.platform_api_contract_boundary.platform_api_calls_allowed !== false || c.platform_api_contract_boundary.platform_api_calls_made !== false || c.platform_api_contract_boundary.raw_account_ids_stored !== false) fail("Unsafe platform boundary"); if (!c.payload_contract_boundary || c.payload_contract_boundary.platform_api_payload_created !== false || c.payload_contract_boundary.platform_api_payload_stored !== false || c.payload_contract_boundary.upload_payload_created !== false || c.payload_contract_boundary.raw_payload_stored !== false) fail("Unsafe payload boundary"); if (!c.network_contract_boundary || c.network_contract_boundary.network_client_implemented !== false || c.network_contract_boundary.network_calls_allowed !== false || c.network_contract_boundary.network_calls_made !== false || c.network_contract_boundary.external_side_effects_allowed !== false || c.network_contract_boundary.external_side_effects_observed !== false) fail("Unsafe network boundary"); if (!c.runtime_contract_boundary || c.runtime_contract_boundary.runtime_config_created !== false || c.runtime_contract_boundary.env_files_created !== false || c.runtime_contract_boundary.secrets_config_created !== false || c.runtime_contract_boundary.credential_config_created !== false || c.runtime_contract_boundary.package_dependency_changes !== false) fail("Unsafe runtime boundary"); if (!c.file_contract_boundary || c.file_contract_boundary.files_created_now !== false || c.file_contract_boundary.implementation_files_created !== false || c.file_contract_boundary.scaffold_files_created !== false || c.file_contract_boundary.package_files_changed !== false || c.file_contract_boundary.runtime_files_created !== false || c.file_contract_boundary.file_mutation_allowed !== false) fail("Unsafe file boundary"); if (!c.validation || c.validation.ready_for_real_upload !== false || c.validation.implementation_files_created !== false || c.validation.scaffold_code_created !== false || c.validation.dependencies_added !== false || c.validation.upload_allowed !== false || c.validation.upload_execution_enabled !== false || c.validation.platform_api_calls_allowed !== false || c.validation.network_calls_allowed !== false || c.validation.credentials_accessed !== false || c.validation.token_accessed !== false || c.validation.media_file_read !== false || c.validation.file_mutation_allowed !== false) fail("Unsafe validation"); const scanValues = (value: unknown): boolean => { if (typeof value === "string") return value.includes("videos.insert") || value.includes("youtube.videos().insert") || value.includes("fetch(") || value.includes("Bearer ") || value.includes("access_token") || value.includes("refresh_token") || value.includes("client_secret") || value.includes("keychain://") || value.includes("/Users/") || value.includes("http://") || value.includes("https://") || value.includes("../") || value.includes("stdout") || value.includes("stderr"); if (Array.isArray(value)) return value.some(scanValues); if (value && typeof value === "object") return Object.values(value as Record<string, unknown>).some(scanValues); return false; }; if (scanValues(contracts)) fail("Unsafe text content"); return { ok: blocking_reasons.length === 0, blocking_reasons, warnings }; }
export function validateRealUploadScaffoldContracts(contracts: unknown): RealUploadScaffoldContractsValidationResult { return validateRealUploadScaffoldContractsShape(contracts); }
export function createRealUploadScaffoldContracts(input: { scaffoldDesign: RealUploadScaffoldDesign; implementationDesign: RealUploadImplementationDesign; finalPreflightGate: FinalRealUploadPreflightGate; dryRunExecutionResult: RealUploadDryRunExecutionResult; executionPlan: RealUploadExecutionPlan; strategyDesign: RealUploadStrategyDesign; realUploadExecutionRequest: RealUploadExecutionRequest; readinessAssessment: RealUploadReadinessAssessment; dryRunUploadSpikeResult: DryRunUploadSpikeResult; uploadExecutionDesign: UploadExecutionDesign; uploadExecutionApproval: UploadExecutionApproval; platformUploadRequest: PlatformUploadRequest; uploadPackageDesign: UploadPackageDesign; localOutputReview: LocalOutputOperatorReview; spikeResult: ControlledProductionRenderSpikeResult; decision?: "draft" | "approved_for_future_contract_tests" | "rejected"; reviewed_by_label?: string; checklist_acknowledged?: boolean; understands_contracts_only?: boolean; understands_no_implementation_files_created?: boolean; understands_no_upload_enabled?: boolean; understands_no_credentials_accessed?: boolean; understands_no_network_calls?: boolean; understands_no_media_reads?: boolean; understands_no_dependencies_added?: boolean; understands_future_contract_test_phase_required?: boolean; decision_note_summary?: string; dryRun: true; }): RealUploadScaffoldContracts { if (input.dryRun !== true) throw new Error("createRealUploadScaffoldContracts: dryRun=true required"); if (!validateRealUploadScaffoldDesign(input.scaffoldDesign).ok || !validateRealUploadImplementationDesign(input.implementationDesign).ok || !validateFinalRealUploadPreflightGate(input.finalPreflightGate).ok || !validateRealUploadDryRunExecutionResult(input.dryRunExecutionResult).ok || !validateRealUploadExecutionPlan(input.executionPlan).ok || !validateRealUploadStrategyDesign(input.strategyDesign).ok || !validateRealUploadExecutionRequest(input.realUploadExecutionRequest).ok || !validateRealUploadReadinessAssessment(input.readinessAssessment).ok || !validateDryRunUploadSpikeResult(input.dryRunUploadSpikeResult).ok || !validateUploadExecutionDesign(input.uploadExecutionDesign).ok || !validateUploadExecutionApproval(input.uploadExecutionApproval).ok || !validatePlatformUploadRequest(input.platformUploadRequest).ok || !validateUploadPackageDesign(input.uploadPackageDesign).ok || !validateLocalOutputOperatorReview(input.localOutputReview).ok || !validateControlledProductionRenderSpikeResult(input.spikeResult).ok) throw new Error("createRealUploadScaffoldContracts: all input artifacts must validate"); if (input.scaffoldDesign.scaffold_design_state !== "approved_for_future_scaffold_contracts") throw new Error("createRealUploadScaffoldContracts: scaffoldDesign must be approved_for_future_scaffold_contracts"); if (input.scaffoldDesign.validation.ready_for_real_upload !== false || input.scaffoldDesign.validation.ready_for_future_scaffold_contracts !== true) throw new Error("createRealUploadScaffoldContracts: scaffoldDesign readiness must remain safe"); const approved = input.decision === "approved_for_future_contract_tests"; if (approved && (!input.checklist_acknowledged || !input.understands_contracts_only || !input.understands_no_implementation_files_created || !input.understands_no_upload_enabled || !input.understands_no_credentials_accessed || !input.understands_no_network_calls || !input.understands_no_media_reads || !input.understands_no_dependencies_added || !input.understands_future_contract_test_phase_required)) throw new Error("createRealUploadScaffoldContracts: acknowledgements required"); const moduleKinds: RealUploadModuleContract["module_kind"][] = ["credential_provider_contract","media_reference_resolver_contract","platform_adapter_contract","payload_builder_contract","network_client_contract","upload_executor_contract","retry_idempotency_contract","rollback_contract","post_upload_verification_contract","audit_event_contract","operator_confirmation_contract","emergency_stop_contract","status_reporter_contract","upload_scaffold_index_contract"]; const contracts: RealUploadScaffoldContracts = { schema_version: "1.0", real_upload_scaffold_contracts_id: crypto.randomUUID(), real_upload_scaffold_design_id: input.scaffoldDesign.real_upload_scaffold_design_id, real_upload_implementation_design_id: input.implementationDesign.real_upload_implementation_design_id, final_real_upload_preflight_gate_id: input.finalPreflightGate.final_real_upload_preflight_gate_id, real_upload_dry_run_execution_result_id: input.dryRunExecutionResult.real_upload_dry_run_execution_result_id, real_upload_execution_plan_id: input.executionPlan.real_upload_execution_plan_id, real_upload_strategy_design_id: input.strategyDesign.real_upload_strategy_design_id, real_upload_execution_request_id: input.realUploadExecutionRequest.real_upload_execution_request_id, real_upload_readiness_assessment_id: input.readinessAssessment.real_upload_readiness_assessment_id, dry_run_upload_spike_result_id: input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id, upload_execution_design_id: input.uploadExecutionDesign.upload_execution_design_id, upload_execution_approval_id: input.uploadExecutionApproval.upload_execution_approval_id, platform_upload_request_id: input.platformUploadRequest.platform_upload_request_id, upload_package_design_id: input.uploadPackageDesign.upload_package_design_id, local_output_review_id: input.localOutputReview.local_output_review_id, production_render_spike_result_id: input.spikeResult.production_render_spike_result_id, final_render_execution_request_id: input.spikeResult.final_render_execution_request_id, render_plan_id: input.spikeResult.render_plan_id, project_id: input.scaffoldDesign.project_id, platform: input.scaffoldDesign.platform, contracts_state: input.decision === "rejected" ? "rejected" : approved ? "approved_for_future_contract_tests" : "draft", created_at: new Date().toISOString(), contracts_mode: approved ? "operator_review_real_upload_scaffold_contracts" : "real_upload_scaffold_contracts_only", required_artifacts: { real_upload_scaffold_design_validated: true, real_upload_implementation_design_validated: true, final_real_upload_preflight_gate_validated: true, real_upload_dry_run_execution_result_validated: true, real_upload_execution_plan_validated: true, real_upload_strategy_design_validated: true, real_upload_execution_request_validated: true, real_upload_readiness_assessment_validated: true, dry_run_upload_spike_result_validated: true, upload_execution_design_validated: true, upload_execution_approval_validated: true, platform_upload_request_validated: true, upload_package_design_validated: true, local_output_review_validated: true, spike_result_validated: true }, contracts_scope: { future_contract_tests_requested: true, contracts_only: true, implementation_files_created: false, scaffold_code_created: false, runtime_enabled: false, current_real_upload_requested: false, current_network_calls_requested: false, current_platform_api_calls_requested: false, current_credential_access_requested: false, current_media_read_requested: false, dependencies_requested: false }, module_contracts: moduleKinds.map((kind, index) => ({ contract_id: `contract-${index + 1}`, module_kind: kind, contract_state: kind === "operator_confirmation_contract" ? "ready_for_future_contract_tests" : "proposed", safe_summary: `Future ${kind.replaceAll("_", " ")} only.`, interface_defined: true, implementation_created: false, runtime_enabled: false, upload_enabled: false, network_enabled: false, platform_api_enabled: false, credential_access_enabled: false, media_read_enabled: false, file_mutation_enabled: false, blocking_reasons: [kind === "credential_provider_contract" ? "Credential access remains disabled." : kind === "media_reference_resolver_contract" ? "Media reads remain disabled." : kind === "platform_adapter_contract" ? "Platform APIs remain disabled." : kind === "payload_builder_contract" ? "Payload construction remains disabled." : kind === "network_client_contract" ? "Network calls remain disabled." : kind === "upload_executor_contract" ? "Upload execution remains disabled." : kind === "retry_idempotency_contract" ? "Retry execution remains disabled." : kind === "rollback_contract" ? "Rollback execution remains disabled." : kind === "post_upload_verification_contract" ? "Verification execution remains disabled." : kind === "audit_event_contract" ? "Raw logging remains disabled." : kind === "operator_confirmation_contract" ? "Operator approval remains required." : kind === "emergency_stop_contract" ? "Emergency stop is contract-only." : "Contract remains safe and local-only."], warnings: [] })), interface_contracts: [{ interface_id: "interface-1", interface_kind: "credential-provider", interface_state: "proposed", safe_summary: "Type-only credential provider interface.", type_only: true, runtime_enabled: false, external_effects_allowed: false, blocking_reasons: ["Credential access remains disabled."], warnings: [] }], data_contracts: { safe_id_contract_defined: true, safe_summary_contract_defined: true, raw_path_contract_defined: false, raw_account_id_contract_defined: false, raw_payload_contract_defined: false, raw_response_contract_defined: false, secret_contract_defined: false, safe_summary: "Data contracts are safe-only.", blocking_reasons: [], warnings: [] }, error_contracts: { safe_error_contract_defined: true, raw_error_payload_allowed: false, raw_stdout_allowed: false, raw_stderr_allowed: false, raw_response_allowed: false, secret_echo_allowed: false, safe_summary: "Errors are safe-only.", blocking_reasons: [], warnings: [] }, audit_contracts: { audit_contract_defined: true, audit_write_enabled: false, raw_payload_logging_allowed: false, raw_response_logging_allowed: false, secret_logging_allowed: false, raw_path_logging_allowed: false, raw_account_id_logging_allowed: false, safe_summary: "Audit stays safe-only.", blocking_reasons: [], warnings: [] }, operator_control_contracts: { operator_confirmation_contract_defined: true, manual_go_no_go_required: true, automatic_execution_allowed: false, emergency_stop_contract_defined: true, reviewed_by_label: input.reviewed_by_label, checklist_acknowledged: !!input.checklist_acknowledged, understands_contracts_only: !!input.understands_contracts_only, understands_no_implementation_files_created: !!input.understands_no_implementation_files_created, understands_no_upload_enabled: !!input.understands_no_upload_enabled, understands_no_credentials_accessed: !!input.understands_no_credentials_accessed, understands_no_network_calls: !!input.understands_no_network_calls, understands_no_media_reads: !!input.understands_no_media_reads, understands_no_dependencies_added: !!input.understands_no_dependencies_added, understands_future_contract_test_phase_required: !!input.understands_future_contract_test_phase_required, decision_note_summary: input.decision_note_summary }, credential_contract_boundary: { credential_contract_defined: true, credentials_required: false, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, credential_reference_stored: false, token_reference_stored: false, secret_material_stored: false }, media_contract_boundary: { media_contract_defined: true, media_file_read: false, raw_media_path_stored: false, media_file_modified: false, media_file_copied: false, media_file_moved: false, media_file_deleted: false }, platform_api_contract_boundary: { platform_api_contract_defined: true, platform_adapter_implemented: false, platform_endpoint_selected: false, platform_api_calls_allowed: false, platform_api_calls_made: false, raw_account_ids_stored: false }, payload_contract_boundary: { payload_contract_defined: true, platform_api_payload_created: false, platform_api_payload_stored: false, upload_payload_created: false, raw_payload_stored: false }, network_contract_boundary: { network_contract_defined: true, network_client_implemented: false, network_calls_allowed: false, network_calls_made: false, external_side_effects_allowed: false, external_side_effects_observed: false }, runtime_contract_boundary: { runtime_contract_defined: true, runtime_config_created: false, env_files_created: false, secrets_config_created: false, credential_config_created: false, package_dependency_changes: false }, file_contract_boundary: { files_created_now: false, implementation_files_created: false, scaffold_files_created: false, package_files_changed: false, runtime_files_created: false, file_mutation_allowed: false }, validation: { scaffold_contracts_complete: true, ready_for_future_contract_tests: approved, ready_for_real_upload: false, implementation_files_created: false, scaffold_code_created: false, dependencies_added: false, upload_allowed: false, upload_execution_enabled: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, media_file_read: false, file_mutation_allowed: false, blocking_reasons: [], warnings: [] }, provenance: { generated_by: "createRealUploadScaffoldContracts", source_real_upload_scaffold_design_id: input.scaffoldDesign.real_upload_scaffold_design_id, source_real_upload_implementation_design_id: input.implementationDesign.real_upload_implementation_design_id, source_final_real_upload_preflight_gate_id: input.finalPreflightGate.final_real_upload_preflight_gate_id, source_real_upload_dry_run_execution_result_id: input.dryRunExecutionResult.real_upload_dry_run_execution_result_id, source_real_upload_execution_plan_id: input.executionPlan.real_upload_execution_plan_id, source_real_upload_strategy_design_id: input.strategyDesign.real_upload_strategy_design_id, source_real_upload_execution_request_id: input.realUploadExecutionRequest.real_upload_execution_request_id, source_real_upload_readiness_assessment_id: input.readinessAssessment.real_upload_readiness_assessment_id, source_dry_run_upload_spike_result_id: input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id, source_upload_execution_design_id: input.uploadExecutionDesign.upload_execution_design_id, source_upload_execution_approval_id: input.uploadExecutionApproval.upload_execution_approval_id, source_platform_upload_request_id: input.platformUploadRequest.platform_upload_request_id, source_upload_package_design_id: input.uploadPackageDesign.upload_package_design_id, source_local_output_review_id: input.localOutputReview.local_output_review_id, source_production_render_spike_result_id: input.spikeResult.production_render_spike_result_id, source_final_render_execution_request_id: input.spikeResult.final_render_execution_request_id, source_render_plan_id: input.spikeResult.render_plan_id } }; if (!validateRealUploadScaffoldContracts(contracts).ok) throw new Error("Unsafe real upload scaffold contracts cannot be created."); return contracts; }
interface RealUploadScaffoldContractsStoreSummaryItem { real_upload_scaffold_contracts_id: string; project_id: string; platform: string; contracts_state: string; created_at: string; }
function safeRealUploadScaffoldContractsSummary(contracts: RealUploadScaffoldContracts): RealUploadScaffoldContractsStoreSummaryItem { return { real_upload_scaffold_contracts_id: safeRealUploadScaffoldContractsString(contracts.real_upload_scaffold_contracts_id, "[unsafe-id]"), project_id: safeRealUploadScaffoldContractsString(contracts.project_id, "[unsafe-project]"), platform: safeRealUploadScaffoldContractsString(contracts.platform, "[unsafe-platform]"), contracts_state: contracts.contracts_state, created_at: contracts.created_at }; }
export function saveRealUploadScaffoldContracts(contracts: RealUploadScaffoldContracts): void { if (!validateRealUploadScaffoldContracts(contracts).ok) throw new Error("Unsafe real upload scaffold contracts cannot be stored."); const store = loadRealUploadScaffoldContractsStore(); const index = store.contracts.findIndex((item) => item.real_upload_scaffold_contracts_id === contracts.real_upload_scaffold_contracts_id); if (index >= 0) store.contracts[index] = contracts; else store.contracts.push(contracts); saveRealUploadScaffoldContractsStore(store); }
export function listRealUploadScaffoldContracts(options?: { project_id?: string; platform?: string; contracts_state?: string; real_upload_scaffold_design_id?: string; real_upload_implementation_design_id?: string }): RealUploadScaffoldContracts[] { return loadRealUploadScaffoldContractsStore().contracts.filter((contracts) => (!options?.project_id || contracts.project_id === options.project_id) && (!options?.platform || contracts.platform === options.platform) && (!options?.contracts_state || contracts.contracts_state === options.contracts_state) && (!options?.real_upload_scaffold_design_id || contracts.real_upload_scaffold_design_id === options.real_upload_scaffold_design_id) && (!options?.real_upload_implementation_design_id || contracts.real_upload_implementation_design_id === options.real_upload_implementation_design_id)).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_scaffold_contracts_id.localeCompare(b.real_upload_scaffold_contracts_id)); }
export function getRealUploadScaffoldContracts(real_upload_scaffold_contracts_id: string): RealUploadScaffoldContracts | null { return loadRealUploadScaffoldContractsStore().contracts.find((contracts) => contracts.real_upload_scaffold_contracts_id === real_upload_scaffold_contracts_id) ?? null; }
export function revokeRealUploadScaffoldContracts(real_upload_scaffold_contracts_id: string, reason: string): RealUploadScaffoldContracts { const contracts = getRealUploadScaffoldContracts(real_upload_scaffold_contracts_id); if (!contracts) throw new Error("Real upload scaffold contracts not found"); const safeReason = safeRealUploadScaffoldContractsString(reason, "[unsafe-reason]"); if (safeReason === "[unsafe-reason]") throw new Error("Unsafe real upload scaffold contracts reason cannot be stored."); const revoked: RealUploadScaffoldContracts = { ...contracts, contracts_state: "revoked", validation: { ...contracts.validation, blocking_reasons: [...contracts.validation.blocking_reasons, safeReason], warnings: [...contracts.validation.warnings, "Real upload scaffold contracts revoked."] }, provenance: { ...contracts.provenance, generated_by: "revokeRealUploadScaffoldContracts" } }; saveRealUploadScaffoldContracts(revoked); return revoked; }
export function getRealUploadScaffoldContractsReport(options?: { project_id?: string; platform?: string }): { total: number; by_state: Record<string, number>; blocked: number; ready_for_operator_review: number; approved_for_future_contract_tests: number; rejected: number; revoked: number; scaffold_contracts_complete: number; ready_for_future_contract_tests: number; ready_for_real_upload: 0; implementation_files_created: 0; scaffold_code_created: 0; dependencies_added: 0; upload_allowed: 0; upload_execution_enabled: 0; platform_api_calls_allowed: 0; network_calls_allowed: 0; credentials_accessed: 0; token_accessed: 0; media_file_read: 0; file_mutation_allowed: 0; module_contracts_total: number; module_contracts_with_blockers: number; contracts: Array<RealUploadScaffoldContractsStoreSummaryItem>; } { const contracts = listRealUploadScaffoldContracts(options); const by_state: Record<string, number> = {}; let blocked = 0, ready = 0, approved = 0, rejected = 0, revoked = 0, complete = 0, readyFuture = 0, moduleContractsTotal = 0, moduleContractsBlocked = 0; const summaries = contracts.map((contract) => { by_state[contract.contracts_state] = (by_state[contract.contracts_state] ?? 0) + 1; if (contract.contracts_state === "blocked") blocked++; if (contract.contracts_state === "ready_for_operator_review") ready++; if (contract.contracts_state === "approved_for_future_contract_tests") approved++; if (contract.contracts_state === "rejected") rejected++; if (contract.contracts_state === "revoked") revoked++; if (contract.validation.scaffold_contracts_complete) complete++; if (contract.validation.ready_for_future_contract_tests) readyFuture++; moduleContractsTotal += contract.module_contracts.length; moduleContractsBlocked += contract.module_contracts.filter((module) => module.blocking_reasons.length > 0).length; return safeRealUploadScaffoldContractsSummary(contract); }); return { total: summaries.length, by_state, blocked, ready_for_operator_review: ready, approved_for_future_contract_tests: approved, rejected, revoked, scaffold_contracts_complete: complete, ready_for_future_contract_tests: readyFuture, ready_for_real_upload: 0, implementation_files_created: 0, scaffold_code_created: 0, dependencies_added: 0, upload_allowed: 0, upload_execution_enabled: 0, platform_api_calls_allowed: 0, network_calls_allowed: 0, credentials_accessed: 0, token_accessed: 0, media_file_read: 0, file_mutation_allowed: 0, module_contracts_total: moduleContractsTotal, module_contracts_with_blockers: moduleContractsBlocked, contracts: summaries }; }

export type RealUploadScaffoldContractTestsMode = "real_upload_scaffold_contract_tests_only" | "operator_review_real_upload_scaffold_contract_tests";
export type RealUploadScaffoldContractTestsState = "draft" | "blocked" | "tested" | "ready_for_operator_review" | "approved_for_future_scaffold_stub_design" | "rejected" | "revoked";
export interface RealUploadScaffoldContractTestsRequiredArtifacts { real_upload_scaffold_contracts_validated: true; real_upload_scaffold_design_validated: true; real_upload_implementation_design_validated: true; final_real_upload_preflight_gate_validated: true; real_upload_dry_run_execution_result_validated: true; real_upload_execution_plan_validated: true; real_upload_strategy_design_validated: true; real_upload_execution_request_validated: true; real_upload_readiness_assessment_validated: true; dry_run_upload_spike_result_validated: true; upload_execution_design_validated: true; upload_execution_approval_validated: true; platform_upload_request_validated: true; upload_package_design_validated: true; local_output_review_validated: true; spike_result_validated: true; }
export interface RealUploadScaffoldContractTestsScope { future_scaffold_stub_design_requested: boolean; contract_tests_only: true; implementation_files_created: false; scaffold_code_created: false; contract_runtime_executed: false; upload_runtime_executed: false; current_real_upload_requested: false; current_network_calls_requested: false; current_platform_api_calls_requested: false; current_credential_access_requested: false; current_media_read_requested: false; dependencies_requested: false; }
export interface RealUploadModuleContractTestResult { test_id: string; module_kind: RealUploadModuleContract["module_kind"]; test_state: "planned" | "blocked" | "passed" | "failed" | "deferred"; safe_summary: string; contract_shape_checked: boolean; implementation_created: false; runtime_executed: false; upload_enabled: false; network_enabled: false; platform_api_enabled: false; credential_access_enabled: false; media_read_enabled: false; file_mutation_enabled: false; raw_payload_created: false; raw_response_stored: false; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadInterfaceContractTestResult { test_id: string; interface_kind: string; test_state: "planned" | "blocked" | "passed" | "failed" | "deferred"; safe_summary: string; type_shape_checked: boolean; runtime_executed: false; external_effects_allowed: false; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadDataContractTestResults { safe_id_contract_checked: boolean; safe_summary_contract_checked: boolean; raw_path_contract_tested: false; raw_account_id_contract_tested: false; raw_payload_contract_tested: false; raw_response_contract_tested: false; secret_contract_tested: false; safe_summary: string; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadErrorContractTestResults { safe_error_contract_checked: boolean; raw_error_payload_tested: false; raw_stdout_tested: false; raw_stderr_tested: false; raw_response_tested: false; secret_echo_tested: false; safe_summary: string; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadAuditContractTestResults { audit_contract_checked: boolean; audit_write_executed: false; raw_payload_logging_tested: false; raw_response_logging_tested: false; secret_logging_tested: false; raw_path_logging_tested: false; raw_account_id_logging_tested: false; safe_summary: string; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadOperatorControlContractTestResults { operator_confirmation_contract_checked: boolean; manual_go_no_go_required: true; automatic_execution_allowed: false; emergency_stop_contract_checked: boolean; reviewed_by_label?: string; checklist_acknowledged: boolean; understands_contract_tests_only: boolean; understands_no_implementation_files_created: boolean; understands_no_upload_enabled: boolean; understands_no_credentials_accessed: boolean; understands_no_network_calls: boolean; understands_no_media_reads: boolean; understands_no_dependencies_added: boolean; understands_future_stub_design_phase_required: boolean; decision_note_summary?: string; }
export interface RealUploadBoundaryTestResults { credential_boundary_checked: boolean; media_boundary_checked: boolean; platform_api_boundary_checked: boolean; payload_boundary_checked: boolean; network_boundary_checked: boolean; runtime_boundary_checked: boolean; file_boundary_checked: boolean; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; media_file_read: false; platform_api_calls_made: false; network_calls_made: false; platform_api_payload_created: false; upload_payload_created: false; response_received: false; file_mutation_allowed: false; }
export interface RealUploadContractTestsExecutionBoundary { implementation_files_created: false; scaffold_code_created: false; dependencies_added: false; contract_runtime_executed: false; upload_runtime_executed: false; upload_allowed: false; upload_execution_enabled: false; platform_api_calls_allowed: false; network_calls_allowed: false; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; media_file_read: false; file_mutation_allowed: false; ready_for_real_upload: false; }
export interface RealUploadScaffoldContractTestsValidationResult { ok: boolean; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadScaffoldContractTests { schema_version: "1.0"; real_upload_scaffold_contract_tests_id: string; real_upload_scaffold_contracts_id: string; real_upload_scaffold_design_id: string; real_upload_implementation_design_id: string; final_real_upload_preflight_gate_id: string; real_upload_dry_run_execution_result_id: string; real_upload_execution_plan_id: string; real_upload_strategy_design_id: string; real_upload_execution_request_id: string; real_upload_readiness_assessment_id: string; dry_run_upload_spike_result_id: string; upload_execution_design_id: string; upload_execution_approval_id: string; platform_upload_request_id: string; upload_package_design_id: string; local_output_review_id: string; production_render_spike_result_id: string; final_render_execution_request_id: string; render_plan_id: string; project_id: string; platform: string; contract_tests_state: RealUploadScaffoldContractTestsState; created_at: string; contract_tests_mode: RealUploadScaffoldContractTestsMode; required_artifacts: RealUploadScaffoldContractTestsRequiredArtifacts; contract_tests_scope: RealUploadScaffoldContractTestsScope; module_contract_test_results: RealUploadModuleContractTestResult[]; interface_contract_test_results: RealUploadInterfaceContractTestResult[]; data_contract_test_results: RealUploadDataContractTestResults; error_contract_test_results: RealUploadErrorContractTestResults; audit_contract_test_results: RealUploadAuditContractTestResults; operator_control_contract_test_results: RealUploadOperatorControlContractTestResults; boundary_test_results: RealUploadBoundaryTestResults; execution_boundary: RealUploadContractTestsExecutionBoundary; validation: { scaffold_contract_tests_complete: boolean; ready_for_future_scaffold_stub_design: boolean; ready_for_real_upload: false; implementation_files_created: false; scaffold_code_created: false; dependencies_added: false; contract_runtime_executed: false; upload_runtime_executed: false; upload_allowed: false; upload_execution_enabled: false; platform_api_calls_allowed: false; network_calls_allowed: false; credentials_accessed: false; token_accessed: false; media_file_read: false; file_mutation_allowed: false; blocking_reasons: string[]; warnings: string[]; }; provenance: { generated_by: "createRealUploadScaffoldContractTests" | "revokeRealUploadScaffoldContractTests"; source_real_upload_scaffold_contracts_id: string; source_real_upload_scaffold_design_id: string; source_real_upload_implementation_design_id: string; source_final_real_upload_preflight_gate_id: string; source_real_upload_dry_run_execution_result_id: string; source_real_upload_execution_plan_id: string; source_real_upload_strategy_design_id: string; source_real_upload_execution_request_id: string; source_real_upload_readiness_assessment_id: string; source_dry_run_upload_spike_result_id: string; source_upload_execution_design_id: string; source_upload_execution_approval_id: string; source_platform_upload_request_id: string; source_upload_package_design_id: string; source_local_output_review_id: string; source_production_render_spike_result_id: string; source_final_render_execution_request_id: string; source_render_plan_id: string; }; }

function safeRealUploadScaffoldContractTestsString(value: unknown, fallback: string): string { return safeRealUploadScaffoldContractsString(value, fallback); }
interface RealUploadScaffoldContractTestsStore { schema_version: "1.0"; created_at: string; contract_tests: RealUploadScaffoldContractTests[]; }
interface RealUploadScaffoldContractTestsStoreSummaryItem { contract_tests_state: RealUploadScaffoldContractTestsState; created_at: string; module_tests_total: number; module_tests_with_blockers: number; safe_summary: string; }
function getRealUploadScaffoldContractTestsPath(): string { return path.join(getRuntimeDir(), "real-upload-scaffold-contract-tests.json"); }
function loadRealUploadScaffoldContractTestsStore(): RealUploadScaffoldContractTestsStore { try { const filePath = getRealUploadScaffoldContractTestsPath(); if (fs.existsSync(filePath)) return JSON.parse(fs.readFileSync(filePath, "utf8")) as RealUploadScaffoldContractTestsStore; } catch {} return { schema_version: "1.0", created_at: new Date().toISOString(), contract_tests: [] }; }
function saveRealUploadScaffoldContractTestsStore(store: RealUploadScaffoldContractTestsStore): void { const filePath = getRealUploadScaffoldContractTestsPath(); fs.mkdirSync(path.dirname(filePath), { recursive: true }); store.contract_tests.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_scaffold_contract_tests_id.localeCompare(b.real_upload_scaffold_contract_tests_id)); fs.writeFileSync(filePath, JSON.stringify(store, null, 2), "utf8"); }
function validateRealUploadScaffoldContractTestsShape(tests: unknown): RealUploadScaffoldContractTestsValidationResult { const blocking_reasons: string[] = []; const warnings: string[] = []; const fail = (reason: string) => blocking_reasons.push(reason); const scan = (value: unknown): boolean => { if (typeof value === "string") { const lower = value.toLowerCase(); return lower.includes("videos.insert") || lower.includes("youtube.videos().insert") || lower.includes("fetch(") || lower.includes("bearer ") || lower.includes("access_token") || lower.includes("refresh_token") || lower.includes("client_secret") || lower.includes("keychain://") || lower.includes("/users/") || lower.includes("http://") || lower.includes("https://") || lower.includes("../") || lower.includes("stdout") || lower.includes("stderr") || lower.includes("process.env"); } if (Array.isArray(value)) return value.some(scan); if (value && typeof value === "object") return Object.entries(value as Record<string, unknown>).some(([key, child]) => { const lowerKey = key.toLowerCase(); return lowerKey.includes("credential_reference") || lowerKey.includes("credentialreference") || lowerKey.includes("access_token") || lowerKey.includes("refresh_token") || lowerKey.includes("client_secret") || lowerKey.includes("code_verifier") || lowerKey.includes("authorization_code") || scan(child); }); return false; }; if (!tests || typeof tests !== "object" || Array.isArray(tests)) return { ok: false, blocking_reasons: ["Invalid scaffold contract tests"], warnings }; const t = tests as RealUploadScaffoldContractTests; if (t.schema_version !== "1.0") fail("Invalid schema version"); if (!t.required_artifacts || t.required_artifacts.real_upload_scaffold_contracts_validated !== true || t.required_artifacts.real_upload_scaffold_design_validated !== true || t.required_artifacts.real_upload_implementation_design_validated !== true || t.required_artifacts.final_real_upload_preflight_gate_validated !== true || t.required_artifacts.real_upload_dry_run_execution_result_validated !== true || t.required_artifacts.real_upload_execution_plan_validated !== true || t.required_artifacts.real_upload_strategy_design_validated !== true || t.required_artifacts.real_upload_execution_request_validated !== true || t.required_artifacts.real_upload_readiness_assessment_validated !== true || t.required_artifacts.dry_run_upload_spike_result_validated !== true || t.required_artifacts.upload_execution_design_validated !== true || t.required_artifacts.upload_execution_approval_validated !== true || t.required_artifacts.platform_upload_request_validated !== true || t.required_artifacts.upload_package_design_validated !== true || t.required_artifacts.local_output_review_validated !== true || t.required_artifacts.spike_result_validated !== true) fail("Unsafe required artifacts"); if (t.contract_tests_state !== "draft" && t.contract_tests_state !== "blocked" && t.contract_tests_state !== "tested" && t.contract_tests_state !== "ready_for_operator_review" && t.contract_tests_state !== "approved_for_future_scaffold_stub_design" && t.contract_tests_state !== "rejected" && t.contract_tests_state !== "revoked") fail("Invalid contract tests state"); if (t.contract_tests_mode !== "real_upload_scaffold_contract_tests_only" && t.contract_tests_mode !== "operator_review_real_upload_scaffold_contract_tests") fail("Invalid contract tests mode"); if (t.contract_tests_scope?.contract_tests_only !== true || t.contract_tests_scope?.implementation_files_created !== false || t.contract_tests_scope?.scaffold_code_created !== false || t.contract_tests_scope?.contract_runtime_executed !== false || t.contract_tests_scope?.upload_runtime_executed !== false || t.contract_tests_scope?.current_real_upload_requested !== false || t.contract_tests_scope?.current_network_calls_requested !== false || t.contract_tests_scope?.current_platform_api_calls_requested !== false || t.contract_tests_scope?.current_credential_access_requested !== false || t.contract_tests_scope?.current_media_read_requested !== false || t.contract_tests_scope?.dependencies_requested !== false) fail("Unsafe contract tests scope"); if (!Array.isArray(t.module_contract_test_results) || new Set(t.module_contract_test_results.map((module) => module.module_kind)).size !== t.module_contract_test_results.length || ["credential_provider_contract","media_reference_resolver_contract","platform_adapter_contract","payload_builder_contract","network_client_contract","upload_executor_contract","retry_idempotency_contract","rollback_contract","post_upload_verification_contract","audit_event_contract","operator_confirmation_contract","emergency_stop_contract","status_reporter_contract","upload_scaffold_index_contract"].some((kind) => !t.module_contract_test_results.some((module) => module.module_kind === kind))) fail("Missing or duplicate module kinds"); if (Array.isArray(t.module_contract_test_results) && t.module_contract_test_results.some((module) => module.contract_shape_checked !== true || module.implementation_created !== false || module.runtime_executed !== false || module.upload_enabled !== false || module.network_enabled !== false || module.platform_api_enabled !== false || module.credential_access_enabled !== false || module.media_read_enabled !== false || module.file_mutation_enabled !== false || module.raw_payload_created !== false || module.raw_response_stored !== false)) fail("Unsafe module contract test results"); if (!t.interface_contract_test_results || t.interface_contract_test_results.some((item) => item.type_shape_checked !== true || item.runtime_executed !== false || item.external_effects_allowed !== false)) fail("Unsafe interface contract test results"); if (!t.data_contract_test_results || t.data_contract_test_results.safe_id_contract_checked !== true || t.data_contract_test_results.safe_summary_contract_checked !== true || t.data_contract_test_results.raw_path_contract_tested !== false || t.data_contract_test_results.raw_account_id_contract_tested !== false || t.data_contract_test_results.raw_payload_contract_tested !== false || t.data_contract_test_results.raw_response_contract_tested !== false || t.data_contract_test_results.secret_contract_tested !== false) fail("Unsafe data contract test results"); if (!t.error_contract_test_results || t.error_contract_test_results.safe_error_contract_checked !== true || t.error_contract_test_results.raw_error_payload_tested !== false || t.error_contract_test_results.raw_stdout_tested !== false || t.error_contract_test_results.raw_stderr_tested !== false || t.error_contract_test_results.raw_response_tested !== false || t.error_contract_test_results.secret_echo_tested !== false) fail("Unsafe error contract test results"); if (!t.audit_contract_test_results || t.audit_contract_test_results.audit_contract_checked !== true || t.audit_contract_test_results.audit_write_executed !== false || t.audit_contract_test_results.raw_payload_logging_tested !== false || t.audit_contract_test_results.raw_response_logging_tested !== false || t.audit_contract_test_results.secret_logging_tested !== false || t.audit_contract_test_results.raw_path_logging_tested !== false || t.audit_contract_test_results.raw_account_id_logging_tested !== false) fail("Unsafe audit contract test results"); if (!t.operator_control_contract_test_results || t.operator_control_contract_test_results.operator_confirmation_contract_checked !== true || t.operator_control_contract_test_results.manual_go_no_go_required !== true || t.operator_control_contract_test_results.automatic_execution_allowed !== false || t.operator_control_contract_test_results.emergency_stop_contract_checked !== true || t.operator_control_contract_test_results.checklist_acknowledged !== true || t.operator_control_contract_test_results.understands_contract_tests_only !== true || t.operator_control_contract_test_results.understands_no_implementation_files_created !== true || t.operator_control_contract_test_results.understands_no_upload_enabled !== true || t.operator_control_contract_test_results.understands_no_credentials_accessed !== true || t.operator_control_contract_test_results.understands_no_network_calls !== true || t.operator_control_contract_test_results.understands_no_media_reads !== true || t.operator_control_contract_test_results.understands_no_dependencies_added !== true || t.operator_control_contract_test_results.understands_future_stub_design_phase_required !== true) fail("Unsafe operator control contract test results"); if (!t.boundary_test_results || t.boundary_test_results.credential_boundary_checked !== true || t.boundary_test_results.media_boundary_checked !== true || t.boundary_test_results.platform_api_boundary_checked !== true || t.boundary_test_results.payload_boundary_checked !== true || t.boundary_test_results.network_boundary_checked !== true || t.boundary_test_results.runtime_boundary_checked !== true || t.boundary_test_results.file_boundary_checked !== true || t.boundary_test_results.credentials_accessed !== false || t.boundary_test_results.token_accessed !== false || t.boundary_test_results.keychain_accessed !== false || t.boundary_test_results.env_accessed !== false || t.boundary_test_results.media_file_read !== false || t.boundary_test_results.platform_api_calls_made !== false || t.boundary_test_results.network_calls_made !== false || t.boundary_test_results.platform_api_payload_created !== false || t.boundary_test_results.upload_payload_created !== false || t.boundary_test_results.response_received !== false || t.boundary_test_results.file_mutation_allowed !== false) fail("Unsafe boundary test results"); if (!t.execution_boundary || t.execution_boundary.implementation_files_created !== false || t.execution_boundary.scaffold_code_created !== false || t.execution_boundary.dependencies_added !== false || t.execution_boundary.contract_runtime_executed !== false || t.execution_boundary.upload_runtime_executed !== false || t.execution_boundary.upload_allowed !== false || t.execution_boundary.upload_execution_enabled !== false || t.execution_boundary.platform_api_calls_allowed !== false || t.execution_boundary.network_calls_allowed !== false || t.execution_boundary.credentials_accessed !== false || t.execution_boundary.token_accessed !== false || t.execution_boundary.keychain_accessed !== false || t.execution_boundary.env_accessed !== false || t.execution_boundary.media_file_read !== false || t.execution_boundary.file_mutation_allowed !== false || t.execution_boundary.ready_for_real_upload !== false) fail("Unsafe execution boundary"); if (!t.validation || t.validation.ready_for_real_upload !== false || t.validation.implementation_files_created !== false || t.validation.scaffold_code_created !== false || t.validation.dependencies_added !== false || t.validation.contract_runtime_executed !== false || t.validation.upload_runtime_executed !== false || t.validation.upload_allowed !== false || t.validation.upload_execution_enabled !== false || t.validation.platform_api_calls_allowed !== false || t.validation.network_calls_allowed !== false || t.validation.credentials_accessed !== false || t.validation.token_accessed !== false || t.validation.media_file_read !== false || t.validation.file_mutation_allowed !== false) fail("Unsafe validation"); if (scan(tests)) fail("Unsafe text content"); return { ok: blocking_reasons.length === 0, blocking_reasons, warnings }; }
export function validateRealUploadScaffoldContractTests(tests: unknown): RealUploadScaffoldContractTestsValidationResult { return validateRealUploadScaffoldContractTestsShape(tests); }
export function createRealUploadScaffoldContractTests(input: {
  ...
  localOutputReview: LocalOutputOperatorReview;
  spikeResult: ControlledProductionRenderSpikeResult;
  decision?: "draft" | "approved_for_future_scaffold_stub_design" | "rejected";
  reviewed_by_label?: string;
  checklist_acknowledged?: boolean;
  ...
  understands_future_stub_design_phase_required?: boolean;
  decision_note_summary?: string;
  dryRun: true;
}): RealUploadScaffoldContractTests {
  if (input.dryRun !== true) throw new Error("createRealUploadScaffoldContractTests: dryRun=true required");
  if (!validateRealUploadScaffoldContracts(input.scaffoldContracts).ok || !validateRealUploadScaffoldDesign(input.scaffoldDesign).ok || !validateRealUploadImplementationDesign(input.implementationDesign).ok || !validateFinalRealUploadPreflightGate(input.finalPreflightGate).ok || !validateRealUploadDryRunExecutionResult(input.dryRunExecutionResult).ok || !validateRealUploadExecutionPlan(input.executionPlan).ok || !validateRealUploadStrategyDesign(input.strategyDesign).ok || !validateRealUploadExecutionRequest(input.realUploadExecutionRequest).ok || !validateRealUploadReadinessAssessment(input.readinessAssessment).ok || !validateDryRunUploadSpikeResult(input.dryRunUploadSpikeResult).ok || !validateUploadExecutionDesign(input.uploadExecutionDesign).ok || !validateUploadExecutionApproval(input.uploadExecutionApproval).ok || !validatePlatformUploadRequest(input.platformUploadRequest).ok || !validateUploadPackageDesign(input.uploadPackageDesign).ok || !validateLocalOutputOperatorReview(input.localOutputReview).ok || !validateControlledProductionRenderSpikeResult(input.spikeResult).ok) throw new Error("createRealUploadScaffoldContractTests: all input artifacts must validate");
  if (input.scaffoldContracts.contracts_state !== "approved_for_future_contract_tests") throw new Error("createRealUploadScaffoldContractTests: scaffoldContracts must be approved_for_future_contract_tests");
  if (input.scaffoldContracts.validation.ready_for_real_upload !== false || input.scaffoldContracts.validation.ready_for_future_contract_tests !== true) throw new Error("createRealUploadScaffoldContractTests: scaffoldContracts readiness must remain safe");
  const approved = input.decision === "approved_for_future_scaffold_stub_design";
  if (approved && (!input.checklist_acknowledged || !input.understands_contract_tests_only || !input.understands_no_implementation_files_created || !input.understands_no_upload_enabled || !input.understands_no_credentials_accessed || !input.understands_no_network_calls || !input.understands_no_media_reads || !input.understands_no_dependencies_added || !input.understands_future_stub_design_phase_required)) throw new Error("createRealUploadScaffoldContractTests: acknowledgements required");
  const kinds = ["credential_provider_contract","media_reference_resolver_contract","platform_adapter_contract","payload_builder_contract","network_client_contract","upload_executor_contract","retry_idempotency_contract","rollback_contract","post_upload_verification_contract","audit_event_contract","operator_confirmation_contract","emergency_stop_contract","status_reporter_contract","upload_scaffold_index_contract"] as const;
  const tests: RealUploadScaffoldContractTests = {
    schema_version: "1.0",
    real_upload_scaffold_contract_tests_id: crypto.randomUUID(),
    ...
    final_render_execution_request_id: input.spikeResult.final_render_execution_request_id,
    render_plan_id: input.spikeResult.render_plan_id,
    project_id: input.scaffoldContracts.project_id,
    platform: input.scaffoldContracts.platform,
    contract_tests_state: approved ? "approved_for_future_scaffold_stub_design" : input.decision === "rejected" ? "rejected" : "draft",
    created_at: new Date().toISOString(),
    contract_tests_mode: approved ? "operator_review_real_upload_scaffold_contract_tests" : "real_upload_scaffold_contract_tests_only",
    required_artifacts: {
    ...
    contract_tests_scope: {
      future_scaffold_stub_design_requested: true,
      contract_tests_only: true,
      implementation_files_created: false,
      scaffold_code_created: false,
      contract_runtime_executed: false,
      upload_runtime_executed: false,
      current_real_upload_requested: false,
      current_network_calls_requested: false,
      current_platform_api_calls_requested: false,
      current_credential_access_requested: false,
    ...
    module_contract_test_results: kinds.map((kind, index) => ({
      test_id: `test-${index + 1}`,
      module_kind: kind,
      test_state: kind === "operator_confirmation_contract" ? "blocked" : "planned",
      safe_summary: `Future ${kind.replaceAll("_", " ")} contract test only.`,
      contract_shape_checked: true,
      ...
      raw_payload_created: false,
      raw_response_stored: false,
      blocking_reasons: [kind === "credential_provider_contract" ? "Credential access remains disabled." : kind === "media_reference_resolver_contract" ? "Media reads remain disabled." : kind === "platform_adapter_contract" ? "Platform APIs remain disabled." : kind === "payload_builder_contract" ? "Payload construction remains disabled." : kind === "network_client_contract" ? "Network calls remain disabled." : kind === "upload_executor_contract" ? "Upload execution remains disabled." : kind === "retry_idempotency_contract" ? "Retry execution remains disabled." : kind === "rollback_contract" ? "Rollback execution remains disabled." : kind === "post_upload_verification_contract" ? "Verification execution remains disabled." : kind === "audit_event_contract" ? "Raw logging remains disabled." : kind === "operator_confirmation_contract" ? "Operator approval remains required." : kind === "emergency_stop_contract" ? "Emergency stop is contract-only." : kind === "status_reporter_contract" ? "Status reporting remains local-only." : "Scaffold indexing remains contract-only."],
      warnings: [],
    })),
    interface_contract_test_results: [{
      test_id: "interface-1",
      interface_kind: "credential-provider",
      test_state: "planned",
      safe_summary: "Type shape only.",
      type_shape_checked: true,
      runtime_executed: false,
      external_effects_allowed: false,
      blocking_reasons: ["Credential access remains disabled."],
      warnings: [],
    }],
    data_contract_test_results: { safe_id_contract_checked: true, safe_summary_contract_checked: true, raw_path_contract_tested: false, raw_account_id_contract_tested: false, raw_payload_contract_tested: false, raw_response_contract_tested: false, secret_contract_tested: false, safe_summary: "Data contracts stay safe-only.", blocking_reasons: [], warnings: [] },
    error_contract_test_results: { safe_error_contract_checked: true, raw_error_payload_tested: false, raw_stdout_tested: false, raw_stderr_tested: false, raw_response_tested: false, secret_echo_tested: false, safe_summary: "Error contracts stay safe-only.", blocking_reasons: [], warnings: [] },
    audit_contract_test_results: { audit_contract_checked: true, audit_write_executed: false, raw_payload_logging_tested: false, raw_response_logging_tested: false, secret_logging_tested: false, raw_path_logging_tested: false, raw_account_id_logging_tested: false, safe_summary: "Audit contracts stay safe-only.", blocking_reasons: [], warnings: [] },
    operator_control_contract_test_results: {
      operator_confirmation_contract_checked: true,
      manual_go_no_go_required: true,
      automatic_execution_allowed: false,
      emergency_stop_contract_checked: true,
      reviewed_by_label: input.reviewed_by_label || "operator-001",
      checklist_acknowledged: !!input.checklist_acknowledged,
      understands_contract_tests_only: !!input.understands_contract_tests_only,
      ...
      understands_future_stub_design_phase_required: !!input.understands_future_stub_design_phase_required,
      decision_note_summary: safeRealUploadScaffoldContractTestsString(input.decision_note_summary, "[contract-tests]"),
    },
    boundary_test_results: { credential_boundary_checked: true, media_boundary_checked: true, platform_api_boundary_checked: true, payload_boundary_checked: true, network_boundary_checked: true, runtime_boundary_checked: true, file_boundary_checked: true, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, media_file_read: false, platform_api_calls_made: false, network_calls_made: false, platform_api_payload_created: false, upload_payload_created: false, response_received: false, file_mutation_allowed: false },
    execution_boundary: { implementation_files_created: false, scaffold_code_created: false, dependencies_added: false, contract_runtime_executed: false, upload_runtime_executed: false, upload_allowed: false, upload_execution_enabled: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, media_file_read: false, file_mutation_allowed: false, ready_for_real_upload: false },
    validation: { scaffold_contract_tests_complete: true, ready_for_future_scaffold_stub_design: approved, ready_for_real_upload: false, implementation_files_created: false, scaffold_code_created: false, dependencies_added: false, contract_runtime_executed: false, upload_runtime_executed: false, upload_allowed: false, upload_execution_enabled: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, media_file_read: false, file_mutation_allowed: false, blocking_reasons: [], warnings: [] },
    provenance: { generated_by: "createRealUploadScaffoldContractTests", source_real_upload_scaffold_contracts_id: input.scaffoldContracts.real_upload_scaffold_contracts_id, source_real_upload_scaffold_design_id: input.scaffoldDesign.real_upload_scaffold_design_id, source_real_upload_implementation_design_id: input.implementationDesign.real_upload_implementation_design_id, source_final_real_upload_preflight_gate_id: input.finalPreflightGate.final_real_upload_preflight_gate_id, source_real_upload_dry_run_execution_result_id: input.dryRunExecutionResult.real_upload_dry_run_execution_result_id, source_real_upload_execution_plan_id: input.executionPlan.real_upload_execution_plan_id, source_real_upload_strategy_design_id: input.strategyDesign.real_upload_strategy_design_id, source_real_upload_execution_request_id: input.realUploadExecutionRequest.real_upload_execution_request_id, source_real_upload_readiness_assessment_id: input.readinessAssessment.real_upload_readiness_assessment_id, source_dry_run_upload_spike_result_id: input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id, source_upload_execution_design_id: input.uploadExecutionDesign.upload_execution_design_id, source_upload_execution_approval_id: input.uploadExecutionApproval.upload_execution_approval_id, source_platform_upload_request_id: input.platformUploadRequest.platform_upload_request_id, source_upload_package_design_id: input.uploadPackageDesign.upload_package_design_id, source_local_output_review_id: input.localOutputReview.local_output_review_id, source_production_render_spike_result_id: input.spikeResult.production_render_spike_result_id, source_final_render_execution_request_id: input.spikeResult.final_render_execution_request_id, source_render_plan_id: input.spikeResult.render_plan_id },
  };
  const validation = validateRealUploadScaffoldContractTests(tests);
  if (!validation.ok) throw new Error(`Unsafe real upload scaffold contract tests cannot be created: ${validation.blocking_reasons.join("; ")}`);
  return tests;
}
export function saveRealUploadScaffoldContractTests(tests: RealUploadScaffoldContractTests): void { const validation = validateRealUploadScaffoldContractTests(tests); if (!validation.ok) throw new Error(`Unsafe real upload scaffold contract tests cannot be stored: ${validation.blocking_reasons.join("; ")}`); const store = loadRealUploadScaffoldContractTestsStore(); const index = store.contract_tests.findIndex((item) => item.real_upload_scaffold_contract_tests_id === tests.real_upload_scaffold_contract_tests_id); if (index >= 0) store.contract_tests[index] = tests; else store.contract_tests.push(tests); saveRealUploadScaffoldContractTestsStore(store); }
export function listRealUploadScaffoldContractTests(options?: { project_id?: string; platform?: string; contract_tests_state?: string; real_upload_scaffold_contracts_id?: string; real_upload_scaffold_design_id?: string }): RealUploadScaffoldContractTests[] { return loadRealUploadScaffoldContractTestsStore().contract_tests.filter((item) => (!options?.project_id || item.project_id === options.project_id) && (!options?.platform || item.platform === options.platform) && (!options?.contract_tests_state || item.contract_tests_state === options.contract_tests_state) && (!options?.real_upload_scaffold_contracts_id || item.real_upload_scaffold_contracts_id === options.real_upload_scaffold_contracts_id) && (!options?.real_upload_scaffold_design_id || item.real_upload_scaffold_design_id === options.real_upload_scaffold_design_id)).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_scaffold_contract_tests_id.localeCompare(b.real_upload_scaffold_contract_tests_id)); }
export function getRealUploadScaffoldContractTests(real_upload_scaffold_contract_tests_id: string): RealUploadScaffoldContractTests | null { return loadRealUploadScaffoldContractTestsStore().contract_tests.find((item) => item.real_upload_scaffold_contract_tests_id === real_upload_scaffold_contract_tests_id) ?? null; }
export function revokeRealUploadScaffoldContractTests(real_upload_scaffold_contract_tests_id: string, reason: string): RealUploadScaffoldContractTests { const tests = getRealUploadScaffoldContractTests(real_upload_scaffold_contract_tests_id); if (!tests) throw new Error("Real upload scaffold contract tests not found"); const safeReason = safeRealUploadScaffoldContractTestsString(reason, "[unsafe-reason]"); if (safeReason === "[unsafe-reason]") throw new Error("Unsafe real upload scaffold contract tests reason cannot be stored."); const revoked: RealUploadScaffoldContractTests = { ...tests, contract_tests_state: "revoked", validation: { ...tests.validation, blocking_reasons: [...tests.validation.blocking_reasons, safeReason], warnings: [...tests.validation.warnings, "Real upload scaffold contract tests revoked."] }, provenance: { ...tests.provenance, generated_by: "revokeRealUploadScaffoldContractTests" } }; saveRealUploadScaffoldContractTests(revoked); return revoked; }
export function getRealUploadScaffoldContractTestsReport(options?: { project_id?: string; platform?: string }): { total: number; by_state: Record<string, number>; blocked: number; tested: number; ready_for_operator_review: number; approved_for_future_scaffold_stub_design: number; rejected: number; revoked: number; scaffold_contract_tests_complete: number; ready_for_future_scaffold_stub_design: number; ready_for_real_upload: 0; implementation_files_created: 0; scaffold_code_created: 0; dependencies_added: 0; contract_runtime_executed: 0; upload_runtime_executed: 0; upload_allowed: 0; upload_execution_enabled: 0; platform_api_calls_allowed: 0; network_calls_allowed: 0; credentials_accessed: 0; token_accessed: 0; media_file_read: 0; file_mutation_allowed: 0; module_tests_total: number; module_tests_with_blockers: number; contract_tests: Array<RealUploadScaffoldContractTestsStoreSummaryItem>; } { const tests = listRealUploadScaffoldContractTests(options); const by_state: Record<string, number> = {}; let blocked = 0, tested = 0, ready = 0, approved = 0, rejected = 0, revoked = 0, complete = 0, readyFuture = 0, moduleTestsTotal = 0, moduleTestsBlocked = 0; const summaries = tests.map((test) => { by_state[test.contract_tests_state] = (by_state[test.contract_tests_state] ?? 0) + 1; if (test.contract_tests_state === "blocked") blocked++; if (test.contract_tests_state === "tested") tested++; if (test.contract_tests_state === "ready_for_operator_review") ready++; if (test.contract_tests_state === "approved_for_future_scaffold_stub_design") approved++; if (test.contract_tests_state === "rejected") rejected++; if (test.contract_tests_state === "revoked") revoked++; if (test.validation.scaffold_contract_tests_complete) complete++; if (test.validation.ready_for_future_scaffold_stub_design) readyFuture++; moduleTestsTotal += test.module_contract_test_results.length; moduleTestsBlocked += test.module_contract_test_results.filter((module) => module.blocking_reasons.length > 0).length; return { contract_tests_state: test.contract_tests_state, created_at: test.created_at, module_tests_total: test.module_contract_test_results.length, module_tests_with_blockers: test.module_contract_test_results.filter((module) => module.blocking_reasons.length > 0).length, safe_summary: safeRealUploadScaffoldContractTestsString(test.validation.blocking_reasons[0] || test.module_contract_test_results[0]?.safe_summary || "Contract tests summary.", "Contract tests summary.") }; }); return { total: summaries.length, by_state, blocked, tested, ready_for_operator_review: ready, approved_for_future_scaffold_stub_design: approved, rejected, revoked, scaffold_contract_tests_complete: complete, ready_for_future_scaffold_stub_design: readyFuture, ready_for_real_upload: 0, implementation_files_created: 0, scaffold_code_created: 0, dependencies_added: 0, contract_runtime_executed: 0, upload_runtime_executed: 0, upload_allowed: 0, upload_execution_enabled: 0, platform_api_calls_allowed: 0, network_calls_allowed: 0, credentials_accessed: 0, token_accessed: 0, media_file_read: 0, file_mutation_allowed: 0, module_tests_total: moduleTestsTotal, module_tests_with_blockers: moduleTestsBlocked, contract_tests: summaries }; }

// ─── VO-7R: Real Upload Scaffold Stub Design ──────────────────────────────

export type RealUploadScaffoldStubDesignMode = "real_upload_scaffold_stub_design_only" | "operator_review_real_upload_scaffold_stub_design";
export type RealUploadScaffoldStubDesignState = "draft" | "blocked" | "ready_for_operator_review" | "approved_for_future_stub_contracts" | "rejected" | "revoked";
export interface RealUploadScaffoldStubRequiredArtifacts { real_upload_scaffold_contract_tests_validated: true; real_upload_scaffold_contracts_validated: true; real_upload_scaffold_design_validated: true; real_upload_implementation_design_validated: true; final_real_upload_preflight_gate_validated: true; real_upload_dry_run_execution_result_validated: true; real_upload_execution_plan_validated: true; real_upload_strategy_design_validated: true; real_upload_execution_request_validated: true; real_upload_readiness_assessment_validated: true; dry_run_upload_spike_result_validated: true; upload_execution_design_validated: true; upload_execution_approval_validated: true; platform_upload_request_validated: true; upload_package_design_validated: true; local_output_review_validated: true; spike_result_validated: true; }
export interface RealUploadScaffoldStubDesignScope { future_stub_contracts_requested: boolean; stub_design_only: true; stub_files_created: false; implementation_files_created: false; runtime_files_created: false; runtime_enabled: false; current_real_upload_requested: false; current_network_calls_requested: false; current_platform_api_calls_requested: false; current_credential_access_requested: false; current_media_read_requested: false; dependencies_requested: false; package_metadata_changes_requested: false; }
export interface RealUploadProposedStubModule { stub_module_id: string; module_kind: "credential_provider_stub" | "media_reference_resolver_stub" | "platform_adapter_stub" | "payload_builder_stub" | "network_client_stub" | "upload_executor_stub" | "retry_idempotency_stub" | "rollback_stub" | "post_upload_verification_stub" | "audit_event_stub" | "operator_confirmation_stub" | "emergency_stop_stub" | "status_reporter_stub" | "upload_scaffold_index_stub"; stub_state: "proposed" | "blocked" | "deferred" | "ready_for_future_stub_contracts"; safe_summary: string; stub_file_created: false; implementation_created: false; runtime_enabled: false; no_op_only: true; upload_enabled: false; network_enabled: false; platform_api_enabled: false; credential_access_enabled: false; media_read_enabled: false; file_mutation_enabled: false; raw_payload_created: false; raw_response_stored: false; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadProposedStubBehavior { behavior_id: string; behavior_kind: string; behavior_state: "proposed" | "blocked" | "deferred" | "ready_for_future_stub_contracts"; safe_summary: string; no_op_behavior: true; runtime_executed: false; external_effects_allowed: false; upload_effect_allowed: false; network_effect_allowed: false; credential_effect_allowed: false; media_effect_allowed: false; file_effect_allowed: false; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadNoOpBoundaries { all_stubs_no_op_only: boolean; no_runtime_execution: true; no_external_effects: true; no_upload_effects: true; no_network_effects: true; no_credential_effects: true; no_media_effects: true; no_file_effects: true; no_raw_payloads: true; no_raw_responses: true; }
export interface RealUploadCredentialStubBoundary { credential_stub_designed: boolean; credentials_required: false; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; credential_reference_stored: false; token_reference_stored: false; secret_material_stored: false; }
export interface RealUploadMediaStubBoundary { media_stub_designed: boolean; media_file_read: false; raw_media_path_stored: false; media_file_modified: false; media_file_copied: false; media_file_moved: false; media_file_deleted: false; }
export interface RealUploadPlatformApiStubBoundary { platform_api_stub_designed: boolean; platform_adapter_implemented: false; platform_endpoint_selected: false; platform_api_calls_allowed: false; platform_api_calls_made: false; raw_account_ids_stored: false; }
export interface RealUploadPayloadStubBoundary { payload_stub_designed: boolean; platform_api_payload_created: false; platform_api_payload_stored: false; upload_payload_created: false; raw_payload_stored: false; }
export interface RealUploadNetworkStubBoundary { network_stub_designed: boolean; network_client_implemented: false; network_calls_allowed: false; network_calls_made: false; external_side_effects_allowed: false; external_side_effects_observed: false; }
export interface RealUploadRuntimeStubBoundary { runtime_stub_designed: boolean; runtime_enabled: false; runtime_files_created: false; runtime_config_created: false; env_files_created: false; secrets_config_created: false; credential_config_created: false; package_dependency_changes: false; }
export interface RealUploadFileStubBoundary { files_created_now: false; stub_files_created: false; implementation_files_created: false; package_files_changed: false; runtime_files_created: false; file_mutation_allowed: false; }
export interface RealUploadStubOperatorReview { reviewed_by_label?: string | undefined; checklist_acknowledged: boolean; understands_stub_design_only: boolean; understands_no_stub_files_created: boolean; understands_no_implementation_files_created: boolean; understands_no_upload_enabled: boolean; understands_no_credentials_accessed: boolean; understands_no_network_calls: boolean; understands_no_media_reads: boolean; understands_no_dependencies_added: boolean; understands_future_stub_contract_phase_required: boolean; decision_note_summary?: string | undefined; }
export interface RealUploadStubExecutionBoundary { stub_files_created: false; implementation_files_created: false; runtime_files_created: false; dependencies_added: false; package_metadata_changed: false; runtime_enabled: false; upload_allowed: false; upload_execution_enabled: false; platform_api_calls_allowed: false; network_calls_allowed: false; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; media_file_read: false; file_mutation_allowed: false; ready_for_real_upload: false; }
export interface RealUploadScaffoldStubDesignValidationResult { ok: boolean; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadScaffoldStubDesign { schema_version: "1.0"; real_upload_scaffold_stub_design_id: string; real_upload_scaffold_contract_tests_id: string; real_upload_scaffold_contracts_id: string; real_upload_scaffold_design_id: string; real_upload_implementation_design_id: string; final_real_upload_preflight_gate_id: string; real_upload_dry_run_execution_result_id: string; real_upload_execution_plan_id: string; real_upload_strategy_design_id: string; real_upload_execution_request_id: string; real_upload_readiness_assessment_id: string; dry_run_upload_spike_result_id: string; upload_execution_design_id: string; upload_execution_approval_id: string; platform_upload_request_id: string; upload_package_design_id: string; local_output_review_id: string; production_render_spike_result_id: string; final_render_execution_request_id: string; render_plan_id: string; project_id: string; platform: string; stub_design_state: RealUploadScaffoldStubDesignState; created_at: string; stub_design_mode: RealUploadScaffoldStubDesignMode; required_artifacts: RealUploadScaffoldStubRequiredArtifacts; stub_design_scope: RealUploadScaffoldStubDesignScope; proposed_stub_modules: RealUploadProposedStubModule[]; proposed_stub_behaviors: RealUploadProposedStubBehavior[]; no_op_boundaries: RealUploadNoOpBoundaries; credential_stub_boundary: RealUploadCredentialStubBoundary; media_stub_boundary: RealUploadMediaStubBoundary; platform_api_stub_boundary: RealUploadPlatformApiStubBoundary; payload_stub_boundary: RealUploadPayloadStubBoundary; network_stub_boundary: RealUploadNetworkStubBoundary; runtime_stub_boundary: RealUploadRuntimeStubBoundary; file_stub_boundary: RealUploadFileStubBoundary; operator_review: RealUploadStubOperatorReview; execution_boundary: RealUploadStubExecutionBoundary; validation: { scaffold_stub_design_complete: boolean; ready_for_future_stub_contracts: boolean; ready_for_real_upload: false; stub_files_created: false; implementation_files_created: false; runtime_files_created: false; dependencies_added: false; package_metadata_changed: false; runtime_enabled: false; upload_allowed: false; upload_execution_enabled: false; platform_api_calls_allowed: false; network_calls_allowed: false; credentials_accessed: false; token_accessed: false; media_file_read: false; file_mutation_allowed: false; blocking_reasons: string[]; warnings: string[]; }; provenance: { generated_by: "createRealUploadScaffoldStubDesign" | "revokeRealUploadScaffoldStubDesign"; source_real_upload_scaffold_contract_tests_id: string; source_real_upload_scaffold_contracts_id: string; source_real_upload_scaffold_design_id: string; source_real_upload_implementation_design_id: string; source_final_real_upload_preflight_gate_id: string; source_real_upload_dry_run_execution_result_id: string; source_real_upload_execution_plan_id: string; source_real_upload_strategy_design_id: string; source_real_upload_execution_request_id: string; source_real_upload_readiness_assessment_id: string; source_dry_run_upload_spike_result_id: string; source_upload_execution_design_id: string; source_upload_execution_approval_id: string; source_platform_upload_request_id: string; source_upload_package_design_id: string; source_local_output_review_id: string; source_production_render_spike_result_id: string; source_final_render_execution_request_id: string; source_render_plan_id: string; }; }
function safeRealUploadScaffoldStubDesignString(value: unknown, fallback: string): string { return safeRealUploadScaffoldDesignString(value, fallback); }
interface RealUploadScaffoldStubDesignsStore { schema_version: "1.0"; created_at: string; designs: RealUploadScaffoldStubDesign[]; }
interface RealUploadScaffoldStubDesignStoreSummaryItem { stub_design_state: RealUploadScaffoldStubDesignState; created_at: string; stub_modules_total: number; stub_modules_with_blockers: number; safe_summary: string; }
function getRealUploadScaffoldStubDesignsPath(): string { return path.join(getRuntimeDir(), "real-upload-scaffold-stub-designs.json"); }
function loadRealUploadScaffoldStubDesignsStore(): RealUploadScaffoldStubDesignsStore { try { const filePath = getRealUploadScaffoldStubDesignsPath(); if (fs.existsSync(filePath)) return JSON.parse(fs.readFileSync(filePath, "utf8")) as RealUploadScaffoldStubDesignsStore; } catch {} return { schema_version: "1.0", created_at: new Date().toISOString(), designs: [] }; }
function saveRealUploadScaffoldStubDesignsStore(store: RealUploadScaffoldStubDesignsStore): void { const filePath = getRealUploadScaffoldStubDesignsPath(); fs.mkdirSync(path.dirname(filePath), { recursive: true }); store.designs.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_scaffold_stub_design_id.localeCompare(b.real_upload_scaffold_stub_design_id)); fs.writeFileSync(filePath, JSON.stringify(store, null, 2), "utf8"); }
function validateRealUploadScaffoldStubDesignShape(design: unknown): RealUploadScaffoldStubDesignValidationResult { const blocking_reasons: string[] = []; const warnings: string[] = []; const fail = (reason: string) => blocking_reasons.push(reason); const scan = (value: unknown): boolean => { if (typeof value === "string") { const lower = value.toLowerCase(); return lower.includes("videos.insert") || lower.includes("youtube.videos().insert") || lower.includes("fetch(") || lower.includes("bearer ") || lower.includes("access_token") || lower.includes("refresh_token") || lower.includes("client_secret") || lower.includes("keychain://") || lower.includes("/users/") || lower.includes("http://") || lower.includes("https://") || lower.includes("../") || lower.includes("stdout") || lower.includes("stderr") || lower.includes("process.env"); } if (Array.isArray(value)) return value.some(scan); if (value && typeof value === "object") return Object.entries(value as Record<string, unknown>).some(([key, child]) => { const lowerKey = key.toLowerCase(); return lowerKey.includes("access_token") || lowerKey.includes("refresh_token") || lowerKey.includes("client_secret") || lowerKey.includes("code_verifier") || lowerKey.includes("authorization_code") || scan(child); }); return false; }; if (!design || typeof design !== "object" || Array.isArray(design)) return { ok: false, blocking_reasons: ["Invalid scaffold stub design"], warnings }; const d = design as RealUploadScaffoldStubDesign; if (d.schema_version !== "1.0") fail("Invalid schema version"); if (d.stub_design_mode !== "real_upload_scaffold_stub_design_only" && d.stub_design_mode !== "operator_review_real_upload_scaffold_stub_design") fail("Invalid stub design mode"); if (d.stub_design_scope?.stub_design_only !== true || d.stub_design_scope?.stub_files_created !== false || d.stub_design_scope?.implementation_files_created !== false || d.stub_design_scope?.runtime_files_created !== false || d.stub_design_scope?.runtime_enabled !== false || d.stub_design_scope?.current_real_upload_requested !== false || d.stub_design_scope?.current_network_calls_requested !== false || d.stub_design_scope?.current_platform_api_calls_requested !== false || d.stub_design_scope?.current_credential_access_requested !== false || d.stub_design_scope?.current_media_read_requested !== false || d.stub_design_scope?.dependencies_requested !== false || d.stub_design_scope?.package_metadata_changes_requested !== false) fail("Unsafe stub design scope"); const requiredKinds = ["credential_provider_stub","media_reference_resolver_stub","platform_adapter_stub","payload_builder_stub","network_client_stub","upload_executor_stub","retry_idempotency_stub","rollback_stub","post_upload_verification_stub","audit_event_stub","operator_confirmation_stub","emergency_stop_stub","status_reporter_stub","upload_scaffold_index_stub"] as const; if (!Array.isArray(d.proposed_stub_modules) || new Set(d.proposed_stub_modules.map((module) => module.module_kind)).size !== d.proposed_stub_modules.length || requiredKinds.some((kind) => !d.proposed_stub_modules.some((module) => module.module_kind === kind))) fail("Missing or duplicate stub module kinds"); if (Array.isArray(d.proposed_stub_modules) && d.proposed_stub_modules.some((module) => module.stub_file_created !== false || module.implementation_created !== false || module.runtime_enabled !== false || module.no_op_only !== true || module.upload_enabled !== false || module.network_enabled !== false || module.platform_api_enabled !== false || module.credential_access_enabled !== false || module.media_read_enabled !== false || module.file_mutation_enabled !== false || module.raw_payload_created !== false || module.raw_response_stored !== false)) fail("Unsafe proposed stub modules"); if (!Array.isArray(d.proposed_stub_behaviors) || d.proposed_stub_behaviors.some((behavior) => behavior.no_op_behavior !== true || behavior.runtime_executed !== false || behavior.external_effects_allowed !== false || behavior.upload_effect_allowed !== false || behavior.network_effect_allowed !== false || behavior.credential_effect_allowed !== false || behavior.media_effect_allowed !== false || behavior.file_effect_allowed !== false)) fail("Unsafe proposed stub behaviors"); if (!d.required_artifacts || d.required_artifacts.real_upload_scaffold_contract_tests_validated !== true || d.required_artifacts.real_upload_scaffold_contracts_validated !== true || d.required_artifacts.real_upload_scaffold_design_validated !== true || d.required_artifacts.real_upload_implementation_design_validated !== true || d.required_artifacts.final_real_upload_preflight_gate_validated !== true || d.required_artifacts.real_upload_dry_run_execution_result_validated !== true || d.required_artifacts.real_upload_execution_plan_validated !== true || d.required_artifacts.real_upload_strategy_design_validated !== true || d.required_artifacts.real_upload_execution_request_validated !== true || d.required_artifacts.real_upload_readiness_assessment_validated !== true || d.required_artifacts.dry_run_upload_spike_result_validated !== true || d.required_artifacts.upload_execution_design_validated !== true || d.required_artifacts.upload_execution_approval_validated !== true || d.required_artifacts.platform_upload_request_validated !== true || d.required_artifacts.upload_package_design_validated !== true || d.required_artifacts.local_output_review_validated !== true || d.required_artifacts.spike_result_validated !== true) fail("Unsafe required artifacts"); if (!d.no_op_boundaries || d.no_op_boundaries.all_stubs_no_op_only !== true || d.no_op_boundaries.no_runtime_execution !== true || d.no_op_boundaries.no_external_effects !== true || d.no_op_boundaries.no_upload_effects !== true || d.no_op_boundaries.no_network_effects !== true || d.no_op_boundaries.no_credential_effects !== true || d.no_op_boundaries.no_media_effects !== true || d.no_op_boundaries.no_file_effects !== true || d.no_op_boundaries.no_raw_payloads !== true || d.no_op_boundaries.no_raw_responses !== true) fail("Unsafe no-op boundaries"); if (!d.credential_stub_boundary || d.credential_stub_boundary.credentials_required !== false || d.credential_stub_boundary.credentials_accessed !== false || d.credential_stub_boundary.token_accessed !== false || d.credential_stub_boundary.keychain_accessed !== false || d.credential_stub_boundary.env_accessed !== false || d.credential_stub_boundary.credential_reference_stored !== false || d.credential_stub_boundary.token_reference_stored !== false || d.credential_stub_boundary.secret_material_stored !== false) fail("Unsafe credential stub boundary"); if (!d.media_stub_boundary || d.media_stub_boundary.media_file_read !== false || d.media_stub_boundary.raw_media_path_stored !== false || d.media_stub_boundary.media_file_modified !== false || d.media_stub_boundary.media_file_copied !== false || d.media_stub_boundary.media_file_moved !== false || d.media_stub_boundary.media_file_deleted !== false) fail("Unsafe media stub boundary"); if (!d.platform_api_stub_boundary || d.platform_api_stub_boundary.platform_adapter_implemented !== false || d.platform_api_stub_boundary.platform_endpoint_selected !== false || d.platform_api_stub_boundary.platform_api_calls_allowed !== false || d.platform_api_stub_boundary.platform_api_calls_made !== false || d.platform_api_stub_boundary.raw_account_ids_stored !== false) fail("Unsafe platform api stub boundary"); if (!d.payload_stub_boundary || d.payload_stub_boundary.platform_api_payload_created !== false || d.payload_stub_boundary.platform_api_payload_stored !== false || d.payload_stub_boundary.upload_payload_created !== false || d.payload_stub_boundary.raw_payload_stored !== false) fail("Unsafe payload stub boundary"); if (!d.network_stub_boundary || d.network_stub_boundary.network_client_implemented !== false || d.network_stub_boundary.network_calls_allowed !== false || d.network_stub_boundary.network_calls_made !== false || d.network_stub_boundary.external_side_effects_allowed !== false || d.network_stub_boundary.external_side_effects_observed !== false) fail("Unsafe network stub boundary"); if (!d.runtime_stub_boundary || d.runtime_stub_boundary.runtime_enabled !== false || d.runtime_stub_boundary.runtime_files_created !== false || d.runtime_stub_boundary.runtime_config_created !== false || d.runtime_stub_boundary.env_files_created !== false || d.runtime_stub_boundary.secrets_config_created !== false || d.runtime_stub_boundary.credential_config_created !== false || d.runtime_stub_boundary.package_dependency_changes !== false) fail("Unsafe runtime stub boundary"); if (!d.file_stub_boundary || d.file_stub_boundary.files_created_now !== false || d.file_stub_boundary.stub_files_created !== false || d.file_stub_boundary.implementation_files_created !== false || d.file_stub_boundary.package_files_changed !== false || d.file_stub_boundary.runtime_files_created !== false || d.file_stub_boundary.file_mutation_allowed !== false) fail("Unsafe file stub boundary"); if (!d.operator_review || d.operator_review.checklist_acknowledged !== true || d.operator_review.understands_stub_design_only !== true || d.operator_review.understands_no_stub_files_created !== true || d.operator_review.understands_no_implementation_files_created !== true || d.operator_review.understands_no_upload_enabled !== true || d.operator_review.understands_no_credentials_accessed !== true || d.operator_review.understands_no_network_calls !== true || d.operator_review.understands_no_media_reads !== true || d.operator_review.understands_no_dependencies_added !== true || d.operator_review.understands_future_stub_contract_phase_required !== true) fail("Unsafe operator review"); if (!d.execution_boundary || d.execution_boundary.stub_files_created !== false || d.execution_boundary.implementation_files_created !== false || d.execution_boundary.runtime_files_created !== false || d.execution_boundary.dependencies_added !== false || d.execution_boundary.package_metadata_changed !== false || d.execution_boundary.runtime_enabled !== false || d.execution_boundary.upload_allowed !== false || d.execution_boundary.upload_execution_enabled !== false || d.execution_boundary.platform_api_calls_allowed !== false || d.execution_boundary.network_calls_allowed !== false || d.execution_boundary.credentials_accessed !== false || d.execution_boundary.token_accessed !== false || d.execution_boundary.keychain_accessed !== false || d.execution_boundary.env_accessed !== false || d.execution_boundary.media_file_read !== false || d.execution_boundary.file_mutation_allowed !== false || d.execution_boundary.ready_for_real_upload !== false) fail("Unsafe execution boundary"); if (!d.validation || d.validation.ready_for_real_upload !== false || d.validation.stub_files_created !== false || d.validation.implementation_files_created !== false || d.validation.runtime_files_created !== false || d.validation.dependencies_added !== false || d.validation.package_metadata_changed !== false || d.validation.runtime_enabled !== false || d.validation.upload_allowed !== false || d.validation.upload_execution_enabled !== false || d.validation.platform_api_calls_allowed !== false || d.validation.network_calls_allowed !== false || d.validation.credentials_accessed !== false || d.validation.token_accessed !== false || d.validation.media_file_read !== false || d.validation.file_mutation_allowed !== false) fail("Unsafe validation"); if (scan(d)) fail("Unsafe text content"); return { ok: blocking_reasons.length === 0, blocking_reasons, warnings }; }
export function validateRealUploadScaffoldStubDesign(design: unknown): RealUploadScaffoldStubDesignValidationResult { return validateRealUploadScaffoldStubDesignShape(design); }
export function createRealUploadScaffoldStubDesign(input: {
  ...
  localOutputReview: LocalOutputOperatorReview;
  spikeResult: ControlledProductionRenderSpikeResult;
  decision?: "draft" | "approved_for_future_stub_contracts" | "rejected";
  reviewed_by_label?: string;
  checklist_acknowledged?: boolean;
  understands_stub_design_only?: boolean;
  ...
  understands_no_media_reads?: boolean;
  understands_no_dependencies_added?: boolean;
  understands_future_stub_contract_phase_required?: boolean;
  decision_note_summary?: string;
  dryRun: true;
}): RealUploadScaffoldStubDesign {
  if (input.dryRun !== true) throw new Error("createRealUploadScaffoldStubDesign: dryRun=true required");
  if (!validateRealUploadScaffoldContractTests(input.scaffoldContractTests).ok || !validateRealUploadScaffoldContracts(input.scaffoldContracts).ok || !validateRealUploadScaffoldDesign(input.scaffoldDesign).ok || !validateRealUploadImplementationDesign(input.implementationDesign).ok || !validateFinalRealUploadPreflightGate(input.finalPreflightGate).ok || !validateRealUploadDryRunExecutionResult(input.dryRunExecutionResult).ok || !validateRealUploadExecutionPlan(input.executionPlan).ok || !validateRealUploadStrategyDesign(input.strategyDesign).ok || !validateRealUploadExecutionRequest(input.realUploadExecutionRequest).ok || !validateRealUploadReadinessAssessment(input.readinessAssessment).ok || !validateDryRunUploadSpikeResult(input.dryRunUploadSpikeResult).ok || !validateUploadExecutionDesign(input.uploadExecutionDesign).ok || !validateUploadExecutionApproval(input.uploadExecutionApproval).ok || !validatePlatformUploadRequest(input.platformUploadRequest).ok || !validateUploadPackageDesign(input.uploadPackageDesign).ok || !validateLocalOutputOperatorReview(input.localOutputReview).ok || !validateControlledProductionRenderSpikeResult(input.spikeResult).ok) throw new Error("createRealUploadScaffoldStubDesign: all input artifacts must validate");
  if (input.scaffoldContractTests.contract_tests_state !== "approved_for_future_scaffold_stub_design") throw new Error("createRealUploadScaffoldStubDesign: scaffoldContractTests must be approved_for_future_scaffold_stub_design");
  if (input.scaffoldContractTests.validation.ready_for_real_upload !== false || input.scaffoldContractTests.validation.ready_for_future_scaffold_stub_design !== true) throw new Error("createRealUploadScaffoldStubDesign: scaffoldContractTests readiness must remain safe");
  if (input.scaffoldContracts.contracts_state !== "approved_for_future_contract_tests" || input.scaffoldContracts.validation.ready_for_real_upload !== false || input.scaffoldContracts.validation.ready_for_future_contract_tests !== true) throw new Error("createRealUploadScaffoldStubDesign: scaffoldContracts readiness must remain safe");
  if (input.scaffoldDesign.scaffold_design_state !== "approved_for_future_scaffold_contracts" || input.scaffoldDesign.validation.ready_for_real_upload !== false || input.scaffoldDesign.validation.ready_for_future_scaffold_contracts !== true) throw new Error("createRealUploadScaffoldStubDesign: scaffoldDesign readiness must remain safe");
  if (input.implementationDesign.design_state !== "approved_for_future_scaffold_design" || input.implementationDesign.validation.ready_for_real_upload !== false || input.implementationDesign.validation.ready_for_future_scaffold_design !== true) throw new Error("createRealUploadScaffoldStubDesign: implementationDesign readiness must remain safe");
  const approved = input.decision === "approved_for_future_stub_contracts";
  if (approved && (!input.checklist_acknowledged || !input.understands_stub_design_only || !input.understands_no_stub_files_created || !input.understands_no_implementation_files_created || !input.understands_no_upload_enabled || !input.understands_no_credentials_accessed || !input.understands_no_network_calls || !input.understands_no_media_reads || !input.understands_no_dependencies_added || !input.understands_future_stub_contract_phase_required)) throw new Error("createRealUploadScaffoldStubDesign: acknowledgements required");
  const kinds = ["credential_provider_stub","media_reference_resolver_stub","platform_adapter_stub","payload_builder_stub","network_client_stub","upload_executor_stub","retry_idempotency_stub","rollback_stub","post_upload_verification_stub","audit_event_stub","operator_confirmation_stub","emergency_stop_stub","status_reporter_stub","upload_scaffold_index_stub"] as const;
  const design: RealUploadScaffoldStubDesign = {
    schema_version: "1.0",
    real_upload_scaffold_stub_design_id: crypto.randomUUID(),
    ...
    final_render_execution_request_id: input.spikeResult.final_render_execution_request_id,
    render_plan_id: input.spikeResult.render_plan_id,
    project_id: input.scaffoldContractTests.project_id,
    platform: input.scaffoldContractTests.platform,
    stub_design_state: approved ? "approved_for_future_stub_contracts" : input.decision === "rejected" ? "rejected" : "draft",
    created_at: new Date().toISOString(),
    stub_design_mode: approved ? "operator_review_real_upload_scaffold_stub_design" : "real_upload_scaffold_stub_design_only",
    required_artifacts: { real_upload_scaffold_contract_tests_validated: true, real_upload_scaffold_contracts_validated: true, real_upload_scaffold_design_validated: true, real_upload_implementation_design_validated: true, final_real_upload_preflight_gate_validated: true, real_upload_dry_run_execution_result_validated: true, real_upload_execution_plan_validated: true, real_upload_strategy_design_validated: true, real_upload_execution_request_validated: true, real_upload_readiness_assessment_validated: true, dry_run_upload_spike_result_validated: true, upload_execution_design_validated: true, upload_execution_approval_validated: true, platform_upload_request_validated: true, upload_package_design_validated: true, local_output_review_validated: true, spike_result_validated: true },
    stub_design_scope: { future_stub_contracts_requested: true, stub_design_only: true, stub_files_created: false, implementation_files_created: false, runtime_files_created: false, runtime_enabled: false, current_real_upload_requested: false, current_network_calls_requested: false, current_platform_api_calls_requested: false, current_credential_access_requested: false, current_media_read_requested: false, dependencies_requested: false, package_metadata_changes_requested: false },
    proposed_stub_modules: kinds.map((kind, index) => ({ stub_module_id: `stub-${index + 1}`, module_kind: kind, stub_state: kind === "operator_confirmation_stub" ? "blocked" : "proposed", safe_summary: `Future ${kind.replaceAll("_", " ")} stub design only.`, stub_file_created: false, implementation_created: false, runtime_enabled: false, no_op_only: true, upload_enabled: false, network_enabled: false, platform_api_enabled: false, credential_access_enabled: false, media_read_enabled: false, file_mutation_enabled: false, raw_payload_created: false, raw_response_stored: false, blocking_reasons: [kind === "credential_provider_stub" ? "Credential access remains disabled." : kind === "media_reference_resolver_stub" ? "Media reads remain disabled." : kind === "platform_adapter_stub" ? "Platform APIs remain disabled." : kind === "payload_builder_stub" ? "Payload creation remains disabled." : kind === "network_client_stub" ? "Network calls remain disabled." : kind === "upload_executor_stub" ? "Upload execution remains disabled." : kind === "retry_idempotency_stub" ? "Retry execution remains disabled." : kind === "rollback_stub" ? "Rollback execution remains disabled." : kind === "post_upload_verification_stub" ? "Verification execution remains disabled." : kind === "audit_event_stub" ? "Raw logging remains disabled." : kind === "operator_confirmation_stub" ? "Operator approval remains required." : kind === "emergency_stop_stub" ? "Emergency stop is stub-only." : kind === "status_reporter_stub" ? "Status reporting remains local-only." : "Stub index remains contract-only."], warnings: [] })),
    proposed_stub_behaviors: [
      { behavior_id: "behavior-1", behavior_kind: "credential_provider_stub", behavior_state: "proposed", safe_summary: "Credential provider stub remains no-op.", no_op_behavior: true, runtime_executed: false, external_effects_allowed: false, upload_effect_allowed: false, network_effect_allowed: false, credential_effect_allowed: false, media_effect_allowed: false, file_effect_allowed: false, blocking_reasons: ["Credential effects remain disabled."], warnings: [] },
      { behavior_id: "behavior-2", behavior_kind: "network_client_stub", behavior_state: "proposed", safe_summary: "Network client stub remains no-op.", no_op_behavior: true, runtime_executed: false, external_effects_allowed: false, upload_effect_allowed: false, network_effect_allowed: false, credential_effect_allowed: false, media_effect_allowed: false, file_effect_allowed: false, blocking_reasons: ["Network effects remain disabled."], warnings: [] },
      { behavior_id: "behavior-3", behavior_kind: "upload_executor_stub", behavior_state: "proposed", safe_summary: "Upload executor stub remains no-op.", no_op_behavior: true, runtime_executed: false, external_effects_allowed: false, upload_effect_allowed: false, network_effect_allowed: false, credential_effect_allowed: false, media_effect_allowed: false, file_effect_allowed: false, blocking_reasons: ["Upload effects remain disabled."], warnings: [] },
      { behavior_id: "behavior-4", behavior_kind: "audit_event_stub", behavior_state: "proposed", safe_summary: "Audit event stub remains no-op.", no_op_behavior: true, runtime_executed: false, external_effects_allowed: false, upload_effect_allowed: false, network_effect_allowed: false, credential_effect_allowed: false, media_effect_allowed: false, file_effect_allowed: false, blocking_reasons: ["Raw logging remains disabled."], warnings: [] }
    ],
    no_op_boundaries: { all_stubs_no_op_only: true, no_runtime_execution: true, no_external_effects: true, no_upload_effects: true, no_network_effects: true, no_credential_effects: true, no_media_effects: true, no_file_effects: true, no_raw_payloads: true, no_raw_responses: true },
    credential_stub_boundary: { credential_stub_designed: true, credentials_required: false, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, credential_reference_stored: false, token_reference_stored: false, secret_material_stored: false },
    media_stub_boundary: { media_stub_designed: true, media_file_read: false, raw_media_path_stored: false, media_file_modified: false, media_file_copied: false, media_file_moved: false, media_file_deleted: false },
    platform_api_stub_boundary: { platform_api_stub_designed: true, platform_adapter_implemented: false, platform_endpoint_selected: false, platform_api_calls_allowed: false, platform_api_calls_made: false, raw_account_ids_stored: false },
    payload_stub_boundary: { payload_stub_designed: true, platform_api_payload_created: false, platform_api_payload_stored: false, upload_payload_created: false, raw_payload_stored: false },
    network_stub_boundary: { network_stub_designed: true, network_client_implemented: false, network_calls_allowed: false, network_calls_made: false, external_side_effects_allowed: false, external_side_effects_observed: false },
    runtime_stub_boundary: { runtime_stub_designed: true, runtime_enabled: false, runtime_files_created: false, runtime_config_created: false, env_files_created: false, secrets_config_created: false, credential_config_created: false, package_dependency_changes: false },
    file_stub_boundary: { files_created_now: false, stub_files_created: false, implementation_files_created: false, package_files_changed: false, runtime_files_created: false, file_mutation_allowed: false },
    operator_review: { reviewed_by_label: input.reviewed_by_label || "operator-001", checklist_acknowledged: !!input.checklist_acknowledged, understands_stub_design_only: !!input.understands_stub_design_only, understands_no_stub_files_created: !!input.understands_no_stub_files_created, understands_no_implementation_files_created: !!input.understands_no_implementation_files_created, understands_no_upload_enabled: !!input.understands_no_upload_enabled, understands_no_credentials_accessed: !!input.understands_no_credentials_accessed, understands_no_network_calls: !!input.understands_no_network_calls, understands_no_media_reads: !!input.understands_no_media_reads, understands_no_dependencies_added: !!input.understands_no_dependencies_added, understands_future_stub_contract_phase_required: !!input.understands_future_stub_contract_phase_required, decision_note_summary: safeRealUploadScaffoldStubDesignString(input.decision_note_summary, "[stub-design]") },
    execution_boundary: { stub_files_created: false, implementation_files_created: false, runtime_files_created: false, dependencies_added: false, package_metadata_changed: false, runtime_enabled: false, upload_allowed: false, upload_execution_enabled: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, keychain_accessed: false, env_accessed: false, media_file_read: false, file_mutation_allowed: false, ready_for_real_upload: false },
    validation: { scaffold_stub_design_complete: true, ready_for_future_stub_contracts: approved, ready_for_real_upload: false, stub_files_created: false, implementation_files_created: false, runtime_files_created: false, dependencies_added: false, package_metadata_changed: false, runtime_enabled: false, upload_allowed: false, upload_execution_enabled: false, platform_api_calls_allowed: false, network_calls_allowed: false, credentials_accessed: false, token_accessed: false, media_file_read: false, file_mutation_allowed: false, blocking_reasons: [], warnings: [] },
    provenance: { generated_by: "createRealUploadScaffoldStubDesign", source_real_upload_scaffold_contract_tests_id: input.scaffoldContractTests.real_upload_scaffold_contract_tests_id, source_real_upload_scaffold_contracts_id: input.scaffoldContracts.real_upload_scaffold_contracts_id, source_real_upload_scaffold_design_id: input.scaffoldDesign.real_upload_scaffold_design_id, source_real_upload_implementation_design_id: input.implementationDesign.real_upload_implementation_design_id, source_final_real_upload_preflight_gate_id: input.finalPreflightGate.final_real_upload_preflight_gate_id, source_real_upload_dry_run_execution_result_id: input.dryRunExecutionResult.real_upload_dry_run_execution_result_id, source_real_upload_execution_plan_id: input.executionPlan.real_upload_execution_plan_id, source_real_upload_strategy_design_id: input.strategyDesign.real_upload_strategy_design_id, source_real_upload_execution_request_id: input.realUploadExecutionRequest.real_upload_execution_request_id, source_real_upload_readiness_assessment_id: input.readinessAssessment.real_upload_readiness_assessment_id, source_dry_run_upload_spike_result_id: input.dryRunUploadSpikeResult.dry_run_upload_spike_result_id, source_upload_execution_design_id: input.uploadExecutionDesign.upload_execution_design_id, source_upload_execution_approval_id: input.uploadExecutionApproval.upload_execution_approval_id, source_platform_upload_request_id: input.platformUploadRequest.platform_upload_request_id, source_upload_package_design_id: input.uploadPackageDesign.upload_package_design_id, source_local_output_review_id: input.localOutputReview.local_output_review_id, source_production_render_spike_result_id: input.spikeResult.production_render_spike_result_id, source_final_render_execution_request_id: input.spikeResult.final_render_execution_request_id, source_render_plan_id: input.spikeResult.render_plan_id }
  };
  const validation = validateRealUploadScaffoldStubDesign(design);
  if (!validation.ok) throw new Error(`Unsafe real upload scaffold stub design cannot be created: ${validation.blocking_reasons.join("; ")}`);
  return design;
}
export function saveRealUploadScaffoldStubDesign(design: RealUploadScaffoldStubDesign): void { const validation = validateRealUploadScaffoldStubDesign(design); if (!validation.ok) throw new Error(`Unsafe real upload scaffold stub design cannot be stored: ${validation.blocking_reasons.join("; ")}`); const store = loadRealUploadScaffoldStubDesignsStore(); const index = store.designs.findIndex((item) => item.real_upload_scaffold_stub_design_id === design.real_upload_scaffold_stub_design_id); if (index >= 0) store.designs[index] = design; else store.designs.push(design); saveRealUploadScaffoldStubDesignsStore(store); }
export function listRealUploadScaffoldStubDesigns(options?: { project_id?: string; platform?: string; stub_design_state?: string; real_upload_scaffold_contract_tests_id?: string; real_upload_scaffold_contracts_id?: string }): RealUploadScaffoldStubDesign[] { return loadRealUploadScaffoldStubDesignsStore().designs.filter((item) => (!options?.project_id || item.project_id === options.project_id) && (!options?.platform || item.platform === options.platform) && (!options?.stub_design_state || item.stub_design_state === options.stub_design_state) && (!options?.real_upload_scaffold_contract_tests_id || item.real_upload_scaffold_contract_tests_id === options.real_upload_scaffold_contract_tests_id) && (!options?.real_upload_scaffold_contracts_id || item.real_upload_scaffold_contracts_id === options.real_upload_scaffold_contracts_id)).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_scaffold_stub_design_id.localeCompare(b.real_upload_scaffold_stub_design_id)); }
export function getRealUploadScaffoldStubDesign(real_upload_scaffold_stub_design_id: string): RealUploadScaffoldStubDesign | null { return loadRealUploadScaffoldStubDesignsStore().designs.find((item) => item.real_upload_scaffold_stub_design_id === real_upload_scaffold_stub_design_id) ?? null; }
export function revokeRealUploadScaffoldStubDesign(real_upload_scaffold_stub_design_id: string, reason: string): RealUploadScaffoldStubDesign { const design = getRealUploadScaffoldStubDesign(real_upload_scaffold_stub_design_id); if (!design) throw new Error("Real upload scaffold stub design not found"); const safeReason = safeRealUploadScaffoldStubDesignString(reason, "[unsafe-reason]"); if (safeReason === "[unsafe-reason]") throw new Error("Unsafe real upload scaffold stub design reason cannot be stored."); const revoked: RealUploadScaffoldStubDesign = { ...design, stub_design_state: "revoked", validation: { ...design.validation, blocking_reasons: [...design.validation.blocking_reasons, safeReason], warnings: [...design.validation.warnings, "Real upload scaffold stub design revoked."] }, provenance: { ...design.provenance, generated_by: "revokeRealUploadScaffoldStubDesign" } }; saveRealUploadScaffoldStubDesign(revoked); return revoked; }
export function getRealUploadScaffoldStubDesignReport(options?: { project_id?: string; platform?: string }): { total: number; by_state: Record<string, number>; blocked: number; ready_for_operator_review: number; approved_for_future_stub_contracts: number; rejected: number; revoked: number; scaffold_stub_design_complete: number; ready_for_future_stub_contracts: number; ready_for_real_upload: 0; stub_files_created: 0; implementation_files_created: 0; runtime_files_created: 0; dependencies_added: 0; package_metadata_changed: 0; runtime_enabled: 0; upload_allowed: 0; upload_execution_enabled: 0; platform_api_calls_allowed: 0; network_calls_allowed: 0; credentials_accessed: 0; token_accessed: 0; media_file_read: 0; file_mutation_allowed: 0; stub_modules_total: number; stub_modules_with_blockers: number; designs: Array<RealUploadScaffoldStubDesignStoreSummaryItem>; } { const designs = listRealUploadScaffoldStubDesigns(options); const by_state: Record<string, number> = {}; let blocked = 0, ready = 0, approved = 0, rejected = 0, revoked = 0, complete = 0, readyFuture = 0, stubModulesTotal = 0, stubModulesBlocked = 0; const summaries = designs.map((design) => { by_state[design.stub_design_state] = (by_state[design.stub_design_state] ?? 0) + 1; if (design.stub_design_state === "blocked") blocked++; if (design.stub_design_state === "ready_for_operator_review") ready++; if (design.stub_design_state === "approved_for_future_stub_contracts") approved++; if (design.stub_design_state === "rejected") rejected++; if (design.stub_design_state === "revoked") revoked++; if (design.validation.scaffold_stub_design_complete) complete++; if (design.validation.ready_for_future_stub_contracts) readyFuture++; stubModulesTotal += design.proposed_stub_modules.length; stubModulesBlocked += design.proposed_stub_modules.filter((module) => module.blocking_reasons.length > 0).length; return { stub_design_state: design.stub_design_state, created_at: design.created_at, stub_modules_total: design.proposed_stub_modules.length, stub_modules_with_blockers: design.proposed_stub_modules.filter((module) => module.blocking_reasons.length > 0).length, safe_summary: safeRealUploadScaffoldStubDesignString(design.validation.blocking_reasons[0] || design.proposed_stub_modules[0]?.safe_summary || "Stub design summary.", "Stub design summary.") }; }); return { total: summaries.length, by_state, blocked, ready_for_operator_review: ready, approved_for_future_stub_contracts: approved, rejected, revoked, scaffold_stub_design_complete: complete, ready_for_future_stub_contracts: readyFuture, ready_for_real_upload: 0, stub_files_created: 0, implementation_files_created: 0, runtime_files_created: 0, dependencies_added: 0, package_metadata_changed: 0, runtime_enabled: 0, upload_allowed: 0, upload_execution_enabled: 0, platform_api_calls_allowed: 0, network_calls_allowed: 0, credentials_accessed: 0, token_accessed: 0, media_file_read: 0, file_mutation_allowed: 0, stub_modules_total: stubModulesTotal, stub_modules_with_blockers: stubModulesBlocked, designs: summaries }; }



// ─── VO-7S: Real Upload Stub Contracts ────────────────────────────────────

export type RealUploadStubContractsMode = "real_upload_stub_contracts_only" | "operator_review_real_upload_stub_contracts";
export type RealUploadStubContractsState = "draft" | "blocked" | "ready_for_operator_review" | "approved_for_future_stub_contract_tests" | "rejected" | "revoked";
export interface RealUploadStubContractsRequiredArtifacts { real_upload_scaffold_stub_design_validated: true; real_upload_scaffold_contract_tests_validated: true; real_upload_scaffold_contracts_validated: true; real_upload_scaffold_design_validated: true; real_upload_implementation_design_validated: true; final_real_upload_preflight_gate_validated: true; real_upload_dry_run_execution_result_validated: true; real_upload_execution_plan_validated: true; real_upload_strategy_design_validated: true; real_upload_execution_request_validated: true; real_upload_readiness_assessment_validated: true; dry_run_upload_spike_result_validated: true; upload_execution_design_validated: true; upload_execution_approval_validated: true; platform_upload_request_validated: true; upload_package_design_validated: true; local_output_review_validated: true; spike_result_validated: true; }
export interface RealUploadStubContractsScope { future_stub_contract_tests_requested: boolean; stub_contracts_only: true; stub_files_created: false; implementation_files_created: false; runtime_files_created: false; runtime_enabled: false; current_real_upload_requested: false; current_network_calls_requested: false; current_platform_api_calls_requested: false; current_credential_access_requested: false; current_media_read_requested: false; dependencies_requested: false; package_metadata_changes_requested: false; }
export interface RealUploadStubModuleContract { stub_contract_id: string; stub_kind: "credential_provider_stub_contract" | "media_reference_resolver_stub_contract" | "platform_adapter_stub_contract" | "payload_builder_stub_contract" | "network_client_stub_contract" | "upload_executor_stub_contract" | "retry_idempotency_stub_contract" | "rollback_stub_contract" | "post_upload_verification_stub_contract" | "audit_event_stub_contract" | "operator_confirmation_stub_contract" | "emergency_stop_stub_contract" | "status_reporter_stub_contract" | "upload_scaffold_index_stub_contract"; contract_state: "proposed" | "blocked" | "deferred" | "ready_for_future_stub_contract_tests"; safe_summary: string; type_shape_defined: boolean; no_op_contract: true; stub_file_created: false; implementation_created: false; runtime_enabled: false; upload_enabled: false; network_enabled: false; platform_api_enabled: false; credential_access_enabled: false; media_read_enabled: false; file_mutation_enabled: false; raw_payload_created: false; raw_response_stored: false; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadStubBehaviorContract { behavior_contract_id: string; behavior_kind: string; contract_state: "proposed" | "blocked" | "deferred" | "ready_for_future_stub_contract_tests"; safe_summary: string; no_op_behavior_contract: true; runtime_execution_allowed: false; external_effects_allowed: false; upload_effect_allowed: false; network_effect_allowed: false; credential_effect_allowed: false; media_effect_allowed: false; file_effect_allowed: false; raw_payload_allowed: false; raw_response_allowed: false; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadNoOpContractBoundary { all_stub_contracts_no_op_only: boolean; no_runtime_execution: true; no_external_effects: true; no_upload_effects: true; no_network_effects: true; no_credential_effects: true; no_media_effects: true; no_file_effects: true; no_raw_payloads: true; no_raw_responses: true; }
export interface RealUploadCredentialStubContractBoundary { credential_stub_contract_defined: boolean; credentials_required: false; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; credential_reference_stored: false; token_reference_stored: false; secret_material_stored: false; }
export interface RealUploadMediaStubContractBoundary { media_stub_contract_defined: boolean; media_file_read: false; raw_media_path_stored: false; media_file_modified: false; media_file_copied: false; media_file_moved: false; media_file_deleted: false; }
export interface RealUploadPlatformApiStubContractBoundary { platform_api_stub_contract_defined: boolean; platform_adapter_implemented: false; platform_endpoint_selected: false; platform_api_calls_allowed: false; platform_api_calls_made: false; raw_account_ids_stored: false; }
export interface RealUploadPayloadStubContractBoundary { payload_stub_contract_defined: boolean; platform_api_payload_created: false; platform_api_payload_stored: false; upload_payload_created: false; raw_payload_stored: false; }
export interface RealUploadNetworkStubContractBoundary { network_stub_contract_defined: boolean; network_client_implemented: false; network_calls_allowed: false; network_calls_made: false; external_side_effects_allowed: false; external_side_effects_observed: false; }
export interface RealUploadRuntimeStubContractBoundary { runtime_stub_contract_defined: boolean; runtime_enabled: false; runtime_files_created: false; runtime_config_created: false; env_files_created: false; secrets_config_created: false; credential_config_created: false; package_dependency_changes: false; }
export interface RealUploadFileStubContractBoundary { files_created_now: false; stub_files_created: false; implementation_files_created: false; package_files_changed: false; runtime_files_created: false; file_mutation_allowed: false; }
export interface RealUploadStubContractsOperatorReview { reviewed_by_label?: string | undefined; checklist_acknowledged: boolean; understands_stub_contracts_only: boolean; understands_no_stub_files_created: boolean; understands_no_implementation_files_created: boolean; understands_no_upload_enabled: boolean; understands_no_credentials_accessed: boolean; understands_no_network_calls: boolean; understands_no_media_reads: boolean; understands_no_dependencies_added: boolean; understands_future_stub_contract_tests_phase_required: boolean; decision_note_summary?: string | undefined; }
export interface RealUploadStubContractsExecutionBoundary { stub_files_created: false; implementation_files_created: false; runtime_files_created: false; dependencies_added: false; package_metadata_changed: false; runtime_enabled: false; upload_allowed: false; upload_execution_enabled: false; platform_api_calls_allowed: false; network_calls_allowed: false; credentials_accessed: false; token_accessed: false; keychain_accessed: false; env_accessed: false; media_file_read: false; file_mutation_allowed: false; ready_for_real_upload: false; }
export interface RealUploadStubContractsValidationResult { ok: boolean; blocking_reasons: string[]; warnings: string[]; }
export interface RealUploadStubContracts { schema_version: "1.0"; real_upload_stub_contracts_id: string; real_upload_scaffold_stub_design_id: string; real_upload_scaffold_contract_tests_id: string; real_upload_scaffold_contracts_id: string; real_upload_scaffold_design_id: string; real_upload_implementation_design_id: string; final_real_upload_preflight_gate_id: string; real_upload_dry_run_execution_result_id: string; real_upload_execution_plan_id: string; real_upload_strategy_design_id: string; real_upload_execution_request_id: string; real_upload_readiness_assessment_id: string; dry_run_upload_spike_result_id: string; upload_execution_design_id: string; upload_execution_approval_id: string; platform_upload_request_id: string; upload_package_design_id: string; local_output_review_id: string; production_render_spike_result_id: string; final_render_execution_request_id: string; render_plan_id: string; project_id: string; platform: string; stub_contracts_state: RealUploadStubContractsState; created_at: string; stub_contracts_mode: RealUploadStubContractsMode; required_artifacts: RealUploadStubContractsRequiredArtifacts; stub_contracts_scope: RealUploadStubContractsScope; stub_module_contracts: RealUploadStubModuleContract[]; stub_behavior_contracts: RealUploadStubBehaviorContract[]; no_op_contract_boundary: RealUploadNoOpContractBoundary; credential_stub_contract_boundary: RealUploadCredentialStubContractBoundary; media_stub_contract_boundary: RealUploadMediaStubContractBoundary; platform_api_stub_contract_boundary: RealUploadPlatformApiStubContractBoundary; payload_stub_contract_boundary: RealUploadPayloadStubContractBoundary; network_stub_contract_boundary: RealUploadNetworkStubContractBoundary; runtime_stub_contract_boundary: RealUploadRuntimeStubContractBoundary; file_stub_contract_boundary: RealUploadFileStubContractBoundary; operator_review: RealUploadStubContractsOperatorReview; execution_boundary: RealUploadStubContractsExecutionBoundary; validation: { stub_contracts_complete: boolean; ready_for_future_stub_contract_tests: boolean; ready_for_real_upload: false; stub_files_created: false; implementation_files_created: false; runtime_files_created: false; dependencies_added: false; package_metadata_changed: false; runtime_enabled: false; upload_allowed: false; upload_execution_enabled: false; platform_api_calls_allowed: false; network_calls_allowed: false; credentials_accessed: false; token_accessed: false; media_file_read: false; file_mutation_allowed: false; blocking_reasons: string[]; warnings: string[]; }; provenance: { generated_by: "createRealUploadStubContracts" | "revokeRealUploadStubContracts"; source_real_upload_scaffold_stub_design_id: string; source_real_upload_scaffold_contract_tests_id: string; source_real_upload_scaffold_contracts_id: string; source_real_upload_scaffold_design_id: string; source_real_upload_implementation_design_id: string; source_final_real_upload_preflight_gate_id: string; source_real_upload_dry_run_execution_result_id: string; source_real_upload_execution_plan_id: string; source_real_upload_strategy_design_id: string; source_real_upload_execution_request_id: string; source_real_upload_readiness_assessment_id: string; source_dry_run_upload_spike_result_id: string; source_upload_execution_design_id: string; source_upload_execution_approval_id: string; source_platform_upload_request_id: string; source_upload_package_design_id: string; source_local_output_review_id: string; source_production_render_spike_result_id: string; source_final_render_execution_request_id: string; source_render_plan_id: string; }; }
function safeRealUploadStubContractsString(value: unknown, fallback: string): string { return safeRealUploadScaffoldStubDesignString(value, fallback); }
interface RealUploadStubContractsStore { schema_version: "1.0"; created_at: string; contracts: RealUploadStubContracts[]; }
interface RealUploadStubContractsStoreSummaryItem { stub_contracts_state: RealUploadStubContractsState; created_at: string; stub_contracts_total: number; stub_contracts_with_blockers: number; safe_summary: string; }
function getRealUploadStubContractsPath(): string { return path.join(getRuntimeDir(), "real-upload-stub-contracts.json"); }
function loadRealUploadStubContractsStore(): RealUploadStubContractsStore { try { const filePath = getRealUploadStubContractsPath(); if (fs.existsSync(filePath)) return JSON.parse(fs.readFileSync(filePath, "utf8")) as RealUploadStubContractsStore; } catch {} return { schema_version: "1.0", created_at: new Date().toISOString(), contracts: [] }; }
function saveRealUploadStubContractsStore(store: RealUploadStubContractsStore): void { const filePath = getRealUploadStubContractsPath(); fs.mkdirSync(path.dirname(filePath), { recursive: true }); store.contracts.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_stub_contracts_id.localeCompare(b.real_upload_stub_contracts_id)); fs.writeFileSync(filePath, JSON.stringify(store, null, 2), "utf8"); }
function validateRealUploadStubContractsShape(contracts: unknown): RealUploadStubContractsValidationResult {
  const blocking_reasons: string[] = [];
  const warnings: string[] = [];
  const fail = (reason: string) => blocking_reasons.push(reason);

  if (!contracts || typeof contracts !== "object" || Array.isArray(contracts)) {
    return { ok: false, blocking_reasons: ["Invalid stub contracts"], warnings };
  }

  const d = contracts as RealUploadStubContracts;
  if (d.schema_version !== "1.0") fail("Invalid schema version");
  if (d.stub_contracts_mode !== "real_upload_stub_contracts_only" && d.stub_contracts_mode !== "operator_review_real_upload_stub_contracts") fail("Invalid stub contracts mode");
  if (d.stub_contracts_scope?.stub_contracts_only !== true || d.stub_contracts_scope?.stub_files_created !== false || d.stub_contracts_scope?.implementation_files_created !== false || d.stub_contracts_scope?.runtime_files_created !== false || d.stub_contracts_scope?.runtime_enabled !== false || d.stub_contracts_scope?.current_real_upload_requested !== false || d.stub_contracts_scope?.current_network_calls_requested !== false || d.stub_contracts_scope?.current_platform_api_calls_requested !== false || d.stub_contracts_scope?.current_credential_access_requested !== false || d.stub_contracts_scope?.current_media_read_requested !== false || d.stub_contracts_scope?.dependencies_requested !== false || d.stub_contracts_scope?.package_metadata_changes_requested !== false) fail("Unsafe stub contracts scope");

  const requiredKinds = [
    "credential_provider_stub_contract",
    "media_reference_resolver_stub_contract",
    "platform_adapter_stub_contract",
    "payload_builder_stub_contract",
    "network_client_stub_contract",
    "upload_executor_stub_contract",
    "retry_idempotency_stub_contract",
    "rollback_stub_contract",
    "post_upload_verification_stub_contract",
    "audit_event_stub_contract",
    "operator_confirmation_stub_contract",
    "emergency_stop_stub_contract",
    "status_reporter_stub_contract",
    "upload_scaffold_index_stub_contract",
  ] as const;

  if (!Array.isArray(d.stub_module_contracts) || new Set(d.stub_module_contracts.map((module) => module.stub_kind)).size !== d.stub_module_contracts.length || requiredKinds.some((kind) => !d.stub_module_contracts.some((module) => module.stub_kind === kind))) fail("Missing or duplicate stub contract kinds");
  if (Array.isArray(d.stub_module_contracts) && d.stub_module_contracts.some((module) => module.stub_file_created !== false || module.implementation_created !== false || module.runtime_enabled !== false || module.no_op_contract !== true || module.upload_enabled !== false || module.network_enabled !== false || module.platform_api_enabled !== false || module.credential_access_enabled !== false || module.media_read_enabled !== false || module.file_mutation_enabled !== false || module.raw_payload_created !== false || module.raw_response_stored !== false)) fail("Unsafe stub module contracts");
  if (!Array.isArray(d.stub_behavior_contracts) || d.stub_behavior_contracts.some((behavior) => behavior.no_op_behavior_contract !== true || behavior.runtime_execution_allowed !== false || behavior.external_effects_allowed !== false || behavior.upload_effect_allowed !== false || behavior.network_effect_allowed !== false || behavior.credential_effect_allowed !== false || behavior.media_effect_allowed !== false || behavior.file_effect_allowed !== false || behavior.raw_payload_allowed !== false || behavior.raw_response_allowed !== false)) fail("Unsafe stub behavior contracts");
  if (d.no_op_contract_boundary?.no_runtime_execution !== true || d.no_op_contract_boundary?.no_external_effects !== true || d.no_op_contract_boundary?.no_upload_effects !== true || d.no_op_contract_boundary?.no_network_effects !== true || d.no_op_contract_boundary?.no_credential_effects !== true || d.no_op_contract_boundary?.no_media_effects !== true || d.no_op_contract_boundary?.no_file_effects !== true || d.no_op_contract_boundary?.no_raw_payloads !== true || d.no_op_contract_boundary?.no_raw_responses !== true) fail("Unsafe no-op contract boundary");
  if (d.credential_stub_contract_boundary?.credentials_required !== false || d.credential_stub_contract_boundary?.credentials_accessed !== false || d.credential_stub_contract_boundary?.token_accessed !== false || d.credential_stub_contract_boundary?.keychain_accessed !== false || d.credential_stub_contract_boundary?.env_accessed !== false || d.credential_stub_contract_boundary?.credential_reference_stored !== false || d.credential_stub_contract_boundary?.token_reference_stored !== false || d.credential_stub_contract_boundary?.secret_material_stored !== false) fail("Unsafe credential boundary");
  if (d.media_stub_contract_boundary?.media_file_read !== false || d.media_stub_contract_boundary?.raw_media_path_stored !== false || d.media_stub_contract_boundary?.media_file_modified !== false || d.media_stub_contract_boundary?.media_file_copied !== false || d.media_stub_contract_boundary?.media_file_moved !== false || d.media_stub_contract_boundary?.media_file_deleted !== false) fail("Unsafe media boundary");
  if (d.platform_api_stub_contract_boundary?.platform_adapter_implemented !== false || d.platform_api_stub_contract_boundary?.platform_endpoint_selected !== false || d.platform_api_stub_contract_boundary?.platform_api_calls_allowed !== false || d.platform_api_stub_contract_boundary?.platform_api_calls_made !== false || d.platform_api_stub_contract_boundary?.raw_account_ids_stored !== false) fail("Unsafe platform boundary");
  if (d.payload_stub_contract_boundary?.platform_api_payload_created !== false || d.payload_stub_contract_boundary?.platform_api_payload_stored !== false || d.payload_stub_contract_boundary?.upload_payload_created !== false || d.payload_stub_contract_boundary?.raw_payload_stored !== false) fail("Unsafe payload boundary");
  if (d.network_stub_contract_boundary?.network_client_implemented !== false || d.network_stub_contract_boundary?.network_calls_allowed !== false || d.network_stub_contract_boundary?.network_calls_made !== false || d.network_stub_contract_boundary?.external_side_effects_allowed !== false || d.network_stub_contract_boundary?.external_side_effects_observed !== false) fail("Unsafe network boundary");
  if (d.runtime_stub_contract_boundary?.runtime_enabled !== false || d.runtime_stub_contract_boundary?.runtime_files_created !== false || d.runtime_stub_contract_boundary?.runtime_config_created !== false || d.runtime_stub_contract_boundary?.env_files_created !== false || d.runtime_stub_contract_boundary?.secrets_config_created !== false || d.runtime_stub_contract_boundary?.credential_config_created !== false || d.runtime_stub_contract_boundary?.package_dependency_changes !== false) fail("Unsafe runtime boundary");
  if (d.file_stub_contract_boundary?.files_created_now !== false || d.file_stub_contract_boundary?.stub_files_created !== false || d.file_stub_contract_boundary?.implementation_files_created !== false || d.file_stub_contract_boundary?.package_files_changed !== false || d.file_stub_contract_boundary?.runtime_files_created !== false || d.file_stub_contract_boundary?.file_mutation_allowed !== false) fail("Unsafe file boundary");
  if (d.execution_boundary?.stub_files_created !== false || d.execution_boundary?.implementation_files_created !== false || d.execution_boundary?.runtime_files_created !== false || d.execution_boundary?.dependencies_added !== false || d.execution_boundary?.package_metadata_changed !== false || d.execution_boundary?.runtime_enabled !== false || d.execution_boundary?.upload_allowed !== false || d.execution_boundary?.upload_execution_enabled !== false || d.execution_boundary?.platform_api_calls_allowed !== false || d.execution_boundary?.network_calls_allowed !== false || d.execution_boundary?.credentials_accessed !== false || d.execution_boundary?.token_accessed !== false || d.execution_boundary?.keychain_accessed !== false || d.execution_boundary?.env_accessed !== false || d.execution_boundary?.media_file_read !== false || d.execution_boundary?.file_mutation_allowed !== false || d.execution_boundary?.ready_for_real_upload !== false) fail("Unsafe execution boundary");
  if (d.validation?.ready_for_real_upload !== false || d.validation?.stub_files_created !== false || d.validation?.implementation_files_created !== false || d.validation?.runtime_files_created !== false || d.validation?.dependencies_added !== false || d.validation?.package_metadata_changed !== false || d.validation?.runtime_enabled !== false || d.validation?.upload_allowed !== false || d.validation?.upload_execution_enabled !== false || d.validation?.platform_api_calls_allowed !== false || d.validation?.network_calls_allowed !== false || d.validation?.credentials_accessed !== false || d.validation?.token_accessed !== false || d.validation?.media_file_read !== false || d.validation?.file_mutation_allowed !== false) fail("Unsafe validation boundary");
  ...
  const forbidden = (value: unknown): boolean => {
    ...
    if (value && typeof value === "object") return Object.entries(value as Record<string, unknown>).some(([key, child]) => {
      ...
      return lowerKey.includes("access_token") || lowerKey.includes("refresh_token") || lowerKey.includes("client_secret") || lowerKey.includes("code_verifier") || lowerKey.includes("authorization_code") || forbidden(child);
    });
    return false;
  };

  if (forbidden(contracts)) fail("Unsafe string content");
  return { ok: blocking_reasons.length === 0, blocking_reasons, warnings };
}

export function validateRealUploadStubContracts(contracts: unknown): RealUploadStubContractsValidationResult {
  return validateRealUploadStubContractsShape(contracts);
}

export function createRealUploadStubContracts(input: any): RealUploadStubContracts {
  if (input.dryRun !== true) throw new Error("createRealUploadStubContracts: dryRun=true required");
  if (!validateRealUploadScaffoldStubDesign(input.stubDesign).ok || !validateRealUploadScaffoldContractTests(input.scaffoldContractTests).ok || !validateRealUploadScaffoldContracts(input.scaffoldContracts).ok || !validateRealUploadScaffoldDesign(input.scaffoldDesign).ok || !validateRealUploadImplementationDesign(input.implementationDesign).ok || !validateFinalRealUploadPreflightGate(input.finalPreflightGate).ok || !validateRealUploadDryRunExecutionResult(input.dryRunExecutionResult).ok || !validateRealUploadExecutionPlan(input.executionPlan).ok || !validateRealUploadStrategyDesign(input.strategyDesign).ok || !validateRealUploadExecutionRequest(input.realUploadExecutionRequest).ok || !validateRealUploadReadinessAssessment(input.readinessAssessment).ok || !validateDryRunUploadSpikeResult(input.dryRunUploadSpikeResult).ok || !validateUploadExecutionDesign(input.uploadExecutionDesign).ok || !validateUploadExecutionApproval(input.uploadExecutionApproval).ok || !validatePlatformUploadRequest(input.platformUploadRequest).ok || !validateUploadPackageDesign(input.uploadPackageDesign).ok || !validateLocalOutputOperatorReview(input.localOutputReview).ok || !validateControlledProductionRenderSpikeResult(input.spikeResult).ok) throw new Error("createRealUploadStubContracts: all input artifacts must validate");
  const approved = input.decision === "approved_for_future_stub_contract_tests";
  if (approved && (!input.checklist_acknowledged || !input.understands_stub_contracts_only || !input.understands_no_stub_files_created || !input.understands_no_implementation_files_created || !input.understands_no_upload_enabled || !input.understands_no_credentials_accessed || !input.understands_no_network_calls || !input.understands_no_media_reads || !input.understands_no_dependencies_added || !input.understands_future_stub_contract_tests_phase_required)) throw new Error("createRealUploadStubContracts: acknowledgements required");
  ...
  const contracts: RealUploadStubContracts = {
    schema_version: "1.0",
    real_upload_stub_contracts_id: crypto.randomUUID(),
    ...
    stub_contracts_state: approved ? "approved_for_future_stub_contract_tests" : input.decision === "rejected" ? "rejected" : "draft",
    created_at: new Date().toISOString(),
    stub_contracts_mode: approved ? "operator_review_real_upload_stub_contracts" : "real_upload_stub_contracts_only",
    required_artifacts: {
    ...
    stub_contracts_scope: {
      future_stub_contract_tests_requested: true,
      stub_contracts_only: true,
    ...
    stub_module_contracts: kinds.map((kind, index) => ({
      stub_contract_id: `stub-contract-${index + 1}`,
      stub_kind: kind,
      contract_state: kind === "operator_confirmation_stub_contract" ? "blocked" : "proposed",
      safe_summary: `Future ${kind.replaceAll("_", " ")} contract only.`,
      type_shape_defined: true,
      no_op_contract: true,
    ...
    stub_behavior_contracts: [
      {
        behavior_contract_id: "behavior-contract-1",
        behavior_kind: "credential_provider_stub_contract",
        contract_state: "proposed",
        safe_summary: "Credential provider contract remains no-op.",
        no_op_behavior_contract: true,
    ...
    no_op_contract_boundary: {
      all_stub_contracts_no_op_only: true,
    ...
    credential_stub_contract_boundary: {
      credential_stub_contract_defined: true,
      credentials_required: false,
      credentials_accessed: false,
      token_accessed: false,
      keychain_accessed: false,
    ...
    media_stub_contract_boundary: {
      media_stub_contract_defined: true,
      media_file_read: false,
      raw_media_path_stored: false,
      media_file_modified: false,
      media_file_copied: false,
    ...
    platform_api_stub_contract_boundary: {
      platform_api_stub_contract_defined: true,
      platform_adapter_implemented: false,
      platform_endpoint_selected: false,
      platform_api_calls_allowed: false,
      platform_api_calls_made: false,
    ...
    payload_stub_contract_boundary: {
      payload_stub_contract_defined: true,
      platform_api_payload_created: false,
      platform_api_payload_stored: false,
      upload_payload_created: false,
      raw_payload_stored: false,
    },
    network_stub_contract_boundary: {
      network_stub_contract_defined: true,
      network_client_implemented: false,
      network_calls_allowed: false,
      network_calls_made: false,
      external_side_effects_allowed: false,
    ...
    runtime_stub_contract_boundary: {
      runtime_stub_contract_defined: true,
      runtime_enabled: false,
      runtime_files_created: false,
    ...
    operator_review: {
      reviewed_by_label: input.reviewed_by_label || "operator-001",
      checklist_acknowledged: !!input.checklist_acknowledged,
      understands_stub_contracts_only: !!input.understands_stub_contracts_only,
      ...
      understands_no_media_reads: !!input.understands_no_media_reads,
      understands_no_dependencies_added: !!input.understands_no_dependencies_added,
      understands_future_stub_contract_tests_phase_required: !!input.understands_future_stub_contract_tests_phase_required,
      decision_note_summary: safeRealUploadStubContractsString(input.decision_note_summary, "[stub-contracts]"),
    },
    execution_boundary: {
      stub_files_created: false,
      implementation_files_created: false,
    ...
    validation: {
      stub_contracts_complete: true,
      ready_for_future_stub_contract_tests: approved,
      ready_for_real_upload: false,
      stub_files_created: false,
      implementation_files_created: false,
      runtime_files_created: false,
    ...
    provenance: {
      generated_by: "createRealUploadStubContracts",
  ...
  const validation = validateRealUploadStubContracts(contracts);
  if (!validation.ok) throw new Error(`Unsafe real upload stub contracts cannot be created: ${validation.blocking_reasons.join("; ")}`);
  return contracts;
}

export function saveRealUploadStubContracts(contracts: RealUploadStubContracts): void {
  const validation = validateRealUploadStubContracts(contracts);
  if (!validation.ok) throw new Error(`Unsafe real upload stub contracts cannot be stored: ${validation.blocking_reasons.join("; ")}`);
  const store = loadRealUploadStubContractsStore();
  const index = store.contracts.findIndex((item) => item.real_upload_stub_contracts_id === contracts.real_upload_stub_contracts_id);
  if (index >= 0) store.contracts[index] = contracts;
  else store.contracts.push(contracts);
  saveRealUploadStubContractsStore(store);
}

export function listRealUploadStubContracts(options?: { project_id?: string; platform?: string; stub_contracts_state?: string; real_upload_scaffold_stub_design_id?: string; real_upload_scaffold_contract_tests_id?: string }): RealUploadStubContracts[] {
  return loadRealUploadStubContractsStore().contracts
    .filter((item) => (!options?.project_id || item.project_id === options.project_id) && (!options?.platform || item.platform === options.platform) && (!options?.stub_contracts_state || item.stub_contracts_state === options.stub_contracts_state) && (!options?.real_upload_scaffold_stub_design_id || item.real_upload_scaffold_stub_design_id === options.real_upload_scaffold_stub_design_id) && (!options?.real_upload_scaffold_contract_tests_id || item.real_upload_scaffold_contract_tests_id === options.real_upload_scaffold_contract_tests_id))
    .sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime() || a.real_upload_stub_contracts_id.localeCompare(b.real_upload_stub_contracts_id));
}

export function getRealUploadStubContracts(real_upload_stub_contracts_id: string): RealUploadStubContracts | null {
  return loadRealUploadStubContractsStore().contracts.find((item) => item.real_upload_stub_contracts_id === real_upload_stub_contracts_id) ?? null;
}

export function revokeRealUploadStubContracts(real_upload_stub_contracts_id: string, reason: string): RealUploadStubContracts {
  const contracts = getRealUploadStubContracts(real_upload_stub_contracts_id);
  if (!contracts) throw new Error("Real upload stub contracts not found");
  const safeReason = safeRealUploadStubContractsString(reason, "[unsafe-reason]");
  if (safeReason === "[unsafe-reason]") throw new Error("Unsafe real upload stub contracts reason cannot be stored.");
  const revoked: RealUploadStubContracts = {
    ...contracts,
    stub_contracts_state: "revoked",
    validation: {
      ...contracts.validation,
      blocking_reasons: [...contracts.validation.blocking_reasons, safeReason],
      warnings: [...contracts.validation.warnings, "Real upload stub contracts revoked."],
    },
    provenance: { ...contracts.provenance, generated_by: "revokeRealUploadStubContracts" },
  };
  saveRealUploadStubContracts(revoked);
  return revoked;
}

export function getRealUploadStubContractsReport(options?: { project_id?: string; platform?: string }): { total: number; by_state: Record<string, number>; blocked: number; ready_for_operator_review: number; approved_for_future_stub_contract_tests: number; rejected: number; revoked: number; stub_contracts_complete: number; ready_for_future_stub_contract_tests: number; ready_for_real_upload: 0; stub_files_created: 0; implementation_files_created: 0; runtime_files_created: 0; dependencies_added: 0; package_metadata_changed: 0; runtime_enabled: 0; upload_allowed: 0; upload_execution_enabled: 0; platform_api_calls_allowed: 0; network_calls_allowed: 0; credentials_accessed: 0; token_accessed: 0; media_file_read: 0; file_mutation_allowed: 0; stub_contracts_total: number; stub_contracts_with_blockers: number; contracts: Array<RealUploadStubContractsStoreSummaryItem>; } {
  const contracts = listRealUploadStubContracts(options);
  const by_state: Record<string, number> = {};
  let blocked = 0;
  let ready = 0;
  let approved = 0;
  ...
  let total = 0;
  let blockers = 0;
  const summaries = contracts.map((item) => {
    by_state[item.stub_contracts_state] = (by_state[item.stub_contracts_state] ?? 0) + 1;
    if (item.stub_contracts_state === "blocked") blocked++;
    if (item.stub_contracts_state === "ready_for_operator_review") ready++;
    if (item.stub_contracts_state === "approved_for_future_stub_contract_tests") approved++;
    if (item.stub_contracts_state === "rejected") rejected++;
    if (item.stub_contracts_state === "revoked") revoked++;
    if (item.validation.stub_contracts_complete) complete++;
    if (item.validation.ready_for_future_stub_contract_tests) readyFuture++;
    total += item.stub_module_contracts.length;
    blockers += item.stub_module_contracts.filter((module) => module.blocking_reasons.length > 0).length;
    return {
      stub_contracts_state: item.stub_contracts_state,
      created_at: item.created_at,
      stub_contracts_total: item.stub_module_contracts.length,
      stub_contracts_with_blockers: item.stub_module_contracts.filter((module) => module.blocking_reasons.length > 0).length,
      safe_summary: safeRealUploadStubContractsString(item.validation.blocking_reasons[0] || item.stub_module_contracts[0]?.safe_summary || "Stub contracts summary.", "Stub contracts summary."),
    };
  });
  return {
    total: summaries.length,
    ...
    blocked,
    ready_for_operator_review: ready,
    approved_for_future_stub_contract_tests: approved,
    rejected,
    revoked,
    stub_contracts_complete: complete,
    ready_for_future_stub_contract_tests: readyFuture,
    ready_for_real_upload: 0,
    ...
    stub_contracts_total: total,
    stub_contracts_with_blockers: blockers,
    contracts: summaries,
  };
}
