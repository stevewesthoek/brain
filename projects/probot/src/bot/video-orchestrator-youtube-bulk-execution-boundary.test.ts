...
import {
  runYouTubeBulkExecutionBoundary,
  createYouTubeBulkExecutionBoundaryReview,
  createYouTubeBulkExecutionBoundarySafeReport,
  revokeYouTubeBulkExecutionBoundaryResult,
  revokeYouTubeBulkExecutionBoundaryReview,
  revokeYouTubeBulkExecutionBoundarySafeReport,
  type YouTubeBulkExecutionBoundaryAdapters,
  type YouTubeBulkExecutionBoundaryInput,
  type YouTubeBulkExecutionBoundaryControlKind,
} from "./video-orchestrator-youtube-bulk-execution-boundary.js";
...
async function readyMultiPair() {
  ...
  return { multiResult, multiReport };
}

test("VO-7CR-BULK-BOUNDARY-1: plans bulk execution boundary without actual bulk upload", async () => {
  const { multiResult, multiReport } = await readyMultiPair();
  const result = await runYouTubeBulkExecutionBoundary(multiReport, multiResult, BULK_INPUT, bulkAdapters(), { id: "youtube-bulk-boundary-result-001", created_at: "2026-05-14T00:00:00.000Z" });
  assert.equal(result.boundary_state, "ready_for_operator_review");
  assert.equal(result.boundary_planned, true);
  assert.equal(result.boundary_implemented, true);
  assert.equal(result.max_items_per_batch, 3);
  assert.equal(result.controls.length, 11);
  assert.equal(result.actual_bulk_uploads_enabled, false);
  assert.equal(result.actual_bulk_uploads_executed, false);
  ...
  assert.equal(result.validation.ready_for_controlled_bulk_execution_review, true);
  assert.equal(result.validation.ready_for_real_upload, false);
  assertDisabledBoundary(result.execution_boundary);
});

test("VO-7CR-BULK-BOUNDARY-2: unsafe input blocks before adapters", async () => {
  const { multiResult, multiReport } = await readyMultiPair();
  let called = false;
  const result = await runYouTubeBulkExecutionBoundary(multiReport, multiResult, { ...BULK_INPUT, allow_actual_bulk_uploads: true as false }, bulkAdapters({ planBoundaryControls: () => { called = true; return bulkControls(); } }));
  assert.equal(called, false);
  assert.equal(result.boundary_state, "blocked");
  assert.equal(result.boundary_planned, false);
  assert.equal(result.actual_bulk_uploads_executed, false);
  assert.equal(result.validation.complete, false);
  assertDisabledBoundary(result.execution_boundary);
});

test("VO-7CR-BULK-BOUNDARY-3: readiness block prevents controlled bulk execution approval", async () => {
  const { multiResult, multiReport } = await readyMultiPair();
  const result = await runYouTubeBulkExecutionBoundary(multiReport, multiResult, BULK_INPUT, bulkAdapters({ checkBoundaryReadiness: () => ({ ready: false, safe_summary: "Boundary blocked.", blocking_reasons: ["Manual pause control missing."] }) }));
  assert.equal(result.boundary_state, "blocked");
  assert.equal(result.validation.complete, false);
  assert.equal(result.validation.blocking_reasons.includes("Manual pause control missing."), true);
  assert.equal(result.actual_bulk_uploads_executed, false);
  assertDisabledBoundary(result.execution_boundary);
});

test("VO-7CS-BULK-BOUNDARY-REVIEW-1: review and safe report require confirmation before controlled bulk execution", async () => {
  const { multiResult, multiReport } = await readyMultiPair();
  const result = await runYouTubeBulkExecutionBoundary(multiReport, multiResult, BULK_INPUT, bulkAdapters());
  const review = createYouTubeBulkExecutionBoundaryReview(result, { id: "youtube-bulk-boundary-review-001", created_at: "2026-05-14T00:00:00.000Z" });
  const report = createYouTubeBulkExecutionBoundarySafeReport(review, result, { id: "youtube-bulk-boundary-safe-report-001", created_at: "2026-05-14T00:00:00.000Z" });
  assert.equal(review.review_state, "approved_for_safe_report");
  assert.equal(review.review_only, true);
  assert.equal(review.boundary_planned, true);
  assert.equal(review.boundary_implemented, true);
  assert.equal(review.actual_bulk_uploads_executed, false);
  ...
  assert.equal(report.safe_report_state, "requires_operator_confirmation_for_controlled_bulk_execution");
  assert.equal(report.safe_report_only, true);
  assert.equal(report.boundary_planned, true);
  assert.equal(report.boundary_implemented, true);
  assert.equal(report.actual_bulk_uploads_executed, false);
...
test("VO-7CS-BULK-BOUNDARY-REVIEW-2: revocation keeps bulk boundary artifacts disabled", async () => {
  const { multiResult, multiReport } = await readyMultiPair();
  const result = await runYouTubeBulkExecutionBoundary(multiReport, multiResult, BULK_INPUT, bulkAdapters());
  const review = createYouTubeBulkExecutionBoundaryReview(result);
  const report = createYouTubeBulkExecutionBoundarySafeReport(review, result);
  assert.equal(revokeYouTubeBulkExecutionBoundaryResult(result).boundary_state, "revoked");
  assert.equal(revokeYouTubeBulkExecutionBoundaryReview(review).review_state, "revoked");
  assert.equal(revokeYouTubeBulkExecutionBoundarySafeReport(report).safe_report_state, "revoked");
});
