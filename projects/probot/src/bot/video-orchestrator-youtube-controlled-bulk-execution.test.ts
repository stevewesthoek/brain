...
import {
  runYouTubeControlledBulkExecution,
  createYouTubeControlledBulkExecutionReview,
  createYouTubeControlledBulkExecutionSafeReport,
  revokeYouTubeControlledBulkExecutionResult,
  revokeYouTubeControlledBulkExecutionReview,
  revokeYouTubeControlledBulkExecutionSafeReport,
  type YouTubeControlledBulkExecutionAdapters,
  type YouTubeControlledBulkExecutionInput,
} from "./video-orchestrator-youtube-controlled-bulk-execution.js";
...
async function readyBulkBoundaryPair() {
  ...
  return { bulkBoundaryResult, bulkBoundaryReport };
}

test("VO-7CT-CONTROLLED-BULK-1: executes controlled bulk items without deletes or metadata changes", async () => {
  const { bulkBoundaryResult, bulkBoundaryReport } = await readyBulkBoundaryPair();
  const result = await runYouTubeControlledBulkExecution(bulkBoundaryReport, bulkBoundaryResult, CONTROLLED_INPUT, controlledBulkAdapters(), { id: "youtube-controlled-bulk-result-001", created_at: "2026-05-14T00:00:00.000Z" });
  assert.equal(result.execution_state, "completed");
  assert.equal(result.controlled_bulk_execution_enabled, true);
  assert.equal(result.controlled_bulk_execution_executed, true);
  assert.equal(result.max_items_to_execute, 2);
  ...
  assert.equal(result.item_results.every((item) => item.delete_performed === false && item.unrelated_metadata_changed === false && item.raw_payload_stored === false && item.raw_response_stored === false), true);
  assert.equal(result.safe_store_result?.stored, true);
  assert.equal(result.validation.complete, true);
  assert.equal(result.validation.ready_for_delete_metadata_boundary_review, true);
  assert.equal(result.validation.ready_for_real_upload, false);
  assertDisabledBoundary(result.execution_boundary);
});

test("VO-7CT-CONTROLLED-BULK-2: unsafe input blocks before adapters", async () => {
  const { bulkBoundaryResult, bulkBoundaryReport } = await readyBulkBoundaryPair();
  let called = false;
  const result = await runYouTubeControlledBulkExecution(bulkBoundaryReport, bulkBoundaryResult, { ...CONTROLLED_INPUT, allow_deletes: true as false }, controlledBulkAdapters({ listApprovedBulkItems: () => { called = true; return []; } }));
  assert.equal(called, false);
  assert.equal(result.execution_state, "blocked");
  assert.equal(result.controlled_bulk_execution_executed, false);
...
test("VO-7CT-CONTROLLED-BULK-3: partial controlled execution is reviewable", async () => {
  const { bulkBoundaryResult, bulkBoundaryReport } = await readyBulkBoundaryPair();
  const result = await runYouTubeControlledBulkExecution(bulkBoundaryReport, bulkBoundaryResult, CONTROLLED_INPUT, controlledBulkAdapters({ executeSingleApprovedItem: (item) => ({ queue_item_id: item.queue_item_id, attempted: true, uploaded: item.queue_item_id.endsWith("001"), skipped: !item.queue_item_id.endsWith("001"), uploaded_video_id: item.queue_item_id.endsWith("001") ? "partial-video" : null, scheduled_publish_at: item.queue_item_id.endsWith("001") ? "2026-05-16T09:00:00.000Z" : null, privacy_status: "private", safe_summary: "Partial controlled item result.", delete_performed: false, unrelated_metadata_changed: false, raw_payload_stored: false, raw_response_stored: false }) }));
...
test("VO-7CU-CONTROLLED-BULK-REVIEW-1: review and safe report require confirmation before delete/metadata boundary", async () => {
  const { bulkBoundaryResult, bulkBoundaryReport } = await readyBulkBoundaryPair();
  const result = await runYouTubeControlledBulkExecution(bulkBoundaryReport, bulkBoundaryResult, CONTROLLED_INPUT, controlledBulkAdapters());
  const review = createYouTubeControlledBulkExecutionReview(result, { id: "youtube-controlled-bulk-review-001", created_at: "2026-05-14T00:00:00.000Z" });
  const report = createYouTubeControlledBulkExecutionSafeReport(review, result, { id: "youtube-controlled-bulk-safe-report-001", created_at: "2026-05-14T00:00:00.000Z" });
  assert.equal(review.review_state, "approved_for_safe_report");
  assert.equal(review.review_only, true);
  assert.equal(review.controlled_bulk_execution_executed, true);
  assert.equal(review.attempted_count, 2);
  assert.equal(review.uploaded_count, 2);
  ...
  assert.equal(report.safe_report_state, "requires_operator_confirmation_for_delete_metadata_boundary");
  assert.equal(report.safe_report_only, true);
  assert.equal(report.controlled_bulk_execution_executed, true);
  assert.equal(report.attempted_count, 2);
  assert.equal(report.uploaded_count, 2);
...
test("VO-7CU-CONTROLLED-BULK-REVIEW-2: revocation keeps controlled bulk artifacts disabled", async () => {
  const { bulkBoundaryResult, bulkBoundaryReport } = await readyBulkBoundaryPair();
  const result = await runYouTubeControlledBulkExecution(bulkBoundaryReport, bulkBoundaryResult, CONTROLLED_INPUT, controlledBulkAdapters());
  const review = createYouTubeControlledBulkExecutionReview(result);
  const report = createYouTubeControlledBulkExecutionSafeReport(review, result);
  assert.equal(revokeYouTubeControlledBulkExecutionResult(result).execution_state, "revoked");
  assert.equal(revokeYouTubeControlledBulkExecutionReview(review).review_state, "revoked");
  assert.equal(revokeYouTubeControlledBulkExecutionSafeReport(report).safe_report_state, "revoked");
});
