...
import {
  runYouTubeMultiAccountPlatformExpansion,
  createYouTubeMultiAccountPlatformExpansionReview,
  createYouTubeMultiAccountPlatformExpansionSafeReport,
  revokeYouTubeMultiAccountPlatformExpansionResult,
  revokeYouTubeMultiAccountPlatformExpansionReview,
  revokeYouTubeMultiAccountPlatformExpansionSafeReport,
  type YouTubeMultiAccountPlatformExpansionAdapters,
  type YouTubeMultiAccountPlatformExpansionInput,
} from "./video-orchestrator-youtube-multi-account-platform-expansion.js";
...
async function readyAutomationPair() {
  ...
  return { automationResult, automationReport };
}

test("VO-7CP-MULTI-EXPANSION-1: checks multi-account/platform queues without executing uploads", async () => {
  const { automationResult, automationReport } = await readyAutomationPair();
  const result = await runYouTubeMultiAccountPlatformExpansion(automationReport, automationResult, MULTI_INPUT, multiAdapters(), { id: "youtube-multi-account-platform-result-001", created_at: "2026-05-14T00:00:00.000Z" });
  assert.equal(result.expansion_state, "ready_for_cross_account_platform_review");
  assert.equal(result.multi_account_checked, true);
  assert.equal(result.multi_platform_checked, true);
  ...
  assert.equal(result.validation.ready_for_bulk_execution_boundary_review, true);
  assert.equal(result.validation.ready_for_real_upload, false);
  assertDisabledBoundary(result.execution_boundary);
});

test("VO-7CP-MULTI-EXPANSION-2: unsafe input blocks before adapters", async () => {
  const { automationResult, automationReport } = await readyAutomationPair();
  let called = false;
  const result = await runYouTubeMultiAccountPlatformExpansion(automationReport, automationResult, { ...MULTI_INPUT, allow_bulk_uploads: true as false }, multiAdapters({ listAccountPlatformQueues: () => { called = true; return []; } }));
  assert.equal(called, false);
  assert.equal(result.expansion_state, "blocked");
  assert.equal(result.multi_account_checked, false);
  assert.equal(result.bulk_uploads_executed, false);
  assert.equal(result.validation.complete, false);
  assertDisabledBoundary(result.execution_boundary);
});

test("VO-7CP-MULTI-EXPANSION-3: readiness block prevents bulk boundary", async () => {
  const { automationResult, automationReport } = await readyAutomationPair();
  const result = await runYouTubeMultiAccountPlatformExpansion(automationReport, automationResult, MULTI_INPUT, multiAdapters({ checkExpansionReadiness: () => ({ ready: false, safe_summary: "Expansion blocked.", blocking_reasons: ["Platform cadence review pending."] }) }));
  assert.equal(result.expansion_state, "blocked");
  assert.equal(result.validation.complete, false);
  assert.equal(result.validation.blocking_reasons.includes("Platform cadence review pending."), true);
  assert.equal(result.upload_executed, false);
  assertDisabledBoundary(result.execution_boundary);
});

test("VO-7CQ-MULTI-EXPANSION-REVIEW-1: review and safe report require confirmation before bulk execution boundary", async () => {
  const { automationResult, automationReport } = await readyAutomationPair();
  const result = await runYouTubeMultiAccountPlatformExpansion(automationReport, automationResult, MULTI_INPUT, multiAdapters());
  const review = createYouTubeMultiAccountPlatformExpansionReview(result, { id: "youtube-multi-account-platform-review-001", created_at: "2026-05-14T00:00:00.000Z" });
  const report = createYouTubeMultiAccountPlatformExpansionSafeReport(review, result, { id: "youtube-multi-account-platform-safe-report-001", created_at: "2026-05-14T00:00:00.000Z" });
  assert.equal(review.review_state, "approved_for_safe_report");
  assert.equal(review.review_only, true);
  assert.equal(review.multi_account_checked, true);
  assert.equal(review.multi_platform_checked, true);
  ...
  assert.equal(report.safe_report_state, "requires_operator_confirmation_for_bulk_execution_boundary");
  assert.equal(report.safe_report_only, true);
  assert.equal(report.multi_account_checked, true);
  assert.equal(report.multi_platform_checked, true);
...
test("VO-7CQ-MULTI-EXPANSION-REVIEW-2: revocation keeps multi-account/platform artifacts disabled", async () => {
  const { automationResult, automationReport } = await readyAutomationPair();
  const result = await runYouTubeMultiAccountPlatformExpansion(automationReport, automationResult, MULTI_INPUT, multiAdapters());
  const review = createYouTubeMultiAccountPlatformExpansionReview(result);
  const report = createYouTubeMultiAccountPlatformExpansionSafeReport(review, result);
  assert.equal(revokeYouTubeMultiAccountPlatformExpansionResult(result).expansion_state, "revoked");
  assert.equal(revokeYouTubeMultiAccountPlatformExpansionReview(review).review_state, "revoked");
  assert.equal(revokeYouTubeMultiAccountPlatformExpansionSafeReport(report).safe_report_state, "revoked");
});
