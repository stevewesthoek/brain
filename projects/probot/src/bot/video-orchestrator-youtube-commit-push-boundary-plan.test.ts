...
import { runYouTubeDeleteMetadataImplementation, createYouTubeDeleteMetadataImplementationReview, createYouTubeDeleteMetadataImplementationSafeReport, type YouTubeDeleteMetadataImplementationAdapters, type YouTubeDeleteMetadataImplementationInput } from "./video-orchestrator-youtube-delete-metadata-implementation.js";
import {
  createYouTubeCommitPushBoundaryPlan,
  createYouTubeCommitPushBoundaryReview,
  createYouTubeCommitPushBoundarySafeReport,
  revokeYouTubeCommitPushBoundaryPlan,
  revokeYouTubeCommitPushBoundaryReview,
  revokeYouTubeCommitPushBoundarySafeReport,
  type YouTubeCommitPushBoundaryInput,
} from "./video-orchestrator-youtube-commit-push-boundary-plan.js";
...
const COMMIT_PUSH_INPUT: YouTubeCommitPushBoundaryInput = { request_id: "youtube-commit-push-boundary-request-001", project_id: "says-the-bible", operator_approval_id: "operator-commit-push-boundary-001", boundary_scope_id: "commit-push-boundary-scope-001", allow_planning_only: true, allow_staging: false, allow_commits: false, allow_pushes: false, allow_destructive_cleanup: false };
...
async function readyDeleteMetadataPair() {
  ...
  const implementationResult = await runYouTubeDeleteMetadataImplementation(boundaryReport, boundaryPlan, IMPLEMENTATION_INPUT, implementationAdapters());
  const implementationReview = createYouTubeDeleteMetadataImplementationReview(implementationResult);
  const implementationReport = createYouTubeDeleteMetadataImplementationSafeReport(implementationReview, implementationResult);
  return { implementationResult, implementationReport };
}

test("VO-7CZ-COMMIT-PUSH-BOUNDARY-1: creates planning-only commit/push boundary", async () => {
  const { implementationResult, implementationReport } = await readyDeleteMetadataPair();
  const plan = createYouTubeCommitPushBoundaryPlan(implementationReport, implementationResult, COMMIT_PUSH_INPUT, { id: "youtube-commit-push-boundary-plan-001", created_at: "2026-05-14T00:00:00.000Z" });
  ...
  assert.equal(plan.controls.every((control) => control.planned_now && !control.implemented_now && !control.staging_enabled_now && !control.commit_enabled_now && !control.push_enabled_now && !control.destructive_cleanup_enabled_now && control.requires_future_confirmation), true);
  assert.equal(plan.staging_enabled, false);
  assert.equal(plan.staged_now, false);
  ...
  assert.equal(plan.destructive_cleanup_enabled, false);
  assert.equal(plan.destructive_cleanup_performed, false);
  assert.equal(plan.validation.complete, true);
  assertDisabledBoundary(plan.execution_boundary);
});

test("VO-7CZ-COMMIT-PUSH-BOUNDARY-2: unsafe input blocks planning", async () => {
  const { implementationResult, implementationReport } = await readyDeleteMetadataPair();
  const plan = createYouTubeCommitPushBoundaryPlan(implementationReport, implementationResult, { ...COMMIT_PUSH_INPUT, allow_commits: true as false });
...
test("VO-7DA-COMMIT-PUSH-REVIEW-1: review and safe report require confirmation before staging/commit/push", async () => {
  const { implementationResult, implementationReport } = await readyDeleteMetadataPair();
  const plan = createYouTubeCommitPushBoundaryPlan(implementationReport, implementationResult, COMMIT_PUSH_INPUT);
  const review = createYouTubeCommitPushBoundaryReview(plan, { id: "youtube-commit-push-boundary-review-001", created_at: "2026-05-14T00:00:00.000Z" });
  const report = createYouTubeCommitPushBoundarySafeReport(review, plan, { id: "youtube-commit-push-boundary-safe-report-001", created_at: "2026-05-14T00:00:00.000Z" });
...
test("VO-7DA-COMMIT-PUSH-REVIEW-2: revocation keeps commit/push boundary disabled", async () => {
  const { implementationResult, implementationReport } = await readyDeleteMetadataPair();
  const plan = createYouTubeCommitPushBoundaryPlan(implementationReport, implementationResult, COMMIT_PUSH_INPUT);
  const review = createYouTubeCommitPushBoundaryReview(plan);
  const report = createYouTubeCommitPushBoundarySafeReport(review, plan);
  assert.equal(revokeYouTubeCommitPushBoundaryPlan(plan).plan_state, "revoked");
  assert.equal(revokeYouTubeCommitPushBoundaryReview(review).review_state, "revoked");
  assert.equal(revokeYouTubeCommitPushBoundarySafeReport(report).safe_report_state, "revoked");
});
