...
import { createRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout, renderRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout, revokeRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout } from "./video-orchestrator-runtime-scheduler-persistent-store-executable-integration-decision-closeout.js";
...
const SAFE_PACKET: RuntimeSchedulerPersistentStoreExecutableIntegrationOperatorDecisionPacket = {
  schema_version: "1.0",
  ...
  plan_step_count: 2,
  allowed_future_scope: ["Prepare runtime import planning artifacts only."],
...
const SAFE_INPUT = {
  ...
  operator_id: "operator-runtime-scheduler-executable-integration-decision-closeout-001",
...
test("VO-7HN-RUNTIME-SCHEDULER-PERSISTENT-STORE-EXECUTABLE-INTEGRATION-DECISION-CLOSEOUT-1: creates closeout without side effects", () => {
  const closeout = createRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout(SAFE_INPUT, SAFE_PACKET);
  ...
  assert.equal(closeout.decision, "approve_future_runtime_import_planning");
  assert.equal(closeout.plan_step_count, 2);
...
test("VO-7HO-RUNTIME-SCHEDULER-PERSISTENT-STORE-EXECUTABLE-INTEGRATION-DECISION-CLOSEOUT-REVIEW-1: defer and reject close out without future scope", () => {
  const defer = createRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout(SAFE_INPUT, { ...SAFE_PACKET, decision: "defer", allowed_future_scope: [] });
  const reject = createRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout(SAFE_INPUT, { ...SAFE_PACKET, decision: "reject", allowed_future_scope: [] });
...
test("VO-7HO-RUNTIME-SCHEDULER-PERSISTENT-STORE-EXECUTABLE-INTEGRATION-DECISION-CLOSEOUT-REVIEW-2: unsafe input or packet blocks closeout", () => {
  const unsafeInput = createRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout({ ...SAFE_INPUT, allow_runtime_imports: true as false }, SAFE_PACKET);
  const unsafePacket = createRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout(SAFE_INPUT, { ...SAFE_PACKET, plan_step_count: 0 });
...
test("VO-7HO-RUNTIME-SCHEDULER-PERSISTENT-STORE-EXECUTABLE-INTEGRATION-DECISION-CLOSEOUT-REVIEW-3: renderer and revocation are safe", () => {
  const closeout = createRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout(SAFE_INPUT, SAFE_PACKET);
  const rendered = renderRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout(closeout);
  const revoked = revokeRuntimeSchedulerPersistentStoreExecutableIntegrationDecisionCloseout(closeout);

  assert.equal(rendered.includes("executable integration decision closeout"), true);
