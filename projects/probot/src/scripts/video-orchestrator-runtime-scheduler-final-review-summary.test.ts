...
import { createRuntimeSchedulerReviewLedgerEntry } from "./video-orchestrator-runtime-scheduler-review-ledger.js";
import {
  createRuntimeSchedulerFinalReviewSummary,
  renderRuntimeSchedulerFinalReviewSummary,
  revokeRuntimeSchedulerFinalReviewSummary,
} from "./video-orchestrator-runtime-scheduler-final-review-summary.js";
...
const FINAL_SUMMARY_INPUT = {
  summary_id: "runtime-scheduler-final-review-summary-001",
  operator_id: "operator-runtime-scheduler-final-review-summary-001",
  allow_summary_only: true,
  allow_package_json_edit: false,
  allow_live_scheduler: false,
  allow_upload_execution: false,
...
function reviewLedger() {
  ...
  const closeout = createRuntimeSchedulerReviewCloseout(REVIEW_CLOSEOUT_INPUT, review);
  return createRuntimeSchedulerReviewLedgerEntry(REVIEW_LEDGER_INPUT, closeout);
}

test("VO-7GP-RUNTIME-SCHEDULER-FINAL-REVIEW-SUMMARY-1: summarizes final review ledger without side effects", () => {
  const summary = createRuntimeSchedulerFinalReviewSummary(FINAL_SUMMARY_INPUT, reviewLedger());

  assert.equal(summary.schema_version, "1.0");
  assert.equal(summary.summary_state, "manual_boundary_confirmed");
  assert.equal(summary.summary_only, true);
  assert.equal(summary.source_ledger_state, "review_ledger_entry_ready");
  assert.equal(summary.decision, "approved_for_package_script_planning");
  ...
  assert.equal(summary.validation.manual_boundary_confirmed, true);
});

test("VO-7GQ-RUNTIME-SCHEDULER-FINAL-REVIEW-SUMMARY-REVIEW-1: unsafe flags block final summary", () => {
  const summary = createRuntimeSchedulerFinalReviewSummary({ ...FINAL_SUMMARY_INPUT, allow_live_scheduler: true as false }, reviewLedger());
...
test("VO-7GQ-RUNTIME-SCHEDULER-FINAL-REVIEW-SUMMARY-REVIEW-2: renderer and revocation are safe", () => {
  const summary = createRuntimeSchedulerFinalReviewSummary(FINAL_SUMMARY_INPUT, reviewLedger());
  const rendered = renderRuntimeSchedulerFinalReviewSummary(summary);
  const revoked = revokeRuntimeSchedulerFinalReviewSummary(summary);

  assert.equal(rendered.includes("runtime scheduler final review summary"), true);
