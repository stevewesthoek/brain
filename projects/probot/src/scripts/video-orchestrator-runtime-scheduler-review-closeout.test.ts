...
import {
  createRuntimeSchedulerReviewCloseout,
  renderRuntimeSchedulerReviewCloseout,
  revokeRuntimeSchedulerReviewCloseout,
} from "./video-orchestrator-runtime-scheduler-review-closeout.js";
...
function review() {
  ...
  const ledger = createRuntimeSchedulerOutcomeLedgerEntry(LEDGER_INPUT, closeout);
  return createRuntimeSchedulerLedgerReview(REVIEW_INPUT, ledger);
}

test("VO-7GL-RUNTIME-SCHEDULER-REVIEW-CLOSEOUT-1: closes ledger review at operator boundary", () => {
  const closeout = createRuntimeSchedulerReviewCloseout(REVIEW_CLOSEOUT_INPUT, review());

  assert.equal(closeout.schema_version, "1.0");
  assert.equal(closeout.closeout_state, "closed_for_operator_boundary");
  assert.equal(closeout.closeout_only, true);
  assert.equal(closeout.source_review_state, "ready_for_manual_review");
  ...
  assert.equal(closeout.validation.closed_for_operator_boundary, true);
});

test("VO-7GM-RUNTIME-SCHEDULER-REVIEW-CLOSEOUT-REVIEW-1: unsafe flags block review closeout", () => {
  const closeout = createRuntimeSchedulerReviewCloseout({ ...REVIEW_CLOSEOUT_INPUT, allow_commit: true as false }, review());
...
test("VO-7GM-RUNTIME-SCHEDULER-REVIEW-CLOSEOUT-REVIEW-2: renderer and revocation are safe", () => {
  const closeout = createRuntimeSchedulerReviewCloseout(REVIEW_CLOSEOUT_INPUT, review());
  const rendered = renderRuntimeSchedulerReviewCloseout(closeout);
  const revoked = revokeRuntimeSchedulerReviewCloseout(closeout);

  assert.equal(rendered.includes("runtime scheduler review closeout"), true);
