...
import {
  createRuntimeSchedulerOutcomeLedgerEntry,
  renderRuntimeSchedulerOutcomeLedgerEntry,
  revokeRuntimeSchedulerOutcomeLedgerEntry,
} from "./video-orchestrator-runtime-scheduler-outcome-ledger.js";
...
function closeout() {
  ...
  return createRuntimeSchedulerOutcomeCloseout(CLOSEOUT_INPUT, outcome);
}

test("VO-7GH-RUNTIME-SCHEDULER-OUTCOME-LEDGER-1: creates ledger entry from closeout without side effects", () => {
  const entry = createRuntimeSchedulerOutcomeLedgerEntry(LEDGER_INPUT, closeout());

  assert.equal(entry.schema_version, "1.0");
  assert.equal(entry.ledger_state, "ledger_entry_ready");
  assert.equal(entry.ledger_only, true);
  assert.equal(entry.recorded_at, "2026-05-15T08:30:00.000Z");
  assert.equal(entry.decision, "approved_for_package_script_planning");
  assert.equal(entry.source_closeout_state, "closed_for_manual_boundary");
  ...
  assert.equal(entry.validation.ledger_entry_ready, true);
});

test("VO-7GI-RUNTIME-SCHEDULER-OUTCOME-LEDGER-REVIEW-1: unsafe flags block ledger", () => {
  const entry = createRuntimeSchedulerOutcomeLedgerEntry({ ...LEDGER_INPUT, allow_file_write: true as false }, closeout());
...
test("VO-7GI-RUNTIME-SCHEDULER-OUTCOME-LEDGER-REVIEW-2: renderer and revocation are safe", () => {
  const entry = createRuntimeSchedulerOutcomeLedgerEntry(LEDGER_INPUT, closeout());
  const rendered = renderRuntimeSchedulerOutcomeLedgerEntry(entry);
  const revoked = revokeRuntimeSchedulerOutcomeLedgerEntry(entry);

  assert.equal(rendered.includes("runtime scheduler outcome ledger entry"), true);
