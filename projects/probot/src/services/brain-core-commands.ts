...
export async function handleBrainCoreCommand(text: string, brainCoreUrl: string): Promise<string | null> {
  ...
  if (subcommand === "approvals") {
    return [
      ...
      `Mind Steward execution flag: ${executionReadiness.mindStewardDryRunExecutionFlagEnabled ? "enabled (still gated)" : "disabled"} · ${executionReadiness.mindStewardDryRunExecutionFlagName}`,
