...
const report = mindRoot
    ? createMindStewardDryRunReport(createMindPathSnapshotFromRoot(mindRoot, createMindRelativePaths()))
    : createMindStewardDryRunReport({ paths: [] });
...
function renderMarkdown(report) {
    const lines = [
        ...
        '',
        '## Wiki Health',
        `- Status: ${report.wikiHealth.status}`,
        `- OK: ${report.wikiHealth.ok}`,
        `- Checked at: ${report.wikiHealth.checkedAt ?? 'unavailable'}`,
        `- Errors: ${report.wikiHealth.summary.errorCount}`,
        `- Warnings: ${report.wikiHealth.summary.warningCount}`,
        `- Stale capture count: ${report.wikiHealth.summary.staleCaptureCount}`,
        `- Failed capture count: ${report.wikiHealth.summary.failedCaptureCount}`,
        `- Oversized wiki page count: ${report.wikiHealth.summary.oversizedWikiPageCount}`,
        `- Missing source trace count: ${report.wikiHealth.summary.missingSourceTraceCount}`,
        ...report.wikiHealth.findings.map((finding) => `- ${finding.severity}: ${finding.path} — ${finding.message}`),
        '',
        '## Maintenance Preview Queue',
        `- Status: ${report.maintenancePreview.status}`,
        `- Total actions: ${report.maintenancePreview.actionCount}`,
        `- Low risk: ${report.maintenancePreview.lowRiskCount}`,
        `- Medium risk: ${report.maintenancePreview.mediumRiskCount}`,
        `- High risk: ${report.maintenancePreview.highRiskCount}`,
        `- Approval required: ${report.maintenancePreview.approvalRequiredCount}`,
        `- Writes to Mind: ${report.maintenancePreview.writesToMind}`,
        ...report.maintenancePreview.topActions.map((action) => `- ${action.risk}: ${action.title}`),
        '',
        '## Loop Plans',
