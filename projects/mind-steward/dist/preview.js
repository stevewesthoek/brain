...
const LINE_LIMITS = { [MIND_PREVIEW_CURRENT_CONTEXT_PATH]: 150 };
...
export function evaluateMindPreviewPolicy(targetPath) {
    ...
    const blockedByExact = MIND_PREVIEW_BLOCKED_EXACT_PATHS.includes(normalized);
    ...
    if (blockedByPrefix)
        reasons.push(`Target path is blocked by prefix ${blockedByPrefix}.`);
    if (blockedByExact)
        reasons.push(`Target path is blocked by exact path ${normalized}.`);
    if (blockedBySuffix)
        reasons.push('Target path is blocked because env files are not allowed.');
    if (blockedByRegistry)
        reasons.push(`Target path is registry-classified as ${registryEntry?.type} and cannot be an active write default.`);
    if (blockedByUnknown)
        reasons.push('Target path is not registered and fails closed.');
    ...
    if (!allowedRoot)
        reasons.push('Target path is not in the preview allowed target list.');
    if (allowedRoot && !blockedRoot && reasons.length === 0)
        reasons.push('Target path is allowed for preview-only planning.');
    return {
...
export function createMindWritePreview(input) {
...
export function applyApprovedMindWritePreview(input) {
    const reasons = [];
    const preview = input.preview;
    const approval = input.approval;
    const approvedAt = (input.now ?? new Date()).toISOString();
    const allowedPreviewHash = preview.newHash;
    if (preview.actionKind !== 'mind-steward-update-current-context') {
    ...
    if (!approval) {
        ...
        if (approval.kind !== 'mind-steward-update-current-context') {
    ...
    return {
        ...
        rollback: {
            ...
            instructions: `Restore ${preview.targetPath} to the content captured by oldHash ${preview.oldHash ?? 'null'} and rerun mind-steward validation.`,
        },
        audit: {
...
export function createMindPreviewArtifact(input) {
...
export function writeMindPreviewArtifact(input) {
...
export function listMindPreviewArtifacts(input = {}) {
    const safeRoot = resolveSafePreviewRoot(input.runtimeRoot);
...
export function readMindPreviewArtifact(input) {
    const safeRoot = resolveSafePreviewRoot(input.runtimeRoot);
...
function normalizeMindPath(targetPath) {
...
function createSimpleUnifiedDiff(path, oldContent, newContent) {
...
function containsLiveLookingSecret(content) {
    const secretPrefixes = MIND_PREVIEW_POLICY.secretPrefixes;
    const privateKeyHeader = ['-----BEGIN', 'PRIVATE KEY-----'].join(' ');
    if (content.includes(privateKeyHeader))
        return true;
...
function createPreviewId(preview) {
    const seed = [
        preview.actionKind,
        preview.targetPath,
        preview.operation,
...
function resolveSafePreviewRoot(runtimeRoot) {
...
function readMindPreviewArtifactByPath(filePath, now) {
