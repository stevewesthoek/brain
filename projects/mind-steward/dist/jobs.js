import { MIND_LEGACY_READ_ONLY_PATHS, MIND_LIVE_FILES, MIND_REQUIRED_INDEX_FILES, MIND_REQUIRED_PATHS, MIND_ROUTER_CONTRACT_FILES, } from './contracts.js';
export const MIND_ROUTER_JOBS = [
    'mind-compile-loop',
    'mind-memory-loop',
    'mind-hygiene-loop',
...
export function createDryRunResult(jobId) {
    return {
        jobId,
        mode: 'dry-run',
        ok: true,
...
export function createAllDryRunResults() {
    return MIND_ROUTER_JOBS.map(createDryRunResult);
}
export function createMindContractDryRunResult(snapshot) {
...
function createPathStatusMap(paths) {
    return new Map(paths.map((pathStatus) => [normalizeMindPath(pathStatus.path), pathStatus]));
}
function findMissing(paths, pathMap) {
    return paths.filter((path) => pathMap.get(normalizeMindPath(path))?.exists !== true);
}
function findPresent(paths, pathMap) {
    return paths.filter((path) => pathMap.get(normalizeMindPath(path))?.exists === true);
}
function normalizeMindPath(path) {
    return path.replace(/^\.\//, '');
}
function toMissingErrors(label, paths) {
...
function createContractWarnings(input) {
    ...
    if (input.failureBufferStatus !== 'real-error-verified') {
        warnings.push(`Failure buffer is not real-error verified; current status: ${input.failureBufferStatus ?? 'unknown'}`);
    }
