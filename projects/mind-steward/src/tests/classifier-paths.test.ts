...
function writeCapture(file: string, title: string): void {
...
test('classifier discovers only inbox/new and ignores retired capture/inbox', async (t) => {
  ...
  assert.equal(
    fs.readFileSync(path.join(mindRoot, 'inbox/new/current.md'), 'utf8').includes('mind_steward_classified'),
    false,
  );
});
...
test('ambiguous or malformed classifier modes fail closed', () => {
  ...
  assert.throws(
    () => resolveMindCaptureExecutionMode({ mode: 'apply', dryRun: true }),
    /conflict/,
  );
  assert.throws(
    () => resolveMindCaptureExecutionMode({ mode: 'unsafe' }),
    /exactly/,
  );
});
