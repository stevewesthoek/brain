...
export function createMindStewardDryRunReport(
  snapshot: MindContractSnapshot,
  now = new Date(),
): MindStewardDryRunReport {
  ...
  const pathStats = snapshot.paths.reduce<MindStewardDryRunReport['snapshotStats']>(
    (acc, item) => {
      acc.pathCount += 1;
    ...
    },
    {
      ...
      captureInboxCount: 0,
    } as MindStewardDryRunReport['snapshotStats'],
  );
...
function createMaintenancePreviewMetadata(wikiHealth: MindStewardWikiHealthReport): MindStewardMaintenancePreviewMetadata {
