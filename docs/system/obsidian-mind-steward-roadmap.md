# Obsidian Mind + Mind Steward Roadmap
...
The `mind` repo will become an unnumbered, Obsidian-first, mind-steward-maintained personal operating memory.

The `brain` repo will own the executable infrastructure: Brain Core API, mind-steward implementation, scheduler integration, skills, orchestrators, and machine-level automations.

Obsidian remains the only primary human dashboard. Brain Core exposes machine and scheduler state. The mind steward continuously keeps `mind` small, coherent, useful, and fast.
...
8. **System** — Brain Core runtime, scheduler jobs, maintenance previews, mind-steward health
...
1. **Mind Steward** — vault maintenance, compilation, memory loops
...
**Dashboard cards:**
- Wiki Health: mind-steward lint status and error/warning counts
- Maintenance Previews: proposed maintenance queue from mind-steward
...
- Next Safe Action: recommended action from Brain Core/mind-steward
...
- Report-only mind-steward dry-run and Brain Core preview/status surfaces are complete.
...
brain    = machine intelligence, mind steward, Brain Core API, scheduler, skills
Brain Core API = safe machine and scheduler boundary
Mind Steward = AI steward for mind + brain workflows
...
## Mind Steward Placement

The mind-steward implementation belongs in `brain`, because it is executable infrastructure.
...
The `mind/router/` folder contains the vault contract consumed by the mind steward, not the implementation.
...
mind/router/mind-steward.md human-readable contract
```
...
The scheduler should run these mind-steward jobs:
...
The mind steward is the LLM interface from the user's perspective. Claude, Codex, Gemini, and local models are interchangeable workers beneath it.
...
When a file exceeds limits, the mind steward must summarize, split, archive, or compile it.
...
```text
Save-to-Mind -> capture/inbox -> mind steward -> wiki/sources/live/archive
```
...
1. **Brain Core owns agent state** — Agents (STB, video, research, design, code, Bible research, mind-steward) are registered entities with persistent runs, plans, roles, and event logs. All state lives in Brain Core, not Mind.
...
1. **Mind Steward** — vault maintenance (compile, memory, hygiene, drift loops)
...
- ✅ All 7 orchestrators registered (mind-steward, video, research, design, code, Bible research, scheduler)
...
- Compiled `wiki/` and `live/` pages are the mind-steward-maintained synthesis layer.
...
- `GET /orchestrators` — registry of all orchestrators (mind-steward, video, design, code, research, Bible research, scheduler, capture)
...
- No direct Mind mutation (goes through mind-steward)
...
- Brain Core exposes scheduler status and mind-steward job results.
...
- The mind steward keeps notes small, deduplicated, linked, and current.
