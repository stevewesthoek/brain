# Codex Prompt — Wire Model Router Dry Runs Into Office Nightly Scheduler
...
Wire the model-router dry-run planner into the Office Nightly Scheduler as read-only reporting jobs. Do not enable writes to `mind` yet.
...
## Task 2 — Add read-only model-router dry-run job integration
