...
function isMeaningfulCode(text) { return /\b(add|build|building|feature|fix|debug|bug|implement|refactor|change|ship|publish|make|improve|authentication|repair|restore|simplify|clean|remove|harden|optimize|reduce|document|normalize)\b/i.test(text) && !/\b(read-only|without changing it|analyze|explain|plan a)\b/i.test(text); }
...
export function composeShadowRequest(input, { catalog = createCapabilityCatalog(), repoRoot: root = repoRoot, currentState = {}, generatedAt = '2026-09-02T00:00:00Z', bounds = DEFAULT_GRAPH_BOUNDS } = {}) {
  ...
  const materialRiskQuestion = packetPlan.route.primaryRouteFamily !== 'web'
    ...
    && /\b(this|old|production|payment|payments)\b/i.test(normalizedText);
  ...
  const inferableSafeQualification = packetPlan.route.primaryRouteFamily === 'code' && packetPlan.route.qualification.required && /\b(authentication|auth|market demand|demand for this product|primary sources?|greek phrase|continue building|map the architecture|safe code analysis|code|file|function|parser|response|validation|configuration|retry|reviewable diff|user flow|data|storage|database|api|frontend|mobile|security|performance|queue|normalization|behavior|environment|error|request|session|redirect)\b/i.test(normalizedText) && !materialRiskQuestion;
  const discussedTargetQuestion = packetPlan.route.primaryRouteFamily === 'code' && /\b(we discussed|the feature)\b/i.test(normalizedText);
  const vagueCodeQuestion = packetPlan.route.primaryRouteFamily === 'code' && /^make code better[.!?]?$/i.test(normalizedText);
  const qualification = (packetPlan.route.qualification.required && !inferableSafeQualification) || materialRiskQuestion || discussedTargetQuestion || vagueCodeQuestion || qaTargetQuestion || combinedBuildVerificationQuestion || memoryPayloadQuestion ? { required: true, question: packetPlan.route.qualification.question ?? (memoryPayloadQuestion ? 'What exact decision or fact should I remember, and should it be a durable fact/decision or only a task note?' : 'What exact target and safe outcome should I use? Why: the target is material to a bounded or safe plan.'), count: 1 } : { required: false, question: null, reason: inferableSafeQualification ? 'The target is inferable and the next step is bounded, read-only, or locally reversible.' : packetPlan.route.qualification.reason };
  graph.metadata.qualificationRequired = qualification.required;
  return { operation: 'shadow_composition_plan', executionExposed: false, providerCalls: 0, externalMutations: 0, mindWrites: 0, profileActivations: 0, clientConfigChanges: 0, automaticResumeAllowed: false, route: packetPlan.route, qualification, taskPacket: packetPlan.taskPacket, evidencePackets: packetPlan.evidencePackets, continuationPacket: packetPlan.continuationPacket, selectedInstructionInspection: packetPlan.selectedInstructionInspection, budget: packetPlan.budget, atomicity: packetPlan.atomicity, graph, mergedEvidence, synthesis, validation: { phase3: packetPlan.validation, graphErrors, valid: packetPlan.validation.valid && graphErrors.length === 0 }, parallelGroups: buildParallelGroups(graph), continuity: { ref: graph.continuationRef, errors: validateContinuationRef(graph, { currentSourceRevision: graph.sourceRevision, contextState: packetPlan.continuity.state, explicitResume: false }) }, metrics: compositionMetrics({ graph, packetPlan, synthesis }) };
}
