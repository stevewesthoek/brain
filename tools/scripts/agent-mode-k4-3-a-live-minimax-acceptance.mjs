...
process.title = 'brain-agent k4-3-a-r1-live-minimax-acceptance';
...
const ACCEPTANCE_GENERATION = 'k4.3-a-r1';
const ROOT_GOAL_ID = 'goal:k4-3-a-r1';
const EVENT_ID = 'event:k4-3-a-r1';
const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
const FAILURE_EVIDENCE_PATH = path.join(REPO_ROOT, 'operations/reports/agent-mode-k4-3-a-r1-live-minimax-recovery-evidence-2026-09-11.md');
...
async function persistFailureEvidence(summary) {
  const body = [
    '# Agent Mode K4.3-A R1 Live MiniMax Recovery Evidence — 2026-09-11',
    '',
...
async function main() {
  ...
  const root = await mkdtemp('/tmp/brain-agent-mode-k4-3-a-r1-');
  ...
  try {
    ...
    const orchestrator = new AgentModeDynamicWorkerOrchestrator({ store, runtime, actionRules: [{ ...K43A_LIVE_MINIMAX_ACTION_RULE, enabled: true }], rootFacts: (id, now) => id === ROOT_GOAL_ID ? rootFacts(now) : undefined, ownerId: 'owner:k4-3-a-r1', controllerRef: K43A_LIVE_MINIMAX_CONTROLLER_REF, now: checkedAt, clock: () => new Date().toISOString() });
