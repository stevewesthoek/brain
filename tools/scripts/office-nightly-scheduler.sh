...
run_mind_compile_loop() {
  local timeout_seconds="${MIND_COMPILE_LOOP_TIMEOUT_SECONDS:-60}"
  local compile_script="${MIND_COMPILE_LOOP_SCRIPT:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/mind-compile-loop.sh}"
  local compile_log="$LOG_DIR/mind-compile-loop.log"
  local command

  if [[ ! -f "$compile_script" ]]; then
    log "skipping job=mind-compile-loop reason=missing_script path=$compile_script"
    return 0
  fi
