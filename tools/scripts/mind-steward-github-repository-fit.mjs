...
function architectureValue(evidence, name) {
  return evidence.architecture_evidence?.signals?.[name]?.value ?? null;
}
...
function evidenceSignals(metadata, evidence) {
  ...
  if (evidence.architecture_status === 'available') {
    const components = architectureValue(evidence, 'architectural_components') ?? [];
    const interfaces = architectureValue(evidence, 'interfaces_apis') ?? [];
    if (components.length || interfaces.length) positive.push(`documented_architecture:${[...components, ...interfaces].slice(0, 3).join(',')}`);
    else missing.push('documented architecture signals');
  } else missing.push('architecture/API/deployment documentation');
...
export function assessGitHubRepositoryFit(evidence, { systemCapabilities = [] } = {}) {
  ...
  return {
    ...
    purpose: { apparent_purpose: description, principal_capabilities: [...topics, ...(documentationValue(evidence, 'main_capabilities') ?? [])], target_users: documentationValue(evidence, 'target_users') ?? [], technology: { primary_language: language, ecosystem: fieldValue(metadata, 'ecosystem') ?? [], documented: documentationValue(evidence, 'supported_technologies') ?? [] }, integration_points: [...(documentationValue(evidence, 'integration_points') ?? []), ...(architectureValue(evidence, 'interfaces_apis') ?? [])], examples_use_cases: documentationValue(evidence, 'examples_use_cases') ?? [], limitations: documentationValue(evidence, 'limitations') ?? [], architecture: { components: architectureValue(evidence, 'architectural_components') ?? [], deployment: architectureValue(evidence, 'deployment_model') ?? [], environments: architectureValue(evidence, 'supported_environments') ?? [], operations: architectureValue(evidence, 'operational_considerations') ?? [] }, maintenance_signals: maintenanceAssessment(metadata) },
    ...
    integration_considerations: { likely_surface: 'human-reviewed evidence and bounded future analysis only', documented_interfaces: architectureValue(evidence, 'interfaces_apis') ?? [], documented_deployment: architectureValue(evidence, 'deployment_model') ?? [], dependencies: fieldValue(metadata, 'ecosystem') ?? [], operational_complexity: 'unknown_without_repository_inspection', maintenance_burden: 'unknown_without_repository_inspection', security_privacy: ['No repository code or dependencies were inspected.', 'External repository trust and privacy require human review.'], licensing: fieldValue(metadata, 'license'), },
    evidence_quality: { source_references: [...evidenceRefs(evidence), ...(evidence.documentation_provenance?.source ? [evidence.documentation_provenance.source] : []), ...(evidence.architecture_evidence?.source ? [evidence.architecture_evidence.source] : [])], freshness: evidence.freshness ?? 'unknown', documentation_freshness: evidence.documentation_freshness ?? 'unknown', architecture_freshness: evidence.architecture_evidence?.freshness ?? 'unknown', confidence: evidence.confidence ?? 0, uncertainty, metadata_status: metadataStatus, documentation_status: evidence.documentation_status ?? 'not_requested', architecture_status: evidence.architecture_status ?? 'not_requested', positive_evidence: signals.positive, negative_evidence: signals.negative, missing_evidence: signals.missing },
