...
const DB_PATH = process.env.DB_PATH || path.join(__dirname, "../../data/google-ads/google_ads.sqlite3");
...
app.get("/api/mutations", (req: Request, res: Response) => {
  const { status = "pending", sort = "id", order = "desc", limit = "50", offset = "0" } = req.query as QueryParams;
  ...
  db.all(query, [status, parseInt(limit), parseInt(offset)], (err, rows: any[]) => {
    if (err) {
...
app.get("/api/mutations/:id", (req: Request, res: Response) => {
  ...
  db.get(mutationQuery, [id], (err, mutation: any) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
    ...
    db.all(auditQuery, [id], (err, auditTrail: any[]) => {
...
app.post("/api/mutations/:id/approve", (req: Request, res: Response) => {
  const { id } = req.params;
...
app.post("/api/mutations/:id/reject", (req: Request, res: Response) => {
  const { id } = req.params;
  const { reason } = req.body;
...
app.post("/api/mutations/:id/preview", (req: Request, res: Response) => {
  const { id } = req.params;

  db.get(`SELECT * FROM pending_mutations WHERE id = ?`, [id], (err, mutation: any) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
...
app.post("/api/mutations/batch-approve", (req: Request, res: Response) => {
  ...
  db.serialize(() => {
    ...
    db.run(
      `UPDATE pending_mutations SET status = ?, updated_at = ? WHERE id IN (${placeholders})`,
      ["approved", now, ...ids],
      function (err) {
        if (err) {
          db.run("ROLLBACK");
...
app.post("/api/mutations/batch-reject", (req: Request, res: Response) => {
  const { ids, reason } = req.body;
  ...
  ws.on("message", (message: string) => {
    try {
      ...
      if (data.action === "subscribe") {
        (ws as any).filters = data.filters || {};
...
function broadcastMutationUpdate(update: any) {
  wsClients.forEach((client) => {
    if (client.readyState === 1) {
      // OPEN
      const filters = (client as any).filters || {};
...
function calculateRiskScore(mutation: any): { score: number; level: string; reasons: string[] } {
