...
function listRuntimeTopology(catalog) {
  return {
    ...
    instances: [...(catalog.runtimeInstances ?? [])].sort((left, right) => left.runtimeInstanceId.localeCompare(right.runtimeInstanceId)).map((instance) => ({
      ...
      runtimeHostId: instance.runtimeHostId,
    ...
    executionConnections: [...(catalog.executionConnections ?? [])].sort((left, right) => left.executionConnectionId.localeCompare(right.executionConnectionId)).map((connection) => ({
      executionConnectionId: connection.executionConnectionId,
      sourceRuntimeInstanceId: connection.sourceRuntimeInstanceId,
      sourceHostId: connection.sourceHostId,
      targetHostId: connection.targetHostId,
      executionTargetRef: connection.executionTargetRef,
      protocol: connection.protocol,
      networkPathId: connection.networkPathId,
      targetRuntimeInstanceId: connection.targetRuntimeInstanceId,
      workspaceRef: connection.workspaceRef,
      healthState: connection.healthState,
      lastObservedAt: connection.lastObservedAt,
      credentialCustodyRef: connection.credentialCustodyRef,
    })),
