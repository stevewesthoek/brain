...
function canaryRow(result, item, index) {
  const v2 = result.v2;
  return { id: item.id ?? `case-${index + 1}`, category: item.category ?? 'CANARY', promptHash: result.receipt.requestHash, expectedQuestion: item.expectedQuestion ?? false, observedQuestion: v2?.qualification?.required === true, selectedPath: result.selectedPath, state: result.state, reason: result.reason, route: v2?.route?.primaryRouteFamily ?? null, owner: v2?.route?.primaryDescriptorId ?? null, specialists: v2?.route?.selectedSpecialistDescriptorIds ?? [], qualification: v2?.qualification?.required === true, risk: v2?.route?.normalizedRequest?.riskClass ?? v2?.route?.riskClass ?? null, confirmation: v2?.route?.normalizedRequest?.confirmationClass ?? v2?.route?.confirmationClass ?? null, selectedCapabilities: v2?.taskPacket?.selectedCapabilityRefs?.map((item) => `${item.capabilityId}:${item.role}`) ?? [], packetStatus: v2?.taskPacket?.state?.status ?? null, taskPacket: Boolean(v2?.taskPacket), evidencePackets: v2?.evidencePackets?.length ?? 0, graph: Boolean(v2?.compositionGraph), qualityGates: (v2?.compositionGraph?.qualityGateNodes ?? []).map(gate), safetyGates: (v2?.compositionGraph?.safetyGateNodes ?? []).map(gate), contextScopes: v2?.route?.normalizedRequest?.domains ?? [], continuity: v2?.continuation?.state ?? 'UNAVAILABLE', metrics: { bootstrapTokens: v2?.budget?.universalBootstrapTargetTokens ?? 0, descriptorTokens: v2?.budget?.descriptorTokens ?? 0, selectedInstructionTokens: v2?.budget?.selectedInstructionTokens ?? 0, contextPackTokens: v2?.budget?.selectedContextPackTokens ?? 0, maxSimultaneousContext: v2?.atomicity?.maxSimultaneousActiveContext ?? 0, unrelatedFullSkillReads: v2?.atomicity?.unrelatedFullBodyReads ?? 0, listFullBodyReads: v2?.atomicity?.listFullSkillBodiesLoaded ?? 0, fullRepositoryLoaded: v2?.taskPacket?.scope?.inScope?.includes('full_repository') ?? false, fullConversationLoaded: false, secretsLoaded: false }, safety: v2?.safety ?? { providerCalls: 0, writesPerformed: 0, executionReady: false }, fallback: result.receipt.fallback, receiptId: result.receipt.receiptId, rawPromptStored: result.receipt.rawPromptStored, executionPerformed: result.receipt.executionPerformed };
}
