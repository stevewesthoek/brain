...
export async function runOperationalRolloutWave1({ writeReport = false } = {}) {
  ...
  const researchDefault = researchPrompts(10).map((item, i) => runClaudeCodeDefaultInvocation({ controller: researchController, catalog, prompt: i === 5 ? 'Research evidence about remote-work patterns.' : item.prompt, fixtureId: `wave1-research-default-${item.id}` }));
