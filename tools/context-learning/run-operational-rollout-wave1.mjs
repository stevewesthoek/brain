...
const promptFamilies = ['Research general public evidence with citations.', 'Research the business market with source-backed claims.', 'Research the technical tradeoffs and cite authority.', 'Research current time-sensitive facts with freshness.', 'Fact check this claim with primary sources.', 'Research contradictory evidence and preserve uncertainty.', 'Research the Bible passage with scholarly sources.', 'Research Hebrew and Greek lexical disagreement for this passage.', 'Research scholarly disagreement about this interpretation.', 'Research the local specialist and location context.'];
...
export async function runOperationalRolloutWave1({ writeReport = false } = {}) {
  ...
  const researchDefault = researchPrompts(10).map((item) => runClaudeCodeDefaultInvocation({ controller: researchController, catalog, prompt: item.prompt, fixtureId: `wave1-research-default-${item.id}` }));
