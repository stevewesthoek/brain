...
async function acquire(source, { fetchImpl = globalThis.fetch, retrievedAt = new Date().toISOString(), maxBytes = MAX_BYTES, freshnessRequirement = 'current', stale = false } = {}) {
  let sourceHost = 'unknown-host';
  try { sourceHost = new URL(source.url).hostname; } catch { /* invalid URL is reported below */ }
  const base = { sourceId: source.sourceId, title: source.title, publisher: source.publisher, ...(source.author ? { author: source.author } : {}), sourceAuthority: source.sourceAuthority ?? source.publisher, url: source.url, retrievedAt, freshnessRequirement, freshnessStatus: stale ? 'STALE' : freshnessRequirement === 'archival' ? 'ARCHIVAL' : 'CURRENT', retrievalMethod: 'https_get_allowlisted', independenceGroup: source.independenceGroup ?? sourceHost, sourceClass: source.sourceClass, claimSynopsis: source.claim ?? source.title, claimRefs: [], confidence: source.sourceClass === 'SUPPLEMENTARY' ? 0.72 : 0.94, uncertainty: 'Bounded retrieval records metadata and a short extracted excerpt; it does not make the source canonical knowledge.', contradictionState: 'NONE' };
  let parsedUrl;
  ...
  try {
    ...
    return { ...base, retrievalStatus: stale ? 'STALE' : bytes.length > maxBytes ? 'WEAK' : 'RETRIEVED', contentDigest, excerpt: contentExcerpt, bytes: bytes.length, sourceClass: source.sourceClass, extractedEvidence: contentExcerpt || 'Retrieved source bytes and identity; no bounded text excerpt was retained for this format.' };
  } catch (error) {
...
function claimLayers({ topic, sources, category, contradiction = false, specialist = false }) {
  const available = sources.filter((source) => ['RETRIEVED', 'WEAK'].includes(source.retrievalStatus));
  ...
  const claims = [
    ...
    { claimId: id(`extracted-${topic}`), layer: 'EXTRACTED_EVIDENCE', statement: available.length ? available.map((source) => source.claimSynopsis ?? source.title).join(' ') : 'No extracted source evidence is available.', sourceRefs: refs.length ? refs : sources.map((source) => source.sourceId), evidenceRefs: [evidenceId], confidence: available.length ? 0.86 : 0, uncertainty: available.length ? 'The extraction is bounded and deterministic; it is not a complete source summary.' : 'Source retrieval was unavailable or failed; no extracted evidence was fabricated.', relation: available.length ? 'SUPPORTS' : 'UNRESOLVED' },
...
export async function runResearchOutput({ taskId = 'research-task', topic, category, question = topic, subquestions = [`What does the available evidence establish about ${topic}?`], sourceSpecs, sourceRevision = 'research-canary', fetchImpl = globalThis.fetch, retrievedAt = new Date().toISOString(), freshnessRequirement = 'current', stale = false, contradiction = false, specialist = false, extraRequest = false, currentState = {}, catalog = createCapabilityCatalog() } = {}) {
  const specs = sourceSpecs ?? RESEARCH_SOURCE_CATALOG[category] ?? RESEARCH_SOURCE_CATALOG.technical;
  const sourceRecords = [];
  for (const source of specs) sourceRecords.push(await acquire(source, { fetchImpl, retrievedAt, freshnessRequirement, stale }));
  ...
  const packet = {
    ...
    research: { mode: RESEARCH_MODE, question, subquestions, sourceRecords, claimLedger: layers.claims, citationChecks: sourceRecords.map((source) => ({ citationId: id(`citation-${topic}-${source.sourceId}`), sourceId: source.sourceId, resolves: ['RETRIEVED', 'WEAK'].includes(source.retrievalStatus), sourceMatchesClaim: source.claimRefs.length > 0, claimBoundedByEvidence: ['RETRIEVED', 'WEAK'].includes(source.retrievalStatus) && Boolean(source.extractedEvidence), freshnessConsidered: Boolean(source.freshnessRequirement), quotedMaterial: false, passed: ['RETRIEVED', 'WEAK'].includes(source.retrievalStatus) && Boolean(source.extractedEvidence), reason: ['RETRIEVED', 'WEAK'].includes(source.retrievalStatus) ? 'Citation resolves to the recorded source and has bounded extracted evidence; weak retrieval remains quality-degraded.' : 'Citation retained as unavailable; no unsupported claim is accepted.' })), sourceIndependence: { sourceCount: sourceRecords.length, independentSourceCount: new Set(sourceRecords.map((source) => source.independenceGroup)).size, independenceGroups: [...new Set(sourceRecords.map((source) => source.independenceGroup))], repeatedUpstreamCount: sourceRecords.length - new Set(sourceRecords.map((source) => source.independenceGroup)).size }, freshness: { requirement: freshnessRequirement, retrievedAt, staleSourceCount: sourceRecords.filter((source) => source.freshnessStatus === 'STALE' || source.retrievalStatus === 'STALE').length, currentEnough: freshnessRequirement === 'archival' || sourceRecords.every((source) => source.freshnessStatus !== 'STALE' && ['RETRIEVED', 'WEAK'].includes(source.retrievalStatus)) }, evidenceLayers: ['SOURCE', 'EXTRACTED_EVIDENCE', 'INTERPRETATION', 'CONCLUSION', ...(contradiction ? ['UNCERTAINTY'] : [])], deepening: { round1: 'MINIMUM_EVIDENCE', decision: extraRequest ? 'GAP_DETECTED' : 'SUFFICIENT', additionalRequests: extraRequest ? [{ requestId: id(`deepening-${topic}`), reason: 'One material gap remained after minimum evidence acquisition.', atomic: true }] : [], finalBudget: { sourceFetches: sourceRecords.length, depthRounds: extraRequest ? 2 : 1, estimatedTokens: Math.ceil(JSON.stringify(sourceRecords).length / 4) } }, stopping: { answerReady: sourceRecords.some((source) => ['RETRIEVED', 'WEAK'].includes(source.retrievalStatus)), importantClaimsSupported: layers.claims.filter((claim) => ['SOURCE', 'EXTRACTED_EVIDENCE', 'CONCLUSION'].includes(claim.layer)).every((claim) => claim.confidence > 0), contradictionsAddressed: !contradiction || sourceRecords.every((source) => source.contradictionState === 'OPEN' || source.contradictionState === 'NONE'), sourceQualityChecked: true, retrievalValueChecked: true, budgetChecked: true }, quality: quality(sourceRecords) }
  };
