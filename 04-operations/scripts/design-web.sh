...
if [ -z "$QUERY" ]; then
  ...
  echo "  bash 04-OPERATIONS/scripts/design-web.sh \"brutalism landing page for fintech\" \"Fintech SaaS\" landing" >&2
  exit 1
fi
...
if [ ! -d "$SKILL_ROOT/data" ] || [ ! -d "$SKILL_ROOT/scripts" ]; then
  ...
  echo "  bash 04-OPERATIONS/scripts/update-ui-ux-pro-max.sh" >&2
  exit 1
fi
